...
| `ask` | Render `directive.question` via `AskUserQuestion` (this harness's binding for the protocol's structured questions — see `question-rendering.md` beside this file; when `AskUserQuestion` is unavailable, e.g. auto permission mode or a headless `kimi -p` run, render the same spec as numbered prose), then feed the human's answer back on the next `report` via `--user-input "<answer>"`. The engine never asks the user itself — it defers the human turn to you. |
