---
name: amadeus-formal-model-check
description: >
  Run the AI-DLC `formal-model-check` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage formal-model-check --single`:
  the engine emits one run-stage directive for formal-model-check and its gate, the
...
# AI-DLC Stage Runner — formal-model-check

Run the `formal-model-check` stage on its own. This is opt-in packaging over
`/amadeus --stage formal-model-check --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kimi-code/tools/amadeus-orchestrate.ts next --stage formal-model-check --single
   ```

   The engine emits one `run-stage` directive for `formal-model-check` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kimi-code/tools/amadeus-orchestrate.ts report --single --stage formal-model-check --result completed
   ```
