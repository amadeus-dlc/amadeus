...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kimi-code/tools/amadeus-orchestrate.ts next --stage market-research --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kimi-code/tools/amadeus-orchestrate.ts report --single --stage market-research --result completed
   ```
