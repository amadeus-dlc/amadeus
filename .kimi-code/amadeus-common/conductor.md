...
`agents/amadeus-architect-agent.md`) and adopt its voice for the stage body — you
...
1. The engine auto-creates `memory.md` from
   `.kimi-code/knowledge/amadeus-shared/memory-template.md` when it issues
...
- **Design deviations** — when the approved requirements or design have to be
  ...
  `"auto-solo-election": true`, put the deviation to an election with
  `bun .kimi-code/tools/amadeus-election.ts open --trigger auto-solo`; on
  `{"opened":null,"reason":"auto-solo-election-disabled"}` no election is created
  and the deviation goes to the user for a ruling. A user-visible spec change is
  always the user's call, whatever the config says.
...
1. Read the team's `## Walking Skeleton` section (resolution order
   `rules/amadeus-org.md` → `amadeus-team.md` → `amadeus-project.md`; the most
...
via `bun .kimi-code/tools/amadeus-state.ts practices-event --type override` before
