...
1. **Detect.** Run `bun .kimi-code/tools/amadeus-utility.ts detect --json`.
...
2. **Read the repertoire.** Read the stock scope definitions under the
   printed `scopesDir` (one `amadeus-<name>.md` per scope) and the grid at the
   printed `scopeGridPath`. Read the stage graph summary in the orchestrator
   SKILL.md (or `.kimi-code/tools/data/stage-graph.json`) for the stage
   list and phases.
...
4. **Validate.** Before the proposal is shown, write the proposed grid to a
   ...
   `bun .kimi-code/tools/amadeus-graph.ts validate-grid --proposal <path> --project-type <greenfield|brownfield>`
...
6. **Write (front/report, after approval).** Author BOTH files - a
   `amadeus-<name>.md` in the printed `scopesDir` (frontmatter: `name`,
   ...
   **NEVER run `amadeus-graph.ts compile` after the write.** Compile is the
...
- Making a scope inferable is an explicit human choice at the gate ("make
  ...
  `bun .kimi-code/tools/amadeus-graph.ts validate-grid --proposal <path> --keywords <granted,csv>`
