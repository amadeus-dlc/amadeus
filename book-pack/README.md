...
処理内容: 新規/fork ステージと agent・scope のコピー → 流用6ステージの `scopes:` に book を追記 → source-owned `stage-identities.json` の番号事前ピン(application-design を 2.9 へ退避し book-structure-design を 2.6 に挿入)→ `amadeus-graph.ts compile`。冪等。
