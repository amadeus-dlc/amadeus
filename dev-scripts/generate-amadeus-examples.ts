...
function normalizeSnapshotNarrative(stepId: string): void {
  ...
  writeWorkspaceFile(".amadeus/steering.md", [
    "# Steering",
    "",
    "## 役割",
    "",
    ...
    "## Intent Layer へ進む基準",
    "",
    "- 目的、主要アクター、主要ドメイン領域が確認済みである。",
    "- 外部システム連携は、対象 Intent の範囲に必要になった時点で確認する。",
...
function inceptionPrompt(): string {
  return [
    ...
    "- 境界づけられたコンテキストは、steering layer の既存 ID である BC004 販売管理を中心にしてください。",
    "- BC004 販売管理は SD004 販売管理に対応させてください。",
    "- この Intent が所有する境界づけられたコンテキストは BC004 販売管理だけにしてください。",
    "- 商品管理、顧客管理、入荷管理、出荷管理、在庫管理は、Intent 配下の境界づけられたコンテキストとして展開せず、必要な依存は BC004 の契約または未確認事項で扱ってください。",
    "- BC004 は、販売可能在庫の参照では在庫管理に依存しますが、在庫引当は今回の範囲外にしてください。",
    "- BC004 は、購入者情報を注文に記録しますが、会員登録、ログイン、顧客台帳、購入履歴管理は今回の範囲外にしてください。",
    "- Unit 候補は、最小購入フローと注文作成です。",
    "- Bolt 候補は、B001 注文作成、B002 注文内容確認、B003 商品選択です。",
    "- R004 注文作成は B001 の責務です。traceability では B002 の要求に R004 を含めないでください。",
    "",
    "実行条件:",
    "- 質問せずに続行してください。",
