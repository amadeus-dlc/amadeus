...
> **Source:** Converted from `ai-driven-development-lifecycle.pdf` (8 pages, Amazon Web Services).
> Two-column layout has been linearized into single-column reading order, and diagrams have been transcribed as structured text for machine readability.
...
### AI-DLC Overview Diagram

The framework maps three columns — **Roles / Rituals**, **Phases**, and **Artifacts** — across three phases:

| Phase | Roles / Rituals | Artifacts produced |
|-------|-----------------|--------------------|
| **Inception** | Product Owner, Developers, AI — ritual: *Mob Elaboration* | An `Intent` decomposes into `Unit 1`, `Unit 2` … `Unit n`. Artifacts: PRFAQ, User Stories, NFRs, Risks |
| **Construction** | Developers, AI — ritual: *Mob Construction*; then Product Owner, Developers, AI — ritual: *Mob Testing* | Units expand into Domain Design, Logical Design, Codes. Each Unit is executed via `Bolt 1`, `Bolt 2` … `Bolt n+1`. Output: Deployment Units (Secured, Instrumented, De-Risked, Tested, Packaged) |
| **Operation** | Product Owner, Developers, AI | Deployment Units running in Production Environment |

### 1. Artefacts

- **Intent** — An Intent in AI-DLC is a high-level statement of purpose that encapsulates what needs to be achieved, whether a business goal, a feature, or a technical outcome (ex. performance scaling). It serves as the starting point for AI-driven decomposition into actionable tasks, aligning human objectives with AI-generated plans.

- **Unit** — A Unit represents a cohesive, self-contained work element derived from an Intent, specifically designed to deliver measurable value. For instance, an Intent to implement a business idea may be decomposed into Units representing independent functional blocks, analogous to Subdomains in DDD or Epics in Scrum. Each Unit encompasses a set of tasks (user stories, in this case, that articulate its functional scope). In the context of AI-DLC, the process of decomposing Intents into Units is driven by AI, with developers and/or Product Owners validating and refining the resulting Units to ensure alignment with business and technical objectives. The units are **loosely coupled**, enabling autonomous development and independent deployment downstream.

- **Bolt** — A Bolt is the smallest iteration in AI-DLC, designed for the rapid implementation of a Unit or a set of tasks within a Unit. Bolts (analogous to Sprints in Scrum) emphasize intense focus and high-velocity delivery, with build-validation cycles measured in hours or days rather than weeks. Each Bolt encapsulates a well-defined scope of work (e.g., a collection of user stories within a Unit), enabling incremental progress while maintaining alignment with the overall objectives of the Unit it supports. A Unit can be executed through one or more Bolts, which may run in parallel or sequentially. AI will plan the Bolts with developers / Product Owners validating it.

- **Domain Design** — The Domain Design artefact models the core business logic of a Unit, independently of the infrastructure components. In the first version of AI-DLC, AI uses domain-driven design principles to create the strategic and tactical modelling elements including aggregates, value objects, entities, domain events, repositories and factories.
...
- **Operations Phase** — centers on the deployment, observability, and maintenance of systems, leveraging AI for operational efficiency. AI actively analyzes telemetry data, including metrics, logs, and traces, to detect patterns, identify anomalies, and predict potential SLA violations, enabling proactive issue resolution. Additionally, AI integrates with predefined incident runbooks, proposing actionable recommendations such as resource scaling, performance tuning, or fault isolation and execute the resolutions when approved by the Developers. Developers serve as validators, ensuring AI-generated insights and proposed actions align with SLAs and compliance requirements.

### 3. The Workflow

Each step builds semantically richer context for the next step.

| # | Phase | Step |
|---|-------|------|
| 1 | **Inception** | Build Context From Existing Codes |
| 2 | **Inception** | Elaborate Intent with User Stories |
| 3 | **Inception** | Plan the Units of Work |
| 4 | **Construction** | Model the Domain, Code & Test |
| 5 | **Construction** | Solve for Non-Functional Requirements |
| 6 | **Construction** | Resolve Deployment Architecture, Code & Test |
| 7 | **Construction** | Generate IaC & Test |
| 8 | **Operations** | Deploy in Production |
| 9 | **Operations** | Monitor, Manage Incidents |
...
### Green-Field Interaction Diagram

A swim-lane interaction between **AI (Domain Specific AI Agent)** and **Product Owner, Developers, AI**, organized by phase and ritual:

- **Inception Phase ↔ Mob Elaboration**
  - User establishes Session & Provides the Intent
  - AI elaborates the Intent → `Intent`
  - AI recommends the Units, Bolts → `Units / Bolts Plan`
  - Verify / Improve Cycles
- **Construction Phase ↔ Mob Programming**
  - Developer establishes Session; AI initiates the Bolt assigned to Developer
  - AI recommends the Domain Design, validated by Developer
  - AI recommends Logical Architecture based on NFRs; Developer + AI makes Architectural Decisions
  - AI generates Codes & Tests, validated by Dev + Product Owner
- **Construction Phase ↔ Mob Testing**
- **Operations Phase**
  - AI analyses telemetry, detects incidents, invokes playbook actions & notifies
  - Developers apply oversight, validate actionable responses & implement
...
## Appendix A — Prompts for Interacting with AI

The following prompts can be used to interact with AI for practicing AI-DLC.

### Setup Prompt

> We will work on building an application today. For every front end and backend component we will create a project folder. All documents will reside in the `aidlc-docs` folder. Throughout our session I'll ask you to plan your work ahead and create an md file for the plan. You may work only after I approve said plan. These plans will always be stored in the `aidlc-docs/plans` folder. You will create many types of documents in the md format. Requirement, features, changes will reside in `aidlc-docs/requirements` folder. User stories must be stored in the `aidlc-docs/story-artifacts` folder. Architecture and Design documents must be stored in the `aidlc-docs/design-artifacts` folder. All prompts in order must be stored in the `aidlc-docs/prompts.md` file. Confirm your understanding of this prompt. Create the necessary folders and files for storage, if they do not exist already.
