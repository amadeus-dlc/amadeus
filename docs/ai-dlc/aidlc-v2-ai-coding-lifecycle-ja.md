...
seal は会話の終点ではなく、次工程が参照できる正本を固定する行為である。
...
amadeus の役割は、Intent Contract を正本にし、Unit、Bolt、Spec の追跡関係を保ったまま cc-sdd の仕様化へ接続することである。

そのため、cc-sdd 由来の Spec 生成コマンドも Intent-aware でなければならない。

Spec が `.amadeus/intents/*` を参照しない場合、SSoT から漂流する。
