...
`.amadeus/domain/bounded-contexts.md` は、全体の境界づけられたコンテキストと Intent 固有モデルへの索引を扱う。

`.amadeus/domain/bounded-contexts/<bounded-context-id>/model.md` は、コンテキスト別の概念間の関係、ライフサイクル、集約候補を扱う。

`.amadeus/domain/bounded-contexts/<bounded-context-id>/contracts.md` は、コンテキスト別の事前条件、不変条件、事後条件を扱う。

Intent 固有のサブドメイン、境界づけられたコンテキスト、概念関係、契約を固定する場合は、Intent 配下に `domain/subdomains.md`、`domain/bounded-contexts.md`、`domain/bounded-contexts/<bounded-context-id>/model.md`、`domain/bounded-contexts/<bounded-context-id>/contracts.md` を置く。

`domain/bounded-contexts.md` は、Unit を切る時に参照するコンテキスト、責務、外部境界を扱う。
