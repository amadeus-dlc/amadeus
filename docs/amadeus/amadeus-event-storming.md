...
`amadeus-event-storming` skill は、この設計に基づく公開入口として扱う。
...
`amadeus-event-storming` は phase を進める主入口ではない。
`amadeus-event-storming` は、Discovery、Intent Init、Inception、Domain Modeling が参照できる分析成果物を作る。
...
`amadeus-event-storming` は、この補助 skill 判断基準の最初の標準例として扱う。

## 責務

`amadeus-event-storming` は、Domain Event を中心に人間へヒアリングする。

`amadeus-event-storming` は次を行う。
...
`amadeus-event-storming` は次を行わない。
...
Discovery のモジュールファイルには、関連 Event Storming への参照を置く。
...
`amadeus-event-storming` は、1回の実行で1つの level を主対象にする。
...
`Event Storming のモジュールファイル` は、目的、対象範囲、関連 Discovery、関連 Intent、実施状態、次に使う skill をまとめる。
...
Handoff は `Event Storming のモジュールファイル` の `Handoff To Domain Modeling` 見出しに保存する。
...
`Event Storming のモジュールファイル` にも関係を残す。
...
`amadeus-event-storming` は、固定上限ではなく質問数の目安を持つ。
