...
**Operation**：v2 の 7 ステージを持ち、実行可否は 2 層で決まる — scope（scope-grid。enterprise や feature などの scope は Operation ステージを EXECUTE で持つ。AMADEUS.md の CONDITIONAL 採用）と workspace steering（default space は `memory/phases/operation.md` により Operation を対象外にし、Operation を含む scope の Intent は理由付き skip で処理する）。現行の validator は、scope によらず全 workspace で Operation の全ステージに `[S]` を追加で要求する — Operation の実行は将来採用である。対象外にする理由（成果物契約、gate、validator、PR 境界）、validator の現行挙動、本家 Operation skill の扱いは [AI-DLC v2 Operation Phase Boundary](../aidlc-v2-operation-phase-boundary.ja.md) に従う。
      ...
      intents/
        ...
        <YYMMDD>-<label>/            # record
          ...
          construction/
            ...
            grillings/
          operation/
            <stage-slug>/           # v2 の 7 ステージ分の scaffold。scope-grid に従い実行し、default space は steering で skip
```
...
| Operation phase | 7 stage を実行対象に含む | v2 と同じ 7 ステージを scope（scope-grid）単位で CONDITIONAL 採用する。既定 workspace の steering（`memory/phases/operation.md`）が対象外の扱いを定める | 対象範囲内の Operation ステージが実行されるかどうかは workspace steering が決めるものであり、成果物契約、gate、validator、PR 境界だけで固定されないため。詳細は [AI-DLC v2 Operation Phase Boundary](../aidlc-v2-operation-phase-boundary.ja.md)。 |
