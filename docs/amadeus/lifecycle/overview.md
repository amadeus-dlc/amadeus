...
| 列 | 意味 | 英語化後のラベル |
|---|---|---|
| Artifact | ステージが読む成果物の record 相対 path または名称。実在する成果物だけを書く。 | Artifact |
...
Inputs 表の一次実測源は、エンジンの stage 定義（`.claude/amadeus-common/stages/<phase>/<slug>.md`）の frontmatter `consumes`（`artifact` / `required` / `conditional_on`）である。
