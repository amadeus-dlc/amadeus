...
**ladder 提案**：walking skeleton の承認直後に一度だけ、残りの Bolt を自律実行するか、Bolt ごとにゲートを続けるかを確認する。

**halt-and-ask**：自律実行中でも、失敗した Bolt は停止して人間に確認する。
...
| 3.5 | Code Generation | ALWAYS | 実行計画の各 Unit で必ず実行 | `amadeus-construction-code-generation` | コード、`plan.md`、`summary.md` |
| 3.6 | Build and Test | ALWAYS | Bolt 内の全 Unit 完了後に 1 回必ず実行 | `amadeus-construction-build-and-test` | `test-results.md` ほか |
...
| Mode | internal |
| 実行単位 | Unit ごと |
| v2 Source | [functional-design.md](https://github.com/awslabs/aidlc-workflows/blob/v2/core/aidlc-common/stages/construction/functional-design.md) |
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| `units.md`（対象 Unit） | 必須 | Stage 2.7 |
| `unit-story-map.md` | 任意 | Stage 2.7 |
| `requirements.md` | 必須 | Stage 2.3 |
| `components.md`、`component-methods.md`、`services.md` | 必須 | Stage 2.6 |
...
| Mode | internal |
| 実行単位 | Unit ごと |
| v2 Source | [nfr-requirements.md](https://github.com/awslabs/aidlc-workflows/blob/v2/core/aidlc-common/stages/construction/nfr-requirements.md) |
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| `business-logic-model.md`、`business-rules.md` | 必須 | Stage 3.1 |
| `requirements.md` | 必須 | Stage 2.3 |
| `technology-stack.md` | brownfield のみ | Stage 2.1 |
...
| Mode | internal |
| 実行単位 | Unit ごと |
| v2 Source | [nfr-design.md](https://github.com/awslabs/aidlc-workflows/blob/v2/core/aidlc-common/stages/construction/nfr-design.md) |
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| NFR Requirements の 5 成果物 | 必須 | Stage 3.2 |
| `business-logic-model.md` | 必須 | Stage 3.1 |
...
| Mode | internal |
| 実行単位 | Unit ごと |
| v2 Source | [infrastructure-design.md](https://github.com/awslabs/aidlc-workflows/blob/v2/core/aidlc-common/stages/construction/infrastructure-design.md) |
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| NFR Design の 5 成果物 | 必須 | Stage 3.3 |
| `components.md`、`services.md` | 必須 | Stage 2.6 |
| `business-logic-model.md` | 必須 | Stage 3.1 |
...
| v2 Source | [code-generation.md](https://github.com/awslabs/aidlc-workflows/blob/v2/core/aidlc-common/stages/construction/code-generation.md) |
...
| `units.md`（対象 Unit） | 必須 | Stage 2.7 |
| `requirements.md` | 必須 | Stage 2.3 |
| Functional Design の成果物 | 実行した場合 | Stage 3.1 |
| NFR Design、Infrastructure Design の成果物 | 実行した場合 | Stage 3.3、3.4 |
...
| `construction/<unit>/code-generation/plan.md` | 実装計画 | code-generation-plan |
| `construction/<unit>/code-generation/summary.md` | 実装結果の要約 | code-summary |

### Notes

旧契約の Bolt preparation と `tasks.md` は `plan.md` が置き換える。
...
| Mode | internal |
| 実行単位 | Bolt ごとに 1 回 |
| v2 Source | [build-and-test.md](https://github.com/awslabs/aidlc-workflows/blob/v2/core/aidlc-common/stages/construction/build-and-test.md) |
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| Bolt 内全 Unit の `plan.md` と `summary.md` | 必須 | Stage 3.5 |
...
| `construction/bolts/<bolt-id>-<slug>/summary.md` | ビルドとテストの要約 | build-and-test-summary |
| `construction/bolts/<bolt-id>-<slug>/test-results.md` | テスト実行結果 | build-test-results |
...
| Mode | internal |
| 実行単位 | Intent ごと |
| v2 Source | [ci-pipeline.md](https://github.com/awslabs/aidlc-workflows/blob/v2/core/aidlc-common/stages/construction/ci-pipeline.md) |
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| `summary.md`、`test-results.md` | 必須 | Stage 3.6 |
...
| `construction/ci-pipeline/questions.md` | 確認した質問と回答 | ci-pipeline-questions |

### Notes

全 Bolt の完了後、phase の `decisions.md` と `traceability.md` を確定し、Intent の完了を `state.json` に記録する。
