...
| record 直下のモジュールファイル `<dirName>.md` | 概要、依存、目標プロファイル | Amadeus 拡張 |
...
Intent のモジュールファイルは Initialization（Stage 0.3）が作り、`概要`、`依存`、`目標プロファイル` だけを持つ。
このステージは `intent-statement.md` に目的、対象、成功条件、契機、範囲を確定し、モジュールファイルの `目標プロファイル` に反映する。
