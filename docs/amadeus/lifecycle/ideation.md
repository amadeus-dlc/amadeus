...
Intent のモジュールファイル（`<dirName>.md`）は GD009 で廃止したため、このステージは作らない（[overview.md](overview.md) の成果物配置を参照）。
...
bugfix のように Ideation の全ステージを実行しない scope では、Ideation の成果物と phase PR を作らずに通過する（[state.md](state.md) の phase 遷移を参照）。
