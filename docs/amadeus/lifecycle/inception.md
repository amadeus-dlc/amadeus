...
|---|---|---|---|---|---|
| 2.1 | Reverse Engineering | CONDITIONAL | brownfield の場合。鮮度維持のため毎回再実行 | `amadeus-inception-reverse-engineering` | `.amadeus/knowledge/codebase/<repo>/` の 9 成果物 |
...
| 2.4 | User Stories | CONDITIONAL | 利用者向け機能、複数ペルソナ、複雑な業務ロジック、チーム横断がある場合 | `amadeus-inception-user-stories` | `stories.md`、`personas.md`、`assessment.md` |
...
| 2.7 | Units Generation | ALWAYS | scope が実行対象にする場合は必ず実行 | `amadeus-inception-units-generation` | `units.md`、`unit-dependencies.md`、`unit-story-map.md` |
...
|---|---|---|
| `.amadeus/knowledge/codebase/<repo>/business-overview.md` | 業務概要 | business-overview |
| `.amadeus/knowledge/codebase/<repo>/architecture.md` | アーキテクチャ | architecture |
| `.amadeus/knowledge/codebase/<repo>/code-structure.md` | コード構造 | code-structure |
| `.amadeus/knowledge/codebase/<repo>/api-documentation.md` | API 文書 | api-documentation |
| `.amadeus/knowledge/codebase/<repo>/component-inventory.md` | コンポーネント一覧 | component-inventory |
| `.amadeus/knowledge/codebase/<repo>/technology-stack.md` | 技術スタック | technology-stack |
| `.amadeus/knowledge/codebase/<repo>/dependencies.md` | 依存関係 | dependencies |
| `.amadeus/knowledge/codebase/<repo>/code-quality-assessment.md` | 品質評価 | code-quality-assessment |
| `.amadeus/knowledge/codebase/<repo>/timestamp.md` | 解析時刻と鮮度 | reverse-engineering-timestamp |

### Notes

成果物は Intent 配下ではなく steering layer 配下に置き、Intent をまたいで再利用する。
...
人間が確認したプラクティスは steering layer の policies へ昇格し、以後の Intent が参照する。

### Inputs

| Artifact | 必須 | 供給元 |
|---|---|---|
| `.amadeus/knowledge/codebase/<repo>/` の成果物 | brownfield のみ | Stage 2.1 |
| steering layer の既存 policies | 任意 | steering layer |
...
| `inception/practices-discovery/timestamp.md` | 発見時刻と鮮度 | practices-discovery-timestamp |

### Notes

昇格先は `.amadeus/steering/policies/` である。
昇格は人間の承認を要する。
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| Intent のモジュールファイル | 任意 | Stage 1.1 または Intake |
...
| `inception/requirements-analysis/questions.md` | 確認した質問と回答 | requirements-analysis-questions |
...
| `business-overview.md`、`component-inventory.md` | brownfield のみ | Stage 2.1 |
| `team-practices.md` | 任意 | Stage 2.2 |
...
| `inception/user-stories/assessment.md` | ストーリーの充足評価 | user-stories-assessment |
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| `wireframes.md`、`user-flow.md` | 必須 | Stage 1.6 |
| `requirements.md` | 必須 | Stage 2.3 |
...
| `inception/refined-mockups/questions.md` | 確認した質問と回答 | refined-mockups-questions |
...
| `requirements.md` | 必須 | Stage 2.3 |
| `stories.md` | 任意 | Stage 2.4 |
| `architecture.md`、`component-inventory.md` | brownfield のみ | Stage 2.1 |
| `team-practices.md` | 任意 | Stage 2.2 |
...
| `inception/application-design/design-decisions.md` | 設計判断 | decisions |

### Notes

`design-decisions.md` はこのステージの設計判断を扱い、phase の `decisions.md` とは分ける。
...
| Artifact | Description | v2 対応 |
|---|---|---|
| `inception/units-generation/units.md` | Unit 一覧 | unit-of-work |
| `inception/units-generation/unit-dependencies.md` | Unit の依存 DAG | unit-of-work-dependency |
| `inception/units-generation/unit-story-map.md` | Unit とストーリーの対応。stories がある場合のみ | unit-of-work-story-map |
...
| `inception/delivery-planning/questions.md` | 確認した質問と回答 | delivery-planning-questions |
