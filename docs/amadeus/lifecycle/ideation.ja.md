# Ideation Phase Stage Reference（Ideation phase ステージリファレンス）
...
Ideation は、Intake の birth 提案を人間が承認し、Initialization（Stage 0.1〜0.3）が record を作った Intent に対して開始する。

この phase では、Requirement、Story、Unit、Bolt、Domain Model、実装を作らない。

phase 共通入力として、全ステージが steering/memory 参照（`org.md`、`team.md`、`project.md`、`phases/ideation.md` = エンジンの rules_in_context）を読む。
...
必須入力の供給ステージを実行しなかった場合の扱いは、[scopes.md](scopes.ja.md) の縮退時の入力代替に従う。
...
|---|---|---|---|---|---|
| 1.1 | Intent Capture & Framing | ALWAYS | 全ワークフローの先頭 | `amadeus-ideation-intent-capture` | `intent-statement.md`、`stakeholder-map.md` |
...
入力テーマから、解決する問題、対象者、成功指標、契機を確認し、`intent-statement.md` と利害関係者の整理を作る。
...
|---|---|---|
| `ideation/intent-capture/intent-statement.md` | 目的（問題）、対象、成功条件、契機、範囲 | intent-statement |
| `ideation/intent-capture/stakeholder-map.md` | 利害関係者、決定者と影響者の区別、コミュニケーション要件 | stakeholder-map |
| `ideation/intent-capture/intent-capture-questions.md` | 確認した質問と回答 | intent-capture-questions |
| `ideation/intent-capture/memory.md` | stage 実行の学習記録（Interpretations、Deviations、Tradeoffs、Open questions） | memory |

### Notes

このステージは `intent-statement.md` に目的、対象、成功条件、契機、範囲を確定する。
成功条件は Intent の受理条件 1（観測可能な成功基準）を満たす形で書く。
Intent のモジュールファイル（`<dirName>.md`）は GD009 で廃止したため、このステージは作らない（[overview.md](overview.ja.md) の成果物配置を参照）。
...
| Artifact | 必須 | 供給元 |
|---|---|---|
| `intent-statement.md` | 必須 | Stage 1.1 |
...
| `ideation/market-research/market-research-questions.md` | 確認した質問と回答 | market-research-questions |
| `ideation/market-research/memory.md` | stage 実行の学習記録（Interpretations、Deviations、Tradeoffs、Open questions） | memory |
...
| `ideation/feasibility/feasibility-questions.md` | 確認した質問と回答 | feasibility-questions |
| `ideation/feasibility/memory.md` | stage 実行の学習記録（Interpretations、Deviations、Tradeoffs、Open questions） | memory |
...
| `ideation/scope-definition/scope-definition-questions.md` | 確認した質問と回答 | scope-definition-questions |
| `ideation/scope-definition/memory.md` | stage 実行の学習記録（Interpretations、Deviations、Tradeoffs、Open questions） | memory |
...
| `ideation/team-formation/team-formation-questions.md` | 確認した質問と回答 | team-formation-questions |
| `ideation/team-formation/memory.md` | stage 実行の学習記録（Interpretations、Deviations、Tradeoffs、Open questions） | memory |
...
| `ideation/rough-mockups/rough-mockups-questions.md` | 確認した質問と回答 | rough-mockups-questions |
| `ideation/rough-mockups/memory.md` | stage 実行の学習記録（Interpretations、Deviations、Tradeoffs、Open questions） | memory |
...
| `ideation/approval-handoff/approval-handoff-questions.md` | 確認した質問と回答 | approval-handoff-questions |
| `ideation/approval-handoff/memory.md` | stage 実行の学習記録（Interpretations、Deviations、Tradeoffs、Open questions） | memory |
...
mvp や poc のように、Ideation の一部を実行しつつこのステージを SKIP にする scope では、最後に実行した Ideation ステージの承認と phase PR が引き継ぎを兼ねる。
bugfix のように Ideation の全ステージを実行しない scope では、Ideation の成果物と phase PR を作らずに通過する（[state.md](state.ja.md) の phase 遷移を参照）。
