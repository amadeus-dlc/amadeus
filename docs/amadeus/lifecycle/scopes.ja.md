...
default space の workspace steering が Operation を対象外にするため、Operation ステージを含む scope（enterprise、feature、infra、security-patch、workshop）は本 workspace では理由付き skip で処理される（lifecycle 契約としては v2 と同じステージ集合を持つ）。現行の validator は、scope によらず全 workspace で Operation の全ステージに `[S]` を追加で要求する — Operation の実行は将来採用である（[AI-DLC v2 Operation Phase Boundary](../aidlc-v2-operation-phase-boundary.ja.md) を参照）。
security-patch の deployment 系ステージと infra の provisioning 系ステージもこの理由付き skip で扱い、必要な場合は Intent の成果物にデプロイ手順を記録して人間に委ねる。
...
コンパイル済み scope grid の数字（bugfix 7/32、refactor 8/32、poc 8/32 など）は v2 と同一であり、差は workspace steering が Operation ステージを理由付き skip で処理する運用にある（ステージ数の削減ではない）。
