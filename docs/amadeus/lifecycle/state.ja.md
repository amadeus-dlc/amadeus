...
承認と遷移の履歴は、record の `audit/` shard（clone ごとの `audit/<host>-<clone>.md`）が追記専用イベントとして持つ。
...
Operation の 7 stage の実行可否は scope（scope-grid）と workspace steering で決まる。default space は steering により対象外とし、該当ステージを `[S]`（`SKIP: out of Amadeus scope`）で処理する。現行の validator は、全 workspace で Operation の全ステージに `[S]` を追加で要求する（Operation の実行は将来採用である）。
...
承認記録は `amadeus-state.md` に持たず、`audit/` の shard のイベントが持つ。
...
`amadeus-state.md` と `audit/` の shard は追記型の生きた台帳であり、phase PR や Bolt PR の断面ごとに状態を巻き戻さない。
...
Operation ステージは scope-grid 上で scope 別に宣言される（CONDITIONAL 採用）が、現行の validator は全 workspace で Operation の全ステージに `[S]` を要求する — Operation の実行は将来採用であり、default space はさらに steering でも対象外と定める。
...
人間向けの Intent 一覧が必要な場合は、正準台帳 `intents.json` から都度生成する（[overview.md](overview.ja.md) を参照）。
...
- scope の実行対象外のステージと Operation の全ステージが `[S]` である（Operation の実行は将来採用である。[AI-DLC v2 Operation Phase Boundary](../aidlc-v2-operation-phase-boundary.ja.md) を参照）。
- `[x]` のステージに、契約が必須とする成果物と `STAGE_COMPLETED` イベントが存在する。
- 必須入力の供給ステージが `[S]` の場合、後続ステージが [scopes.md](scopes.ja.md) の縮退時の入力代替に従っている。
