...
scope は Intent の `state.json` に記録し、ステージ解決の入力にする。
...
v2 のコンパイル済み scope grid（bugfix 7/32、refactor 8/32、poc 8/32 など）との差は、Amadeus が対象外にした initialization 3 ステージと operation 7 ステージの分である。
...
`state.json` の Unit 単位ステージと Bolt の記録では、識別子として `implicit` を使う。
