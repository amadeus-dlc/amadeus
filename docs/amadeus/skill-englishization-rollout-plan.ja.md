# Skill Englishization Rollout Plan（skill 段階的英語化ロールアウト計画）
...
Issue #395 の [Skill Language Policy](skill-language-policy.ja.md)、Issue #400 の代表 skill 英語化、Issue #401 の [AI-DLC v2 Difference Response Plan](aidlc-v2-difference-response-plan.ja.md) を前提にする。

注記: 以下の表の skill 名は、この計画を書いた時点（Issue #402）のものである。その後、phase 接頭辞なしの `amadeus-<stage>` 形式への改名または退役が行われた。現在の一覧は [skills/amadeus/references/stage-catalog.md](../../skills/amadeus/references/stage-catalog.md) を参照する。
