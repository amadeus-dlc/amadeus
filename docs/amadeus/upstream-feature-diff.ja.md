...
- Amadeus: 本リポジトリ（`skills/amadeus*`、`.agents/amadeus/` エンジン、`harness/`、validator、インストーラ）
...
| 対話提示 | — | ハーネスの question rendering | `amadeus-grilling` へ結線（1 問ずつ、推奨回答つき）= 38 stage skill の宣言済み適応点 | `AMADEUS.md`（skill 適応方針）、`skills/amadeus-grilling/` |
...
| Codex guard ファイル | 対象外 | `dist/codex/.agents/skills/aidlc-*/agents/openai.yaml`（38 件、生成物） | 38 件を `skills/amadeus-*/agents/openai.yaml` へ採用（+ 昇格コピー）。guard 内容の同一性は sha256 で検証済み | `harness/codex/provenance.md`（写像表 + ハッシュ） |
