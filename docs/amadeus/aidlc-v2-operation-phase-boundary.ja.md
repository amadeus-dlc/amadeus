...
| Incident Response | 対象外。インシデントからの学びは、Issue、PR、CI 結果を入力として §13 learnings ritual（`amadeus-learnings.ts`）で取り込める。対応作業そのものは扱わない。 |
...
| Feedback & Optimization | 対象外。フィードバックの受け口は `amadeus` Intake の合流判定とスコープバックログ、および §13 learnings ritual である。改善の実施は新しい Intent として起票する。 |
