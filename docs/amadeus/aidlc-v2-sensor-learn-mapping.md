...
- sensor 宣言: 各 stage 定義の frontmatter `sensors:`、Learn: `core/aidlc-common/protocols/stage-protocol.md` §13

## 判断

Amadeus DLC は、sensor 実行機構（`.aidlc-sensors/` への検査結果出力、sensor-fire hook）と、learnings ritual の決定論ツール（`aidlc-learnings.ts` 相当）を採用しない。
...
| `linter` | プロジェクトの linter 実行（結果を `<record>/.aidlc-sensors/` へ出力） | Construction の設計 4 stage、code-generation、ci-pipeline |
...
`.aidlc-sensors/` 相当の検査結果ディレクトリは追加しない。検査結果の記録先は、構造検証が validator の結果報告、build とテストが Bolt record の `build-test-results.md` である。
...
| 項目 | 理由 |
|---|---|
| sensor 実行機構（sensor-fire hook、`.aidlc-sensors/` 出力） | 配布契約（単一公開入口と skill 一式）に hook 実行基盤を追加しない。決定論的検査は `amadeus-validator` と Build and Test、CI が既に担う。 |
| learnings ritual の決定論ツール（`aidlc-learnings.ts` 相当） | 候補の表面化と分類は `amadeus-history-review` と `amadeus-learning-review` の契約で担い、定着は人間 gate を経る既存契約を維持する。 |
