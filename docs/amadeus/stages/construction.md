...
Functional Design は、詳細な Domain Model と契約の管理元である。
Aggregate、Entity、Value Object、Domain Service、Domain Event、事前条件、不変条件、事後条件は、対象 Unit の Functional Design で扱う。

Functional Design が承認済みになった後、共有境界として採用する内容だけを Domain Map と Context Map へ反映できる。
Domain Map と Context Map は、候補ではなく `adopted` または `retired` の現在の索引を扱う。
...
8. Functional Design が `passed` になった場合、共有境界として採用する内容を Domain Map と Context Map へ反映できる。
9. Domain Map と Context Map へ反映する場合は、Functional Design、Construction の判断、追跡のいずれかを根拠として残す。
...
| `.amadeus/domain-map.md` | 共有境界として採用する内容がある場合だけ更新する現在の索引 |
| `.amadeus/context-map.md` | コンテキスト間依存として採用する内容がある場合だけ更新する現在の索引 |
...
この契約定義の後続作業は、次の issue に分ける。

1. Steering の template と skill から旧 `domain/**` 生成を外す。
2. Inception の Bounded Context 参照契約を Domain Map 前提へ移す。
3. Construction から Domain Map と Context Map を更新する契約を追加する。
4. validator と eval を新契約へ移行する。
5. examples を後方互換なしで再生成する。
