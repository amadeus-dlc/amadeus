...
Inception phase は、Ideation 完了済み Intent を、必要な場合の Codebase Analysis、Requirement、必要な場合の User Story、Use Case、Unit、Unit Design Brief、Bolt へ分解する phase である。
...
Codebase Analysis は、既存コードに載せる Intent の場合だけ充足対象に含める。

greenfield の Intent では `state.json.inception.codebaseAnalysis` に対象外理由を残し、`codebase-analysis.md` を必須成果物に含めない。
...
|---|---|---|---|---|---|
| 2.0 | Codebase Analysis | CONDITIONAL | 既存コードに載せる Intent の場合 | `amadeus-inception-codebase-analysis` | `codebase-analysis.md`、`state.json.inception.codebaseAnalysis` |
...
| Condition | 既存コードに載せる Intent の場合 |
| Lead Skill | `amadeus-inception-codebase-analysis` |
| Mode | internal |

### Purpose

Codebase Analysis は、既存コードに載せる Intent で、既存能力、統合点、ギャップ、リスク、証拠、鮮度を整理する。

この stage は AI-DLC v2 の Reverse Engineering 相当を Amadeus DLC の小さい成果物契約へ写したものである。

Impact Mapping は独立 stage にしない。

既存コードとの対応は Codebase Analysis Gate と `traceability.md` の `既存コード分析からの追跡` で扱う。
...
1. 対象 Intent が既存コードに載るかを判定する。
2. 既存コードに載る場合は `state.json.inception.codebaseAnalysis.requirement` を `required` にする。
3. greenfield、既存コードなし、対象外の場合は `requirement` を `not_required`、`status` を `skipped` にし、`skipReason` を残す。
4. 対象コード、既存能力、統合点、ギャップ、リスクを `codebase-analysis.md` に記録する。
5. 判断に使ったファイル、実行結果、コミット、確認日時を証拠と鮮度として記録する。
6. Unit Design Brief に渡す入力を `Inception への入力` に整理する。
7. 必要な入力が不足している場合は `status` を `blocked` にし、`blockedReason` を残す。

### Outputs

| Artifact | Description |
|---|---|
| `inception/codebase-analysis.md` | 既存コード分析 |
| `state.json.inception.codebaseAnalysis` | Codebase Analysis の必要性、状態、証拠、対象範囲、鮮度 |
...
後続 stage は brownfield 判定を個別に持たず、この状態を参照する。

### Approval Gate

brownfield で Unit Design Brief を作る場合は、Codebase Analysis Gate を通す。

`requirement` が `required` で `status` が `passed` または `ready_for_approval` の場合は、`codebase-analysis.md`、`evidence`、`targetScope` を揃える。

### Notes

AI-DLC v2 の9成果物は optional detail の候補として扱い、初期導入では必須化しない。

詳細成果物が必要な場合だけ、`inception/codebase-analysis/` 配下に置く。
...
- `inception/codebase-analysis.md`、必要な場合
...
6. `state.json.inception.codebaseAnalysis.requirement` が `required` の場合は、Codebase Analysis Gate を通してから Unit Design Brief を作る。
7. brownfield の場合は、既存能力、統合点、ギャップを読んでから Unit Design Brief を作る。
8. Unit の `コンテキスト` は Domain Map の adopted Bounded Context、または Inception で採用判断する境界候補として扱う。
9. 既存 Boundary を参照する場合、Unit の `コンテキスト` は Domain Map の adopted Bounded Context を参照する。
10. 新規 Boundary を採用する場合、Traceability Finalization で Domain Map へ adopted Bounded Context として反映し、`根拠` は Inception の判断を指す。
11. Domain Map に adopted Bounded Context として反映できない場合は、`state.json.inception.gate` を `passed` にしない。
12. 対応する Bounded Context が未確認の場合は、推測で Intent 固有の Domain Model 成果物を作らず、未確認事項として残す。
...
2. `既存コード分析からの追跡` では、`codebase-analysis.md`、Requirement、Use Case、Unit、Bolt、Unit Design Brief を接続する。
3. greenfield で `codebase-analysis.md` を作らない場合は、`既存コード分析からの追跡` に対象外理由を残す。
4. Inception の境界、粒度、対象外、greenfield または brownfield の判断を `decisions.md` と `decisions/**` に残す。
5. `state.json.phase` を `inception` にし、Inception の必須成果物と `state.json.inception.codebaseAnalysis` を反映する。
6. Story が不要な場合は、`state.json.inception.requiredStoryArtifacts` を空配列にする。
7. 既存 Boundary を参照する場合、Unit から参照する Bounded Context が Domain Map で adopted であることを確認する。
8. 新規 Boundary を採用する場合、Domain Map へ adopted Bounded Context として反映し、`根拠` に Inception の判断を置く。
9. Unit から参照する Bounded Context が Domain Map で adopted ではない場合、`state.json.inception.gate` は `not_ready` にする。
10. Bounded Context と Unit 参照が採用済み Boundary として確定している場合は `passed` にしてよい。
11. validator で対象 Intent を検証する。
