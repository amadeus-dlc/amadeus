...
Inception phase は、Ideation 完了済み Intent を、Requirement、User Story、Use Case、Unit、Unit Design Brief、Bolt へ分解する phase である。

この phase は、Construction で Task 化できるだけの要求、アクター価値、システム相互作用、Unit、Bolt、追跡を固定する。
...
| 2.2 | User Stories | ALWAYS | 要件定義済みの場合 | `amadeus-inception-user-stories` | `user-stories.md`、`user-stories/**` |
| 2.3 | Use Cases | ALWAYS | User Stories 作成済みの場合 | `amadeus-inception-use-cases` | `use-cases.md`、`use-cases/**` |
...
User Stories は、Requirement からアクター価値を切り出す。
...
2. Story が Requirement の言い換えだけになっている場合は、粒度不足を疑う。
3. 自然な粒度であれば、後続の追跡確定で理由を残せるように根拠を本文に残す。
...
| Phase | Inception |
| Execution | ALWAYS |
| Condition | `user-stories.md`、`user-stories/**` が存在する |
| Lead Skill | `amadeus-inception-use-cases` |
| Mode | internal |

### Purpose

Use Cases は、User Story からシステムとの相互作用を切り出す。
...
1. User Story から、利用者とシステムの相互作用を Use Case として切る。
2. Use Case が Requirement の箇条書きになっている場合は、相互作用として書き直す。
...
Use Cases の結果が User Story または Requirement の意味を変える場合は、前段の stage に戻す。
...
5. Bounded Context と Unit 参照が確定している場合は `passed` にしてよい。
