...
**Discovery ID** は Discovery ディレクトリ名全体である。
...
`詳細` は `discoveries/<discovery-date>-<slug>/brief.md` を指す。
...
入力テーマ、確認した前提、判定理由は `brief.md` に置く。
...
Intent 候補の進行状態は `brief.md` の Intent 候補表に置く。
...
Intent 候補の状態と Intent へのリンクは `brief.md` の表で管理する。
...
`state.json.decision` と `brief.md` の `判定` は一致させる。
...
`decision: single_intent` の場合、`brief.md` に Intent Draft を含める。
...
`decision: multi_intent` の場合、`brief.md` に複数の Intent 候補を置く。
...
作成済みなら `../../intents/<intent-id>/intent.md` への相対リンクを書く。
...
- `brief.md`：`decision: single_intent` では、`推奨次アクション` に作成済み Intent へのリンクと次に使う phase skill を書く。
- `brief.md`：`decision: multi_intent` では、対象候補の `状態` を `initialized` にし、`Intent` に作成済み Intent へのリンクを書く。
- `.amadeus/discoveries.md`：対象 Discovery の `状態`、`判定`、`推奨次アクション`、`詳細` を `state.json` と `brief.md` に合わせる。
- `state.json`：`status`、`decision`、`gate` を再判定する必要がある場合だけ更新する。

`state.json` に Intent へのリンクや候補状態は持たせない。

Intent 作成、Discovery `brief.md` 更新、`.amadeus/discoveries.md` 更新、必要な `state.json` 更新は一体の操作として扱う。
いずれかを完了できない場合は停止し、部分更新を成功として扱わない。

## Validator 方針

`amadeus-intent-validator` は Discovery layer も検証対象にする。
...
- `詳細` リンクが存在する。
- `.amadeus/discoveries.md` の対象行が `state.json` と `brief.md` に対応している。
- 各 Discovery に `brief.md` と `state.json` がある。
- `state.json.phase` が `discovery` である。
- `state.json.status` が許可値である。
- `state.json.gate` が許可値である。
- `state.json.decision` が許可値である。
- `state.json.decision` と `brief.md` の `判定` が一致する。
