...
**Discovery ID** は Discovery のモジュールファイルと Discovery のモジュールディレクトリの stem 全体である。
...
入力テーマ、確認した前提、判定理由は `Discovery のモジュールファイル` に置く。
...
Intent 候補の進行状態は `Discovery のモジュールファイル` の Intent 候補表に置く。
...
Intent 候補の状態と Intent へのリンクは `Discovery のモジュールファイル` の表で管理する。
...
`Discovery のモジュールファイル` は、人間向けの判断記録である。
...
`decision: single_intent` の場合、`Discovery のモジュールファイル` に Intent Draft を含める。
...
`decision: multi_intent` の場合、`Discovery のモジュールファイル` に複数の Intent 候補を置く。
...
必要な場合は `Discovery のモジュールファイル` に調査すべき論点を残す。
現時点では `.amadeus/research/**` は作らない。

`no_intent` は、Amadeus の Intent lifecycle に載せる価値がない入力に使う。
ただし Discovery を起こした場合は、Intent にしなかった理由を `Discovery のモジュールファイル` に残す。
...
- `Discovery のモジュールファイル`：`decision: single_intent` では、`推奨次アクション` に作成済み Intent へのリンクと次に使う phase skill を書く。
- `Discovery のモジュールファイル`：`decision: multi_intent` では、対象候補の `状態` を `initialized` にし、`Intent` に作成済み Intent へのリンクを書く。
- `.amadeus/discoveries.md`：対象 Discovery の `状態`、`判定`、`推奨次アクション`、`詳細` を `state.json` と `Discovery のモジュールファイル` に合わせる。
- `state.json`：`status`、`decision`、`gate` を再判定する必要がある場合だけ更新する。

`state.json` に Intent へのリンクや候補状態は持たせない。

Intent 作成、Discovery のモジュールファイル更新、`.amadeus/discoveries.md` 更新、必要な `state.json` 更新は一体の操作として扱う。
...
- `詳細` リンクが存在する。
- `.amadeus/discoveries.md` の対象行が `state.json` と `Discovery のモジュールファイル` に対応している。
- 各 Discovery に `Discovery のモジュールファイル` と `state.json` がある。
- `state.json.phase` が `discovery` である。
- `state.json.status` が許可値である。
- `state.json.gate` が許可値である。
- `state.json.decision` が許可値である。
- `state.json.decision` と `Discovery のモジュールファイル` の `判定` が一致する。
