...
Issue #395 の [Skill Language Policy](skill-language-policy.ja.md) と、Issue #400 の代表 skill 英語化を前提にする。
...
| 4 | #394 Operation phase の対象外理由 | 境界の明確化 | #394 時点で、Operation phase は Amadeus DLC の対象外として扱われており、#391、#392、#393 の Construction までの判断後に境界を明文化するのが自然であったため（現行の 2 層構造は [AI-DLC v2 Operation Phase Boundary](aidlc-v2-operation-phase-boundary.ja.md) を参照）。 | Operation phase を対象外にする理由を、成果物契約、gate、validator、PR 境界の観点で説明する。Operation skill の追加や取り込みは含めない。 |
...
| #392 | Build and Test の失敗時処理が従う契約。`build-test-results.md`（本家では `test-results.md`）に残す内容。Bolt gate との関係。 |
| #394 | #394 時点で Operation phase が対象外として扱われた理由。AI-DLC v2 Operation skill 一覧と Amadeus 側の扱い。将来対応する場合の別 Issue または roadmap。 |
