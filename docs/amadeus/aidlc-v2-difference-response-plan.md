...
| 2 | #393 sensor と Learn の写像 | 意味変更の判断 | sensor と Learn は、#391 の reviewer 判断と同じく、stage 完了前後の確認と知見記録に関わるため。 | sensor ごとの検証先を `amadeus-validator`、build and test、stage 固有成果物へ写像する。Learn は `memory.md`、`decisions.md`、`traceability.md`、`grillings` のどこへ記録するかを明記する。`.aidlc-sensors/` 相当の仕組み追加は含めない。 |
