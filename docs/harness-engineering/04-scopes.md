...
A scope is the dial that decides *which* of the framework's 32 stages run for a given kind of work, and which sit out. A fix doesn't need market research or a deployment pipeline; a regulated enterprise feature needs all of it. Rather than asking the user to hand-pick stages every time, AI-DLC ships a catalog of named scopes — each one a curated EXECUTE/SKIP verdict over the full stage set, paired with a default depth and test strategy. Pick the scope and the rest cascades.
...
For the full scope catalog with use cases and the routing table users read, see [Scopes, Depth, and Test Strategy](../guide/05-scopes-and-depth.md) in the User Guide. This chapter is the authoring side of that same data.
...
|-------|----------|--------------|
| `name` | Yes | The scope name. Must equal the filename stem (`amadeus-feature.md` → `feature`). |
...
4. **Verify the scope resolves and is accepted.** Run `/amadeus --doctor`. Then confirm an init under the new scope produces a state file with the right `Scope:` line, and that it's accepted as an env default and as a mid-workflow `--scope` change.
...
7. **(Optional) Generate a typeable runner.** A scope is fully usable via `/amadeus --scope <name>` the moment its file lands — no runner needed. If you want a one-word command (`/amadeus-hotfix`), generate a runner skill: `bun .claude/tools/amadeus-runner-gen.ts scopes --all` emits `skills/amadeus-<scope>/SKILL.md` for every scope file (or edit the generator's `FIRST_BATCH` to ship a curated subset). Each runner is a ~6-line shell that drives `amadeus-orchestrate next --scope <name>` to `done` with the scope baked in; the runner packages an already-runnable scope, and the scope file is its definition. It carries no `hooks:` block: the deterministic spine (audit, sensors, runtime-compile, state validation) is registered project-wide in `settings.json`, so every runner inherits it for free. Re-run the generator (or `scopes --check`) whenever you add or rename a scope file.
...
- If you gave it `keywords`, freeform `/amadeus <text>` auto-detects it as soon as the frontmatter list is populated — no SKILL.md prose edit, just the table regeneration.
...
For the normative scope-shape and runner contract — how a `.claude/scopes/` file drives which stages a workflow visits, and how the generator turns it into a typeable `/amadeus-<scope>` skill — see [Skill System §5 (scope shape) and §4 (runners)](../reference/17-skill-system.md) in the Developer Reference.
