...
4. **Make changes** -- Edit the harness-neutral source in `packages/framework/core/` (tools, stages, agents, hooks, rules, knowledge) or a harness surface in `packages/framework/harness/<name>/` (the orchestrator skill, settings). Then run `npm run dist` to regenerate `dist/` — never hand-edit `dist/`, the drift guard (`npm run dist:check` / `package.ts --check`) will fail CI
