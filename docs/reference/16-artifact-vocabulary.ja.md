...
権威あるソースは、すべてのステージファイルにある `produces[]` フィールドです。
`dist/claude/.claude/tools/amadeus-graph.ts` のヘルパーがコンパイル済みステージグラフを読み取り、
その和集合をセットとして返します — これはスコープ(`amadeus-lib.ts:772` の `validScopes()`)や
エージェント(`amadeus-lib.ts:794` の `loadAgents()`)で使われるのと同じパターンです。レジストリを
この章の外に置くことで、並行して手作業で維持されるリストが招くドリフトを防ぎます。
...
他のステージは `consumes[]` で同じ識別子を参照し、読み取り依存を宣言します。識別子は短い
ケバブケース文字列です — ファイル拡張子なし、フォルダプレフィックスなし、スラッシュなし。
...
- **状態のプラミング。** `amadeus-state.md`、`audit.md`、`.amadeus-recovery.md` は
  ツール(`amadeus-state.ts`、フックスクリプト)によって管理され、ステージが `produces[]` 経由で
...
1. **ステージファイルが権威。** 各ステージの `produces:` リストは、そのステージが
   発行するすべての正規名を宣言します。`consumes:` はそのステージが依存する正規文字列を
   指定します。
2. **レジストリは記述されるのではなく計算される。**
   ...
   `stage-graph.json` からすべてのステージの `produces[]` を和集合にします。
...
32個のステージファイルすべてが `produces:` を宣言するため、導出は完全なレジストリを返します。
ツールは空データに対しても well-defined です — `produces:` を持たないステージは単に何も
寄与しません — が、出荷されるフレームワークではすべてのステージが populated です。
...
2つのステージが `produces[]` リストで同じ正規名を宣言することは**禁止**されています。
レジストリはセットであり、名前はグローバルに一意でなければなりません。同じ基礎概念が2つの
ステージによって発行される場合、曖昧さを解消する2つの別々の名前を選びます。
...
ことがあります。ファイルシステムではなく、ステージの `produces:` リストと
`bun amadeus-graph.ts artifacts` を真実のソースとして使ってください。
...
1. 生成ステージの `.md` ファイルを編集し、その `produces:` リストに正規名を追加する。
...
1. 生成ステージの `produces:` エントリでリネームする。
