...
`intent-mirror.github.project.targets`は`{ project, phase-field?, status-names? }`の配列です。`project`は`"<owner>/<number>"`に一致し、numberは正の整数です。ゼロ埋め・小数・その他の不正値はcoerceせず拒否します。`phase-field`は空でないfield名で、既定値は`Intent Phase`です。`status-names`のキーはclosedなphase語彙`ideation | inception | construction | operation | done`で、値は空でない文字列です。未知の要素キー、未知のphaseキー、あるいは1要素の不正は、部分的なリストを作らずその層全体を拒否します。`intent-mirror.github.issue.consent`と`intent-mirror.github.project.targets`は独立に解決され、キーごとに有効な値を持つ最後の層が勝ち、勝った`intent-mirror.github.project.targets`は前の層の対象リストを全置換します。
