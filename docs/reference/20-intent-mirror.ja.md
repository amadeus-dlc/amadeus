...
`phase-field`が指すfieldをlifecycle reconcileとcompletion gateの正本とします。`Status`は補助同期であり、進行中は`In progress`、完了時は`Done`、parked中は現在値を維持し、補助同期の失敗はreconcileやcloseを阻害しません。
