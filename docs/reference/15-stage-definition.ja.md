...
> 意味 **しません**。スコープが生成側を除外する場合(例: `fix` は
...
`produces:` と同じ kebab-case の文字列リストです。Unit によっては生成するものの、
すべての Unit には要求しない成果物を指定します。条件付きの出力がないステージでは、
このフィールドを省略します。

Unit ごとの完了判定が確認するのは `produces[]` だけです。この区別により、
`functional-design` はバックエンドだけの Unit で `frontend-components` を省略でき、
`infrastructure-design` は共有インフラがない場合に `shared-infrastructure` を省略できます。
ステージの承認ゲートへ進むためだけに、N/A と書いたダミーファイルを作る必要はありません。

任意の成果物も、それ以外の処理では通常の成果物として扱います。

- run-stage directive は、解決済みの必須パスと任意パスをすべて
  `directive.produces` に含め、条件付きのパスだけを
  `directive.optional_produces` にも含めます。必須パスは生成が必要です。
  任意パスは、対応する `CONDITIONAL` 指示を満たす場合だけ生成する候補です。
- `artifactsRegistry()` と `producersOf()` は両方のリストを参照します。
- required-sections センサーは、実際に生成された任意の成果物もテンプレート検証の
  対象に含めます。

各 `optional_produces` には、どの条件で生成するかを示す `CONDITIONAL` の指示を、
ステージ本文と `outputs:` の両方に記載してください。このフィールドを使えるのは、
Unit によって本当に不要となる成果物だけです。必須の成果物を移すと、決定論的な
Unit 完了判定の対象から外れてしまいます。各出力リスト内で同じ成果物を重複させず、
必須と任意の両方に同じ成果物を指定しないでください。
