...
[Stage Protocol](04-stage-protocol.md) を補完するものです。本章はステージファイルが
...
です。[Contributing: Adding an Agent](11-contributing.md#adding-an-agent) を参照。
...
フロントマター([Agent System](05-agent-system.md#frontmatter-contract) を参照)は
...
予約された名前空間のパターンは、監査タクソノミー([State Machine](12-state-machine.md))に
...
- `dist/claude/.claude/amadeus-common/protocols/stage-definition.md` — 本章が
  ナラティブ化する権威ある仕様。
- [Stage Protocol](04-stage-protocol.md) — ランタイム実行の挙動。
- [Agent System](05-agent-system.md) — エージェントファイルの並行する YAML ファースト
  契約。
- [State Machine](12-state-machine.md) — ステージ実行が監査イベントを emit する場所。
