...
- ステージスキップのセマンティクス(`PHASE_SKIPPED` イベント)。`tests/integration/t39.test.ts` は既知の 9 個のスコープ名をスコープごとのループにハードコードしています — 新しいスコープはそのリストが拡張されるまで実行されません。同じ PR の一部として、新しいスコープをそのループに追加してください。
