...
canonical execution event set that crossed the StartPermit projection barrier.
The runtime-state digest selects that committed event set, and the root/digest
pair is copied from the audit journal rather than recomputed from runtime state.
Recompiling the runtime graph therefore preserves the same required-projected
cursor instead of advancing to a newer, not-yet-projected canonical event.
...
- **The audit log this graph is derived from** - the event taxonomy
