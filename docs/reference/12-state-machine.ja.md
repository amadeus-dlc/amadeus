...
- [Orchestrator](03-orchestrator.md) — `/amadeus --status`、session チェック、resume パスがステートマシンのシグナルをどう消費するか。
- [Stage Protocol](04-stage-protocol.md) — `[?]` / `[R]` 遷移を駆動する承認ゲート UX を含む、ステージレベルの挙動契約。
- [Hooks and Tools](06-hooks-and-tools.md) — フックのライフサイクル、CLI ツールリファレンス、監査イベントカタログ。
- [Testing](09-testing.md) — ドリフトテストの仕組みと実行タイミング。
