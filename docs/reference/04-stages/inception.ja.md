...
| fix           | 2.1(ブラウンフィールドの場合 -- バグの特定), 2.3(最小限 -- バグの記述) |
| chore            | なし(Inception は完全にスキップ)                              |
| refactor         | 2.1(ブラウンフィールドの場合 -- 現行コードの理解), 2.3(最小限) |
...
- fix および refactor スコープでは、既存コードの理解が不可欠なため、ブラウンフィールドでは常にこのステージが実行される。グリーンフィールドではスキャン対象の既存コードが無いためスキップされ、Requirements Analysis(2.3)へ直行する。
...
| Phase            | Inception                                                              |
| Stage #          | 2.3                                                                    |
| Condition        | ALWAYS -- エンジンが解決した深度が成果物の分量をスケールする           |
...
Requirements Analysis は、ユーザーの intent とリバースエンジニアリングによるコードベース理解を、正式で構造化された要件へと変換する。リクエストの明確さ、タイプ、スコープ、複雑さを評価し、エンジンが解決した深度を確認し、既知の事項を抽出し、6つの次元にわたる完全性分析を実行し、明確化のための質問を生成し、正式な要件文書を生成する。

このステージは常に実行される。深度は run-stage ディレクティブの `directive.depth` フィールド(フォールバック: `amadeus-state.md` の `**Depth**`)としてエンジンから配達され、生成する要件の分量をスケールする。複雑さの評価は advisory であり、解決済み深度と大きく食い違う場合はステージ自身が深度を変更せず `--depth` オーバーライドを提案する。
...
4. **深度の確認** -- run-stage ディレクティブの `directive.depth` フィールドから解決済みの深度を読む(フォールバック: `amadeus-state.md` の `**Depth**`)。深度の権威はエンジンであり(stage-protocol.md §8)、ステージが複雑さの評価から再導出することはない。評価と解決済み深度が大きく食い違う場合は、`--depth` によるオーバーライドを提案する1行のアドバイザリーを提示するにとどめ、ステージ自身が深度を変更することはない。
...
- 深度はスコープの既定値としてエンジンが解決する: fix/poc には最小限、feature には標準、enterprise には包括的。
- fix スコープでは、このステージが最小限の深度でバグの記述を捉える。
