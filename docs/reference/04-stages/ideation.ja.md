...
5. **成果物の生成** -- intent ステートメントとステークホルダーマップを生成する。Issue 起点の intent では、`issue-evidence fetch --issues <n[,n...]>` を実行して起票 Issue とそのクロスレビューコメントを record へ取り込む。`gh` の失敗は非0終了で何も書かず、ステージは自由文のリクエストで続行する。
...
| `issue-evidence.md`           | オプション(`optional_produces`)。起票 Issue 本文とクロスレビューコメントの逐語+取得 provenance — ステージ 2.1 / 2.3 が再導出せず消費する上流入力 |
