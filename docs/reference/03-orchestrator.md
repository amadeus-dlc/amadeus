...
> Languages: **English** | [日本語](03-orchestrator.ja.md)
...
### Workspace management commands -- List, create, and switch

The `space`, `space-create`, and `intent` commands manage spaces and intents. Each command finishes after completing the requested operation; it does not run or advance a workflow stage.
...
| `fix` | 0.1-0.3, 2.1 (always), 2.3 (minimal), 3.5, 3.6 | 7 / 32 | Minimal | Minimal |
...
- **fix** -- No Ideation. Reverse Engineering always included (to find the bug) plus minimal Requirements Analysis. Code Generation and Build and Test only.
...
- **refactor** -- No Ideation. Same Inception start as fix. Adds Functional Design (as refactoring plan).
...
|---|---|---|
| Minimal | poc, fix, chore, refactor, security-patch | Minimal artifacts, brief analysis, optional stages skipped |
...
Autonomy is selected for the whole Intent as `none`, `semi`, or `full`; it is not selected by a post-walking-skeleton ladder. In `none`, humans decide stage gates, phase gates, and questions. In `full`, a human-issued Intent-scoped grant may decide all four interaction kinds through Intent completion — but only where the derivation singles out one option: a ruling point reserved to the user, or a derivation that ends contested or empty, goes to a person in an interactive session and into waiting in a non-interactive one. `semi` is `full` minus two human milestones, the phase boundary and the walking skeleton; everything else, including the Bolt swarm's batch boundaries, runs unattended (recorded as `AUTO_DECIDED`). The walking-skeleton milestone fires only where the Skeleton Stance resolves `on`. Legacy standing grants and `AUTONOMY_MODE_SET` records remain available for replay and diagnosis but confer no authority.
