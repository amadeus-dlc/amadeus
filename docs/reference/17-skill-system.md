...
1. **Identity** lives in one file per scope at `dist/claude/.claude/scopes/amadeus-<name>.md` — frontmatter (`name`, `depth`, `keywords`, `description`) plus prose describing the scope. The shipped set is `bugfix`, `enterprise`, `feature`, `infra`, `mvp`, `poc`, `refactor`, `security-patch`, `workshop`.
...
|------------|------|-------|
| `prepare --batch <n> --units <a,b,c> [--base <branch>] [--degraded-from <subagent\|ultracode>]` | Fork an isolated git worktree per unit (composing `amadeus-worktree create` + `amadeus-bolt start --worktree`). Runs before any worker, so it cannot fold into `check`. | `SWARM_STARTED` (plus `SWARM_DEGRADED` when a loud downgrade is reported). |
...
**The driver seam.** `AMADEUS_USE_SWARM=1` selects an inline Dynamic Workflow driver (the conductor authors a `Workflow` whose JS owns the per-unit pipeline and the iteration cap); unset selects the subagent floor (N parallel `Task` calls in one message, one per unit). If `=1` but the Workflow tool is unavailable, the conductor **loud-degrades** to the floor and passes `--degraded-from ultracode` so the referee emits `SWARM_DEGRADED`. The runaway backstop is not a cap inside the tool - it is the harness's Stop-hook ceiling, which is 8 blocks on this autonomous-Construction path (§3).
