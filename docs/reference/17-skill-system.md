...
| `finalize --batch <n> --units <a,b,c> --claimed <a,b> --check-cmd <cmd> [--test-file <path>] [--reasons <unit>=<reason>,…] [--target <branch>] [--base <branch>] [--strategy <squash\|merge\|rebase>] [--repo <name>]` | The authoritative gate: **re-run the check on every claimed unit** before any merge (a unit named in `--claimed` but red on disk is refused the merge and lands in the failure envelope — the lying-conductor guard), then serialised HOLD-MERGE merge-back of the genuine passes: AIDLC data (`complete --merge`) then Git source (`amadeus-worktree merge`). A genuine unit requires `--target` or `--base` (prepare's base). A unit is not converged unless both land. Exits 0 (batch converged and merged) or 2 (failure envelope). | `SWARM_UNIT_CONVERGED` / `SWARM_UNIT_FAILED` / `SWARM_BATON_RETURNED` / `SWARM_COMPLETED`. |
