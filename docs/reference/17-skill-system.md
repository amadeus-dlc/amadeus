...
|------------|------|-------|
| `prepare --batch <n> --units <a,b,c> [--base <branch>] [--concurrency <1..4>] [--degraded-from <subagent\|claude-ultra\|codex-ultra>]` | Resolve `max-parallel-units`, initialize the canonical FIFO pool, and fork an isolated git worktree per submitted Unit. | `SWARM_STARTED`, `UNIT_POOL_EVENT_SET_COMMITTED` (plus `SWARM_DEGRADED` when a loud downgrade is reported). |
