...
| `invoke-swarm` | Yes | The engine granted an eligible Construction batch to the swarm. The conductor fans out the units in `directive.units` and runs the convergence loop, consulting the swarm referee (see §6). Emitted for an eligible Construction batch under an `autonomous` OR `gated` grant; under `gated` the engine emits an `ask` batch-end gate between batches, which `amadeus-bolt approve-batch --batch <n>` clears. |
...
1. **Identity** lives in one file per scope at `dist/claude/.claude/scopes/amadeus-<name>.md` — frontmatter (`name`, `depth`, `keywords`, `description`) plus prose describing the scope. The shipped set is `bugfix`, `chore`, `enterprise`, `feature`, `infra`, `mvp`, `poc`, `refactor`, `security-patch`, `workshop`.
...
|------------|------|-------|
| `prepare --batch <n> --units <a,b,c> [--base <branch>] [--degraded-from <subagent\|claude-ultra\|codex-ultra>]` | Fork an isolated git worktree per unit (composing `amadeus-worktree create` + `amadeus-bolt start --worktree`). Runs before any worker, so it cannot fold into `check`. | `SWARM_STARTED` (plus `SWARM_DEGRADED` when a loud downgrade is reported). |
...
**The driver seam.** `AMADEUS_USE_SWARM` is a three-value enum — its only valid states are **unset**, `claude-ultra`, and `codex-ultra`. The conductor resolves the value against the running harness once per batch (via `amadeus-swarm.ts resolve --harness <self>`) before any worktree or spawn, and the resolution is a static function of `(value, harness)`. **unset** selects the subagent floor (N parallel `Task` calls in one message, one per unit) on every harness. `claude-ultra` selects the inline Dynamic Workflow driver on the Claude harness (its JS owns the per-unit pipeline and the iteration cap), and `codex-ultra` selects native fan-out at reasoning effort=ultra on the Codex harness. An ultra value requested on a harness that is not its native one **loud-degrades** to the subagent floor with `--degraded-from <requested>`, so the referee emits `SWARM_DEGRADED` (the requested value is preserved in the audit). Every other value — the legacy `1`, the empty string, any unrecognised string — is **rejected fail-closed** before any worktree, spawn, or `SWARM_STARTED`; there is no compatibility shim. The runaway backstop is not a cap inside the tool - it is the harness's Stop-hook ceiling, which is 8 blocks on this autonomous-Construction path (§3).
