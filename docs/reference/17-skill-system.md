...
| `done` | Yes | Branch on `directive.terminal`. `true` — the workflow (or single-stage run) is complete: present the completion summary and STOP. `false` — the `report` committed a mid-workflow transition and state advanced; the loop continues with the next `next`. The field is required, so a `done` is never a stop signal by `kind` alone (#2762). |
...
Text description of the diagram: `next` (passed `$ARGUMENTS` verbatim) returns one directive. The conductor branches on `directive.kind`. For `run-stage`, `ask`, `invoke-swarm`, and run-then-continue `print` directives it performs the named move and calls `report`, which loops back to `next`. For terminal `print`, `error`, and a `done` whose `terminal` is `true` it stops the loop; a `done` whose `terminal` is `false` is the report's acknowledgement that a mid-workflow transition committed, and the loop returns to `next`.
