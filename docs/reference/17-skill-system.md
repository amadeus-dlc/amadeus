...
| `check <unit> --check-cmd <cmd> [--test-file <path>]` | Stateless single-unit verdict: run the project's own check command (exit 0 = green, the authoritative signal — a worker's self-claim is never trusted) plus an anti-tamper compare of the protected file against the prepared fork SHA. Prints `{converged, tampered, reason}`; exits 0 iff genuinely converged. | None (advisory; informs the conductor's retry decision). |
| `finalize --batch <n> --units <a,b,c> --claimed <a,b> --check-cmd <cmd> [--test-file <path>] [--reasons <unit>=<reason>,…] [--target <branch>] [--strategy <squash\|merge\|rebase>] [--repo <name>]` | The authoritative gate: **re-run the check on every claimed unit** before any merge (a unit named in `--claimed` but red on disk is refused the merge and lands in the failure envelope — the lying-conductor guard), then per genuine Unit: release the HOLD-MERGE lock, reconcile workflow metadata (`complete --merge`), and integrate the Unit's committed source into the prepared base (`--target` overrides; strategy defaults to `squash`). A failed source merge downgrades the Unit, blocks the remaining transactions, and exits 2 — never a false `converged`. Exits 0 (batch converged and merged) or 2 (failure envelope). | `SWARM_UNIT_CONVERGED` / `SWARM_UNIT_FAILED` / `SWARM_BATON_RETURNED` / `SWARM_COMPLETED`. |
