# Amadeus-DLC Workflows 1.0 Skywork向けブリーフ

## 目的

この資料は、Amadeus-DLC Workflows 1.0をSkyworkでスライド化するための入力資料である。単なる機能一覧ではなく、「AIコーディングを、再現可能で監査可能な開発ライフサイクルに変える仕組み」として説明する。

## 一言でいうと

Amadeus-DLC Workflows 1.0は、AIエージェントによるソフトウェア開発を、5フェーズ・32ステージ・
11ドメインエージェント・承認ゲート・監査ログ・学習ループで統制する、AWS発の
AI-Driven Development Life Cycleのマルチハーネス実装である。

1.0の中核は「ひとつのハーネス非依存なcoreを、Claude Code、Kiro IDE、Kiro CLI、Codex CLIなど複数の実行環境へ配布する」点にある。方法論は一箇所で定義され、各ハーネスは薄い実行面だけを持つ。
...
Amadeus-DLC Workflows 1.0は、この課題に対して「工程」「役割」「成果物」「承認」「監査」「学習」を明示し、AI作業を開発プロセスとして扱える形にする。
...
Amadeus-DLC Workflows 1.0は、ソフトウェア開発を次の5フェーズに分ける。

- Initialization: ワークスペースを検出し、`amadeus/`ワークスペースとintentのレコード(状態ファイルを含む)を初期化する。
...
1.0は、細かすぎる専門エージェントを大量に作るのではなく、広い責務を持つ11エージェントで進める。
...
すべてのタスクに32ステージを強制するわけではない。1.0は9つのスコープを持ち、タスクに応じて実行ステージ数と詳細度を調整する。
...
1.0では、intentのレコード配下の`amadeus-state.md`(`amadeus/spaces/<space>/intents/<record>/amadeus-state.md`)が現在位置と進捗の正本になる。各ステージは未開始、進行中、承認待ち、修正中、完了、スキップの状態を持つ。

一方、レコード配下の監査シャード(`amadeus/spaces/<space>/intents/<record>/audit/`のper-clone分割)は追記型の監査ログである。ワークフロー開始、ステージ開始、質問回答、ゲート承認、成果物更新、サブエージェント完了、センサー結果、学習ルール追加などを記録する。1.0では67種類の監査イベントが定義されている。
...
- Tier 2: チームが管理するプロジェクト固有知識。`amadeus/spaces/<space>/knowledge/`に置く。
...
Amadeus-DLC Workflows 1.0: AIコーディングを開発ライフサイクルへ

話すこと:

- AI-DLCはAI-Driven Development Life Cycle。
- 1.0はAWS発の方法論を、複数AI開発ハーネスで動く実装として再構成したもの。
...
### 3. Amadeus-DLC Workflows 1.0の全体像
...
- `amadeus-state.md`が現在位置、`audit.md`が履歴を担う。
...
- Amadeus-DLC Workflows 1.0は、AIにコードを書かせるだけの仕組みではない。
...
- 1.0はGA Previewであり、インターフェース、ステージ定義、エージェント構成、インストールモデルは変わりうる。
