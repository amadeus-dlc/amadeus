...
- Initialization: ワークスペースを検出し、`amadeus-docs/`と状態ファイルを初期化する。
...
1.0では、`amadeus-docs/amadeus-state.md`が現在位置と進捗の正本になる。各ステージは未開始、進行中、承認待ち、修正中、完了、スキップの状態を持つ。

一方、`amadeus-docs/audit.md`は追記型の監査ログである。ワークフロー開始、ステージ開始、質問回答、ゲート承認、成果物更新、サブエージェント完了、センサー結果、学習ルール追加などを記録する。1.0では67種類の監査イベントが定義されている。
...
- Tier 2: チームが管理するプロジェクト固有知識。`amadeus-docs/knowledge/`に置く。
