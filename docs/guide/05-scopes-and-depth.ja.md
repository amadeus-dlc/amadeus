...
`requirements.md` については `depth-budget` センサーが結果を測定します。番号付き `FR-n` 1件あたりのバイト数を、Minimal は 1,800、Standard は 2,400 の上限と照合します(Comprehensive は上限なし)。他のセンサーと同様に advisory であり、超過は承認ゲートで確認事項として提示されるだけで、ブロックはしません。
