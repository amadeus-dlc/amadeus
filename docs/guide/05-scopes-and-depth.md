...
The `depth-budget` sensor measures the result for `requirements.md`: bytes per numbered `FR-n` against a ceiling of 1,800 for Minimal and 2,400 for Standard (Comprehensive declares none). Like every sensor it is advisory — an overrun is reported at the approval gate as something to look at, never as a block.
