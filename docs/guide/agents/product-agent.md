...
The amadeus-product-agent works closely with the amadeus-architect-agent on feasibility and dependencies, the amadeus-design-agent on UX alignment, and the amadeus-delivery-agent on capacity and scope validation. Its outputs (requirements, stories, scope) are consumed by nearly every downstream agent.
