...
The amadeus-pipeline-deploy-agent leads four stages spanning Inception, Construction, and Operation. It has Bash access for running pipeline tools, deployment scripts, and smoke test commands.
...
The amadeus-pipeline-deploy-agent does not support any stages in an advisory capacity.

## What to Expect

When the amadeus-pipeline-deploy-agent is active, it asks about your existing CI/CD infrastructure, deployment targets, branching strategy, and rollback requirements. It produces pipeline configurations (CI config files, quality gate definitions), deployment strategies (blue-green, canary, rolling), and rollback runbooks. During Deployment Execution, it orchestrates the actual deployment, runs smoke tests, and monitors health metrics.

## How It Collaborates

The amadeus-pipeline-deploy-agent receives buildable source and test suites from the amadeus-developer-agent, quality gate definitions from the amadeus-quality-agent, and environment endpoints from the amadeus-aws-platform-agent. Its deployed services are handed off to the amadeus-operations-agent for observability setup, and deployment artifacts go to the amadeus-quality-agent for performance validation.
