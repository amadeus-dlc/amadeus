...
All framework TypeScript hooks (`amadeus-mint-presence.ts`, `amadeus-audit-logger.ts`, `amadeus-sensor-fire.ts`, `amadeus-runtime-compile.ts`, `amadeus-log-subagent.ts`, `amadeus-stop.ts`, `amadeus-validate-state.ts`, `amadeus-sync-statusline.ts`, `amadeus-session-start.ts`, `amadeus-plugin-compose.ts`, `amadeus-session-end.ts`, `amadeus-statusline.ts`) require `bun`. If `bun` is missing or not on PATH for non-interactive shells, these hooks will not fire.
...
It checks: prerequisite (`bun`), **Self-install projection freshness**, hook availability (every hook `settings.json` wires — all framework hooks — must exist in `.claude/hooks/`, and a wired-but-missing hook fails loudly), project structure (`settings.json`), workspace shell readiness (`.claude/` + `amadeus/spaces/default/memory/`), state/audit consistency, hook heartbeats, graph integrity (no cycles, every graph entry has a file), scope validation across all 10 scopes, stage schema + graph references, and keyword overlap across scopes. In the self-development repository, the projection check runs build-backed `scripts/promote-self.ts --check` against every managed delivery tree; each `DIFFERS`, `MISSING`, `ORPHAN`, or `MISPLACED` result makes doctor exit 1, while an absent script produces one passing N/A row. It also surfaces two advisory rows that always pass (they never change the exit code): **Rule drift** (team/project rules that overlap a populated org-policy heading, flagged for contradiction review) and **Paired sensor coverage** (rules carrying a `pairing:` whose named Sensor resolves to a stage). Exits 0 on full pass, 1 on any failure; the report writes to stdout either way. `--doctor` is **read-only** with respect to tracked files and projection trees: on a fresh installed shell with no intent yet it creates nothing, while the self-development projection check may regenerate `dist/` and create the gitignored `amadeus/active-space` runtime cursor when it is absent. Once an intent exists it records a `HEALTH_CHECKED` (and `GUARDRAIL_LOADED`) audit row.
