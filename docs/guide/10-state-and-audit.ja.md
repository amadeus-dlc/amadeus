...
> 言語: [English](10-state-and-audit.md) | **日本語**
...
| **旧常任グラント観測** | 3 | `GRANT_ISSUED`, `GRANT_REVOKED`, `GATE_AUTHORIZATION_SELECTED`(replay/doctor専用、新しい認可なし) |
...
| **Construction Bolt + 旧Mode** | 4 | `BOLT_STARTED`, `BOLT_COMPLETED`, `BOLT_FAILED`, `AUTONOMY_MODE_SET`(旧観測) |
...
- **`event`** — 正準イベントタイプのいずれか(自由記述の `append-raw` レコードでは `null` になり、本文は `rawBody` に入る)
