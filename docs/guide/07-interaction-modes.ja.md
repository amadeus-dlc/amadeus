...
stage が questions ファイル（`<stage>-questions.md`）を作ると、人間には 4 通りの答え方が示される。Claude Code では、この順序の 4 択として表示される。

- **Guide me**：会話の中で、質問を小さなまとまりごとに確認しながら進める。
...
harness ごとの実際の見え方は、[question-rendering annex](../../core/skills/amadeus/references/question-rendering.md) が定義する。例えば Codex では選択肢として 3 つが並び、Chat は専用の選択肢ではなく自由記述（custom）から到達する。
...
これは、[amadeus-grilling skill](../../core/skills/amadeus-grilling/SKILL.md) が他の場面で従うのと同じ規律である。
