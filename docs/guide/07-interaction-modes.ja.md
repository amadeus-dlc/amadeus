...
エージェントがあなたを執拗にインタビューします — 決定空間を設計ツリーとして捉え、枝刈り後のフロンティア全体をラウンド単位で提示し、各質問に推奨される回答とその根拠を添えて — ツリーに入ったすべての分岐が決着し、あなたとエージェントが共通の理解に達するまで。ステージが成果物を生成する前に、計画や設計をストレステストしたい場合に最適です。

- エージェントがコードベースや先行成果物から判定できる事実は、尋ねられずに調べられます。決定事項のみがあなたに提示されます
- 不確かな事実は、確認のために確信度タグ付きの見積もりとして提示されます
- 深度は質問数の予算ではなく重要度のしきい値です: どのノードがツリーに入るかを決め、枝刈りされたノードは黙って捨てられるのではなく合意サマリーに列挙されます
- セッションは、枝刈り後のフロンティアが空になるか、あなたが「done」と言ったときに終わります。深度ガイドラインの 3 倍に置かれた遮断器(Minimal 12 / Standard 24 / Comprehensive 36)が開示された上限であり、発火した場合は未解決のフロンティアが開示されます
