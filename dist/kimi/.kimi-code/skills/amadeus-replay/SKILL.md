...
description: >
  ...
  `amadeus-runtime.ts summary`; prose comes from the audit trail and
...
```bash
bun .kimi-code/tools/amadeus-runtime.ts summary --json
```
...
Run `bun .kimi-code/tools/amadeus-runtime.ts summary --json`.
...
- The artefacts under `amadeus-docs/<phase>/<stage>/` — for what each
  stage produced.
...
them at `/amadeus-outcomes-pack` (the skill that writes a file) rather
