...
- **Stop is observation-only**: Kimi's external hook payload does not provide a trustworthy main-vs-subagent caller identity. The adapter therefore never forwards `Stop` to the stateful core hook for either caller shape and always returns empty stdout/stderr with exit 0. The explicit forwarding loop above, or the next human turn, drives subsequent workflow progress. The product and architecture reviewer profiles expose only `Read`, `Grep`, and `Glob`; Kimi re-checks that tool allowlist before execution, so reviewer Bash cannot invoke the engine or state tools. Stage `support_agents` are loaded inline rather than dispatched, the developer/architect workers retain Bash for their declared tasks, and Kimi's built-in `explore` profile is already read-only.
