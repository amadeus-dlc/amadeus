...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kimi-code/tools/amadeus-orchestrate.ts next --stage deployment-pipeline --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kimi-code/tools/amadeus-orchestrate.ts report --single --stage deployment-pipeline --result completed
   ```
