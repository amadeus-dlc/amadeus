...
- **`/amadeus-grilling`** (standalone) — the read-only session skill at
  `.pi/skills/amadeus-grilling/SKILL.md`.
...
| Audit log | REQUIRED — `bun .pi/tools/amadeus-log.ts decision` before each question, `... answer` after each answer (per question; existing event types only) | None — read-only classification, no audit events |
