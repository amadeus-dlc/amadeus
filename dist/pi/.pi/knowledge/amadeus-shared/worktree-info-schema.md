...
```
bun .pi/tools/amadeus-worktree.ts info --slug <kebab-slug>
```
...
- Implementation: `.pi/tools/amadeus-worktree.ts` (`handleInfo` handler)
- Test: `tests/unit/t72-worktree-info.sh`
- Caller: `.pi/skills/amadeus/SKILL.md` (per-Bolt-loop halt-and-ask flow)
...
- Audit-format spec: `.pi/knowledge/amadeus-shared/audit-format.md` `WORKTREE_CREATED` row
