...
import { supplyResourceAttribute } from "../otel/resource-suppliers.ts";
...
if (sessionId) {
  try {
    ...
    // already supplied in this process
  }
}
...
try {
  ...
  if (uncompiledStages.length > 0) {
    driftNote =
      ...
      `Run \`bun ${harnessDir()}/tools/amadeus-graph.ts compile\` to include them, then start a fresh workflow (an in-flight workflow keeps its original stage set).\n`;
  }
} catch {
