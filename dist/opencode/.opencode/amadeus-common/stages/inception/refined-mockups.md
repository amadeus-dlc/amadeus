...
Load amadeus-design-agent persona from `agents/amadeus-design-agent.md` and knowledge from `.opencode/knowledge/amadeus-design-agent/`.
...
Create mid-to-high fidelity mockups (per user story/screen), interaction specification document (use `.opencode/knowledge/amadeus-design-agent/component-spec-template.md` as the format for component-level specifications), design system mapping, responsive behavior specification, and accessibility compliance checklist.

For non-UI: create API developer experience specification.

### Step 6: Update State

Mark refined-mockups as `[x]` completed in `<record>/amadeus-state.md`.
