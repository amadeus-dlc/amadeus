...
- Read team practices via `.opencode/amadeus-rules/amadeus-team.md` (fall back to `org.md` per `knowledge/amadeus-shared/rules-reading.md`); match the affirmed branching strategy to one of the five in `branching-strategies.md`
- Resolve `amadeus-worktree` flags (`--slug`, `--base`, `--target`, `--strategy`, optional `--message`) per the chosen strategy's runbook
- Invoke `bun .opencode/tools/amadeus-worktree.ts` from the main repo checkout; `amadeus-worktree` itself emits the audit event audit-first before invoking git
...
1. `.opencode/amadeus-rules/` -- execution guardrails
2. `.opencode/knowledge/amadeus-shared/` -- shared methodology
3. `.opencode/knowledge/amadeus-pipeline-deploy-agent/` -- agent-specific methodology
4. `.opencode/amadeus-rules/` -- team-affirmed practices (read per `knowledge/amadeus-shared/rules-reading.md`)
