...
1. Pass the directive width to preparation: `bun .claude/tools/amadeus-swarm.ts prepare --batch <n> --units <all> --concurrency <directive.cap> ...`. `max-parallel-units` resolves project → space → intent, defaults to the hard cap 4, and an invocation may only narrow it.
