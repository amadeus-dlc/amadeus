...
Log each question via `bun .claude/tools/amadeus-log.ts decision` BEFORE presenting it. Log each answer via `bun .claude/tools/amadeus-log.ts answer` after the user responds.
...
After writing the four artifacts, emit `PRACTICES_DISCOVERED` via `bun .claude/tools/amadeus-state.ts practices-event --type discovered --field "Sources Scanned: <list>" --field "Drafts: team-practices.md, discovered-rules.md"` (the tool wraps the audit emission so events stay tool-owned per the audit-first invariant).

### Step 5: Affirmation Gate

Compliance with `stage-protocol.md` checklist:

1. `bun .claude/tools/amadeus-state.ts gate-start practices-discovery` BEFORE the affirmation question.
2. `bun .claude/tools/amadeus-log.ts decision` for the affirmation question.
3. A structured question presents `team-practices.md` and `discovered-rules.md` for review. Options:
   - **Approve** — promote affirmed content to `.claude/rules/amadeus-team.md` and `.claude/rules/amadeus-project.md` (Step 6).
...
4. `bun .claude/tools/amadeus-log.ts answer` after the user answers.
5. `bun .claude/tools/amadeus-orchestrate.ts report --stage practices-discovery --result approved --user-input "<exact label>"` (or `bun .claude/tools/amadeus-state.ts reject practices-discovery --feedback "<text>"`) — auto-emits the gate-approved/gate-rejected audit events through the owning tools.
...
```
bun .claude/tools/amadeus-state.ts practices-promote \
...
- Review path: `<record>/inception/practices-discovery/` AND `.claude/rules/amadeus-team.md` AND `.claude/rules/amadeus-project.md`
