...
- Read team practices via `.cursor/amadeus-rules/amadeus-team.md` (fall back to `org.md` per `knowledge/amadeus-shared/rules-reading.md`); match the affirmed branching strategy to one of the five in `branching-strategies.md`
- Resolve `amadeus-worktree` flags (`--slug`, `--base`, `--target`, `--strategy`, optional `--message`) per the chosen strategy's runbook
- Invoke `bun .cursor/tools/amadeus-worktree.ts` from the main repo checkout; `amadeus-worktree` itself emits the audit event audit-first before invoking git
...
1. `.cursor/amadeus-rules/` -- execution guardrails
2. `.cursor/knowledge/amadeus-shared/` -- shared methodology
3. `.cursor/knowledge/amadeus-pipeline-deploy-agent/` -- agent-specific methodology
4. `.cursor/amadeus-rules/` -- team-affirmed practices (read per `knowledge/amadeus-shared/rules-reading.md`)
