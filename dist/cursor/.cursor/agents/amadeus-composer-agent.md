...
1. **Detect.** Run `bun .cursor/tools/amadeus-utility.ts detect --json`.
...
4. **Validate.** Before the proposal is shown, write the proposed grid to a
   ...
   `bun .cursor/tools/amadeus-graph.ts validate-grid --proposal <path> --project-type <greenfield|brownfield>`
...
- Making a scope inferable is an explicit human choice at the gate ("make
  ...
  `bun .cursor/tools/amadeus-graph.ts validate-grid --proposal <path> --keywords <granted,csv>`
