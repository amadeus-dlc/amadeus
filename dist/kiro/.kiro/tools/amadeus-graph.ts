...
// the doctor handler (see amadeus-utility.ts handleDoctor) and the
...
export interface GraphStage extends StageEntry {
  ...
  // (validateStageFrontmatter at amadeus-stage-schema.ts rejects unknown
 ...
 *  workspace-root amadeus/spaces/default/memory/ relative to this tool's location
...
// declaration; the resolver attaches the matching amadeus-phase-<name>.md
...
// `amadeus-` prefix now that they live under the neutral amadeus/ roof); the
...
// Phase rule files live in phases/<phase>.md (the flat amadeus-phase-<phase>.md
 ...
 *  skips single-line comments; rule files (e.g. amadeus-org.md's
...
export interface SensorFile {
  id: string;
  path: string;        // ".claude/sensors/amadeus-<id>.md"
 ...
 *  the stage's artifact set — the dispatcher (amadeus-sensor.ts) and the
 *  PostToolUse fire hook (amadeus-sensor-fire.ts) both hold the GraphStage and
 ...
 *  .claude/scopes/amadeus-x.md that no stage names yet (valid scope, empty
...
// in `.claude/scopes/amadeus-<name>.md`, not here.
      ...
      if (json !== expected) {
        console.error(
          `export --check: bundle drift vs ${fixturePath}. ` +
            `Regenerate with: bun amadeus-graph.ts export > ${fixturePath}`
        );
        process.exit(1);
      }
      return;
...
Common forms:
  ...
  amadeus-graph validate-grid --proposal <path> [--strict] [--project-type <t>] [--keywords <csv>]
