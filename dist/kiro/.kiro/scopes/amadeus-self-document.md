...
genuinely open question. The sibling incremental scopes (`self-fix`,
`self-refactor`) skip it since their subject is already fixed by an
...
`self-refactor` — restructuring needs a target shape — applied to prose
...
`/amadeus --scope self-document`. Making it inferable is a separate,
...
Lightening review 2026-07-28: only one completed self-document intent
exists, too few for the evidence-mining methodology that lightened
self-feature. Revisit once completed intents accumulate.
