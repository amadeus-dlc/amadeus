...
- Park: after recording the human answer, run `amadeus-bolt park --stage <stage> --slug <slug> --reason <reason>`. The Unit becomes non-runnable but does not settle the stage; independent siblings in the same batch may continue, while dependent batches and the stage gate remain blocked. This human-authorized Unit disposition remains valid in `autonomous` mode; it is distinct from parking the entire unattended workflow. Worktree at `<path>` is preserved. Resume with `amadeus-bolt resume --stage <stage> --slug <slug> --reason <reason>`.
