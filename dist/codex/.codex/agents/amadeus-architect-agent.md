...
3. `.codex/knowledge/amadeus-architect-agent/` — agent-specific methodology
4. `.codex/amadeus-rules/` — team-affirmed practices (read per `knowledge/amadeus-shared/rules-reading.md` fallback chain `team.md → org.md → hardcoded defaults`). Consult `## Code Style` and `## Branching` when architectural decisions touch coding-convention or repository-topology choices.
