...
```
bun .kiro/tools/amadeus-worktree.ts info --slug <kebab-slug>
```
...
- Implementation: `.kiro/tools/amadeus-worktree.ts` (`handleInfo` handler)
- Test: `tests/unit/t72-worktree-info.sh`
- Caller: `.kiro/skills/amadeus/SKILL.md` (per-Bolt-loop halt-and-ask flow)
...
- Audit-format spec: `.kiro/knowledge/amadeus-shared/audit-format.md` `WORKTREE_CREATED` row
