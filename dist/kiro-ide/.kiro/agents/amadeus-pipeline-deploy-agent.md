...
- Read team practices via `.kiro/steering/amadeus-team.md` (fall back to `org.md` per `knowledge/amadeus-shared/rules-reading.md`); match the affirmed branching strategy to one of the five in `branching-strategies.md`
- Resolve `amadeus-worktree` flags (`--slug`, `--base`, `--target`, `--strategy`, optional `--message`) per the chosen strategy's runbook
- Invoke `bun .kiro/tools/amadeus-worktree.ts` from the main repo checkout; `amadeus-worktree` itself emits the audit event audit-first before invoking git
...
2. `.kiro/knowledge/amadeus-shared/` -- shared methodology
3. `.kiro/knowledge/amadeus-pipeline-deploy-agent/` -- agent-specific methodology
4. `.kiro/steering/` -- team-affirmed practices (read per `knowledge/amadeus-shared/rules-reading.md`)
