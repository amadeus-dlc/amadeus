...
1. The engine auto-creates `memory.md` from
   `.kiro/knowledge/amadeus-shared/memory-template.md` when it issues
...
- **Design deviations** — when the approved requirements or design have to be
  ...
  `bun .kiro/tools/amadeus-election.ts open --trigger auto-solo`; on
...
via `bun .kiro/tools/amadeus-state.ts practices-event --type override` before
