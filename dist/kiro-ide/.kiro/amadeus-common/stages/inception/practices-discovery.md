...
1. **amadeus-pipeline-deploy-agent** (lead) — Reads `.kiro/knowledge/amadeus-pipeline-deploy-agent/branching-strategies.md`. Scans git history (branch names, merge patterns, lifetime), CI config, deployment cadence. Returns: branching strategy match, deployment frequency, environment topology.
...
Log each question via `bun .kiro/tools/amadeus-log.ts decision` BEFORE presenting it. Log each answer via `bun .kiro/tools/amadeus-log.ts answer` after the user responds.
...
After writing the four artifacts, emit `PRACTICES_DISCOVERED` via `bun .kiro/tools/amadeus-state.ts practices-event --type discovered --field "Sources Scanned: <list>" --field "Drafts: team-practices.md, discovered-rules.md"` (the tool wraps the audit emission so events stay tool-owned per the audit-first invariant).

### Step 5: Affirmation Gate

Compliance with `stage-protocol.md` checklist:

1. `bun .kiro/tools/amadeus-state.ts gate-start practices-discovery` BEFORE the affirmation question.
2. `bun .kiro/tools/amadeus-log.ts decision` for the affirmation question.
3. A structured question presents `team-practices.md` and `discovered-rules.md` for review. Options:
   - **Approve** — promote affirmed content to `.kiro/steering/amadeus-team.md` and `.kiro/steering/amadeus-project.md` (Step 6).
...
4. `bun .kiro/tools/amadeus-log.ts answer` after the user answers.
5. `bun .kiro/tools/amadeus-orchestrate.ts report --stage practices-discovery --result approved --user-input "<exact label>"` (or `bun .kiro/tools/amadeus-state.ts reject practices-discovery --feedback "<text>"`) — auto-emits the gate-approved/gate-rejected audit events through the owning tools.
...
```
bun .kiro/tools/amadeus-state.ts practices-promote \
...
- Reads both drafts and both target files (`.kiro/steering/amadeus-team.md` and `.kiro/steering/amadeus-project.md`); fails closed before any write if any input is missing.
...
2. Update `Practices Affirmed Timestamp` in `<record>/amadeus-state.md` via `bun .kiro/tools/amadeus-state.ts set "Practices Affirmed Timestamp=NOW"` (the `NOW` literal expands to the current ISO 8601 timestamp; the field is part of the v7 state template).
...
- Review path: `<record>/inception/practices-discovery/` AND `.kiro/steering/amadeus-team.md` AND `.kiro/steering/amadeus-project.md`
