...
| `ask` | Render `directive.question` as numbered prose per `question-rendering.md`. If `directive.response_action.kind` is `select-intent`, render exactly its `options`, resolve the chosen number or text to the option slug, run `bun .kiro/tools/amadeus-utility.ts intent "<slug>"`, then re-run `next`; never pass the raw ordinal to a tool and never call `report` for this action. Otherwise feed the resolved option label or free text back on the next `report` via `--user-input`. The engine never asks the user itself — it defers the human turn to you. |
