...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .kiro/tools/amadeus-orchestrate.ts next --stage feasibility --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .kiro/tools/amadeus-orchestrate.ts report --single --stage feasibility --result completed
   ```
