...
// spawns `amadeus-sensor.ts fire <id> --stage <slug> --output-path <path>`
// per matching entry.
//
...
// matcher; recursion guard skips writes to `amadeus-docs/.amadeus-sensors/`.
//
...
// of amadeus-audit-logger.ts:42 — the includes-filter works precisely
...
// (it resolves the active record root). Keep the flat `amadeus-docs/.amadeus-sensors/`
...
if (
  ...
  filePath.includes("amadeus-docs/.amadeus-sensors/") ||
  filePath.includes("amadeus-docs\\.amadeus-sensors\\")
) {
  process.exit(0);
}
...
// do not fire. The framework artifact glob is `**/{amadeus-docs,intents}/**`
...
for (const entry of applicableSensors) {
  ...
  //
  // TPL note: this hook invokes the DISPATCHER (amadeus-sensor.ts fire), not the
  ...
  try {
    ...
    // for `--doctor` to surface (mirrors amadeus-runtime-compile.ts:115-121):
