...
Discovery、Intent Init、Inception、Domain Modeling が参照できる補助分析成果物を作ります。
...
`state.json` は Intent のモジュールディレクトリ直下に残し、`initialization/` には移しません。

### 5. Intent を Ideation へ進める
...
| ライフサイクル進行 | `amadeus-intent-init` | Intent 登録、`Intent のモジュールファイル`、`state.json` を作る | Ideation 以降の成果物 |
