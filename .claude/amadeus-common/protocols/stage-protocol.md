...
2. [ ] **Log questions via `amadeus-log.ts`** — before presenting a structured question: `bun .claude/tools/amadeus-log.ts decision --stage <slug> --decision "<summary>" --options "<csv>"`. After response: `bun .claude/tools/amadeus-log.ts answer --stage <slug> --details "<exact choice>"`. (§3)
...
2. Do NOT present the approval question. Report once, forwarding the carrier verbatim: `bun .claude/tools/amadeus-orchestrate.ts report --stage <slug> --result approved --standing-grant-id <id> --standing-grant-route-id <route-id>`.
...
- Workflow-specific obligations on top of the protocol:
  ...
  - Audit per question, existing contract only: `bun .claude/tools/amadeus-log.ts decision ...` before presenting, `bun .claude/tools/amadeus-log.ts answer ...` after the response — one `decision`/`answer` pair per question, with the same write-back, audit, and fresh-timestamp discipline as Step 3a. No new event types.
...
1. Advance state atomically via CLI tool (see "Silent bookkeeping writes" below):
   `bun .claude/tools/amadeus-state.ts advance "<completed-slug>" "<next-slug>"`
...
2. AFTER user response: `bun .claude/tools/amadeus-orchestrate.ts report --stage <slug> --result approved --user-input "<choice>"` or `bun .claude/tools/amadeus-state.ts reject <slug> --feedback "<text>"`. `report` emits any missing gate row, then `GATE_APPROVED` + `STAGE_COMPLETED`, and auto-advances to the next in-scope stage (or completes the workflow if this was the final stage). `reject` likewise emits any missing gate row, then `GATE_REJECTED` + `STAGE_REVISING`, and leaves the stage in `[R]`.

At each question interaction:
1. BEFORE presenting the question: `bun .claude/tools/amadeus-log.ts decision --stage <slug> --decision "<summary>" --options "<A,B,C>"` (emits `DECISION_RECORDED`).
2. AFTER response: `bun .claude/tools/amadeus-log.ts answer --stage <slug> --details "<summary of answers>"` (emits `QUESTION_ANSWERED`).
...
```bash
bun .claude/tools/amadeus-state.ts finalize "completed-slug"
```
...
```bash
bun .claude/tools/amadeus-state.ts complete-workflow "completed-slug"
```
...
```bash
bun .claude/tools/amadeus-state.ts lookup phase-of SLUG          # → phase name
bun .claude/tools/amadeus-state.ts lookup next-stage SLUG SCOPE   # → next in-scope slug
bun .claude/tools/amadeus-state.ts lookup agent-for SLUG          # → lead agent name
bun .claude/tools/amadeus-state.ts lookup validate-stage SLUG     # → JSON with slug, phase, number, valid
```
...
| **Scope** | Controls which stages execute and at what depth. Ten built-in scopes, one file per scope under `.claude/scopes/amadeus-<name>.md`: enterprise, feature, mvp, poc, bugfix, chore, refactor, infra, security-patch, workshop. Custom scopes can be added without editing this file. |
...
1. **Build the authoritative declared pass-list.** Before spawning the reviewer, pass the unchanged current `run-stage` directive JSON on stdin to:
   ...
   ```bash
   bun .claude/tools/amadeus-reviewer-runtime.ts scope
   ```
...
2. **Invoke the reviewer sub-agent.** Delegate to the exact checker named in `directive.reviewer`. Pass the stage definition, validation tools named by that definition, and only the authoritative paths returned by `scope`, together with its `invocationId`, the current positive iteration, and an empty transient Scope decision transcript. Preserve this `invocationId + iteration` exactly through prompt, request, decision, and result. The reviewer result starts with `Reviewer: <directive.reviewer>` and carries invocation ID, verdict, iteration, summary, findings, transcript, and requested-read paths. The reviewer does not write the artifact.
   ...
   ```bash
   bun .claude/tools/amadeus-reviewer-runtime.ts check-read
   ```
...
2. **Surface candidates (the tool reads memory.md).** Run:
   ```bash
   bun .claude/tools/amadeus-learnings.ts surface --slug <stage-slug>
   ```
...
└── Sensor (two-write install): scaffold a project-tier manifest at
    .claude/sensors/amadeus-<id>.md with a matches: glob, AND append its id to
...
```bash
bun .claude/tools/amadeus-state.ts reuse-artifact <stage-slug> \
