...
1. The engine auto-creates `memory.md` from
   `.claude/knowledge/amadeus-shared/memory-template.md` when it issues
...
- **Design deviations** — when the approved requirements or design have to be
  ...
  `bun .claude/tools/amadeus-election.ts open --trigger auto-solo`; on
...
via `bun .claude/tools/amadeus-state.ts practices-event --type override` before
