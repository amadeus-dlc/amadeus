...
- 依存する既存 Intent はあるか。
- `Intent 正本ファイル` の成功条件や範囲に最低限入れる制約はあるか。
...
この場合、Grilling Decision Trail の `反映先` は `Intent 正本ファイル` に限定する。
...
不足している目的、成功条件、範囲は `未確認` として `Intent 正本ファイル` に残す。
...
- Discovery Brief から作成した場合は、対象 Discovery の `Discovery 正本ファイル` と `.amadeus/discoveries.md`
