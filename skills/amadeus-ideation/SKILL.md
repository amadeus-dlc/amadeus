...
| 状態 | 自動選択 | 理由 |
|---|---|---|
| Intent Record がないが、入力テーマまたは Discovery Brief があり、確定判断の記録への参照が入力に存在して Ideation の判断項目を導ける | `scaffold-only` | Intent Capture & Framing から Ideation 完了まで一続きで作れるため |
...
| `state.json.phase` が `ideation` で `ideation.intentCapture.status` が `completed` だが、`scope.md` 以降、`inception/**`、`construction/**` が未作成であり、確定判断の記録への参照が入力に存在して Ideation の判断項目を導ける | `scaffold-only` | 中断した Ideation を同じ実行契約で完了まで進めるため |
...
ユーザーが明示した場合、または Intent Record がなく入力テーマか Discovery Brief があり、確定判断の記録への参照が入力に存在して Ideation の判断項目を導ける場合だけ使う。

確定判断の記録とは、次の 3 種である。

- GitHub Issue の確定判断（目的、対象、対象外、受け入れ条件）。
- Grilling Decision Trail（`grillings/Gxxx-*.md` の確定判断）。
- Discovery Brief の確定済み判定と候補判断。

確定判断の記録への参照がない入力では、auto 判定は scaffold-only を選ばず、`guided` として不足判断を一問ずつ確認する。
scaffold-only を選んだ場合は、参照した確定判断の記録を選択理由として示す。
