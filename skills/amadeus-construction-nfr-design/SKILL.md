...
対象 Intent の `state.json` で、`stages["nfr-design"]` が実行対象であり、対象 Unit の `stages["nfr-design"].units["<unit-id>"].state` が `pending`、`active`、`awaiting_approval`、`revising` のいずれかであることを前提にする。
...
NFR Requirements を実行しなかった場合は、成果物を作らず対象 Unit の状態を `skipped` にし、理由を記録して `amadeus` へ戻る。
...
質問を行った場合は `construction/<unit-id>-<slug>/nfr-design/questions.md` に記録する。

## テンプレート

優先順位は次である。

1. `.amadeus/settings/templates/intents/construction/nfr-design/`
...
- 質問を行った場合は `construction/<unit-id>-<slug>/nfr-design/questions.md`
...
2. 対象 Unit の `stages["nfr-design"].units["<unit-id>"].state` を `active` にする。
