...
1. 状態が `pending` の場合だけ Condition を判定する。プラクティスの発見が不要な場合は `skipped` を記録して終了する。`active`、`awaiting_approval`、`revising` からの再開では再判定しない。
2. `stages["practices-discovery"].state` を `active` にする。
3. brownfield は codebase 知識と repo 内の証拠（CI 設定、既存 PR、設定ファイル）からプラクティスを抽出する。greenfield は質問で確認する。
4. `team-practices.md`、`discovered-rules.md`、`evidence.md`、`timestamp.md` を作る。
5. `stages["practices-discovery"].state` を `awaiting_approval` にし、ゲートを提示する。
6. 承認後、確認済みプラクティスの steering policies への昇格を提案する。昇格は人間が承認した項目だけを `.amadeus/steering/policies/` に反映する。
