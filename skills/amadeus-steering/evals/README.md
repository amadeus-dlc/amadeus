...
| モード | 状態 | 確認内容 | 証拠 |
|---|---|---|---|
| `scaffold-only` | 完了 | 質問なしで最小 steering layer を作り、空欄ではなく `未確認` を残し、Intent を作らない。Execution Validator が pass する。 | `/tmp/amadeus-steering-scaffold-only.GrzMDR` |
...
```sh
ruby -rjson -e 'JSON.parse(File.read("skills/amadeus-steering/evals/evals.json")); puts "evals.json: ok"'
