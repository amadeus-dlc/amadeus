...
- `.amadeus/intents/<intent-id>-<slug>/state.json`
- 新規 Boundary を採用する場合だけ、`.amadeus/domain-map.md`
- 新規 Boundary の依存を採用する場合だけ、`.amadeus/context-map.md`
...
7. 新規 Boundary の依存を同時に採用する場合だけ、Context Map に依存を反映し、`根拠` に Inception の判断を置く。
8. Unit から参照する Bounded Context が Domain Map で `adopted` ではない場合、`state.json.inception.gate` は `not_ready` にする。
9. Bounded Context と Unit 参照が採用済み Boundary として確定している場合は `passed` にしてよい。
10. 親 skill から記録対象の質問と回答が渡された場合だけ、`amadeus-grilling` の構造に従って Grilling Decision Trail を同じ変更で更新する。
11. 構造矛盾がないか validator で対象 Intent を検証する。
