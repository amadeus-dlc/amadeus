...
既存の Domain Event や候補を上書きする場合は、理由を `hotspots.md` または `Event Storming 正本ファイル` の `Supersession` に残す。
...
`Event Storming 正本ファイル` は次の見出しを持つ。
...
`Event Storming 正本ファイル` の `Handoff To Domain Modeling` は、`Candidate` に `AGCnnn` または `BCCnnn` の ID を書く。
...
`Event Storming 正本ファイル` の `Supersession` に、`Supersedes`、`Superseded By`、`Reason` を残す。
