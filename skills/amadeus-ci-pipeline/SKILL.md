---
name: amadeus-ci-pipeline
description: >
  Run the AI-DLC `ci-pipeline` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage ci-pipeline --single`:
...
`/amadeus --stage ci-pipeline --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage ci-pipeline --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage ci-pipeline --result completed
   ```
