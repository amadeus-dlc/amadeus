---
name: amadeus-scope-definition
description: >
  Run the AI-DLC `scope-definition` stage (ideation phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage scope-definition --single`:
...
`/amadeus --stage scope-definition --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage scope-definition --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage scope-definition --result completed
   ```
