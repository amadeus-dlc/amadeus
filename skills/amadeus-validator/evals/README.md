...
- Construction 段階では、対象 Bolt の `construction-design.md`、`state.json.construction.bolts[].designGate`、`Construction Design からの追跡` を検証する。
...
| `construction-design-ready` | 完了 | Design Gate ready 時点で `construction-design.md`、Bolt 単位 gate、`Construction Design からの追跡` がある。 | `examples/05-construction-design-ready` が `pass`。一時コピーで `Logical Design` 本文、Bolt gate、設計追跡を壊すと `fail`。 |
...
bun run skills/amadeus-validator/validator/AmadeusValidator.ts examples/05-construction-design-ready 20260628-discovery-brief-creation
