...
- `codebase-analysis.md` が存在する場合、`state.json.inception.codebaseAnalysis.requirement` が `required` の場合、または `state.json.inception.requiredArtifacts` に含まれる場合、必須見出しを検証する。
- `codebase-analysis.md` が存在せず、`state.json.inception.codebaseAnalysis.requirement` が `required` でもなく、`state.json.inception.requiredArtifacts` にも含まれない場合は不足にしない。
...
| `codebase-analysis-state` | 完了 | `state.json.inception.codebaseAnalysis` の requirement、status、evidence、targetScope、freshness を検証する。 | required の場合に `codebase-analysis.md` がないと `fail`。greenfield skipped は `pass`。 |
