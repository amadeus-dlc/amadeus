...
`lifecycle-v2` は、`amadeus-state.md` の scope、depth、Stage Progress、Phase Progress、Current Status と、`audit/audit.md` のイベント（`WORKFLOW_STARTED`、`STAGE_COMPLETED`、`PHASE_VERIFIED`、`PHASE_SKIPPED`、`BOLT_COMPLETED`、`WORKFLOW_COMPLETED`）、completed ステージの必須成果物を検査する。
