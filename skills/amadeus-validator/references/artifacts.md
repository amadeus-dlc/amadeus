...
同じ表の中で `識別子` を重複させない。
`詳細` は `.amadeus/discoveries/<discovery-id>/brief.md` を指す相対リンクにする。
...
- `construction.bolts[]` は対象 Bolt の `id` と `designGate` を持つ。
- `construction.bolts[].designGate.status` は `not_started`、`draft`、`ready`、`passed`、`failed` のいずれかである。
- `construction.bolts[].designGate.evidence` は対象 Bolt の `bolts/<bolt-id>-<slug>/construction-design.md` を指す。
...
Construction 段階の `bolts/<bolt-id>/construction-design.md` は、次の見出しを持つ。
...
`Domain Design`、`Logical Design`、`実装設計`、`検証設計` では、対象 Task を `B001/T001` の形式で明示する。
