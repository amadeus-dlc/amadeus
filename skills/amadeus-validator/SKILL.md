...
- 対象 Intent の `state.json.schemaVersion` が `2` の場合、scope、depth、status、phase、`stages` のキー集合と scope の実行対象の一致、ステージ状態、approval evidence、`phaseGates`、`bolts`、completed ステージの必須成果物を v2 互換ライフサイクルの契約として検証する。
