...
十分な CI が既にある場合は、成果物を作らず `ci-pipeline` の checkbox を `[S]` にし、理由を記録して `amadeus` へ戻る。
...
1. checkbox が `[ ]` の場合だけ Condition を判定する。十分な CI が既にある場合は `[S]` を記録して終了する。`[-]`、`[?]`、`[R]` からの再開では再判定しない。
