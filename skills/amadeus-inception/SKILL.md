...
description: >-
  ...
  既存コード分析、Unit、Unit Design Brief、Bolt、Task、追跡、判断を作成、点検、補修、または煮詰める。`state.json.phase` が `ideation` で gate passed の
...
この skill は、Intent から要求、ユーザーストーリー、ユースケース、Unit、Unit Design Brief、Bolt、Task への分解と追跡を固定する。
...
- 分かっている場合は、要求、利用者価値、相互作用、Unit 境界、Unit Design Brief 候補、Bolt 境界、Task 候補。
...
- Unit Design Brief が不足しており、Bolt や Task が要求やユースケースから直接作られている箇所。
...
`Inception への入力` には、Unit Design Brief、Bolt、`tasks.md` へ渡す判断材料を書く。
...
Task や Bolt の都合ではなく、要求、ユースケース、価値境界から導く。
...
複数 Unit をまたぐ Bolt では、`設計` に複数の Unit Design Brief へのリンクを書き、`bolt.md` に理由を残す。
...
`tasks.md` は、同じ Bolt の `bolt.md` と、Bolt が参照する Unit Design Brief を入力にして作る。
...
同じ Bolt の `bolt.md`、対象 Unit の `design.md`、対象要求、対象ユースケースを読み、次を確認する。

- 対象 Bolt の要求が、少なくとも1つの Task に対応している。
- Unit Design Brief の設計戦略、責務境界、検証観点、Bolt 分割方針が Task に反映されている。
...
要求、ユースケース、Unit Design Brief、Bolt の不足や矛盾が原因の場合は、`tasks.md` を書かずに止める。
その場合は、不足している成果物と該当見出しを示し、上流成果物の refine に戻す。

Amadeus の Task 形式では、Kiro の `_Boundary:_`、`_Depends:_`、`(P)` は使わない。
境界は Unit Design Brief と Bolt、依存は `依存`、並列可否や任意性は必要なら `作業` または `traceability.md` に自然文で残す。
...
`設計からの追跡` では、Unit Design Brief がどの Unit、要求、ユースケース、Bolt、Task に展開されたかを追跡する。
表は、次の列を持つ。
...
- Unit Design Brief なしで `tasks.md` を作らない。
- Task の `作業` を省略しない。
