...
`inception.gate` を `passed` にする場合は、対象 Intent の解決領域として少なくとも1件の境界づけられたコンテキストを `.amadeus/domain/**` または既存の domain layer で確認し、`inception/units.md` の `コンテキスト` に同じ `BCnnn` を参照させる。
steering layer に既存の `BCnnn` がある場合、同じ識別子を別名または別役割の境界づけられたコンテキストとして再定義しない。
...
| `amadeus-inception-user-stories` | User Stories | `user-stories.md`, `user-stories/**` |
...
Inception 全体を進める場合は、5つの内部 skill を上から順に使う。
...
既存の `Intent のモジュールファイル`、`ideation/scope.md`、`ideation/ideation.md`、`ideation/traceability.md`、`ideation/decisions.md`、steering layer、domain layer から分かることは質問しない。
...
`Intent のモジュールファイル`、`scope.md`、`ideation.md`、steering layer、domain layer を読み、次を確認する。
...
Unit を切るときは、`.amadeus/domain/**` と steering layer の境界づけられたコンテキストを参照する。
...
Inception は Intent 固有の正式な Domain Model や Contracts を作らない。
`inception.gate` を `passed` にする場合は、`inception/units.md` の `コンテキスト` が既存の Bounded Context、または人間が確定した Bounded Context を参照している必要がある。
