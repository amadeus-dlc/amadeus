---
name: amadeus-units-generation
description: >
  Run the AI-DLC `units-generation` stage (inception phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage units-generation --single`:
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage units-generation --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage units-generation --result completed
   ```
