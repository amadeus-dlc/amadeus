...
複数の Bolt を扱う場合は、`bolts.md` の `依存関係` 表から wave（並行実行できる Bolt の集合）を導出できる。

wave は、依存がすべて前の wave までに完了する Bolt の集合を wave 1 から順に導出する。
同じ依存表からは常に同じ wave 分割を導出する。
依存に循環がある場合は wave を導出せず、`bolts.md` の依存表の補修へ戻す。
wave は実行順序の判断であり、`state.json` に新しいフィールドを追加しない。

wave 並行は、同じ wave に互いに依存しない Bolt が複数入り、worktree 分離で実行できる場合に使える。
先頭の wave に Bolt が 1 件しかなくても、後続の wave に複数の Bolt が入る場合は、その wave から並行にできる。
条件を満たさない場合は、従来どおり Bolt を 1 件ずつ直列に実行する。
どちらの場合も、Bolt ごとの内部プロセスの順序（Bolt 実行準備、実装実行、検証と堅牢化、追跡と状態確定）と Task Generation Gate の契約は変わらない。

wave 並行を使う場合は、次の順で進める。

1. wave 分割（wave 番号、Bolt、並行可否）を実行計画として人間に提示する。
2. 同じ wave 内の複数 Bolt の Bolt 実行準備をまとめて行い、まとめて `ready_for_approval` にして停止する。承認は Bolt ごとの Task Generation Gate 契約のまま、まとめて処理できる。承認判断の decision と approval evidence は Bolt ごとに記録する。
3. 承認済みの wave 内の Bolt を、Bolt ごとに分離した worktree で並行実行する。同一 worktree 内の Bolt と検証は直列のままにする。
4. wave 内の全 Bolt の実装と検証の完了後に並行 branch を統合し、共有成果物を整合させ、標準検証を通してから次の wave の Bolt 実行準備へ進む。wave 内の Bolt の検証が失敗した場合は、修正して検証を通すまで次の wave へ進まない。

worktree の分離と統合の運用は、対象 workspace の steering layer に並行運用の判断基準（steering policy）がある場合はそれに従う。
