...
Amadeus は `skills/amadeus*`、`.agents/skills/amadeus*`、validator、template、docs、`aidlc/` 成果物を扱う。
...
Amadeus DLC はこれらの意味論を、単一入口 `amadeus`、日本語 Markdown の成果物、`aidlc-state.md`、validator、grilling、PR による人間ゲートとして具体化する。
...
**target workspace**：変更差分、自己開発用 `aidlc/`、作業中の成果物を置く作業場所である。
...
例として、source skill、昇格先 skill、target workspace の `aidlc/` 成果物を含む。
target artifacts は、skill の実行能力そのものではなく、変更対象または検証対象として扱う。

**Space**：対象 workspace で複数 Intent が共有する入れ物である。
Space は `aidlc/spaces/<space>/` に置き、`memory/`（組織、チーム、プロジェクトの前提）、`knowledge/`（用語、ドメイン知識、Event Storming）、`codekb/`（コードベース知識）、`intents/`（Intent Record と Intent Registry）を持つ。
Space の既定名は `default` であり、対象 workspace の `aidlc/active-space` が現在の Space を指す。
...
たとえば `aidlc/spaces/<space>/intents/<dirName>.md` は、`aidlc/spaces/<space>/intents/<dirName>/` と対になるモジュールファイルである。
モジュールファイルは、対象成果物そのものの目的、責務、範囲、関連成果物を扱う。

**モジュールディレクトリ**：同じ階層にある同じ stem のモジュールファイルと対になるディレクトリである。
たとえば `aidlc/spaces/<space>/intents/<dirName>/` は、`aidlc/spaces/<space>/intents/<dirName>.md` と対になるモジュールディレクトリである。
...
Event Storming の概要は `aidlc/spaces/<space>/knowledge/event-storming/<event-storming-id>.md`、または `aidlc/spaces/<space>/intents/<dirName>/event-storming/<event-storming-id>.md` に置く。
...
Intent の概要、依存、目標プロファイルは `aidlc/spaces/<space>/intents/<dirName>.md` に置く。
Intent の目的、対象、成功条件、契機、範囲は、scope が Intent Capture & Framing を実行対象にする場合は `intent-statement.md`、実行しない場合は `requirements.md` に置く。
Intent の状態と phase ごとの成果物は `aidlc/spaces/<space>/intents/<dirName>/` に置く。
...
Intent Record は、Intent のモジュールファイル、モジュールディレクトリ、`aidlc/spaces/<space>/intents/intents.json` の行、`aidlc-state.md` で構成する。
Intent のモジュールファイルには、概要、依存、目標プロファイルを置く。
目的、対象、成功条件、契機、範囲は、scope が Intent Capture & Framing を実行対象にする場合は `intent-statement.md`、実行しない場合は `requirements.md` に置く。
Intent Record の骨格と `aidlc-state.md` は、Birth 提案の承認を受けて Initialization が作る。
内容の具体化は、scope が Intent Capture & Framing を実行対象にする場合に Stage 1.1 が行う。

**Aidlc State**：Intent Record の実行状態を唯一保持する成果物である。
Aidlc State は record 直下の `aidlc-state.md` に置き、AI-DLC v2 の state template のセクション構成をそのまま使う。
...
承認と差し戻しの根拠は、Aidlc State ではなく Audit Trail が持つ。

**Initialization**：Birth 提案の承認直後に Intent Record の骨格と Aidlc State を作る、決定論的な初期化 phase である。
...
Intent Phase Directory Layout では、Intent のモジュールファイルは `aidlc/spaces/<space>/intents/<dirName>.md` に置き、`aidlc-state.md` は `aidlc/spaces/<space>/intents/<dirName>/aidlc-state.md` に置く。
...
Stage Memory はゲート提示の直前に更新し、Aidlc State や Audit Trail の代わりにはならない。

**Intents**：Amadeus DLC 全体で扱う Intent 群である。
Intents の正準台帳は Intent Registry であり、人間向け一覧は `aidlc/spaces/<space>/intents/intents.md` に置く。
Intent が別の Intent の成果を前提にする場合は、依存関係を記録する。

**Intent Registry**：Intent の正準 ID と一覧を追記していく台帳である。
Intent Registry は `aidlc/spaces/<space>/intents/intents.json` に置き、各行に `{uuid, slug, dirName, scope, repos, status}` を持つ。
`uuid` は UUIDv7 であり、衝突しない正準 ID である。
`aidlc/spaces/<space>/intents/intents.md` は Intent Registry から再生成する人間向け索引であり、Intent Registry と矛盾させない。
...
登録層は、Intent のモジュールファイル、モジュールディレクトリ、`intents.json`、`aidlc-state.md`、`intent-statement.md`、目標プロファイル、目的、成功条件、範囲、依存関係を扱う。
...
Bolt の実行状態は `aidlc-state.md` の Project Information にある `Bolt Refs` に置く。
...
Unit 単位の状態は `aidlc-state.md` の CONSTRUCTION PHASE にある `Per unit: <unit>` ブロックの `functional-design` checkbox に記録する。
...
`aidlc/spaces/<space>/knowledge/glossary.md` に置く。
Intent 配下には置かない。
