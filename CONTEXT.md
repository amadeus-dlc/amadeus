...
Amadeus DLC はこれらの意味論を、単一入口 `amadeus`、日本語 Markdown の成果物、`state.json`、validator、grilling、PR による人間ゲートとして具体化する。
...
Intent Record の骨格は、`amadeus` の Intake で Birth 提案が承認された直後に作る。
内容の具体化は、scope が Intent Capture & Framing を実行対象にする場合に Stage 1.1 が行う。
Birth 直後の `state.json.phase` は、scope が Ideation の全ステージを SKIP にする場合は `inception`、それ以外は `ideation` である。
...
Bolt の実行状態と gate は `state.json.bolts` に置き、Bolt PR の merge を gate evidence にする。
...
Unit 単位の状態は `state.json.stages["functional-design"].units` に記録する。
...
Domain Map は Steering の初期入力ではなく、Construction の承認済み成果物（Functional Design の反映候補の採用判断）から更新される。
...
Context Map は Steering の初期入力ではなく、Construction の承認済み成果物から更新される。
