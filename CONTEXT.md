...
Intent の概要、依存、目標プロファイルは `amadeus/spaces/<space>/intents/<dirName>.md` に置く。
...
Intent Record は、Intent のモジュールファイル、モジュールディレクトリ、`amadeus/spaces/<space>/intents/intents.json` の行、`amadeus-state.md` で構成する。
...
Aidlc State は record 直下の `amadeus-state.md` に置き、AI-DLC v2 の state template のセクション構成をそのまま使う。
...
Intent Phase Directory Layout では、Intent のモジュールファイルは `amadeus/spaces/<space>/intents/<dirName>.md` に置き、`amadeus-state.md` は `amadeus/spaces/<space>/intents/<dirName>/amadeus-state.md` に置く。
...
Intents の正準台帳は Intent Registry であり、人間向け一覧は `amadeus/spaces/<space>/intents/intents.md` に置く。
...
`amadeus/spaces/<space>/intents/intents.md` は Intent Registry から再生成する人間向け索引であり、Intent Registry と矛盾させない。
...
登録層は、Intent のモジュールファイル、モジュールディレクトリ、`intents.json`、`amadeus-state.md`、`intent-statement.md`、目標プロファイル、目的、成功条件、範囲、依存関係を扱う。
