...
Amadeus DLC では、DDD Module ごとのモデルを `domain/bounded-contexts/<bounded-context-id>-<slug>/models/<ddd-module-id>-<slug>.md` に置く。
...
Unit の `コンテキスト` は `.amadeus/domain/bounded-contexts.md` の BC を参照する。
該当 BC が未確認の場合は、`domain-notes.md` に未確認事項を残す。
...
正式化された内容は、対象範囲に応じて `.amadeus/domain/` または Construction の Functional Design に昇格する。
