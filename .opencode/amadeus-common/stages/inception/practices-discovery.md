...
- **Greenfield**: skip Step 2; the interview asks all five practice areas using `amadeus-org.md` defaults as suggested answers.
...
1. **amadeus-pipeline-deploy-agent** (lead) — Reads `.opencode/knowledge/amadeus-pipeline-deploy-agent/branching-strategies.md`. Scans git history (branch names, merge patterns, lifetime), CI config, deployment cadence. Returns: branching strategy match, deployment frequency, environment topology.
...
Present structured questions to surface five practice areas, one per `amadeus-team.md` section heading: Way of Working, Walking Skeleton, Testing Posture, Deployment, Code Style.
...
**Greenfield**: ask all five practice areas. Use `amadeus-org.md` section content as the source of suggested-answer text. `amadeus-org.md` and `amadeus-team.md` share the same Title Case heading set (`## Way of Working`, `## Walking Skeleton`, `## Testing Posture`, `## Deployment`, `## Code Style`) — read via `extractMarkdownSection` with the matching heading.

**Re-run pre-fill**: if `amadeus-team.md` already has affirmed content, read each section via `extractMarkdownSection(content, "## Way of Working")` etc. and present the existing text as the default option.

Log each question via `bun .opencode/tools/amadeus-log.ts decision` BEFORE presenting it. Log each answer via `bun .opencode/tools/amadeus-log.ts answer` after the user responds.

### Step 4: Consolidate

Write four artifacts to `<record>/inception/practices-discovery/`:

1. **team-practices.md** — descriptive, team voice. Five sections matching `amadeus-team.md` headings (`## Way of Working`, `## Walking Skeleton`, `## Testing Posture`, `## Deployment`, `## Code Style`). Each section is 1-3 sentences of plain prose synthesising Step 2 evidence + Step 3 answers.
...
After writing the four artifacts, emit `PRACTICES_DISCOVERED` via `bun .opencode/tools/amadeus-state.ts practices-event --type discovered --field "Sources Scanned: <list>" --field "Drafts: team-practices.md, discovered-rules.md"` (the tool wraps the audit emission so events stay tool-owned per the audit-first invariant).

### Step 5: Affirmation Gate

Compliance with `stage-protocol.md` checklist:

1. `bun .opencode/tools/amadeus-state.ts gate-start practices-discovery` BEFORE the affirmation question.
2. `bun .opencode/tools/amadeus-log.ts decision` for the affirmation question.
3. A structured question presents `team-practices.md` and `discovered-rules.md` for review. Options:
   - **Approve** — promote affirmed content to `.opencode/amadeus-rules/amadeus-team.md` and `.opencode/amadeus-rules/amadeus-project.md` (Step 6).
...
4. `bun .opencode/tools/amadeus-log.ts answer` after the user answers.
5. `bun .opencode/tools/amadeus-orchestrate.ts report --stage practices-discovery --result approved --user-input "<exact label>"` (or `bun .opencode/tools/amadeus-state.ts reject practices-discovery --feedback "<text>"`) — auto-emits the gate-approved/gate-rejected audit events through the owning tools.

### Step 6: Promote (On Approve Only)

Cross-row promotion of affirmed content from per-workflow audit trail into team-authored harness config is delegated to a single tool subcommand. The orchestrator does NOT read or write the target files directly — `amadeus-state.ts practices-promote` does the read+splice+write atomically and emits `PRACTICES_AFFIRMED` on success or `PRACTICES_OVERRIDE` on failure. This keeps the cross-row writes deterministic and out of the LLM's judgment path.

Run:

```
bun .opencode/tools/amadeus-state.ts practices-promote \
...
- Reads both drafts and both target files (`.opencode/amadeus-rules/amadeus-team.md` and `.opencode/amadeus-rules/amadeus-project.md`); fails closed before any write if any input is missing.
...
2. Update `Practices Affirmed Timestamp` in `<record>/amadeus-state.md` via `bun .opencode/tools/amadeus-state.ts set "Practices Affirmed Timestamp=NOW"` (the `NOW` literal expands to the current ISO 8601 timestamp; the field is part of the v7 state template).
...
- Review path: `<record>/inception/practices-discovery/` AND `.opencode/amadeus-rules/amadeus-team.md` AND `.opencode/amadeus-rules/amadeus-project.md`
...
- Prescriptive rule → `.opencode/amadeus-rules/amadeus-phase-<phase>.md` (phase-scoped)
  or `.opencode/amadeus-rules/amadeus-<org|team|project>.md` (cross-cutting)
- Verification check → new manifest at `.opencode/sensors/amadeus-<id>.md`
