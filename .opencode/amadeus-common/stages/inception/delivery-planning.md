...
Load amadeus-delivery-agent persona from `agents/amadeus-delivery-agent.md` and knowledge from `.opencode/knowledge/amadeus-delivery-agent/`.
...
**If practices-discovery executed**, read `.opencode/amadeus-rules/amadeus-team.md` via `extractMarkdownSection` for three sections that influence Bolt planning:
...
Use these affirmed practices when populating `bolt-plan.md`. If `amadeus-team.md` is empty (practices-discovery skipped), fall back to scope defaults from `rules/amadeus-org.md`.
