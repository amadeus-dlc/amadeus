...
under `.opencode/amadeus-common/stages/`. The schema (`stage-schema.ts`), the
...
`.opencode/tools/data/stage-graph.json` from the YAML sources; the runtime
...
| `lead_agent` | string | yes | agent slug; validated dynamically against `.opencode/agents/*.md` via `loadAgents()` — no hardcoded enum |
...
`.opencode/tools/data/stage-graph.json`. Consumers continue to read the compiled
