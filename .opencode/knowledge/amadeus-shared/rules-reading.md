...
at workflow start: `org → team → project → phase → stage`. `amadeus-org.md`
holds framework defaults; `amadeus-team.md` carries this team's affirmed
practices (populated by practices-discovery); `amadeus-project.md` adds
project-scoped specialisation; `amadeus-phase-<phase>.md` attaches because
the stage's frontmatter `phase: <name>` field is the pull import for the
matching phase-rule filename. Stage rules (`amadeus-stage-<slug>.md`) are
...
Heading shapes can drift between `amadeus-team.md`, `amadeus-org.md`, and the
...
1. **`.opencode/amadeus-rules/amadeus-team.md`** — team-affirmed practices. The
   primary source.
2. **`.opencode/amadeus-rules/amadeus-org.md`** — framework defaults written in team
...
def read_practice(topic):
  for layer in [amadeus-team.md, amadeus-org.md]:
...
1. `amadeus-team.md` `## Way of Working` → empty (fresh template).
2. `amadeus-org.md` `## Way of Working` → "trunk-based; base `main`, target
   `main`; squash-merge".
3. Returns `{base: "main", strategy: "squash"}` to the orchestrator
   alongside the agent's invocation of `amadeus-worktree`.

If `amadeus-team.md` `## Way of Working` had read "We use GitFlow with
