...
1. **Detect.** Run `bun .opencode/tools/amadeus-utility.ts detect --json`.
...
2. **Read the repertoire.** Read the stock scope definitions under the
   ...
   SKILL.md (or `.opencode/tools/data/stage-graph.json`) for the stage
   list and phases.
...
4. **Validate.** Before the proposal is shown, write the proposed grid to a
   ...
   `bun .opencode/tools/amadeus-graph.ts validate-grid --proposal <path> --project-type <greenfield|brownfield>`
...
- Making a scope inferable is an explicit human choice at the gate ("make
  ...
  `bun .opencode/tools/amadeus-graph.ts validate-grid --proposal <path> --keywords <granted,csv>`
