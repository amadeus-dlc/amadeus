...
- **Never hand-edit `dist/`**: it is generated and drift-guarded. Edit `packages/framework/core/` or `packages/framework/harness/<name>/`, then run `bun scripts/package.ts` and commit the regenerated trees (`bun scripts/package.ts --check` is the CI drift guard).
