...
- **Pre-commit hook** (lefthook, `lefthook.yml`, #1984): `bun install` auto-installs a `pre-commit` git hook (via the `prepare` script) that runs `bun run typecheck`, `bun run lint`, and `bun scripts/precommit-related-unit-tests.ts` — the last one runs only the `tests/unit/*.test.ts` files whose `covers: file:<path>` header names a staged file, so the hook stays fast (whole-repo typecheck+lint plus a small diff-scoped unit slice, well under the 30s budget) regardless of how many staged files touch unrelated tests. Skip with `git commit --no-verify` for an emergency/WIP commit — CI (`bun run check` + the full test suite) always re-verifies afterward, so skipping never lets a real problem land unnoticed. See `docs/reference/09-testing.md` § Trigger Points.
