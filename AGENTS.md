...
- For Amadeus self-development, explicitly select `amadeus-feature`,
  `amadeus-bugfix`, or `amadeus-refactor` based on the change type. The canonical
...
- **Test timeouts on constrained VMs**: `bun run test:ci` runs files in parallel. On a CPU-constrained cloud VM a handful of heavy integration files (notably the Codex-harness / upstream-migration / team-up suites such as `t227-codex-migration-walking-skeleton`, `t-codex-hooks-ownership`, `t-codex-hooks-migration`, `t-team-up-codex-resume`) can exceed Bun's default per-test timeout on their first (cold-compile) run and report as failures. They are **flaky timeouts, not real failures**: rerun the affected file(s) in isolation with a raised timeout, e.g. `bun test --timeout 120000 <file>`, and they pass. Expect heavy wall-clock drift on integration tests here.
...
- **Never hand-edit `dist/`** (nor the promoted root `AGENTS.md`/`CLAUDE.md` suffix): they are generated and drift-guarded. Only the hand-authored region of root `AGENTS.md` above the `# AI-DLC on Codex CLI` marker is preserved by `bun scripts/promote-self.ts` — content appended after it is dropped and fails `promote:self:check`. Edit framework sources under `packages/framework/core/` or `packages/framework/harness/<name>/`, then run `bun scripts/package.ts` and commit the regenerated trees (`bun scripts/package.ts --check` and `bun run promote:self:check` are the CI drift guards).
