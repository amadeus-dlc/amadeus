...
claude_member_cmd() {
  local m="$1" wt="${2:-$BASE/$1}" args="" mailbox_cmd
  if [ "$CONTINUE" = "1" ]; then
    args="--continue"
  fi
  ...
  printf 'cd %q && mise trust -q 2>/dev/null; %sAMADEUS_OPERATING_MODE=team CLAUDE_IDENTITY=%q %q %s %q; exec $SHELL -l' \
    "$wt" "$mailbox_cmd" "$CLAUDE_IDENTITY" "$REPO/scripts/run-claude.sh" "$args" "/agmsg mode monitor"
}
...
codex_member_cmd() {
  local m="$1" wt="${2:-$BASE/$1}" role prompt command="codex" resume_arg="" resume_uuid="" codex_home mailbox_cmd
  role="$(member_role "$m")"
  ...
  printf 'cd %q && mise trust -q 2>/dev/null; %sAMADEUS_OPERATING_MODE=team CODEX_IDENTITY=%q CODEX_HOME=%q AGMSG_CODEX_ROLE=%q %q --project %q --codex-command %q -- %s %q; exec $SHELL -l' \
    "$wt" "$mailbox_cmd" "$CODEX_IDENTITY" "$codex_home" "$role" "$CODEX_MONITOR" "$wt" "$command" "$resume_arg" "$prompt"
}
