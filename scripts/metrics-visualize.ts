...
export function main(argv: string[]): number {
  ...
  if (parsed.mode === "check") {
    ...
    if (existing !== html) {
      console.error(`${out} is stale: ${Buffer.byteLength(existing)} bytes on disk vs ${bytes} bytes rendered — run --write`);
      return 1;
    }
    console.log(`${out} is up to date (${series.length} snapshots, ${bytes} bytes)`);
    return 0;
  }
