...
function planBody(plan: Omit<MigrationPlan, "planDigest">): Omit<MigrationPlan, "planDigest"> {
  return plan;
}
...
function isReceipt(value: unknown): value is MigrationReceipt {
  ...
  return typeof row.operationId === "string" &&
    typeof row.electionId === "string" &&
    typeof row.planDigest === "string" &&
    ["planned", "applied", "verified", "failed"].includes(row.status as string) &&
    Array.isArray(row.completedSteps) &&
    (row.afterDigest === null || typeof row.afterDigest === "string") &&
    (row.failureDetail === null || typeof row.failureDetail === "string") &&
    Array.isArray(row.recoveryDetails);
}
...
function prepareMove(
  ...
  if (!sourceExists) {
    ...
    progress.recoveryDetails.push("repaired completed move from source-absent target-present evidence");
...
function commitRegistry(root: string, plan: MigrationPlan, progress: ApplyProgress): void {
  if (currentRegistryMatches(root, plan.registryBefore)) {
    ...
    progress.recoveryDetails.push("repaired completed registry update from exact registry evidence");
  } else {
...
export function applyMigrationPlan(
  ...
  if (prior?.failureDetail !== null && prior?.failureDetail !== undefined) {
    progress.recoveryDetails.push(`previous failure: ${prior.failureDetail}`);
  }
  if (prior?.status === "verified") {
    progress.recoveryDetails.push(`previous verified digest: ${prior.afterDigest ?? "missing"}`);
  }
...
function readJsonFile<T>(path: string | null, label: string): T {
...
export function main(args = process.argv.slice(2)): number {
  ...
  const plan = readJsonFile<MigrationPlan>(options.planPath, "plan");
  if (options.mode === "verify") {
    const fidelity = verifyMigrationPlan(root, plan);
    process.stdout.write(`${JSON.stringify(fidelity, null, 2)}\n`);
    return fidelity.match ? 0 : 1;
  }
  const approval = readJsonFile<MigrationApproval>(options.approvalPath, "approval");
