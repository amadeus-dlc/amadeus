...
| [domain/](domain/) | 全体のサブドメイン、境界づけられたコンテキスト、コンテキスト別モデル、契約を扱う。 |
| [discoveries.md](discoveries.md) | Intent 化前の入力テーマと判定記録を扱う。 |
...
6. [domain/bounded-contexts.md](domain/bounded-contexts.md) で全体の境界づけられたコンテキストを確認する。
7. [discoveries.md](discoveries.md) で Intent 化前の判定記録を確認する。
8. [intents.md](intents.md) で対象 Intent と依存関係を確認する。

## Intent Layer へ進む基準

次のどれかに該当する場合は、`intents/<intent-id>-<slug>/` 配下へ進む。

- 特定 Intent の要求、契約、受け入れ状態を確認する。
- 特定 Intent のユーザーストーリー、ユースケース、ユニット、ボルト、タスクを確認する。
- 特定 Intent のドメインモデル要素や契約を確認する。
