...
| BR003 | wave 並行の適用条件は「同じ wave に互いに依存しない Bolt が複数入り、worktree 分離で実行できる場合」とし、条件を満たさない場合の既定は従来どおりの直列実行のままにする。先頭の wave が 1 件でも後続の wave に複数の Bolt が入る場合は、その wave から並行にできる。既存の内部プロセスの順序（5 つの内部 skill を上から順に使う）は Bolt ごとに維持する。 | R004 | accepted |
