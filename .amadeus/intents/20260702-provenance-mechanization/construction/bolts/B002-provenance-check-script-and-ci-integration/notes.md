...
- GitHub Actions の既定 checkout は shallow clone（fetch-depth: 1）であり、記録された commit が履歴に存在しないため `provenance:check` が commit 不一致を誤検出する。実 workspace 検証を CI で成立させる前提条件として、`.github/workflows/ci.yaml` の checkout に `fetch-depth: 0` を設定した（R003 の前提）。
- rebase で commit SHA が変わった場合、旧 SHA を参照する記録は commit 不一致になる。これは check の設計どおりの検出であり、記録の再生成（`provenance:generate` の再実行）で解消する。
