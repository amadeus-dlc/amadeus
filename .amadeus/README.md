...
`codebase-analysis.md` は、既存コードに載せる brownfield Intent で、ユニット、ボルト、設計を作る前に書く。
greenfield で既存コードがない場合は作らず、`traceability.md` に対象外理由を書く。

`codebase-analysis.md` は、次の見出しを持つ。
...
既存能力、統合点、ギャップ、リスクは分けて書く。
既存コードにある事実と、これから設計で決める仮説を混同しない。
`Inception への入力` には、ユニット、Unit `design.md`、ボルト、`tasks.md` へ渡す判断材料を書く。
実装手順や Spec はここで先取りしない。

## Inception 段階の追跡

Inception 段階の `traceability.md` は、次の見出しを持つ。
...
`既存コード分析からの追跡` では、`codebase-analysis.md` がどの要求、ユースケース、ユニット、ボルト、設計の入力になったかを書く。
表は、次の列を持つ。
...
Unit 詳細は `units/<unit-id>-<slug>/unit.md` に置く。
同じディレクトリに Unit Design Brief として `design.md` を置く。

`unit.md` には、ユニット単位の価値境界、対象要求、検証観点、関連成果物を書く。
`関連成果物` から同じディレクトリの `design.md` へリンクする。

`units.md` の `詳細` は、必ず `units/<unit-id>-<slug>/unit.md` を指す。
旧構造の `units/<unit-id>.md` と `units/<unit-id>-<slug>.md` は使わない。

`design.md` には、課題解決の設計戦略、責務境界、構成候補、データと契約候補、検証観点、Bolt 分割方針、Construction への引き継ぎを書く。
タスクは、同じボルトの `bolt.md` と参照先 Unit `design.md` を入力にして作る。

## タスク内の項目

タスクは、同じボルトの `bolt.md` と参照先 Unit `design.md` を入力にして作る。
参照先 Unit `design.md` がない状態で `tasks.md` を作らない。
`tasks.md` を書く前に、要求、ユースケース、ボルト、設計、依存、観測できる完了状態を確認する。
要求、ユースケース、ボルト、設計に不足や矛盾がある場合は、タスクを作らず上流成果物へ戻る。
...
- 要求が少なくとも1つのタスクに対応している。
- 参照先 Unit `design.md` の設計戦略、責務境界、構成候補、データと契約候補、検証観点がタスクに反映されている。
- 各タスクに、具体的な `作業`、要求、ユースケース、依存、証拠候補がある。
- 暗黙の前提、準備作業、外部依存、順序制約が隠れていない。
- タスクが複数ボルトの責務をまたぐ場合は、統合タスクとして理由を明示している。
...
| `steering.md` | 任意 | `.amadeus/` 直下の成果物が増えた時 | steering layer の入口、読む順序、Intent layer との責務境界を扱う | README で十分なら作らない |
| `knowledge.md` | 任意 | 継続的に参照する背景知識が必要な時 | Amadeus DLC を進めるための背景知識と前提を扱う | README や glossary で十分なら作らない |
| `policies.md` | 任意 | 守る方針や禁止事項を分離したい時 | 成果物作成時に守る方針、禁止事項、判断基準を扱う | README で十分なら作らない |
...
| `codebase-analysis.md` | 条件付き必須 | 既存コードに載せる brownfield Intent で Unit、Bolt、設計を作る前 | 既存能力、統合点、ギャップ、リスク、Inception への入力を扱う | greenfield なら作らず、`traceability.md` に対象外理由を書く |
| `units.md` | 必須 | 最初のユニットを作る時 | ユニット一覧と参照する境界づけられたコンテキストを扱う | ユニットがなければ作らない |
| `units/<unit-id>/unit.md` | 必須 | ユニット作成時 | 個別ユニットの価値境界、検証観点、関連成果物を扱う | ユニットがなければ作らない |
| `units/<unit-id>/design.md` | 必須 | ユニット作成後、ボルト作成前 | 課題解決の設計戦略、責務境界、構成候補、データと契約候補、検証観点、Bolt 分割方針、Construction への引き継ぎを扱う | ユニットがなければ作らない |
| `bolts.md` | 必須 | 最初のボルトを作る時 | ボルト一覧を扱う | ボルトがなければ作らない |
| `bolts/<bolt-id>/bolt.md` | 必須 | ボルト作成時 | ボルトの範囲、対象ユニット、参照する Unit Design、完了条件を扱う | ボルトがなければ作らない |
| `bolts/<bolt-id>/tasks.md` | 必須 | ボルト作成後 | タスクを識別子付きチェックリストで扱う。入力は同じボルトの `bolt.md` と参照先 Unit `design.md` | ボルトがなければ作らない |
...
- [ ] 要求が関係するアクターを確認した。
- [ ] 要求が関係する外部システムを確認した。
- [ ] 要求が観測できる振る舞い、制約、受け入れ状態を表していることを確認した。
- [ ] 要求に実装方法、技術選定、内部構造を混ぜていないことを確認した。
- [ ] `scope.md` の対象と対象外を確認した。
- [ ] ストーリーが必要か判断した。
- [ ] ストーリーを作る場合は、`actors.md` にアクターが定義されていることを確認した。
- [ ] ユースケースが必要か判断した。
- [ ] ユースケースを作る場合は、アクターまたは外部システムへの参照があることを確認した。
- [ ] スコープ、目的、アクター、外部システム、ドメイン用語、契約に矛盾がないことを確認した。
- [ ] 受け入れ状態を記録した。
- [ ] 追跡表を更新した。

## 既存コード分析チェック

- [ ] 既存コードに載せる Intent か、greenfield かを確認した。
- [ ] 既存コードに載せる場合は、`codebase-analysis.md` を作成した。
- [ ] greenfield の場合は、`traceability.md` に対象外理由を記録した。
- [ ] 既存能力、統合点、ギャップ、リスクを分けて記録した。
- [ ] 既存コードにある事実と、設計で決める仮説を混同していないことを確認した。
- [ ] Unit、Unit `design.md`、Bolt、`tasks.md` へ渡す入力を記録した。
- [ ] 実装手順や Spec を先取りしていないことを確認した。
