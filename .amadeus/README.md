...
`units/<unit-id>.md` には、ユニット単位の価値、要求、境界、実装単位への参照を書く。

| 項目 | 必須度 | 役割 |
|---|---|---|
| 目標 | 必須 | ユニットで成立させる価値を示す |
| 要求 | 必須 | ユニットが満たす Requirement ID を示す |
| ストーリー | 任意 | ユーザーストーリーがある場合に Story ID を示す |
| コンテキスト | 必須 | ユニットが参照する境界づけられたコンテキスト ID を示す |
| 境界 | 必須 | ユニットが扱う外部境界を示す |
