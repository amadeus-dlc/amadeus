...
3. **If either step above created `.codex/hooks.json` during THIS task, classify the current assignment before deciding whether to stop**:
   - **AI-DLC engine work**: if this task will start, run, or resume an AI-DLC workflow, mutate the engine, or otherwise requires hooks or `HUMAN_TURN` in this task, tell the user a task restart (same worktree) is required, and stop. An already-running Codex task does not reload `hooks.json`, so a hooks file created mid-task — by `bun run build` or by `activate`, it makes no difference — leaves the UserPromptSubmit hook dark until the next task starts.
   ...
   - **No-intent delegated work**: continue only when the assignment will not invoke or mutate the AI-DLC engine and does not require hooks or `HUMAN_TURN` during this task — for example, a delegated no-intent builder implementing code directly. Treat `.codex/hooks.json` and other ignored self-install surfaces created by the build as ignored disposable generated output; do not stage or commit it.
   - **Real changes remain protected**: only known ignored build output gets this allowance. Tracked or user-owned changes are not disposable; inspect and handle them under the repository's normal safety rules.
