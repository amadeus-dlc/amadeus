...
| `present-gate` | A solo standing grant was revoked or expired before commit-time revalidation. The stage quality rituals are already complete; present the normal numbered approval gate and report `approved` after a human response. |

The orchestration engine emits eight kinds today: `run-stage`, `invoke-swarm`, `present-gate`, `ask`, `print`, `error`, `done`, `parked` (`invoke-swarm` is emitted only for an eligible Construction batch under an `autonomous` grant; `present-gate` is the fail-closed fallback for a solo standing-grant race). `dispatch-subagent` remains a documented placeholder.
...
`run-stage` folds the approval-gate decision into its `gate` field. The engine has already decided whether this stage gates for every deterministic case — bootstrap initialization stages auto-proceed (`gate: false`), an ordinary uncovered gate uses `gate: true`, and a solo-mode standing grant covering an ordinary gate uses `gate: "granted"`. One case is **not** deterministic and arrives as the sentinel `gate: "unresolved"`:
...
- **`gate: "granted"`** — run the same stage body, reviewer step, stage-completion verification, and §13 learnings ritual as `gate: true`, but do not stop for a human approval turn. Immediately call `report --stage "<directive.stage>" --result grant-approved`. The state tool revalidates the grant at commit time and records its `Grant Id`. If the grant expired or was revoked meanwhile, the engine emits `present-gate`; present the normal human approval gate and report `approved` after the response. Never convert this route into delegated approval: team mode retains its existing leader/delegation flow and does not emit `"granted"`.
