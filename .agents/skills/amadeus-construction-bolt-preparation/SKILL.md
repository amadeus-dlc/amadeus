...
Task Generation Gate を `ready_for_approval` へ到達させ、停止して人間の承認を待つ。
承認を得た場合だけ `passed` にして、実装実行へ渡せる状態にする。
...
11. `state.json.construction.bolts[]` に対象 Bolt の `taskGeneration` を作り、実装へ渡せる粒度なら `status` を `ready_for_approval` にする。更新は同梱スクリプトで行える: `bun run .agents/skills/amadeus-validator/scripts/StateScaffold.ts <workspace> bolt-preparation --intent <intent-id>-<slug> --bolt <bolt-id>-<slug> --unit <unit-id>-<slug>`（evidence と requiredBoltArtifacts は実在ファイルの走査で確定される）。
12. `ready_for_approval` へ到達したら、実装へ進まず停止して人間の承認を待つ。
13. 承認を得てから `status` を `passed` にし、`approval` evidence を追加する。承認を得ていない場合は、`ready_for_approval` のまま停止を続ける。
14. 親 skill から記録対象の質問と回答が渡された場合だけ、`amadeus-grilling` の構造に従って Grilling Decision Trail を同じ変更で更新する。
15. 実装やテスト実行は行わない。
