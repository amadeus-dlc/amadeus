---
name: amadeus-practices-discovery
description: >
  Run the AI-DLC `practices-discovery` stage (inception phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage practices-discovery --single`:
...
`/amadeus --stage practices-discovery --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage practices-discovery --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage practices-discovery --result completed
   ```
