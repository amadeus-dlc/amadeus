---
name: amadeus-feasibility
description: >
  Run the AI-DLC `feasibility` stage (ideation phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage feasibility --single`:
...
`/amadeus --stage feasibility --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage feasibility --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage feasibility --result completed
   ```
