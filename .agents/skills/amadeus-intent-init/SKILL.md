...
description: >-
  ...
  `.amadeus/intents/<intent-id>-<slug>.md`、`state.json` の作成が必要な場面では必ず使う。Ideation、requirements、use-cases、
...
1. `.amadeus/settings/templates/intents/initialized.md` と `.amadeus/settings/templates/intents/initialized/`
2. この skill に同梱された `templates/intents/initialized.md` と `templates/intents/initialized/`

`.amadeus/settings/templates/intents/initialized.md` と `.amadeus/settings/templates/intents/initialized/` は、プロジェクト固有の上書きとして扱う。
存在しない場合は、`templates/intents/initialized.md` と `templates/intents/initialized/` の標準テンプレートを使う。
...
- 依存する既存 Intent はあるか。
- `Intent のモジュールファイル` の成功条件や範囲に最低限入れる制約はあるか。
...
この場合、Grilling Decision Trail の `反映先` は `Intent のモジュールファイル` に限定する。
`scope.md`、`ideation.md`、`requirements.md` など、Intent 初期化ではまだ作らない成果物を `反映先` にしない。
...
不足している目的、成功条件、範囲は `未確認` として `Intent のモジュールファイル` に残す。
...
- Discovery Brief から作成した場合は、対象 Discovery のモジュールファイルと `.amadeus/discoveries.md`
...
| `詳細` | `[<intent-id>-<slug>.md](intents/<intent-id>-<slug>.md)` |
