---
name: amadeus-user-stories
description: >
  Run the AI-DLC `user-stories` stage (inception phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage user-stories --single`:
...
`/amadeus --stage user-stories --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage user-stories --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage user-stories --result completed
   ```
