...
description: >-
  ...
  components.md、component-methods.md、services.md、component-dependency.md、design-decisions.md を
...
対象 Intent の `state.json` で、`stages["application-design"]` が実行対象であり、状態が `pending`、`active`、`awaiting_approval`、`revising` のいずれかであることを前提にする。
...
既存コンポーネントの修正だけの場合は、成果物を作らず `stages["application-design"]` を `skipped` にし、理由を記録して `amadeus` へ戻る。

少なくとも次を読む。

- `inception/requirements-analysis/requirements.md`
- `inception/user-stories/stories.md`（実行した場合）
- `.amadeus/knowledge/codebase/<repo>/architecture.md` と `component-inventory.md`（brownfield の場合）
- `inception/practices-discovery/team-practices.md`（実行した場合）
- `.amadeus/domain-map.md` と Event Storming の成果物（存在する場合。境界の判断材料にする）
- `state.json`
...
質問を行った場合は `inception/application-design/questions.md` に記録する。
...
1. `.amadeus/settings/templates/intents/inception/application-design/`
...
- `inception/application-design/design-decisions.md`（このステージの設計判断）
- `state.json`（`stages["application-design"]` の状態と approval evidence）
- 質問を行った場合は `inception/application-design/questions.md`
...
2. `stages["application-design"].state` を `active` にする。
3. 要求、ストーリー、既存アーキテクチャ、ドメインの判断材料を読み、不足論点を質問で確認する。
4. 5 つの成果物を作る。各コンポーネントは対応する要求の識別子を参照する。
5. `stages["application-design"].state` を `awaiting_approval` にし、ゲートを提示する。
...
承認されたら `stages["application-design"].state` を `completed` にし、`stages["application-design"].approval` に `approvedAt` と `via: "conversation"` を記録する。
差し戻されたら `stages["application-design"].state` を `revising` にする。
Accept as-is が選ばれた場合は、`stages["application-design"].state` を `completed` にし、`stages["application-design"].approval` に `approvedAt`、`via: "conversation"`、`"acceptedAsIs": true` を記録し、この判断を `inception/decisions.md` に記録する。
