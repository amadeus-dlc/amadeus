...
対象 Intent ID が指定され、`.amadeus/intents/<intent-id>-<slug>/state.json` の `phase` が `ideation` の場合は、Ideation 段階の Intent として少なくとも次を対象にする。
...
対象 Intent ID が指定され、`.amadeus/intents/<intent-id>-<slug>/state.json` の `phase` が `initialized` の場合は、Initialized 段階の Intent として少なくとも次を対象にする。
...
`識別子` は Intent ID と slug を連結した Intent ディレクトリ名として扱う。
