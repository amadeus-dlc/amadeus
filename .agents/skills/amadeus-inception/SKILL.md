...
description: >-
  Amadeus の Ideation 完了済み Intent を Inception 段階へ進め、要求、受け入れ状態、必要な場合のユーザーストーリー、ユースケース、
...
この skill は、Intent から要求、必要な場合のユーザーストーリー、ユースケース、Unit、Unit Design Brief、Bolt への分解と追跡を固定する。
...
少なくとも次が存在しない場合は、作業を止めて `amadeus-ideation` または `amadeus-ideation` を案内する。
...
- 分かっている場合は、要求、利用者価値、ユースケース、Unit 境界、Unit Design Brief 候補、Bolt 境界、Construction へ渡す作業粒度の候補。
...
テンプレートの `<...>` は、Ideation 成果物、steering layer、Domain Map、Context Map、回答内容から分かる値に置き換える。
分からない項目は空欄にせず、本文と未確認事項に `未確認` として残す。
`codebase-analysis.md` は brownfield の場合だけ必須成果物に含める。
greenfield の場合は `state.json.inception.codebaseAnalysis` に対象外理由を残す。
Inception は Intent 固有の正式な Domain Model や Contracts を作らない。
`inception.gate` を `passed` にする場合は、`inception/units.md` の `コンテキスト` に Domain Map の `adopted` Bounded Context を参照させる。
既存 Boundary を参照する場合は、Domain Map の既存 `adopted` Bounded Context を使う。
新規 Boundary を採用する場合は、Finalization で Domain Map へ `adopted` Bounded Context として反映し、`根拠` に Inception の判断を置く。
Domain Map に反映できない境界候補を参照する場合は、推測で `passed` にせず、未確認事項を残して `state.json.inception.gate` を `not_ready` にする。
モデル詳細と契約条件が未確認の場合は、Construction Functional Design へ引き継ぐ未確認事項として残す。
...
| 内部 skill | 役割 | 主な成果物 |
|---|---|---|
| `amadeus-inception-codebase-analysis` | Codebase Analysis | `codebase-analysis.md`, `state.json.inception.codebaseAnalysis`、既存コードに載せる場合 |
| `amadeus-inception-requirements-definition` | 要件定義 | `requirements.md`, `requirements/**`, `acceptance.md` |
| `amadeus-inception-user-stories` | User Stories | `user-stories.md`, `user-stories/**`、人間アクターのユーザー価値表現が必要な場合 |
| `amadeus-inception-use-cases` | Use Cases | `use-cases.md`, `use-cases/**` |
| `amadeus-inception-units-generation` | Units Generation | `units.md`, `units/**`, `bolts.md`, `bolts/**` |
| `amadeus-inception-traceability-finalization` | 追跡と状態確定 | `traceability.md`, `decisions.md`, `decisions/**`, `state.json` |

`codebase-analysis.md`、`state.json.inception.codebaseAnalysis` を作成または補修する場合は、必ず `amadeus-inception-codebase-analysis` を使う。

`requirements.md`、`requirements/**`、`acceptance.md` を作成または補修する場合は、必ず `amadeus-inception-requirements-definition` を使う。

`user-stories.md`、`user-stories/**` を作成または補修する場合は、必ず `amadeus-inception-user-stories` を使う。

`use-cases.md`、`use-cases/**` を作成または補修する場合は、必ず `amadeus-inception-use-cases` を使う。

`units.md`、`units/**`、`bolts.md`、`bolts/**` を作成または補修する場合は、必ず `amadeus-inception-units-generation` を使う。

`traceability.md`、`decisions.md`、`decisions/**`、`state.json` を作成または補修する場合は、必ず `amadeus-inception-traceability-finalization` を使う。

Inception 全体を進める場合は、内部 skill を上から順に使う。

Codebase Analysis は、既存コードに載せる Intent の場合だけ `codebase-analysis.md` を作る。

greenfield の Intent では `codebase-analysis.md` を作らず、`state.json.inception.codebaseAnalysis` に対象外理由を残す。

User Stories は、人間アクターのユーザー価値表現が必要な場合だけ実行する。

システムまたは外部システムだけが相互作用主体である場合は、User Stories を省略して Use Cases へ進む。
...
- 既存の `inception/requirements.md`
- 既存の `inception/acceptance.md`
- 既存の `inception/user-stories.md`、存在する場合
- 既存の `inception/use-cases.md`
- 既存の `inception/codebase-analysis.md`
- 既存の `inception/units.md`
- 既存の `inception/bolts.md`
- 既存の `inception/units/*/design.md`
- steering layer
...
| 状態 | 自動選択 | 理由 |
|---|---|---|
| `Intent のモジュールファイル` または `state.json` がない | 停止 | Intent Recordが不足しているため |
...
既存の `Intent のモジュールファイル`、`ideation/scope.md`、`ideation/ideation.md`、`ideation/traceability.md`、`ideation/decisions.md`、steering layer、Domain Map、Context Map から分かることは質問しない。
...
- Construction の Task Generation で Task 化するために、Bolt へ渡す作業粒度、依存、証拠候補は何か。
- Intent、Requirement、Story、Use Case、Unit、Bolt のピラミッド構造が崩れる場合、その理由は何か。
- User Stories を省略できる相互作用主体か。

質問した場合は、その場で成果物を作らず、ユーザーの回答を待つ。
回答を受け取ってから Inception 成果物を作る。
回答に記録対象の判断が含まれる場合は、Inception 成果物への反映と同じ変更で `inception/grillings.md` と `inception/grillings/Gxxx-*.md` を対象 Intent 配下に更新する。
この場合、親 skill は記録対象の質問、確認理由、推奨回答、推奨理由、ユーザー回答、確定判断、反映先を、委譲先の内部 skill へ明示的に渡す。
Grilling Decision Trail の実ファイル更新は、該当する Inception 成果物を更新する内部 skill が同じ変更で行う。
...
ユーザーが明示した場合だけ使う。
質問せず、既存 Ideation 成果物、Domain Map、Context Map から分かる範囲で Inception 成果物を作る。
...
既存の `inception/grillings.md` または `inception/grillings/Gxxx-*.md` が存在し、構造だけが壊れている場合は、Grilling Decision Trail の索引、session ファイル名、必須見出し、表列、相対リンク、状態、反映先、判断 ID、置き換え先、質問記録の参照だけを補修してよい。
...
- `inception/codebase-analysis.md`
- `inception/units.md`
- `inception/bolts.md`
- `inception/bolts/*.md`
...
- `.amadeus/intents/<intent-id>-<slug>/inception/user-stories.md`、必要な場合
- `.amadeus/intents/<intent-id>-<slug>/inception/user-stories/<story-id>-<slug>.md`、必要な場合
...
- `.amadeus/intents/<intent-id>-<slug>/state.json`
- 記録対象の質問と回答が発生した場合だけ、`.amadeus/intents/<intent-id>-<slug>/inception/grillings.md`
- 記録対象の質問と回答が発生した場合だけ、`.amadeus/intents/<intent-id>-<slug>/inception/grillings/Gxxx-*.md`

`Intent のモジュールファイル`、`scope.md`、`ideation.md` は原則として既存内容を尊重する。
...
User Story は、人間アクターのユーザー価値表現が必要な場合だけ作る。

Use Case は Requirement を参照し、User Story がある場合だけ Story を参照する。
...
`Intent のモジュールファイル`、`scope.md`、`ideation.md`、steering layer、Domain Map、Context Map を読み、次を確認する。

- 要求が、利用者、運用者、外部システムから観測できる振る舞い、制約、受け入れ状態を表している。
- 要求が、実装方法、技術選定、内部 API、ファイル構造、データモデル、コンポーネント構成を固定していない。
- `scope.md` の対象境界、実行制御、成果物深度、検証戦略、`ideation.md` の未確定事項が要求案に反映されている。
...
- steering layer に既存の `BCnnn` がある場合、同じ識別子を別名または別役割で使っていない。
...
`Inception への入力` には、Unit Design Brief、Bolt、Construction の Task Generation へ渡す判断材料を書く。
...
Unit を切るときは、Domain Map の `adopted` Bounded Context、または Inception で採用判断する境界候補を参照する。
Domain Map 未登録の Bounded Context を仮参照する場合は、Finalization で Domain Map へ反映する必要がある未確認事項として残す。

`詳細` は `units/<unit-id>-<slug>.md` を指す。
Unit のモジュールファイルと同じ stem のモジュールディレクトリに `design.md` を置く。
...
`識別子` は `ITnnn` にする。
`repository` と `path` は実装対象を識別する最小情報であり、分からない場合は `未確認` と書く。
`branch`、`PR`、`CI` は分かっている場合だけ記録し、該当しない場合は `なし`、未確定の場合は `未確認` と書く。
複数リポジトリまたは複数 path を扱う場合は、実装対象行を分ける。

`関連成果物` には、対応するモジュールディレクトリ配下の `design.md` への相対リンクを置く。
...
複数 Unit をまたぐ Bolt では、`設計` に複数の Unit Design Brief へのリンクを書き、Bolt のモジュールファイルの `複数 Unit を扱う理由` に理由を残す。
...
`実装対象` の表は Unit と同じ列を持つ。
Bolt が複数リポジトリをまたぐ場合は、変更対象ごとに `ITnnn` の行を分ける。
`repository` と `path` は未確定なら `未確認` と書き、`branch`、`PR`、`CI` は `なし` または `未確認` を許容する。

## Construction Task 生成への引き継ぎ

Inception では `tasks.md` を作らない。
Task は Construction phase の Task Generation で、Functional Design、Unit Design Brief、対象 Bolt のモジュールファイルを根拠に生成する。

Bolt には、Task Generation で Task 化できるように次を残す。

- 対象 Unit と Unit Design Brief への参照。
- 対象要求、対象ユースケース、完了条件。
- repository、path、branch、PR、CI を含む実装対象。値が未確定の場合は `未確認` として残す。
- 実施境界、依存、未確認事項。
- Functional Design と Task Generation で確定する未確認事項。
...
`対象境界からの追跡` では、`ideation/scope.md` の採用済み `SC-IN-*` がどの Requirement、Story、Use Case、Unit、Bolt に展開されたかを追跡する。
対象外境界は、Inception 成果物に混入させず、必要な場合は対象外制約として `decisions.md` または個別判断に残す。
`要求からの追跡` の `ボルト` 列と `ボルトからの追跡` の `要求` 列は双方向に一致させる。
たとえば `要求からの追跡` で R004 が B001 のみを参照する場合、`ボルトからの追跡` で B002 の要求に R004 を含めない。
表は、次の列を持つ。
...
Story が必要な場合だけ、`inception/user-stories.md` を `requiredArtifacts` に含める。
Story が不要な場合は、`requiredStoryArtifacts` を空配列にする。
...
Inception は Intent 固有の正式な Domain Model や Contracts を `requiredArtifacts` に含めない。
Inception は Intent 固有の正式な Domain Model や Contracts を作らない。
`inception.gate` を `passed` にする場合は、`inception/units.md` の `コンテキスト` が Domain Map の `adopted` Bounded Context を参照している必要がある。
新規 Boundary を採用する場合は、Finalization で Domain Map へ `adopted` Bounded Context として反映し、`根拠` に Inception の判断を置く。
Domain Map に反映できない Bounded Context を仮参照する場合は、`inception.gate` を `passed` にしない。

## 禁止事項

- `state.json.ideation.gate` が `passed` でない Intent を Inception へ進めない。
- Domain Map の `adopted` Bounded Context を参照または反映しないまま `inception.gate` を `passed` にしない。
- 詳細な domain model、contract を推測で作らない。
- Intent 固有の正式な Domain Model や Contracts を Inception で作らない。
...
- 用語、概念、Domain Map、Context Map、詳細な Domain Model、契約が不足している場合: `amadeus-domain-grilling` または `amadeus-domain-modeling`
