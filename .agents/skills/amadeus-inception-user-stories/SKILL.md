...
description: >-
  Amadeus Inception の内部 skill。要件定義済み Intent に対して、ユーザーストーリーだけを実行し、
...
要件定義の成果物から、アクター価値としてのユーザーストーリーを作る。
...
1. Requirement と受け入れ状態から、アクター価値をユーザーストーリーとして切る。
2. ユーザーストーリーが Requirement の言い換えだけになっている場合は、粒度不足を疑う。
3. 自然な粒度であれば、後続の追跡確定で理由を残せるように根拠を本文に残す。
4. 親 skill から記録対象の質問と回答が渡された場合だけ、`amadeus-grilling` の構造に従って Grilling Decision Trail を同じ変更で更新する。
