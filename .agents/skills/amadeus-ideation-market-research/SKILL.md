...
差し戻されたら `stages["market-research"].state` を `revising` にする。
Accept as-is が選ばれた場合は、`stages["market-research"].state` を `completed` にし、`stages["market-research"].approval` に `approvedAt`、`via: "conversation"`、`"acceptedAsIs": true` を記録し、この判断を `ideation/decisions.md` に記録する。
