...
回答が不足していても、作業を止め続けない。
分からない項目は `未確認` とし、未確認事項に確認すべき問いを残す。
...
4. brownfield または `discovery` の場合は、既存の README、設計資料、ドメイン資料、`.amadeus/` を読み、既存の正本を尊重する。
5. `guided` または `discovery` の場合は、足りない情報だけを最大5問で質問する。
...
10. 昇格済みの `amadeus-execution-validator` が使える場合は、全体成果物だけを検証する。
