...
`.amadeus/discoveries.md` が存在しない場合は、`bun run .agents/skills/amadeus-validator/scripts/IndexGenerate.ts <workspace>` を実行して作成してよい。
...
`amadeus-discovery dry-run` は、読み取り専用で Discovery 候補を表示する入口である。
この skill で `dry-run` を扱う場合は、候補表示に必要な入力境界を確認する。

必要に応じて、`dry-run` は `amadeus-history-review` の過去分析結果と `amadeus-learning-review` の学習分類結果を入力にできる。
...
- `.amadeus/discoveries.md`（`IndexGenerate.ts` の再生成による更新。手書きしない）
...
`.amadeus/discoveries.md` は、`discoveries/*.md` の H1 と `## 推奨次アクション`、`discoveries/*/state.json` の `status` と `decision` を情報源にして、`bun run .agents/skills/amadeus-validator/scripts/IndexGenerate.ts <workspace>` で再生成する共有インデックスである。
`一覧` へ行を手書きで追加しない。
...
| 列 | 意味 |
|---|---|
| `識別子` | Discovery のモジュールディレクトリ名（`<discovery-id>`）。 |
| `テーマ` | `discoveries/<discovery-id>.md` の H1 から導出する入力テーマ。 |
| `状態` | `discoveries/<discovery-id>/state.json` の `status`。 |
| `判定` | `discoveries/<discovery-id>/state.json` の `decision`。 |
| `推奨次アクション` | `discoveries/<discovery-id>.md` の `## 推奨次アクション` から導出する、次に使う skill または no-action 理由。 |
| `詳細` | `[Discovery のモジュールファイル](discoveries/<discovery-id>.md)` |

行の並び順は識別子の辞書順で決定論的に生成される。
...
2. `bun run .agents/skills/amadeus-validator/scripts/IndexGenerate.ts <workspace>` で `.amadeus/discoveries.md` を再生成し、対象 Discovery の詳細リンクが含まれる。
