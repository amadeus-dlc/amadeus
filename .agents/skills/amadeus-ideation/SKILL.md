...
description: >-
  Amadeus の既存 Intent に対して Ideation 段階のプロセスを進める公開入口。`Intent のモジュールファイル` と `state.json` を持つ
...
`state.json` が存在しないが `Intent のモジュールファイル` は存在する場合は、Intent 登録状態を確認し、`amadeus-intent-init` で補修するよう案内する。
...
ユーザーが明示した場合だけ使う。
質問せず、既存 `Intent のモジュールファイル` と `state.json` から分かる範囲で Ideation 成果物を作る。
