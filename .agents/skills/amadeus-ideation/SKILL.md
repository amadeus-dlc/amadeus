...
description: >-
  Amadeus の既存 Intent に対して Ideation 段階のプロセスを進める公開入口。`Intent 正本ファイル` と `state.json` を持つ
...
`state.json` が存在しないが `Intent 正本ファイル` は存在する場合は、Intent 登録状態を確認し、`amadeus-intent-init` で補修するよう案内する。
...
| 状態 | 自動選択 | 理由 |
|---|---|---|
| `Intent 正本ファイル` または `state.json` がない | 停止 | Intent の入れ物が不足しているため |
...
既存の `Intent 正本ファイル`、`state.json`、`.amadeus/intents.md`、steering layer から分かることは質問しない。
...
ユーザーが明示した場合だけ使う。
質問せず、既存 `Intent 正本ファイル` と `state.json` から分かる範囲で Ideation 成果物を作る。
