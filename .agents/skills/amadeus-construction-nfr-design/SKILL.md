...
NFR Requirements を実行しなかった場合は、成果物を作らず対象 Unit の checkbox を `[S]` にし、理由を記録して `amadeus` へ戻る。
...
1. checkbox が `[ ]` の場合だけ Condition を判定する。偽なら対象 Unit を `[S]` にして終了する。`[-]`、`[?]`、`[R]` からの再開では再判定しない。
