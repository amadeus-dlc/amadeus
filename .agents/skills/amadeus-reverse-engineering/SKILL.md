---
name: amadeus-reverse-engineering
description: >
  Run the AI-DLC `reverse-engineering` stage (inception phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage reverse-engineering --single`:
...
`/amadeus --stage reverse-engineering --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage reverse-engineering --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage reverse-engineering --result completed
   ```
