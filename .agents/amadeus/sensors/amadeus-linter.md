...
id: linter
kind: deterministic
command: bun .claude/tools/amadeus-sensor-linter.ts
...
1. **`lint:check` script** — when the workspace's `package.json` declares a
   ...
   error-severity violation carrying the harness's own diagnostic output.
   This keeps the sensor generic (no workspace-specific paths) while making
   gate-time fires reflect the project's real lint rules.
2. **eslint fallback** — without a `lint:check` script, the sensor probes
   eslint (`bunx eslint`). Absent tool or config exits 127, which the
   dispatcher reclassifies as a quiet PASS (tool-unavailable). Multi-language
