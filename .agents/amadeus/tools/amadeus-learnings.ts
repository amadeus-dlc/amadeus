...
// amadeus/spaces/<space>/memory/ (neutral names, no `aidlc-` prefix) — NOT a
...
function handleSurface(args: string[], projectDir: string): void {
  ...
  if (!slug) {
    fail("Usage: amadeus-learnings.ts surface --slug <stage-slug> [--project-dir <path>]", 1);
  }
  ...
  // `amadeus/spaces/<space>/intents/<dirName>` など可変長）。phase は末尾から
...
// when the heading is absent (DETERMINISM safety net, amadeus-lib.ts), so the
...
function handlePersist(args: string[], projectDir: string): void {
  ...
  if (!selectionsJson) {
    fail(
      "Usage: amadeus-learnings.ts persist --slug <stage-slug> --selections-json <path> [--project-dir <path>]",
      1
    );
  }
...
// shipped manifest shape (dist/claude/.claude/sensors/amadeus-linter.md).
...
function printHelp(): void {
  process.stdout.write(
    [
      "amadeus-learnings.ts — §13 learning-gate tool (tool-as-actor).",
      "",
      "Subcommands:",
      "  surface --slug <stage-slug> [--project-dir <path>]",
...
function main(): void {
  ...
  if (cmd === undefined) {
    fail("Usage: amadeus-learnings.ts <surface|persist|--help>", 2);
  }
