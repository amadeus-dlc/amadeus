...
export interface StageFrontmatter {
  ...
  // Read by the stage-completion artifact guard in amadeus-state.ts (issue #366).
  // Absent -> false.
...
// for the naming rules and amadeus-graph.ts for the derived registry.
...
export function validateStageFrontmatter(
  ...
  // (amadeus-graph.ts guards the whole reviewer block on `reviewer` being
