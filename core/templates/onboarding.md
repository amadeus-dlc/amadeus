...
- **Session skills** (read-only, user-invocable): `{{HARNESS_DIR}}/skills/amadeus-session-cost/`, `{{HARNESS_DIR}}/skills/amadeus-replay/`, `{{HARNESS_DIR}}/skills/amadeus-outcomes-pack/` — typed as `{{INVOKE}}-session-cost`, `{{INVOKE}}-replay`, `{{INVOKE}}-outcomes-pack`. Each pulls every count from `bun {{HARNESS_DIR}}/tools/amadeus-runtime.ts summary --json` (no LLM-side counting). Classified `read-only`: they never advance the workflow stage pointer and never emit audit events. `amadeus-session-cost` and `amadeus-replay` print to the terminal only; `amadeus-outcomes-pack` is the only one that writes a file (`OUTCOMES.md`).
