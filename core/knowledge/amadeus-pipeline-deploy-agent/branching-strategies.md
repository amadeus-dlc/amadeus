...
**Worktree mapping.** Create: `bun .claude/tools/amadeus-worktree.ts create --slug <bolt-slug> --base main`. Merge: `--target main --strategy squash`. Each Bolt = one squash commit on `main`.
...
3. **Create**: invoke `bun .claude/tools/amadeus-worktree.ts create --slug <bolt-slug> --base main`.
4. **Merge** (after Bolt gate approval): caller must be on `main` at the main checkout. Invoke `bun .claude/tools/amadeus-worktree.ts merge --slug <bolt-slug> --target main --strategy squash --message "<commit message>"`.
...
3. **Create**: `bun .claude/tools/amadeus-worktree.ts create --slug <bolt-slug> --base main`.
4. **Merge**: `bun .claude/tools/amadeus-worktree.ts merge --slug <bolt-slug> --target main --strategy <squash|merge> [--message "<msg>"]`. With `--strategy merge`, a no-fast-forward merge commit preserves the bolt branch's individual commits.
...
3. **Create**: `bun .claude/tools/amadeus-worktree.ts create --slug <bolt-slug> --base <line>`.
4. **Merge**: caller on `<line>` at the main checkout. `bun .claude/tools/amadeus-worktree.ts merge --slug <bolt-slug> --target <line> --strategy merge`.
...
The orchestrator's halt-and-ask quotes the `detail` field verbatim. See `amadeus-common/protocols/stage-protocol.md` § "Halt-and-ask on failure" and `core/skills/amadeus/SKILL.md` § "Halt-and-ask failure handling" for the full prompt shape and preservation invariant.
