...
```
bun {{HARNESS_DIR}}/tools/amadeus-worktree.ts info --slug <kebab-slug>
```
...
- Implementation: `{{HARNESS_DIR}}/tools/amadeus-worktree.ts` (`handleInfo` handler)
- Test: `tests/unit/t72-worktree-info.sh`
- Caller: `{{HARNESS_DIR}}/skills/amadeus/SKILL.md` (per-Bolt-loop halt-and-ask flow)
...
- Audit-format spec: `{{HARNESS_DIR}}/knowledge/amadeus-shared/audit-format.md` `WORKTREE_CREATED` row
