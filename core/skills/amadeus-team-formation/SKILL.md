---
name: amadeus-team-formation
description: >
  Run the AI-DLC `team-formation` stage (ideation phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage team-formation --single`:
...
`/amadeus --stage team-formation --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage team-formation --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage team-formation --result completed
   ```
