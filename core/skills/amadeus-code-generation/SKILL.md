---
name: amadeus-code-generation
description: >
  Run the AI-DLC `code-generation` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage code-generation --single`:
...
`/amadeus --stage code-generation --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage code-generation --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage code-generation --result completed
   ```
