---
name: amadeus-refined-mockups
description: >
  Run the AI-DLC `refined-mockups` stage (inception phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage refined-mockups --single`:
...
`/amadeus --stage refined-mockups --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage refined-mockups --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage refined-mockups --result completed
   ```
