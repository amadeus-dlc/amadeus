---
name: amadeus-infrastructure-design
description: >
  Run the AI-DLC `infrastructure-design` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage infrastructure-design --single`:
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage infrastructure-design --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage infrastructure-design --result completed
   ```
