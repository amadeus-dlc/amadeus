---
name: amadeus-approval-handoff
description: >
  Run the AI-DLC `approval-handoff` stage (ideation phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage approval-handoff --single`:
...
`/amadeus --stage approval-handoff --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage approval-handoff --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage approval-handoff --result completed
   ```
