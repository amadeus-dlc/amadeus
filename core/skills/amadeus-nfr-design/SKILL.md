---
name: amadeus-nfr-design
description: >
  Run the AI-DLC `nfr-design` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/amadeus --stage nfr-design --single`:
...
`/amadeus --stage nfr-design --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts next --stage nfr-design --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .agents/amadeus/tools/amadeus-orchestrate.ts report --single --stage nfr-design --result completed
   ```
