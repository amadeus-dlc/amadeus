...
const skipped: Collector = { name: "bugs", collect: () => ({ ok: "skipped", name: "bugs", reason: "no token" }) };
describe("t221 metrics snapshot CLI", () => {
  ...
  test("skipped collectors are named after the count and excluded from it", () => {
    expect(runCli(["--write"], { env, collectors: [good, skipped], writer: () => "/tmp/metrics/x.json" }).line).toBe("OK 1 collectors (skipped: bugs) /tmp/metrics/x.json");
    expect(runCli(["--check"], { env, collectors: [good, skipped] }).line).toBe("CHECK OK 1 collectors (skipped: bugs)");
  });
