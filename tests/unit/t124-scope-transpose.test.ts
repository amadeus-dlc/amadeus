...
// canonical sole-writer. `compile --check` now validates source compilation
// invariants without comparing generated bytes.
//
...
// `compile --check`'s exit codes with clean / stale / missing generated grids — drive
...
describe("amadeus-graph compile / --check (Bun spawnSync env seam)", () => {
  ...
  test("compile --check does not require generated scope-grid bytes [.sh test 11]", () => {
    ...
    const gridPath = mkTempPath("grid");
    copyFileSync(GRAPH_JSON, graphPath);
    // gridPath never created: source compilation remains the authority.
    expect(existsSync(gridPath)).toBe(false);
    const r = runGraph(["compile", "--check"], graphPath, gridPath);
    expect(r.status).toBe(0);
    expect(`${r.stdout ?? ""}${r.stderr ?? ""}`).toContain(
...
describe("compile preserves composed scope-grid entries", () => {
  test("a composed entry survives recompile while the cross-harness invariant names its divergence", () => {
    const graphPath = mkTempPath("graph");
    const gridPath = mkTempPath("grid");
    copyFileSync(GRAPH_JSON, graphPath);
    copyFileSync(GRID_JSON, gridPath);
    ...
    // A one-surface composed entry is valid runtime state, but it is not
    // cross-harness-equal and the semantic invariant reports that fact.
