...
import {
  ...
  renderHtml,
  svgLinePath,
} from "../../scripts/metrics-visualize";
...
describe("chart axes", () => {
  test("y-axis uses five evenly spaced ticks for a changing range", () => {
    expect(axisTicks(80, 84)).toEqual([80, 81, 82, 83, 84]);
  });
  test("a flat range uses one centered value tick", () => {
    expect(axisTicks(7, 7)).toEqual([7]);
  });
  test("x-axis labels are limited to five representative snapshots", () => {
    expect(axisTickIndices(10)).toEqual([0, 2, 5, 7, 9]);
...
describe("renderHtml", () => {
  ...
  test("charts include numeric y-axis ticks, horizontal grid lines, and date labels", () => {
    expect(html).toContain('class="grid-line"');
    expect(html).toContain('class="axis-label y-axis-label"');
    expect(html).toContain('class="axis-label x-axis-label"');
    expect(html).toContain(">80.5</text>");
    expect(html).toContain(">81.25</text>");
    expect(html).toContain(">2026-07-01</text>");
    expect(html).toContain(">2026-07-02</text>");
  });
