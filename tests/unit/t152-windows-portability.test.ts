...
describe("t152 Windows portability guard", () => {
  ...
  test("Windows path separators are normalized at harness and sensor boundaries", () => {
    const sensor = read("dist/claude/.claude/tools/amadeus-sensor.ts");
  ...
  test("swarm convergence check runs via a portable shell, not a hardcoded bash argv", () => {
    // #5: amadeus-swarm.ts checkConverged() previously launched the check command
  ...
  test("Windows graph and type-check sensor paths avoid Bun substrate traps", () => {
    const lib = read("dist/claude/.claude/tools/amadeus-lib.ts");
    expect(lib).toContain("function loadScopeGridForMapping()");
    expect(lib).not.toMatch(/const \{ loadScopeGrid \} = require\("\.\/amadeus-graph\.ts"\)/);

    const typeCheck = read("dist/claude/.claude/tools/amadeus-sensor-type-check.ts");
