...
describe("stateFilePath()", () => {
  ...
  test("never falls back to the flat amadeus-docs/ root (P9 end state)", () => {
    expect(stateFilePath(PROJ)).not.toBe(join(PROJ, "amadeus-docs", "amadeus-state.md"));
    expect(stateFilePath(PROJ).includes(`${SEP}amadeus-docs${SEP}`)).toBe(false);
  });
});
...
describe("stateFilePath() / auditFilePath() relationship", () => {
  test("both build under the same intents record root but DIFFER", () => {
    ...
    expect(stateLeaf).toBe("amadeus-state.md");
