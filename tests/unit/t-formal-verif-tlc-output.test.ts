...
describe("TLC 1.7.4 -tool closed output grammar", () => {
  ...
  test("accepts the temporal-property lifecycle emitted by the production model", () => {
    ...
    expectHarnessError(
      temporalOutput
        .replace(
          "Checking temporal properties for the complete state space with 2 total distinct states at (2026-07-21 09:26:25)",
          "Checking temporal properties for a partial state space",
        ),
    );
    expectHarnessError(
      temporalOutput
        .replace(
          "Finished checking temporal properties in 1ms at 2026-07-21 09:26:25",
          "Finished checking temporal properties incorrectly",
        ),
    );
  });
