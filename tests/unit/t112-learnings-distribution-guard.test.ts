// covers: cli:amadeus-learnings(persist)
//
// t112 — isFrameworkDistributionPath guard recognises the RELOCATED framework
// tree (v0.6.0 milestone 0 — amadeus-claude-code/ -> dist/claude/). Migrated from
// tests/unit/t112-learnings-distribution-guard.sh (TAP plan 3, 3 bun spawns).
//
// Mechanism: cli. The guard `isFrameworkDistributionPath` (amadeus-learnings.ts
...
// the scaffolded `amadeus-bad.md` are observables only reachable through the
...
// the real tool).
//
// Source under test (dist/claude/.claude/tools/amadeus-learnings.ts):
//   :502 manifestPath = sensorManifestPath(projectDir, sensorId)
//          => join(projectDir, ".claude", "sensors", `amadeus-${sensorId}.md`)
...
//          - the RELOCATED segment (was join("amadeus-claude-code", ...))
...
//        -> "refusal leaves no amadeus-bad.md scaffolded under the framework tree"
 ...
 * .md under .claude/amadeus-common/stages/inception/, a runtime-graph, and a
...
function mkTempRoot(): string {
  const d = toPortablePath(mkdtempSync(join(process.env.TMPDIR || tmpdir(), "amadeus-t112-")));
  tempDirs.push(d);
  return d;
}

describe("t112 amadeus-learnings persist — framework-distribution guard (migrated from t112-learnings-distribution-guard.sh, plan 3)", () => {
  // ===========================================================================
  ...
  // is .../dist/claude/.claude/sensors/amadeus-bad.md, which
  ...
  test("relocated framework path (dist/claude/.claude/sensors) is refused with exit 1 [.sh test 1]", () => {
    ...
    // (amadeus-learnings.ts:507) names the offending manifest path, proving the
  ...
  test("ordinary project path is accepted and the manifest lands under the project's .claude/sensors [.sh test 3]", () => {
    ...
    expect(existsSync(join(projroot, ".claude", "sensors", "amadeus-bad.md"))).toBe(true);
    // STRONGER than the .sh: the scaffolded manifest carries the sensor id, and
    // the two-write bind (amadeus-learnings.ts:532) appended the id to the seeded
    // stage's `sensors:` frontmatter — proving the accept arm ran to completion,
    // not merely past the guard.
    const manifest = readFileSync(
      join(projroot, ".claude", "sensors", "amadeus-bad.md"),
      "utf-8",
    );
