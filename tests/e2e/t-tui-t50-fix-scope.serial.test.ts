...
// interactive fix journey a user actually lives was never tested. This drives the
...
//   - a fix workflow starts from `/amadeus fix` on a fresh BROWNFIELD workspace
//     (statusline leaves `ready` for a live phase),
//   - the answer-gate clears the Initialization + Inception gates (fix SKIPS all
...
//       * the born intent's amadeus-state.md records the `fix` scope + a brownfield
...
//
// WHY PATTERN B (not A): fix is a MULTI-STEP journey (Initialization +
...
//
// WHY a MILESTONE terminator (not full fix completion): the .sh ITSELF never
// asserted full fix completion — its strongest stage assertion (test 13) was
...
// (init 3 + the 2 Inception stages fix executes: reverse-engineering +
...
// were `assert_gt`/`skip`-on-miss soft probes; FINDING: full fix completion
...
// sets 600). A fix run-through (Initialization + the heavy reverse-engineering
...
function skipReason(): string | null {
  if (process.env.AMADEUS_TUI_LIVE !== "1") {
    return "set AMADEUS_TUI_LIVE=1 to run the live fix journey (uses Bedrock tokens)";
  }
...
describe("t-tui-t50-fix-scope (answering gates advances fix lifecycle on disk)", () => {
  test.skipIf(SKIP_REASON !== null)(
    ...
    async () => {
      ...
      try {
        ...
        // --- submit the fix workflow command --------------------------------
        // Use the EXPLICIT `--scope fix` flag, not the bare freeform `fix`
        // keyword. The shipped distributable settings.json pins
        // AMADEUS_DEFAULT_SCOPE=workshop, so a bare freeform `/amadeus fix ...`
        ...
        // workflow START (fix vs the workshop env-default vs keyword-autodetect's
        ...
        // job, not t50's.) The explicit `--scope fix` flag WINS silently and
        // gatelessly (SKILL.md:105 "explicit CLI flag wins" + :170a auto-confirm;
        // proven live by t29's override case), so t50 cleanly tests the fix
        // LIFECYCLE without the env-default derail. The trailing description still
        // flows to the known-scope handler (SKILL.md:122: `--scope fix` with no
        // state "behaves like /amadeus fix") so the step-6 free-text "what to
        ...
        // Inception stages fix executes: reverse-engineering +
        ...
        // as a backstop, and it false-fired here: fix's first Inception stage is
        ...
        // workspace-scaffold init stage for every scope, fix included).
        ...
        // > 200 bytes. fix on a brownfield workspace ALWAYS runs RE, so this is
