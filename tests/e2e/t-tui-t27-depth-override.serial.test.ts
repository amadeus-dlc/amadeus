...
// AMADEUS_TUI_LIVE=1 so a bare `--e2e` on a laptop SKIPs; tmux/claude/distributable
...
//
// SPAWN, not import (D-TUI-7): Bun spawns tui-drive.ts on every platform. The tui-drive.ts
...
// Windows Bun.Terminal backend (run later via SSM) captures identically — authored for
