...
// a conductor that never opens the election SKILL still reaches the automatic solo
...
// automatic solo config — if the hook were only in the SKILL, the turn would end at
...
// `--trigger auto` invocation in the transcript, is the execution proof.
//
...
describe("t-exec-codex-autosolo-s13 — §13 reaches the automatic solo election without the SKILL (#1735)", () => {
  test.skipIf(SKIP_REASON !== null)(
    `a protocol-only §13 run opens the election with --trigger auto${SKIP_REASON ? ` [SKIP: ${SKIP_REASON}]` : ""}`,
    () => {
      ...
      try {
        ...
        expect(r.out).toContain("--trigger auto");
        ...
        expect(r.out).not.toContain("solo-election-manual-trigger-required");
      } finally {
        cleanup();
      }
    },
