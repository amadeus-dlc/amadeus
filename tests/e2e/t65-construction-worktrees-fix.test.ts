// covers: subcommand:amadeus-bolt:dispatch-event, subcommand:amadeus-utility:init, audit:MERGE_DISPATCH_INVOKED, scope:fix
//
// bun:test port of tests/e2e/t65-construction-worktrees-fix.sh (TAP
...
// `fix` scope — a skeleton-off (incremental) scope. The four SKILL.md
...
//       scope-mapping.json). fix.stages["code-generation"] === "EXECUTE"
//       (incremental scopes still build code) and
//       fix.stages["practices-discovery"] === "SKIP" (incremental scopes
...
//   .sh assert_scope_codegen_mode "fix" "EXECUTE"   -> "1 (none): scope-grid fix code-generation = EXECUTE"
//   .sh assert_dispatch_event_runs_for_scope fix    -> "2 (cli): dispatch-event MERGE_DISPATCH_INVOKED emits for fix"
//   .sh PD_MODE == SKIP                                 -> "3 (none): scope-grid fix practices-discovery = SKIP"
...
// stub integration project + `amadeus-utility init --force --scope fix`, then
...
describe("t65 Construction-worktrees per-scope contract — fix (migrated from t65-construction-worktrees-fix.sh, plan 4)", () => {
  // ===========================================================================
  // Data assertions (mechanism none): read the compiled scope-grid.json.
  // ===========================================================================
  test("1 (none): scope-grid fix code-generation = EXECUTE [.sh assert_scope_codegen_mode]", () => {
    // The .sh ran `bun -e "require(grid).fix.stages['code-generation']"` and
  ...
  test("3 (none): scope-grid fix practices-discovery = SKIP [.sh PD_MODE check]", () => {
    // The .sh ran `bun -e "require(MAPPING).fix.stages['practices-discovery']"`
  ...
  // ===========================================================================
  // CLI assertions (mechanism cli): one shared inited fix project.
  // Mirrors setup_construction_project("fix"):
  //   setup_integration_project --with-greenfield-stub
  //   bun amadeus-utility init --project-dir <p> --force --scope fix
  // ===========================================================================
  let proj: string;
  ...
  test("2 (cli): dispatch-event MERGE_DISPATCH_INVOKED emits for fix [.sh assert_dispatch_event_runs_for_scope]", () => {
