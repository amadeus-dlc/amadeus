...
describe("t01 — shipped-tree file-structure invariant (mechanism: none)", () => {
  ...
  // .sh enforced.
  test("asserts EXACTLY 71 shipped paths (TAP plan 63 + 2 reviewer agents + 1 presence hook + the composer + 4 additional hooks) [.sh L9]", () => {
    const paths: string[] = [
      ...
      ...AGENTS.map((a) => at("agents", `amadeus-${a}-agent.md`)), // 20-33 (14)
      ...Object.entries(STAGES).flatMap(([phase, stages]) =>
        stages.map((s) => at("amadeus-common", "stages", phase, `${s}.md`)),
      ), // 34-65 (32)
      at("settings.json.example"), // 66
      at("settings.local.json.example"), // 67
      at("knowledge", "amadeus-shared", "state-template.md"), // 68
      mem("org.md"), // 69 — method relocated to amadeus/spaces/default/memory/
      mem("project.md"), // 70
      at("CLAUDE.md.example"), // 71
    ];
