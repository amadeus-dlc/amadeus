...
The vocabulary is closed and has exactly three members — `type-only`, `catch-arm`, `dispatch-case` — because those are the three the AST can decide. Any other value, including an empty string, fails the gate as a malformed entry rather than being skipped. The field is optional and the check is a ratchet: entries whose ranges are none of the three classes stay undeclared and unchecked, and every entry whose range IS decidable carries the pin (they were bulk-pinned mechanically once, and new ones take it on as they are added). Add it whenever the range is decidable, and drop it if a later edit makes the range something else — a stale declaration is meant to be loud.
