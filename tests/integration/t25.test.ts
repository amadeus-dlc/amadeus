...
// (plan 6). Drives the real `/amadeus --phase ideation` through the Claude Agent
...
//
// WHY THIS PORT EXISTS. The .sh grepped the post-run amadeus-state.md and audit.md
...
// resolves (amadeus-jump.ts handleResolve:121-133 -> firstInScopeStageOfPhase,
// amadeus-lib.ts:1408) to the FIRST in-scope ideation stage = intent-capture.
// intent-capture's display order is BEFORE functional-design, so direction =
// backward (amadeus-jump.ts:143-145). The SKILL.md Stage/Phase Jump handler routes
// this through `amadeus-jump.ts execute --direction backward`, which atomically:
...
//
// Any BENIGN ERROR_LOGGED probes the run emits (e.g. amadeus-state get
...
// ---------------------------------------------------------------------------
// Timeout budget. The .sh set AMADEUS_TEST_TIMEOUT=300. The
...
const TARGET_STAGE = "intent-capture"; // firstInScopeStageOfPhase("ideation","feature"), amadeus-lib.ts:1408
...
describe("t25 /amadeus --phase ideation backward jump (sdk)", () => {
  // -------------------------------------------------------------------------
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        // seeded file. P4: the jump tool does NOT migrate (only intent-birth does,
        // amadeus-utility.ts:2022) — the seed stays at the flat layout, so
        ...
        // state to assert.
        const r = await driveAidlc("/amadeus --phase ideation", {
          projectDir: proj,
        ...
        // A jump leaves Status=Running (amadeus-jump.ts:309 never terminates),
        ...
        // verbatim field line. These are the exact bytes amadeus-jump.ts +
        // amadeus-audit.ts wrote — the deterministic equivalent of the .sh greps.
