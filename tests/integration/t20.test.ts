...
// (plan 7). Drives the real /amadeus --status through the Claude Agent SDK and
...
// (SKILL.md:67,71) routes `--status` to `bun .claude/tools/amadeus-utility.ts
// status` via Bash and prints its stdout VERBATIM. That stdout is the
// deterministic surface: handleStatus (amadeus-utility.ts:181) reads the fields
// straight off amadeus-state.md (Scope/Phase/Current Stage) and renders a fixed
...
//   5 no new .md files                -> no *.md under amadeus-docs/ beyond the seeded state/audit after the run
...
//   - status block fields:  amadeus-utility.ts handleStatus() :297-311 (Scope/Phase/Current Stage lines)
//   - no-workflow literal:   "No active AI-DLC workflow found." :185
//   - --status dispatch:     SKILL.md:67,71 -> `bun .claude/tools/amadeus-utility.ts status` via Bash, stdout verbatim
...
// the .sh set AMADEUS_TEST_TIMEOUT=180). The bun:test per-test cap is that value;
...
const STATUS_HEADER = "AI-DLC Workflow Status"; // amadeus-utility.ts:297
const NO_WORKFLOW = "No active AI-DLC workflow found."; // amadeus-utility.ts:185
...
/** Markdown files under <proj>/amadeus-docs/, excluding the seeded state/audit and
 *  the recovery scratch file — mirrors the .sh test-5 find filter. */
function docMdFiles(proj: string): string[] {
  const docs = join(proj, "amadeus-docs");
  ...
  return readdirSync(docs).filter(
    (f) =>
      ...
      f !== "amadeus-state.md" &&
      f !== "audit.md" &&
      f !== ".amadeus-recovery.md",
  );
}

describe("t20 /amadeus --status (sdk)", () => {
  // -------------------------------------------------------------------------
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        // .sh test 5: no new .md artifacts created in amadeus-docs/.
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        // after the stdout write, amadeus-utility.ts:193).
        const statusCall = r.toolResults.find(
