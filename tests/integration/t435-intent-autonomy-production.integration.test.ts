...
describe("Intent-scoped autonomy production path", () => {
  test("production adapters fail closed when no active Intent exists", () => {
    ...
    expect(listProductionAutoDecisions({ projectDir })).toEqual({ ok: false, error: "review-target-not-found" });
    expect(getProductionAutoDecision({ projectDir, decisionId: "missing-decision" })).toEqual({
      ok: false,
      error: "review-target-not-found",
    });
    expect(commitProductionDecisionReview({ projectDir, decisionId: "missing-decision", choice: "accept" }))
      .toEqual({ ok: false, error: "active-source-and-review-target-required" });
  });
