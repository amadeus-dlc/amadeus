// covers: subcommand:amadeus-state:acknowledge-compaction, subcommand:amadeus-state:gate-start, subcommand:amadeus-state:advance
//
...
// are derived faithfully from the three amadeus-state subcommands it drives
...
// which — amadeus-state acknowledge-compaction — is otherwise UNCOVERED.)
//
...
// produces (amadeus-lib.ts:1637), the chmod 0444 permission denial that only a
...
//          locked audit.md makes appendAuditEntry throw (amadeus-audit.ts:272
...
//          (amadeus-audit.ts:189-199) recreates a missing audit.md; exit 0.
//          F4: audit emit succeeds (audit.md writable) but writeStateFile on a
//          0444 state.md throws -> main() catch -> error() -> emitError emits
//          ERROR_LOGGED (state.md exists, so the guard at amadeus-lib.ts:1604
...
// "Error Logged", with Tool/Command/Error fields (amadeus-lib.ts:1619-1629,
// amadeus-audit.ts:151).
//
...
//       exits 1, and amadeus-state.md is byte-identical to its pre-injection snapshot.
...
//   .sh assert 2 (state file unchanged after audit failure)    -> "F1: amadeus-state.md byte-unchanged (audit-first holds)"
...
//     "no Scope field" (amadeus-state.ts:334).
//   - F4 assert 8: beyond the bare count delta, pin the new row's Tool field to
//     "amadeus-state", proving the row came from the state tool's emitError path.
//
...
// under <record>/audit/<host>-<clone>.md — not the flat amadeus-docs/ trio. After
...
/**
 * `AMADEUS_WORKFLOW_INTENT=chaos bun amadeus-utility.ts init --scope fix
...
function init(p: string): CliResult {
  const res = spawnSync(
    BUN,
    [UTIL, "init", "--scope", "fix", "--project-dir", p],
    {
      encoding: "utf-8",
...
// exits 1 and amadeus-state.md is byte-unchanged. (.sh asserts 1-2)
// ============================================================

describe("t47 F1 — read-only audit shard during acknowledge-compaction (audit-first holds)", () => {
  runIfChmod(
    "F1: acknowledge-compaction exits 1 when the audit shard is read-only + amadeus-state.md byte-unchanged",
    () => {
      const p = proj();
      ...
      // stable per-clone token, not the PID — amadeus-lib.ts:955-971), so chmod-ing
      // that one shard read-only denies its append. (The .sh chmod-ed the flat
      // amadeus-docs/audit.md; here the equivalent is the born record's shard.)
...
describe("t47 F2 — missing audit.md before gate-start (ensureAuditFile recovers)", () => {
  test(
    ...
    () => {
      ...
      // whole dir is the equivalent of the .sh's `rm "$PROJ/amadeus-docs/audit.md"`
...
describe("t47 F3 — corrupted state.md before advance (refuses, names Scope)", () => {
  test("F3: advance exits 1 on corrupted state + error message names the missing Scope field", () => {
    ...
    // (seededStateFile) — a flat amadeus-docs/ seed would not be the file the tool
...
`,
    ...
    // STRONGER: pin the exact refusal sentence (amadeus-state.ts:332-336).
...
describe("t47 F4 — read-only state.md before gate-start (ERROR_LOGGED emitted)", () => {
  runIfChmod(
    ...
    () => {
      ...
      // amadeus-lib.ts:1604 let the row through).
      ...
      // from the state tool's emitError (Tool: "amadeus-state",
      // amadeus-lib.ts:1619-1623 / amadeus-state.ts:1751-1756).
