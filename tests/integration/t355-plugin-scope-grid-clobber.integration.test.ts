// covers: function:compileStageGraph, function:applyPluginScopeBindings, function:mergeComposedScopes
// size: medium
//
// t355 (integration) — #1630 end-to-end: `amadeus-graph compile` over a real
// plugin host whose host config binds a plugin stage to an EXISTING composed scope.
//
// The reported failure was a full compile: with `self-feature` carrying 18
// EXECUTE stages on disk, assigning a plugin stage through host configuration
// recompiled the row down to 1 EXECUTE (the
...
function stageFixture(slug: string): string {
  return [
    ...
    "condition: Host scope binding fixture.",
 ...
 */
function makeHost(boundScopes: string[] | null): string {
  ...
  if (boundScopes !== null) {
    ...
    const content = stageFixture(STAGE_SLUG);
  ...
  writeFileSync(join(root, "amadeus", "config.json"), JSON.stringify({
    plugin: {
      "scope-bindings": boundScopes === null || boundScopes.length === 0
        ? {}
        : { [PLUGIN]: { [STAGE_SLUG]: boundScopes } },
    },
  }));
...
describe("compile: host-owned plugin scope binding over a real plugin host (#1630)", () => {
  test("a host binding to a composed scope joins its plan instead of replacing it", async () => {
  ...
  test("a host binding to a stock scope joins it without disturbing existing cells", async () => {
  ...
  test("a host binding to a brand-new scope gets a row holding only its own cell", async () => {
  ...
  // drop. That GC destroyed the approved plan — an unbound stage (`scopes: []`)
  ...
  test("drop -> compose is lossless for an unbound plugin's composed-scope cell", async () => {
    // `scopes: []` — the host-unbound plugin shape, so the
    ...
    // The composer's approved plan: the plugin stage added to the composed row.
