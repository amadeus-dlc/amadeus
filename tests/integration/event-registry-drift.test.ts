...
describe("registry internal consistency (VER-1)", () => {
  ...
  test("canonical cardinality is pinned at 94 (C11/FR-9 +DELEGATED_MERGE_RECORDED) — vacuous equality banned", () => {
    expect(EXPECTED_CANONICAL_COUNT).toBe(94);
    expect(canonicalAuditEvents().length).toBe(94);
    expect(SETS.registryCanonical.size).toBe(94);
    expect(SETS.auditVocabulary.size).toBe(94);
  });
