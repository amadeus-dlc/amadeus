...
describe("registry internal consistency (VER-1)", () => {
  ...
  test("canonical cardinality is pinned at 87 (#1672, #1602, #1919, #2095, #2096) — vacuous equality banned", () => {
    expect(EXPECTED_CANONICAL_COUNT).toBe(87);
    expect(canonicalAuditEvents().length).toBe(87);
    expect(SETS.registryCanonical.size).toBe(87);
    expect(SETS.auditVocabulary.size).toBe(87);
  });
