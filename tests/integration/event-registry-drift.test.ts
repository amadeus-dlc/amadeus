...
describe("registry internal consistency (VER-1)", () => {
  ...
  test("canonical cardinality is pinned at 81 (#1672, #1602, #1919) — vacuous equality banned", () => {
    expect(EXPECTED_CANONICAL_COUNT).toBe(81);
    expect(canonicalAuditEvents().length).toBe(81);
    expect(SETS.registryCanonical.size).toBe(81);
    expect(SETS.auditVocabulary.size).toBe(81);
  });
