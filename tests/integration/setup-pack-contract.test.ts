...
import { PackContract, type PackReport, runBunPackDryRun } from "../lib/setup-pack-contract.ts";
...
beforeAll(async () => {
  ...
  if (result.type === "err") {
    throw new Error(`bun pm pack --dry-run failed against packages/setup: ${result.error.detail}`);
  }
...
describe("pack contract — bun pm pack --dry-run (FR-018, BR-P01-P03)", () => {
  ...
  test("edge case: a declared file dropped from `files` is detected as missing (REL-P02a)", () => {
    ...
    const result = runBunPackDryRun(dir);
    if (result.type === "err") throw new Error(`bun pm pack --dry-run failed against mutated copy: ${result.error.detail}`);
