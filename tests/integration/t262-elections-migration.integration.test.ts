...
import { createMigrationPlan, main } from "../../scripts/amadeus-election-migrate.ts";
...
describe("migration CLI", () => {
  ...
  test("verify is write-free and apply rejects a mismatched approval or altered plan", () => {
    const plan = createMigrationPlan(root, {
      electionId: "E-ONLY",
      operationId: "migration-e-only",
      createdAt: "2026-08-13T00:00:00Z",
    ...
    const approvalPath = join(projectDir, "approval.json");
    writeFileSync(planPath, JSON.stringify(plan));
    writeFileSync(approvalPath, JSON.stringify({ approved: true, planDigest: `sha256:${"0".repeat(64)}` }));
    const before = [
      readFileSync(join(root, "E-ONLY", "election.json")),
      readFileSync(join(root, "E-ONLY", "ledger.json")),
      readFileSync(join(root, "E-ONLY", "timeline.json")),
    ...
    expect(main(["--project-dir", projectDir, "--plan", planPath, "--verify"])).toBe(1);
    expect([
      readFileSync(join(root, "E-ONLY", "election.json")),
      readFileSync(join(root, "E-ONLY", "ledger.json")),
      readFileSync(join(root, "E-ONLY", "timeline.json")),
    ...
    const altered = { ...plan, electionId: "E-OTHER" };
    ...
    expect(() => main([
      ...
      "--approval", approvalPath,
      "--apply",
    ])).toThrow(/integrity|corrupt|digest/i);
    expect(existsSync(join(root, plan.sourcePath))).toBe(true);
    ...
    const afterApply = [
      readFileSync(join(root, plan.targetPath, "election.json")),
      readFileSync(join(root, plan.targetPath, "ledger.json")),
      readFileSync(join(root, plan.targetPath, "timeline.json")),
    ];
    expect(main(["--project-dir", projectDir, "--plan", planPath, "--verify"])).toBe(0);
    expect([
      readFileSync(join(root, plan.targetPath, "election.json")),
      readFileSync(join(root, plan.targetPath, "ledger.json")),
      readFileSync(join(root, plan.targetPath, "timeline.json")),
