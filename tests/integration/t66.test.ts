...
// =============================================================================
// compile --check semantic invariants (.sh:821-849, 3 assertions)
...
describe("t66 compile --check invariants (spawnSync CLI exit-code)", () => {
  test("generated graph mutations do not participate in source invariant checks", () => {
    ...
    // Mutate the generated graph. The source-only check recompiles from source
    // and does not compare against committed or existing generated bytes.
...
describe("t66 designer export (spawnSync CLI-boundary)", () => {
  ...
  test("export element counts: stages=32, scopes=15, artifacts=122, agents=15", () => {
    const res = spawnSync(BUN, [GRAPH_TS, "export"], { encoding: "utf8" });
    ...
    expect(Object.keys(out.scopes).length).toBe(15);
...
describe("t66 rules_in_context resolution (in-process + spawnSync seams)", () => {
  ...
  // .sh:1000-1009 — compile --check validates edited rule sources directly.
  // MUST STAY SPAWN: compile then --check (process.exit boundary) after adding a rule.
  test("compile --check validates edited rule sources without a byte comparison", () => {
...
`,
  ...
  // .sh:1106-1116 — compile --check validates edited sensor sources directly.
  ...
  test("compile --check validates edited sensor sources without a byte comparison", () => {
