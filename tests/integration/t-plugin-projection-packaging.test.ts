...
describe("t-plugin-projection-packaging — U09 FR-6 item 19", () => {
  ...
  test("self-install stays the closed six faces (not widened to every package face)", () => {
    expect([...SELF_INSTALL_HARNESSES].sort()).toEqual(["claude", "codex", "cursor", "kimi", "opencode", "pi"]);
