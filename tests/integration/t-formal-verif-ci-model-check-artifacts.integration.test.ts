...
function createRun(root: string, kind: "warm-up" | "measured", index: number): CiModelCheckRunEvidence {
  ...
  const relativeDirectory = `${model}/runs/${kind}-${index}`;
...
describe("CI model-check artifact verifier", () => {
  ...
  test("rejects byte drift and an unexecuted Docker inspection", () => {
    ...
    const receiptPath = join(notRun.root, "FormalElection/runs/measured-1/env-receipt.json");
    ...
    rewriteJsonArtifact(notRun.root, "FormalElection/runs/measured-1", "env-receipt.json", (value) => value);
  ...
  test("accepts manifest-bound empty stderr but rejects empty stdout and traversal", () => {
    ...
    writeFileSync(join(emptyStderr.root, "FormalElection/runs/measured-2/tlc-stderr.bin"), "");
    resealArtifact(emptyStderr.root, "FormalElection/runs/measured-2", "tlc-stderr.bin");
    expect(verifyCiAcceptanceArtifacts(emptyStderr.root).ok).toBe(true);

    const emptyStdout = fixture();
    writeFileSync(join(emptyStdout.root, "FormalElection/runs/measured-2/tlc-stdout.bin"), "");
    resealArtifact(emptyStdout.root, "FormalElection/runs/measured-2", "tlc-stdout.bin");
  ...
  test("rejects a manifest-bound stderr stream above the 16 MiB U3 cap", () => {
    ...
    writeFileSync(
      join(oversized.root, "FormalElection/runs/measured-2/tlc-stderr.bin"),
      Buffer.alloc(16 * 1024 * 1024 + 1, 1),
    );
    resealArtifact(oversized.root, "FormalElection/runs/measured-2", "tlc-stderr.bin");
  ...
  test("fails closed for malformed roots, acceptance JSON, manifests, and directories", () => {
    ...
    writeJson(join(malformedManifest.root, "FormalElection/runs/warm-up-0/manifest.json"), {});
    expect(verifyCiAcceptanceArtifacts(malformedManifest.root).ok).toBe(false);

    const unavailable = fixture();
    rmSync(join(unavailable.root, "FormalElection/runs/warm-up-0"), { recursive: true });
    expect(verifyCiAcceptanceArtifacts(unavailable.root).ok).toBe(false);

    const linked = fixture();
    const runPath = join(linked.root, "FormalElection/runs/warm-up-0");
    const targetPath = join(linked.root, "FormalElection/runs/warm-up-target");
  ...
  test("rejects malformed manifest inventories and unavailable artifacts", () => {
    ...
    rewriteJsonArtifact(expected.root, "FormalElection/runs/warm-up-0", "manifest.json", (manifest) => {
    ...
    rewriteJsonArtifact(duplicate.root, "FormalElection/runs/warm-up-0", "manifest.json", (manifest) => {
    ...
    unlinkSync(join(missing.root, "FormalElection/runs/warm-up-0/tlc-stdout.bin"));
    expect(verifyCiAcceptanceArtifacts(missing.root).ok).toBe(false);

    const nonFile = fixture();
    unlinkSync(join(nonFile.root, "FormalElection/runs/warm-up-0/tlc-stdout.bin"));
    mkdirSync(join(nonFile.root, "FormalElection/runs/warm-up-0/tlc-stdout.bin"));
    expect(verifyCiAcceptanceArtifacts(nonFile.root).ok).toBe(false);

    const badEntry = fixture();
    rewriteJsonArtifact(badEntry.root, "FormalElection/runs/warm-up-0", "manifest.json", (manifest) => {
  ...
  test("rejects malformed receipt identities, matrices, values, and markers", () => {
    for (const mutate of [
      ...
      const value = fixture();
      rewriteJsonArtifact(value.root, "FormalElection/runs/warm-up-0", "env-receipt.json", (receipt) => {
        mutate(receipt);
        return receipt;
      });
    ...
    const receiptPath = join(malformedReceipt.root, "FormalElection/runs/warm-up-0/env-receipt.json");
    writeFileSync(receiptPath, "{");
    const manifestPath = join(malformedReceipt.root, "FormalElection/runs/warm-up-0/manifest.json");
    const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
    ...
    rewriteJsonArtifact(marker.root, "FormalElection/runs/warm-up-0", "completion-marker.json", (value) => ({
      ...value,
      complete: false,
    }));
    ...
    rewriteJsonArtifact(terminal.root, "FormalElection/runs/warm-up-0", "manifest.json", (value) => ({
      ...value,
      partial: true,
    }));
