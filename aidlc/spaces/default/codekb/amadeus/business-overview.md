...
このリポジトリは Amadeus 本体の自己開発 workspace を兼ね、`aidlc/spaces/default/` に自身の Intent record を蓄積する。

## 中核の運用モデル

- ライフサイクルの公開入口は `amadeus` skill の 1 個で、実行はエンジン駆動（`amadeus-orchestrate.ts` の next / report forwarding loop）である。
- Intent の正準台帳は `intents/intents.json`、進行台帳は各 record の `aidlc-state.md` と `audit/`（追記型）である。
