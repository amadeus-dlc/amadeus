...
- target workspace の `aidlc/spaces/default/` を、Amadeus 本体開発用の Space として扱う。
...
- `aidlc-state.md` と成果物の phase、status、required artifacts が整合していること。
...
承認待ちの確認には、`aidlc/spaces/<space>/intents/*/aidlc-state.md` を横断して、`[?]`（AwaitingApproval）のステージ、`STAGE_COMPLETED` イベントが未記録の `[x]` ステージ、`BOLT_COMPLETED` イベントが未記録の Bolt、`PHASE_VERIFIED` イベントが未記録の phase（Phase Progress が `Verified` になっていない phase）を確認する。
...
- 定型文・実機確認の本体: `aidlc/spaces/default/intents/260705-agmsg-trial-docs/construction/agmsg-trial-docs/code-generation/multi-agent-trial-record.md`（完了済み record であり書き換えない）
