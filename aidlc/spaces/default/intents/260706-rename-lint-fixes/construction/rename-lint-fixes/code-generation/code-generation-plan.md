...
B003 の実装着手は NFR-2 に従い engineer3 の #528 PR merge 後とする。
...
- [ ] **Step 6: B003 設計確定（FR-3.2 / FR-3.3 / AC-6）** — 後掲「B003 設計仕様」節を参照。本 step は設計確定地点（bugfix scope は functional-design を持たないため code-generation-plan.md が設計確定地点）として AC-6 を充足する。
- [ ] **Step 7: B003 実装（FR-3.1 / FR-3.2）** — #528 PR merge 後に着手する（NFR-2）。`amadeus-sensor-linter.ts` と `amadeus-linter.md` を「B003 設計仕様」に従って更新し、B003 eval の RED→GREEN を確認する。parity 宣言と skills/ 反映を同時に行う。
- [ ] **Step 8: 検証一式** — `npm run test:all` pass、`npm run test:it:rename-leftovers` pass、B003 eval pass、`bun run .agents/skills/amadeus-validator/validator/AmadeusValidator.ts . 260706-rename-lint-fixes` pass を確認する。
- [ ] **Step 9: code-summary.md 作成** — 変更ファイル、主要判断、RED→GREEN 証跡、検証結果、逸脱を記録する。B003 完了後に最終化する。
...
- stdout JSON: `{ "pass": false, "errorCount": >= 1, "violations": [{ "file": <stub-file>, "rule": "no-stub-compat", ... }] }`.
