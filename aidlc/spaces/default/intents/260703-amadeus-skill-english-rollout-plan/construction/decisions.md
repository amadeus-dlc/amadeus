...
| CD011 | Build and Test の失敗時処理は、AI-DLC v2 の「診断と修正を最大 2 回試行」へ寄せず、halt-and-ask と Code Generation 責務分離の現行契約を意図的差分として維持する。 | コード変更を Code Generation の記録経由に限定して記録の真実性と Bolt gate の承認対象を保つ。halt-and-ask は autonomous mode の安全弁であり、本家の有限試行の意図（人間へ委ねる）は最初の失敗で即座に満たす。 | 理由と対比は docs/amadeus/aidlc-v2-build-and-test-failure-handling.md、lifecycle construction の Stage 3.6 Notes、overview の構造差分表、skill 注記に記録した（#392）。 |
| CD012 | Operation phase は対象外を維持し、理由を成果物契約（実環境への作用と観測は record で真実性が閉じない）、gate（PR merge で承認を表現できない）、validator（実環境状態の検証手段を持たない）、PR 境界（実行単位が PR にならない）の 4 観点で明文化する。将来対応は専用 Issue の起票と Intake 経由の新 Intent として分離する。 | 対象外の宣言（SKIP 注記、AMADEUS.md、skill-language-policy の維持契約）は既にあるが、理由の説明がなく、本家を extends する観点で境界が追跡できなかった。 | 理由と本家 Operation skill 7 件の扱いは docs/amadeus/aidlc-v2-operation-phase-boundary.md と lifecycle overview に記録した（#394）。 |
