...
| CD005 | Construction phase PR は、Issue #399 の完了判断に必要な成果物確定 PR とする。 | 全 Bolt が完了し、ci-pipeline は skip 判定済みである。一方で `PHASE_VERIFIED` と `WORKFLOW_COMPLETED` は phase PR merge 確認後に記録する必要がある。 | PR #414 merge 後に、Construction phase verification、workflow completion、registry completed、Issue #399 close を記録する。 |
