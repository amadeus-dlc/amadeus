# Logical Components — u002-kanban-sync-cli
...
論理コンポーネントは application-design の C-1〜C-4 のとおり（scan.ts の IntentScanner / ColumnMapper、board.ts の ProjectsClient、kanban-sync.ts の SyncRunner）。追加しない。
