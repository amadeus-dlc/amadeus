...
| R004 独自 skill 整理 | 5 skill 削除に加え、旧 stage skill 22 個と amadeus-steering も削除（CD010。examples 廃止により provenance 制約が解消） | test:all、parity:check | 充足 |
...
| R010 検証 green と examples | test:all green。examples は再生成ではなく機構ごと廃止し、エンジン sandbox e2e で代替（CD010、人間承認） | test:all（engine e2e を含む） | 充足（要求の意図「実行結果の検証が green」を e2e で満たす） |
