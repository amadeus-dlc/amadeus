...
| CD010 | examples/ と skill-provenance 機構を廃止し、実行結果の検証はエンジン sandbox e2e（決定論、LLM 不要）で代替する。これに伴い旧 stage skill 22 個と amadeus-steering を削除する | 人間承認（PR #427 レビュー中の判断）。現行生成ハーネスは新アーキテクチャで構造的に不成立（エンジン未配布、旧契約プロンプト、gate の human presence）で、上流に examples の概念はなく「重複系は削除」基準に該当する | CD005 と CD006 の後続作業が解消し、R004 と R010 の残課題が閉じる。real provider 生成基盤（runner、llm-templates の steering e2e）も退役 |
