...
> フレームワークのデフォルト。team.md、project.md と順に加算して読み込まれる。下位レイヤーは上位ルールを上書きせず、矛盾する規則は保存前の admission check で拒否される。

## Way of Working

私たちは**トランクベース開発**を採用する。すべての作業は短命なフィーチャーブランチ(通常1〜2日以内に解決)を経由して `main` にマージする。長命ブランチはマージ負債を蓄積するため避ける。

`amadeus/` ワークスペース(record、memory、codekb、knowledge)はバージョン管理し、ワークフローのパーク時、ステージ完了時、セッションまたは1日の終わりにチェックポイントとしてコミットする。per-clone の監査シャードは append-only とし、チェックポイントのコミットへ含める。
...
チームがより厳格な姿勢を確約する場合は、矛盾しない追加規則として team.md に記録する。
