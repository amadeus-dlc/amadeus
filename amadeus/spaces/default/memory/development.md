...
- 対象 record の `amadeus-state.md`（State Version 7）の Phase Progress、Current Status の `Current Stage` を確認してから作業を進める。
- 変更種別ごとの完了条件は `memory/team.md` の判断基準に従う。
...
6. `amadeus` skill のルーティングに従い、`amadeus-state.md` が解決した次ステージの内部 skill で成果物を作成または更新する。
7. 実装や文書変更を行う前に、対象の変更種別と完了条件を確認する。
8. `aidlc/` 成果物を作成または更新した場合は、`amadeus-validator` で対象 workspace と対象 Intent を検証する。
...
13. merge は人間が実行する。merge 後、`amadeus` skill が `PHASE_VERIFIED` または `BOLT_COMPLETED` イベントを `audit/audit.md` に追記する。
...
| phase | 目的 | 完了時に確認すること |
|---|---|---|
| Ideation | Intent の目的、スコープ境界、スコープバックログ、実現可能性を整理する。 | scope が実行対象にするステージが `[x]`（Completed）または `[S]`（Skipped）で、phase PR の merge を `PHASE_VERIFIED` イベントとして記録している（Phase Progress が `Verified`）。 |
| Inception | 要求、必要なストーリーとモック、アプリケーション設計、Unit の依存 DAG、Bolt 計画を整理する。 | Units と Delivery Planning まで追跡でき、phase PR の merge を `PHASE_VERIFIED` イベントとして記録している（Phase Progress が `Verified`）。 |
| Construction | Bolt 単位で設計を具体化し、実装、ビルド、テスト、必要な CI 整備を進める。 | 各 Bolt の `BOLT_COMPLETED` イベント（PR merge）と Stage 3.7 の完了または skip を記録し、Current Status の `Status` が `Completed` である。 |
...
- 変更種別ごとの必須条件が満たされている。
- skill 変更では、挙動差分の要約、skill-forge 確認の記録、粒度制約の確認が必須条件に含まれる。詳細は `memory/team.md` の判断基準に従う。
