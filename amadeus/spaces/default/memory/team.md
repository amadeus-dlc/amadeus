...
- 並行 builder のディスパッチプロンプトに「モニタ/バックグラウンド待ちでターンを終了しない — 検証は同期(フォアグラウンド)で完遂し、完了報告までを1タスクとする」を標準文言として含める — c5(無応答検知→引き取り)は事後対処であり、事前抑止の定型文言が無いと背景 gate 待ち停止が反復する(batch8 U5 で2回反復→明示 nudge で即解消の実測。E-L65 裁定 全会一致) (learned 2026-07-11) <!-- cid:code-generation:builder-prompt-sync-completion --> (learned 2026-07-11) <!-- cid:code-generation:c-el65-1 -->
- 上流のクロスレビューコメント・裁定を要件へ昇格するときは、結論部だけでなく留保部(「要再判断」「未決」等)も転記し、留保付き事項は決定事項として書かず pre-approved 分岐か「実装時判断」へ落とす — 留保の脱落は未決の設計判断を builder に既決として渡し、無申告の構造追加を誘発する(batch8 U7 FR-7 で e4 コメントの留保を落として references/ パスを断定 → product-lead レビューが全 harness 実測で捕捉し REVISE。E-L38=引用の実在・E-L56=列挙の完全性の隙間である「引用の完全性(留保の保存)」を埋める。E-L62 裁定) (learned 2026-07-11) <!-- cid:requirements-analysis:citation-reservation-preservation --> (learned 2026-07-11) <!-- cid:requirements-analysis:c-el62-1 -->
