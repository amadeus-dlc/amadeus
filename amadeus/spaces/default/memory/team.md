...
- codecov の commit report line_coverage は carryforward により未計測行を hit と誤表示する — patch 判定は pulls/{id} エンドポイントの patch 集計を権威とし、per-line 確定(compare/segments)が GitHub rate limit で不可の場合も pulls 集計で代替できる(E-L51 2026-07-11 採用 6/6、#855 waiver 確定の実測。codecov-waiver-line-confirmation / external-status-triage の追補) (learned 2026-07-11) (learned 2026-07-11) <!-- cid:code-generation:codecov-carryforward-patch-authority -->
- bun の spawnSync/spawn は runtime で変更した process.env を child に自動で畳み込まない — env override に依存するテスト・ツールは spawn 呼び出しに env: process.env を明示する(欠くと override 非継承のまま誤 pass)。E-L33 batch4 候補2(spawnSync env スナップショット)と同一機序を統合(E-L51 2026-07-11 採用 6/6、#865 U6 実測) (learned 2026-07-11) (learned 2026-07-11) <!-- cid:code-generation:bun-spawn-env-snapshot -->
- 選挙裁定が特定の欠陥機序を前提とするとき、実装者は修正適用後に元症状の非再現(閉包)まで実測し、前提不成立を検知したら実装を止めて再選挙に回す — 裁定準拠の実装完了と症状の閉包は別物(E-L51 2026-07-11 採用 6/6、E-B5b→E-B5e の #831 実例。implementation-deviation-election の裁定前提面への拡張) (learned 2026-07-11) (learned 2026-07-11) <!-- cid:code-generation:ruling-premise-closure-verification -->
