...
- leader からの指示・配信(タスクディスパッチ・選挙配信・ゲート連絡・ノルム通知等)を受けたメンバーは、受領時に必ず短い ack を返信する — agmsg は不達が無音(send.sh は未登録宛先でも成功を返す)のため、返信だけが leader 側の配送確認になる。leader は ack 未着を「待機・作業中」と区別される配送異常のシグナルとして扱い、agmsg-recipient-typo の診断(team.sh 実名確認・identity 再確認)を両側で即時発動する(e2 の resume 時 identity 引きずりで pong・選挙票・ack が約3時間無音不達になった実障害 2026-07-12 より。ack は受領確認であり、完了・ブロッカーの自発報告義務 = push-reporting とは別軸) (user decision 2026-07-12) <!-- cid:requirements-analysis:dispatch-ack-required -->
