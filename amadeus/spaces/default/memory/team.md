...
- ユーザー常任マージ承認(2026-08-15 実 HUMAN_TURN、逐語「CI greenであればマージしてよいです。以後も」): 本則「AI は PR のマージを自発実行しない…承認後にスカッシュマージ」(cid:requirements-analysis:leader-executes-merge)の個別承認要求を、**必須 CI green と収束(未解決スレッドゼロ)を実測した場合に限り**常任承認で代替してよい、へ緩和する。マージ前の再実測義務(mergeable・現 head 必須 CI・verdict)と merge queue 経由・Ruleset 非バイパスは不変。head が実質変化していれば増分再レビューのうえ実測をやり直す。この常任承認はマージのみに適用され、リリース・publish・その他の不可逆外部操作へは拡張しない。次回蒸留で本則へ統合する (learned 2026-08-15, intent 260814-priority-bug-batch, ユーザー直接裁定) <!-- cid:ci-pipeline:standing-merge-approval-ci-green -->
