...
- leader からの指示・配信(タスクディスパッチ・選挙配信・ゲート連絡・ノルム通知等)を受けたメンバーは、受領時に必ず短い ack を返信する — agmsg は不達が無音(send.sh は未登録宛先でも成功を返す)のため、返信だけが leader 側の配送確認になる。leader は ack 未着を「待機・作業中」と区別される配送異常のシグナルとして扱い、agmsg-recipient-typo の診断(team.sh 実名確認・identity 再確認)を両側で即時発動する(e2 の resume 後 identity 引きずり(プロセス起動 23:42:04Z の ps 実測)で pong・選挙票2通・ノルム ack・再稼働報告の計5通が約3時間46分(23:46Z〜03:32Z)無音不達になった実障害 2026-07-12 より。ack は受領確認であり、完了・ブロッカーの自発報告義務 = push-reporting とは別軸) (user decision 2026-07-12) <!-- cid:requirements-analysis:dispatch-ack-required -->
...
- agmsg send.sh は未登録の宛先名でも成功を返すため、宛先 typo は無音で不達になる(claude-3 の再起動後全報告が『leader』宛で不達になった実障害、2026-07-09)。対策: 宛先は送信前に team.sh で実測した現登録名のみを使う — 登録名の実名例をノルム本文に焼き込まない(旧例示『claude-leader / claude-engineer-N / codex-engineer-N』が改名後に stale 化し、2026-07-12 の不達2件(e2 の旧名 FROM/TO 計5通・e1 の claude-engineer-5 宛レビュー依頼)の共通原因になった実測より、例示を削除し送信前 team.sh 確認を必須化)。重要な報告で leader から反応がない場合は宛先名を疑って team.sh で実名確認する。長時間の無応答を検知した側(leader 含む)も、相手の送信不達の可能性を診断に含める (learned 2026-07-09) (amended 2026-07-12 user decision) <!-- cid:requirements-analysis:requirements-analysis:agmsg-recipient-typo -->
