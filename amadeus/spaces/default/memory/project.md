...
- target workspace の `amadeus/spaces/default/` は、Amadeus 本体開発用の Space（memory、knowledge）と Intent record を扱う。
...
- PR 作成時は、対応 Issue と `amadeus/spaces/default/intents/...` をリンクする。
...
- upstream-coverage sensor は questions ファイルにも consumes 参照を要求する。questions 生成時点で codekb 等の上流成果物への参照段落を含める。sensor は fail 時に .amadeus-sensors/ へ detail ファイル、pass 時に audit へ SENSOR_FIRED + SENSOR_PASSED を記録する（fail の解消後は amadeus-sensor.ts fire で pass を記録し直す） (learned 2026-07-06) <!-- cid:260706-pr-gate-discipline:requirements-analysis:c3 -->
