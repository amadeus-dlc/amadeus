...
実装時は `core/` または `harness/<name>/` を編集元とし、`dist/` とセルフインストールツリーは生成物として `bun scripts/package.ts` と `bun run promote:self` で同期する。
...
TypeScript/ESM と Bun 直接実行を前提に、既存の `amadeus-` プレフィックス、`core/` のハーネス中立層と `harness/<name>/` のハーネス別表層という境界を守る。フォーマッタは無効、lint は Biome、型検査は `tsc --noEmit` で行い、ツール・フックには実行ビットを要求しない。
...
- phase 最終ステージ(approval-handoff / delivery-planning 等)の approve 前に verification/phase-check-<phase>.md の作成を定型タスクとして含める — エンジンの phase-boundary ガードが実在を要求して approve を拒否する。成果物の正名は approval-handoff:c2 に従う(同項と相互参照。E-PM1 PM-e2-1 2026-07-16 採用 4/4、delivery-planning approve の「phase-check-inception.md does not exist」拒否実測。bolt-pr-taskization 同型の指令ループ外規範) <!-- cid:approval-handoff:phase-check-before-final-approve -->
- conductor はゲート準備完了報告の前に、宣言センサー(sensors_applicable)を全成果物へ手動発火し、FAILED を是正してから報告する — PostToolUse 依存では発火漏れが起きうる(E-PM1 M2 2026-07-16 採用 4/4、approval-handoff/practices-discovery の2ステージ連続で報告前捕捉の実測) <!-- cid:requirements-analysis:manual-sensor-fire-before-gate-report -->
...
- scope-change を実行したら、現在ステージの checkbox 状態を確認し、awaiting-approval だった場合は「next 再入 → amadeus-state.ts checkbox <slug>=in-progress → gate-start → approve」の順で正規復旧する — scope-change の Stage Progress 再構築が completed/in-progress/skipped 以外の状態(awaiting-approval/revising)を pending へ落とすため(原因機構: amadeus-utility.ts:3227-3230 の三項マッピング、状態型は amadeus-lib.ts:58。ツール側修正は Issue #1015)。実測: 2026-07-16 scope-definition で SCOPE_CHANGED → approve still pending 拒否×2 → 復旧列 GATE_APPROVED(e2 監査シャード 2fec7b288e5f.md :1069/:1237/:1423/:1454)(E-CS2 L1 2026-07-16 採用 4/4) <!-- cid:scope-definition:scope-change-checkbox-recovery -->
- practices-promote に渡す discovered-rules.md では Mandated/Forbidden セクション配下に説明文を置かない(説明は別セクションへ)— parseRules(amadeus-state.ts:2556-2561)が非空・非コメント・非見出しの全行をルールとして project.md へ append するため。**Issue #1013 のパーサ側修正が着地するまでの暫定運用であり、修正マージ時に失効棚卸しの対象とする**。実測: 2026-07-16 practices-discovery で説明文2行が誤 append → 復元 git diff 0(E-CS2 L2 2026-07-16 採用 4/4) <!-- cid:practices-discovery:promote-no-prose-in-rule-sections -->
