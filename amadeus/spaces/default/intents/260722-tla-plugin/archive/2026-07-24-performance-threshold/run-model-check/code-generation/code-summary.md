...
- TLC process は shell-free argv、120秒 deadline、stdout/stderr 各16 MiB上限で実行する。timeout/overflow 時は process group を `SIGTERM`、`SIGKILL` の順で停止・回収する。
