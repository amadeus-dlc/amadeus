...
- `EnvReceipt` — `{ readonly plannerIdentity: string; readonly verified: readonly string[]; readonly declaredNotApplied: readonly { check: string; reason: string }[] }`(宣言的非適用の記録面 — BR-U3-6)
...
- EnvReceipt.declaredNotApplied は Docker planner では必ず sandbox-receipt / host-jdk-snapshot の2件を含む(空なら契約違反としてテストで固定)
