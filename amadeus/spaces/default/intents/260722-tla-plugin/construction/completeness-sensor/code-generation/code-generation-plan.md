...
- framework 配布物として自己完結する `packages/framework/core/tools/amadeus-formal-verif-model-map.ts` が `ModelMap` / `ModelMapEntry` / `ModelMapDrift`、parse、diff、canonical identity の1定義である。host 側の `scripts/formal-verif/tla-model-map.ts` と `canonical.ts` は同moduleを明示的に再exportし、U1/U5で型・parse・identityを再定義しない。core toolを全harnessへ投影した後もhost projectの `scripts/` に依存しないことを配布境界テストで固定する。
...
2. [x] **Step 2: read-only checker の境界を実装する** — `packages/framework/core/tools/amadeus-sensor-model-completeness.ts` に薄い CLI adapter と in-process test seam を設け、project root・map path・clock・filesystem/hash port を注入可能にする。checker は同じ framework core 配布面の canonical moduleを静的importし、source core・生成済みharnessの双方で自己完結した同じ定義を消費する。mapの不在、mapの空・UTF-8/JSON/schema不正をすべて exit非0ではなく有効な fail verdict JSONへ写像する。戻り値は `CompletenessVerdict` の判別unionとし、成功値だけを evaluator へ渡す。対応: FR-4.1/4.2/4.4、BR-U5-2/3/4/6、Reliability Design、NFR-3。
