...
- BR-U1-3(fail-closed 読込): モデル/cfg の不在・空・読取不能は Result エラーとして返し、呼び出し元で loud fail。無言の既定値・スキップを持たない
...
- unit: loadTlaModelSource の正常系+不在/空/読取不能の3エラー系(ENOENT 注入 — ポータブル)
