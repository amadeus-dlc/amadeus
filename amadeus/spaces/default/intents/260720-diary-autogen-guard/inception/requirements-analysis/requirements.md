...
- **FR-1(明示 intent アンカー)**: 【E-DAGRA1 裁定 = A(03:17:37Z、3-0、留保なし)— **前提訂正は E-DAGRAX で追認済み(2026-07-20 開票、3-0 全会一致 GoA 1x3 — E-DAGRA1 の3投票者が同前提引用を当事者申告のうえ追認)**】ensureStageDiary の駆動を cursor 再解決に依存させず**明示 intent アンカー**で行う(audit 系と対称化 — 裁定の本質)。ただし裁定選択肢 A の文中の「memory_path 由来(slug 既含)」は reviewer C-1 で反証された(§2 訂正参照)— **アンカー材料の調達手段(cursor 非依存の intent 特定: 発行フロー上流で解決済みの intent の明示受け渡し等)は design(CG plan)の未決事項として委譲**し、前提訂正は E-DAGRAX(record: leader tree、081a8c29e push 済み)で追認確定 — 裁定本質(明示 intent アンカー・audit 対称化・diary 経路限定)は不変、アンカー材料の調達手段は design 委譲(ruling-premise-closure-verification の完全履行)。受け入れ基準: RE の再現条件(多 intent+cursor 非解決 pd)で「intent 実在 ⇒ diary 生成」の不変条件が成立(現行コードでは fail する回帰テスト)。
