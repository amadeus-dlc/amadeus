...
- directive の memory_path は intent slug を既に含む(memoryPathFor、orchestrate.ts:593)— アンカー材料は chokepoint に既在。
- 過去 ❌ 時点の env 実値は復元不能(gitignored+未ログ)— 回帰は再現条件の注入で固定する。

## 3. 機能要件

- **FR-1(明示 intent アンカー)**: 【E-DAGRA1 裁定 = A(03:17:37Z、3-0、留保なし)】ensureStageDiary の駆動を cursor 再解決に依存させず、**directive の memory_path(intent slug を含む)由来の明示アンカー**で行う — audit 系と対称化。受け入れ基準: RE の再現条件(多 intent+cursor 非解決 pd)で「intent 実在 ⇒ diary 生成」の不変条件が成立(現行コードでは fail する回帰テスト)。
...
- directive.memory_path は全発行経路で正しい intent slug を含む(memoryPathFor は recordPrefix と独立に slug を構成 — RE 実測)。
