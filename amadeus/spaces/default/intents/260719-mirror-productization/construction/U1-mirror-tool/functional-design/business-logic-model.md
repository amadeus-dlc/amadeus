...
main("status", intentArg)
  ...
  → readMirrorIssue(snapshot, gh)      …フィールド不在/Issue 不在が gh 実文で確定 → findings に mirror-missing。view 失敗が不在と確定できない場合(ネットワーク等)→ precondition(exit 2)【ND RD-U1-1 の精密化を反映 — 双方向申告】
