...
ask への回答・auto-sync の実行痕跡は決定的記録(BR-U4-5 の冪等入力)— 記録失敗時に再 ask される側へ倒す(重複 ask は冗長だが安全、無音スキップは危険 — fail-safe 方向の明示)。

## RL-U4-3: mirror ツール失敗の非伝播

auto-sync print 指令の実行(mirror sync)が失敗しても workflow は停止しない(C-01 — 当該機能のみ不可)。失敗は conductor に見える形(コマンドの exit code)で顕在化。
