...
Step 1 は必ず status 実行。exit code で分岐案内: 0(clean)→「乖離なし」報告のみ / 1(diverged)→ findings 種別に応じ create(`mirror-missing`) / sync(`issue-drifted`) / syncとcloseの両候補(`stale-status-line`、BR-U2-5)を案内 / 2(precondition)→ 理由(gh 不在等)と復旧手順を提示。
...
## BR-U2-5: stale-status-line の案内条件(コード生成 reviewer i1 Major の是正)

`stale-status-line` はfinding kindだけを分類し、detail内のrecord状態その他の自由文を解析しない。Step 2では`sync`と`close`を両方提示し、`sync`はopen mirrorを更新すること、`close`はツール側のfail-closedなclose-after-landing検査を実行して未着地なら拒否され得ることを説明する。人間が最終verbを明示選択するまでどちらも実行しない。phase境界ask(C4)経路にcloseは現れない — SKILL/手動が唯一のclose導線。

## BR-U2-7: 任意intent指定の引数境界

実行可能なbash fenceは引数なしの固定4コマンドだけとする。任意の`--intent`指定は別手順で扱い、active space内に実在するintent directoryの正確なbasenameだけを受理する。path separator、`.`、`..`、制御文字、shell metacharacterを拒否し、shell command stringの構築や補間をせず、`--intent`に続く単一引数として渡す。単一argvを保持できない実行境界では停止する。
