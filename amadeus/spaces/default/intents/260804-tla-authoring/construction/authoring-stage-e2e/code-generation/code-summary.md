...
5. **CI Project Coverage Gate 赤 → ソロ選挙 E-TLA-U5COV 裁定 A(2-0、2026-08-05)**: t450 の in-process 動的 import による temp host コピーの lcov 混入(SF 110 中 100)で universe 膨張(17,648→116,489 行、92.19%→75.66%)。裁定 A = テスト側 spawn 化 + collector 恒久修正は Issue #2315。**留保転記(subagent-1、GoA 2)**: (a) collector Issue はマージ前起票し番号を PR 本文と record に残す(#2315 起票済み — 黙った先送りは no-silent-scope-narrowing) (b) spawn driver は非 0 exit・非 JSON stdout・module 解決失敗を loud fail させ、BR-U5-09 の観測事実を JSON 経由でも逐語保持する (c) 変換後閉包の実測 — merged lcov の repo 外 SF 0 件を grep 出力で示し、project % 回復を CI 出力から転記する
6. **裁定 A の閉包実測(2026-08-05/06)**: 是正3コミット(053e5ea58 spawn 化 / a729a559a fixture 分離 — driver in-process import の 0-hit 搭載を解消 / 729a89930 loud fail 強化)。ローカル閉包: merged lcov の repo 外 SF 0 件・driver SF 0 件(grep 実測)、patch gate PASS 46/46 waiver 0、project gate OK 92.1998%、t450 13 pass。**CI 閉包(PR #2312 head cc246466f、run 実文転記)**: `project coverage gate: OK — current 92.2233%` / `Patch coverage gate: PASS`、必須 check 13 pass / 未解決スレッド 0
