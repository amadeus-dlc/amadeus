...
> E-OC1証跡: Guide meによるソロモード・ユーザー本人のHUMAN_TURN直接回答。Q1〜Q4の個別回答後、Q5で合意サマリーを確認した。Q8はConstruction開始後のchange requestとしてユーザー本人が明示承認した。leader（ユーザー本人）承認: 2026-07-29T09:05:42Z。
...
`amadeus/config.json`が`"auto-mirror": "auto"`であるにもかかわらず、Mirror Issue未作成時にcreate／sync／skipの確認が表示される不具合をどう扱いますか。

Q8の「このIntentへ追加する」は、作成するIssue本文に記載した次の期待動作・原因境界・受け入れ条件を8件目の正本として採用することを含む。

- `auto + Mirror Issueなし`では人間へ質問せず、安全ガード付きcreate lifecycleを自動実行する
- `auto + Mirror Issueあり`では安全ガード付きsyncを自動実行する
- `prompt`だけがcreate／sync／skipを質問し、`off`はGitHub mutationも質問も行わない
- `amadeus-orchestrate.ts`に残る暫定`decideMirrorBoundary`が`auto + issueなし`を`ask`へ落とす二重policyを除去する
- create／syncのoperation選択は既存mirror coordinator／lifecycle policyへ委譲し、operationをentrypointで直接決定しない
- pending receiptと同一boundary instanceの再試行を維持し、remote create成功後のlocal失敗でも重複Issueを作らない
- mode × Mirror存在状態の6象限をunit／integrationで固定し、generated harnessのe2eと配布drift guardを通す
- `prompt`だけが質問する設定契約へ英語・日本語のlayered config文書を同期する

- A. [#1681](https://github.com/amadeus-dlc/amadeus/issues/1681)としてこのIntentへ追加する — 8件目として1 Issue = 1 Bolt = 1 PRで修正する
- B. 別のBugfix Intentへ分離する — 現在のIntentは既存7件だけを扱う
- C. Issueだけ作成し、このIntentでは修正しない
- X. Other (please specify)

[Answer]: A — #1681としてこのIntentへ追加する（2026-07-29T09:05:42Z、ユーザー直接回答。画面上の選択肢1をAへ解決）
