...
`reviewerReadScope(unit: UnitRef, consumes: readonly ArtifactRef[]): ReadScope`をorchestrator-ownedな正準入口とする。engine directiveから`stage_file`、current Unitの必須`produces`、実在するoptional `produces`、present `consumes`を一度だけ列挙し、重複を除いた決定的pass-listを返す。Q&Aは`directive.consumes`に明示された場合だけ含める。この§12a declared pass-listをauthoritative scopeとする。missing optional、`memory.md`、`plan.md`、reasoning、record root、sibling Unit artifact、未宣言Q&Aは含めない。
...
追加requestは、current Unit artifactの具体的integration ID、passed contractが解決する単一owner path、claimed shape確認に必要な非空reason、browse/search由来でない単一file path、の4条件を`check-read`でAND評価する。承認時だけ当該invocationへ1 pathを追加し、2 file目や次iterationへの継承を認めない。`complete-review`はtranscript全件を同じ正準入力から再計算する。

owner 0件・複数、path不一致、reason空、directoryやpatternならaccepted requestは0件である。workloadはsibling Unit数へ比例せず、current Unitと明示passed contractの大きさだけで有界となる。actual read捕捉のproxy/sandboxは追加しない。
