...
| Provenance | 既存Review/subagent/audit records | Scope decision transcriptはprompt/result間だけのtransient carrierとし、最終Reviewへ再検証済みprojectionを永続記録する。既存auditはinvocation identity/completionを保持する。新eventなし。 |
...
公開seamは`reviewerReadScope(unit, consumes)`と`runtimeReviewIdentity(persona, utcDate)`の正準2関数だけに限定し、pass-list builder、owner resolver、`check-read`、transcript validator、date command runnerは内部helperとする。
...
- 新audit event、read ledger、別provenance store、全6 harness read proxy/sandbox、独自retention/alerting基盤。
...
pureな4条件predicateとidentity validationはunit、real path/declared pass-list/transient Scope decision transcript/subagent/永続Review projectionはintegration、配布journeyは必要最小e2eへ置く。actual invisible readの完全捕捉はtest対象にしない。push前local lcovでpatch追加行未カバー0を確認し、spawn blind spotは実測後に計測済みmoduleへのin-process seamで覆う。waiverは既決条件の明示証拠がある残余行だけに限定する。

## Decision consequences

既存stack維持により新しいsecurity/operations surfaceを増やさず、reviewのdeclared scopeとprovenanceを決定的にできる。一方、owner contractが曖昧なintegrationは追加requestで補えずfindingとして戻る。これは既決fail-closed policyであり、本Unitでpermissionを広げない。
