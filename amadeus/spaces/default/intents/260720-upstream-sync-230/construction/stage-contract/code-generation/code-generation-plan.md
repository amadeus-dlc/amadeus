...
1. [ ] RED: 公開契約と厳格な不正入力拒否を unit test で固定する。
2. [ ] RED: graph/runtime/directive/coverage/guard と既定互換を integration test で固定する。
3. [ ] GREEN: 共通型・frontmatter parser/emitter/schema validator を最小変更で実装する。
4. [ ] GREEN: graph projection、Bolt DAG の unit kind、成果物選択を共有契約へ接続する。
5. [ ] GREEN: directive、coverage、artifact guard に同じ成果物選択規則を接続し、空集合を vacuous success とする。
6. [ ] GREEN: 4 つの既定ステージと active unit DAG に正準の種別マッピングを追加する。
7. [ ] 正準ジェネレーターで package/self-install surface を再生成する。
8. [ ] 検証: focused test、typecheck、lint、complexity gate、`dist:check`、`promote:self:check` を実行する。
9. [ ] 変更・検証・既知の制約を `code-summary.md` に記録する。
