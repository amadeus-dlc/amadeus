...
| Gate revision | timestamp順audit shards | Revision Count、gate/revision audit rows | approve transaction |
...
全audit shardを読み、各blockから`Timestamp`と元buffer positionを取得する。`Timestamp`昇順、同値はposition昇順で次の6 eventをinterleaveする。
...
- Revision: bug flow、clean approve、reviewer append、recorded reject、non-produces write、autonomous、off-switch、no anchor、stage-start fallback、mid-stage coaching、multi-shard timestamp orderを対照testにする。
