...
- FR-3.2: 導入コマンド・post-install verification（doctor、validator）の実行例と出力例は、隔離 workspace での実実行結果を貼る（NFR-1）。
...
- NFR-1: 実測駆動 — 全コマンド例は隔離 workspace（本番 `amadeus/` を変更しない一時ディレクトリ）での実実行で検証し、コピペで動くことを確認する。出力例は実物（無関係部の省略は「…」で明示）。
