...
- イテレーション1: NOT-READY — 1件(`repository.directory` の出典追跡不能+正値未定義)→ 出典は ideation raid-log I2 の対応欄に実在。是正: 出典の明示+正値の定義(フィールド削除)+変更対象表への伝播+バッジ行の明示
- イテレーション2(最終): **READY**(raid-log 実読・3ファイル伝播・U4 との責務一致を確認済み)。非ブロッキングの表記ゆれ(4要素/5要素)も5点列挙に統一済み
