...
- E2E テスト(計測区間を明記): タイマーは **CLI プロセス起動から終了まで**。フィクスチャアーカイブ使用時はネットワーク項(解決+取得 ≤25秒)が消えるため、**U1 ローカル I/O 項(≤6秒)+U2 分(≤25秒)= ≤31秒**をアサートする。実ネットワークのスモーク(リリース前 `--release` 層)は ≤60秒をアサート
