...
| BR-P02 | 同梱物契約は `PackContract.current()` の単一定義から導出する。テスト・package.json `files`・手順書に同じリストを手書き複製しない |
...
| BR-P09 | `packages/setup/package.json` は root の I1/I2 不備を継承しない。root 自体の是正も本 Unit のスコープ(公開前必須) |
