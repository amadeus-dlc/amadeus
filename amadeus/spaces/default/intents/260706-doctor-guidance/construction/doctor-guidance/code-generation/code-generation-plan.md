...
| Step | 対応要求 | 対象 |
|---|---|---|
| Step 1 | FR-3.1 / FR-3.2 | RED 先行: installer eval へ #573 一連（11 検査）を追加 |
...
- [x] **Step 1: installer eval の RED（FR-3.1/FR-3.2）** — makeFreshWorkspace()（memory 非 seed）を追加し、「導入 → installer exit 0 + info 行 → doctor exit 0 + advisory marker + dist/ 文言なし → 実 CLI birth → advisory 消滅 → harness dir 削除 → doctor fail + installer 再実行誘導」の 11 検査を追加。実装前に 7 検査の FAIL（RED）を確認（birth 後の 3 検査は現行挙動で pass = FR-1.3 の回帰ガード）。
...
- [x] **Step 5: 回帰確認** — installer eval 11/11 GREEN、typecheck pass、parity:check ok、test:all exit 0（#554/#451 領域の既存 eval 含む）。
