...
| `dev-scripts/evals/installer/check.ts` | FR-3 | makeFreshWorkspace() + #573 一連 11 検査（RED 先行 7 FAIL → GREEN 11/11） |
...
- GREEN: 11/11 ok。eval 精密化 2 件（FR-3.2 の破損状態は .claude + .agents 両方削除、dist/ assertion は shell 行に限定）は diary の Interpretations に記録。
