...
- **BR-U1-5(回路遮断器)**: depth 指定時のみ、総質問数(提示済み全ラウンドの合計、estimate 確認・followup 込み)が目安×3(M12/S24/C36)に達したら「ツリー未完走」を明示開示して停止。silent 打ち切りの禁止を明文化。Free は適用外。【裁定 E-GFR-CG1 2026-08-10(2-0、GoA 2x2)による申告付き追補 — PR #2828 の外部レビュー(CodeRabbit Major)起点、cid:code-generation:c1-external-review-contract-change 準拠】遮断器境界でラウンドは原子的(追記された ⇔ 提示された): 残り枠がラウンド全体を提示できない場合、そのラウンドは questions ファイルへ追記も提示もせず、frontier を未解決として C-4 の開示対象へ渡す — 空 [Answer] タグの残骸を作らないことで stage-protocol §3 Step 4 の空タグ検査による遮断器後の無音再開経路を構造的に封鎖する。帰結として遮断器は数値上限手前のラウンド境界で発火しうる(count は上限であって目標でない)。[FR-PROTO-8]
