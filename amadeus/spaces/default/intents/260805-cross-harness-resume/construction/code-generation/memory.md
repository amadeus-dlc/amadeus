...
- 2026-08-05T14:55:00Z — verb 配置は amadeus-utility.ts でなく amadeus-state.ts(session-takeover)— RECOVERY_COMPLETED の単一 emitter 契約(12-state-machine.md:214、t48 逆検査)を維持するため。builder の裁量2件(FR-5 は既存章拡張 / t453 assert の精密化)は conductor が妥当と判定
...
- 2026-08-05T14:55:00Z — conductor 裏取りで builder 指定のテストパス誤り(t451-caller-denial-diagnosis→実名 t451-caller-denial-reason)により bun が1ファイルを無音除外(4指定→3実行)— ls 実測で正名確認し再実行、全4ファイル green(cid:build-and-test:test-path-set-completeness の実践)
...
- coverage patch gate の正規判定は PR CI で確定(allowlist waiver 2件除去の妥当性 = t451 の in-process driver 化)
