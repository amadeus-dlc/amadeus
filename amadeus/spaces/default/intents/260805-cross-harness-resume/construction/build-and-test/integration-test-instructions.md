...
|---|---|
| `tests/integration/t448-caller-denial-reason.integration.test.ts`(新規) | FR-1: 拒否4原因値の判別(C1/C2/C3/C5 相異、C6=C1 同一の (b))+ FR-2: 原因別メッセージの復旧コマンド名 |
| `tests/integration/t449-kimi-session-start-recovery.integration.test.ts`(新規) | FR-3: C1/C2/C3 合成 → SessionStart 相当 → authorized の閉包(`.lock` 残存の解除含む) |
| `tests/integration/t450-session-takeover.integration.test.ts`(新規) | FR-4 (a)〜(f)(人間確認 fail-closed / 再バインド / role 明示 / audit / --project-dir / unpark 疎通) |
...
| `tests/integration/t-coverage-mechanism-ratchet.test.ts` | mechanism 台帳(EXPECTED_NONE_TO_CLI への t449/t450 登録) |
