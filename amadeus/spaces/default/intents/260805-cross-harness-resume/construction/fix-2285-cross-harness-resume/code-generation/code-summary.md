...
| Step | Red | Green |
|---|---|---|
| FR-1/FR-2(t451) | 10 fail / 2 pass | 12 pass |
| FR-3(t452) | 1 fail / 3 pass(fail は FR-3 が例示した `.lock` 残存そのもの) | 4 pass |
| FR-4(t453) | 10 fail / 3 pass | 13 pass |
...
なし(`.lock` 是正は FR-3 本文が明示的に授権)。裁量2件を申告: (1) FR-5 手順書は新章でなく `11-session-management.md` の拡張(章番号空間の衝突回避 — shared-ledger-insert-collision)(2) t453 の当初過剰 assert(audit バイト不変)を要件の実契約(RECOVERY_COMPLETED 不在+state 不変)へ精密化
