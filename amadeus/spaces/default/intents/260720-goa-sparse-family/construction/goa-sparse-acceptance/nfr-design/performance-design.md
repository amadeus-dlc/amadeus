...
- 同一入力`E-ABC-`と`E-ABC-x`を両所有面へ与え、`ECODE_RE`がvalid prefix `E-ABC`を返す一方で`GoaLineCode.parse`がwhole valueを拒否する対照を1つのtable-driven testでassertする。

## 引き渡し

code-generationはpure scan seamとwrapperを同一正本で実装し、unit層で`N=1/2/4`を駆動する。あわせて`ECODE_RE`の旧/新count対照、複節全長match、同一入力でのscanner prefix受理/validator全値拒否対照を同じ正本testへ置く。build-and-testは実測`H`、offset単調性、`execCalls=H+1`、E-code occurrence count前後一致、関連test/全CI/patch coverageを確定値として記録する。
