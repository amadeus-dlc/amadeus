...
- **R-22 の是正完了(§12a iteration-1 BLOCKER の閉包、2026-08-16)**: 当初この unit は R-22(ゲート改訂復元)を NOT 実装のまま出荷し(旧記載: 「semiの `[R]` revise loop がゲート改訂復元を失う — これはR-22が禁じる退行そのもの…ここでは是正しなかった」)、R-23 の「投影変更と同一変更列」条件に違反する実退行が main に存在した(実測: `amadeus-state.ts:4165` が投影値 `isAutonomousMode(content)` を渡し、semi でも `amadeus-lib.ts` が `not-needed` を返す)。§12a iteration 1 が BLOCKER として検出 → 既決 FD 契約(R-22)の執行として conductor が in-intent 修復を実施: 新 reader `declaredFullAutonomy`(`amadeus-intent-autonomy.ts`、R-21 の answer path と同型の宣言 mode 束縛)+ call site 差し替え(`amadeus-state.ts:4168`)+ t481 正典書込テストへの第 7 読者 pin(semi → false / full → true。TDD Red 1 fail 実測 → Green)。**PR #3146 で MERGED**(CI green・converged: true 実測 → 常任承認 queue)。R-23 の「同一変更列」の字義は満たせなかったが(履歴事実として本行に記録)、同一 intent 内・build-and-test 前に退行は閉じた。
