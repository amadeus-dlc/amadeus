...
- basis.fingerprint は不透明 SHA-256(算出は下流)— 本 unit は書式検証のみで、偽 fingerprint による裁定偽装は下流の照合(監査)で検出〔事後注記 2026-08-16: 「本 unit」は C1 語彙モジュール(amadeus-recommendation.ts)を指すと確定 — E-260816-R6-FINGERPRINT 1-1 tie のユーザー裁定 B。導出段 C2 での算出実装は設計意図の実現。詳細は code-generation/code-summary.md 申し送り節〕
