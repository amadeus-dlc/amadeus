# Deployment Architecture — u001-registry-issues-field
...
デプロイは存在しない。成果物は git リポジトリ内のファイル（`intents.json` と検証コード）であり、PR merge がそのまま「配備」である。実行環境は各作業マシンのローカル（Bun のみ。gh CLI は不要）に限る。クラウドリソースは使わない。
