...
4. C5が`<scratch-project>/.claude/settings.json`へ`{"hooks":{}}`だけを生成し、`CredentialBinding`をchild allow-listへ注入する。base env keysは`PATH`,`HOME`(scratch),`TMPDIR`(scratch),`LANG`,`LC_ALL`,`NO_COLOR`。authはnative keychain（env追加なし）または`ANTHROPIC_API_KEY` leaseだけを初期対応とし、それ以外はcapability unsupported（**訂正 2026-08-08 / Issue #2235**: native keychain は `HOME` 隔離と両立せず、正しくは `ANTHROPIC_API_KEY` のみ。根拠は business-rules.md の BR-I06 訂正注記）。user/local settings、hooks、source auth/config pathは渡さない。
