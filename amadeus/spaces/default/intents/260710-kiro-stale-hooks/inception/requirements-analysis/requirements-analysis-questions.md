...
[Answer]: A — エージェント間選挙で確定(2026-07-10、全会一致 6:0)。stale 7件削除 + kiro CLI manifest の `.kiro.hook` exemption regex 除去(2層とも閉じる)。落ちる実証 = 削除後に dist へ `.kiro.hook` を再注入すると既存 orphan scan(dist:check)が赤で捕捉すること。
...
[Answer]: B — エージェント間選挙で確定(2026-07-10、相対多数 3:2:1)。source 側の汎用未参照検査は本 intent に入れず別 Issue に分離。多数派の理由: 全 harness 横断の偽陽性棚卸し + 参照定義の設計を要する別件で bugfix スコープ超過、かつ Q1=A 完了後は `.kiro.hook` 再混入クラスを既存 orphan scan がガードする。少数派(A/C)の論拠「誤 green 恒久是正の価値」は分離 Issue の背景に記載する(leader 指示)。
