...
| DS-4 Mappingの再現と承認 | quality leadが、corpus標本・label・距離統計から `W = max(nearest-rank p95, min + 1)` とmodeを再計算し、lower/upper-bound saturationの分類を承認できる | FR-SWP-1〜4、FR-TST-3 |
...
| DS-4 | schema・承認labels・W/mode期待値 | primary: index/sweep・統計・生成mapping・一致test | build後のmapping drift検査 |
...
DS-4を中心に、DS-1〜3/5/6が必要とするmapping schema、承認fixture、測定条件、lower/upper-bound saturationの期待値を提供する。内部の受け入れ確認はschemaとfixtureだけで閉じ、実行可能なindex/sweepや生成結果を所有しない。これはUnit間の経済的な実装順序ではない。

### `numeric-provenance-sensor-cli`

DS-1〜3とDS-6のruntime振る舞いに加え、DS-4のDesign-time Artifact Index、sweep generator、機械生成report/mappingをprimaryで実装し、DS-5のproducer面を持つ。内部の受け入れ確認は、U1 contract適合、pure seamの境界fixture、cutoff/分類、CLI verdict、性能、既存dispatcher integrationをそれぞれ独立に判定可能とする。
