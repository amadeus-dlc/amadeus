...
- **FR-PRED-2**: provenance 受理語彙を次に固定する。(a) 同一近傍窓内のバッククォートコードに `git|grep|rg|wc|find|ls|jq|gh|bun` のいずれかがコマンド境界で存在する、(b) `測定 ref|measurement ref|observed at|HEAD|origin/main` のいずれか、(c) 7-40桁の hex SHA、(d) Markdown link の宛先が同一 record 内の測定成果物または codekb `re-scans/` を指す。単なる「測定」「確認済み」やリンクのない裸ファイル名は受理しない。語彙変更は別 requirements 変更とする。
...
- **FR-PRED-4**: 適用候補は intent record 配下の Ideation/Inception/Construction/Operation の stage 成果物 `.md` とし、`*-questions.md`、`memory.md`、`verification/**`、audit/state、定型 ack、100行未満かつ数値候補0件の軽量報告を除外する。残る成果物種別×意味クラスを FR-SWP-2 の規則で enforcement / measurement-only に分類し、1件以上 enforcement となった stage slug のみ frontmatter `sensors:` へ配線する。成果物パス→stage slug は runtime graph の宣言済み produces から機械導出し、手書き推定しない。分類表と配線 stage 集合は FR-SWP-4 の成果物を正本とする。`matches` は dispatcher の `matchesGlob` と hook の `Bun.Glob` の両エンジンで一致することをテストで検証する(RE §2 t514 様式)。
...
- **Date:** 2026-08-10T09:47:51Z
- **Iteration:** 1
- **Scope decision:** none

数値 provenance ガードの目的、適用限定、advisory 方針は明確です。しかし、検出対象・近傍窓・閾値・配線範囲という中核契約が後工程へ未確定のまま委譲され、閾値と verdict の関係にも矛盾があります。現状では実装者とQAが同一の合否判定を導けないため、着手可能な要件とは判定できません。

### Findings

- BLOCKER | FR-PRED-1〜4、FR-SWP-4、Open questions が、対象クラス、regex、近傍窓、対象成果物、配線 stage 集合を corpus sweep または設計段で確定するとしています。これらはセンサーの発火有無を変える材料契約です。requirements-analysis の承認前に、確定値または客観的かつ一意な決定規則と承認主体を定義してください。construction 成果物へ委譲したままでは、本レビューで要求動作を検証できません。
- BLOCKER | FR-SEN-3 は pass = findings.length === 0 と規定する一方、FR-SWP-2 は findings 許容量または flag-rate の閾値を要求し、FR-SWP-3 は測定のみへの降格も認めています。閾値が verdict、finding 生成、適用対象選別、または設計時の採否のどれを制御するのか不明です。各モードについて同じ入力から導かれる pass/skipped/findings の期待結果を固定してください。
- BLOCKER | FR-SWP-1〜3 と FR-TST-3 は既存コーパスの偽陽性率を成功判定に使いますが、未併記検出を真の違反と偽陽性へ分類する正解ラベル、標本抽出、判定者、分母、合格上限が定義されていません。単なる未併記率では偽陽性率を証明できません。QAが再現可能な評価手順と合格基準を定義してください。
- BLOCKER | NFR-3 の「既存センサー並み」「数百 ms オーダー」「fire timeout 既定内」は、測定環境、入力サイズ、試行回数、統計量、上限値がなく合否判定できません。100KB 敵対入力を含む具体的な性能予算と測定手順を明記してください。
- FOLLOW-UP | requirements.md は技術実測の正本として pass-list 外の re-scans/260810-numeric-provenance-guard.md、また FR-PRED-5／FR-TST-4 で team.md・project.md を参照しています。後続レビューでも根拠を追跡できるよう、正式な consume に含めるか、必要な契約と測定結果を承認対象成果物内へ自足的に記載してください。
- FOLLOW-UP | Open questions の .claude/sensors/ と core manifest の8バイト差は、FR-DIST-1〜2の配布同一性に影響する可能性があります。実装開始前の調査条件と、不一致が残った場合の許容可否を明記してください。

## Review — Iteration 2

- **Verdict:** NOT-READY
- **Reviewer:** amadeus-product-lead-agent
- **Date:** 2026-08-10T09:52:49Z
- **Iteration:** 2
- **Scope decision:** none

iteration 1 の4件中、threshold と verdict の関係、偽陽性評価、性能予算は解消されています。対象クラス・近傍窓・分類・配線にも一意な導出規則と承認主体が追加されました。ただし、適用除外と provenance 受理条件に実装者ごとに判定が分かれる未定義語が残り、同じ入力に対する合否をQAが一意に導けないため READY にはできません。

### Findings

- BLOCKER | FR-PRED-4 は「定型 ack」を検査対象外としますが、該当成果物を識別する path glob、frontmatter、runtime graph の artifact type その他の機械判定規則がありません。数値を含む同一ファイルが実装者によって skipped にも enforcement finding にもなり得るため、iteration 1 の対象成果物に関する材料契約は未解消です。識別規則を固定し、ack／非ackの境界 fixture で pass/fail を検証可能にしてください。
- BLOCKER | FR-PRED-2(d) の「同一 record 内の測定成果物」が未定義です。任意の Markdown link を provenance として受理するのか、特定ディレクトリ・artifact type・declared produces のみに限定するのかで enforcement verdict が変わります。許容パス、相対リンク解決、record 外参照の扱いを一意に定め、受理／拒否 fixture を固定してください。
- FOLLOW-UP | iteration 1 の残る技術根拠追跡事項として、requirements.md は pass-list 外の re-scan、project.md、team.md を引き続き参照しています。材料契約は概ね requirements.md 内へ転記されましたが、後続レビューが実測根拠を監査できるよう正式な consume 化を検討してください。
- FOLLOW-UP | .claude/sensors/ と core manifest の8バイト差は Open questions のままです。FR-DIST-1〜3 の build・配送先実行・drift 検査で出荷可否は判定できますが、差が残る場合の許容条件と記録先を実装前に明示すると手戻りを防げます。
