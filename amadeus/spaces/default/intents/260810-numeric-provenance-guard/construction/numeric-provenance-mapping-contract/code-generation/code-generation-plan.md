...
- [x] **Step 6: sample identity preimageの曖昧性を修正する。** 同一pathの`line=5, normalizedText=10件`と`line=51, normalizedText=0件`が区切りなし連結で衝突する回帰testを先に追加し、旧schema/fixtureを検出する2 failを確認した。UTF-8 `JSON.stringify([relativePath,line,normalizedText])`（空白なし・要素順固定）のlowercase hex SHA-256へ上位契約、schema、fixture、digest chainを同期して9 pass / 0 failへ戻した。Trace: FR-SWP-1、BR-U1-04、SampleIdentity。
