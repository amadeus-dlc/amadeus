...
- **E-OC1 判定**: **選挙不要**。根拠種別は「既決規範の機械的執行」— FD(business-logic-model.md)が 3 層置換・判定表・結線 3 点・検証面(t451〜t453)まで確定済みであり、NFR 設計分岐は requirements.md §Non-functional requirements の逐条照合と `cid:nfr-design:c1` から一意に導出できる。
...
- 後続へ委ねる判断: なし(実装は FD の t451〜t453 設計に従う)
- 上流との矛盾: **なし**
