...
| D6 | 引き取り C(quality-waiver 収載 assert) | t459 に `PROHIBITED_EFFECTS` の `quality-waiver` 収載を assert するテストを置く(`amadeus-intent-autonomy-production.ts:69-75` 実測 verbatim を §C16 手順 2 が転記済み)。落ちる実証: 配列から `quality-waiver` を除去すると赤 | 引き取り C(`unit-of-work.md` §未確定事項の引き取り) |
| D7 | テスト層と seam | t457(C16 unit — 写像・registry・翻訳の純関数部)/ t458(integration — FR-ADV-1/2 の guard 貫通と receipt 記録、実 FS)/ t459(unit — D6 の収載 assert + C17 受理 3 点)。`amadeus-advisory-choice.ts` の in-process 駆動は export 済み関数経由 | `unit-of-work.md` §テスト番号の予約 / `cid:code-generation:fs-tests-integration-first` |
