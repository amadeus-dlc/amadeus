...
## Open questions
- 2026-08-06T01:10:00Z — [ブロッカー/停止] CG approve report が「advisory choice store shape is invalid」で fail-closed(決定的再現 2 回)。原因 = aar の FR-ADV-3 置き換え(schema 1 store 非翻訳)× 本 intent の live store が schema 1+pending 1 件(formal-model-check)— 移行経路が製品に存在せず、hold の「聞き直し」機構が store 不読で起動不能。Issue #2330(bug/P1/S2)起票+PR #2318 へレビュー観点コメント済み。ユーザー指示「parkしてpush」により停止(engine park は autonomous モードで拒否 — 設計どおり)。再開時: #2330 の回復経路確定(人間裁定を要する)→ pending advisory 再裁定 → approve report。record/push は da783aaaa まで反映済み
