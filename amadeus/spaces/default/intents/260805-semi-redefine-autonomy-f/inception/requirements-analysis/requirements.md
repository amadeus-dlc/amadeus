...
- **FR-CLI-3(再宣言の意味論)** — 「既に設定済みか」の判別に **state フィールドの有無や値を使わない**。state テンプレートは birth 時点で `- **Intent Autonomy Mode**: none` を書く(`tools/amadeus-utility.ts:4635` 実測、HEAD 断面)ため、フィールドの実在を「設定済み」と読むと新規 Intent への `--autonomy semi` が常に異値扱いになり FR-CLI-1 の主用途が構造的に成立しない。判別子は `modeProvenance` とする — 初期投影は `{ kind: "system-default", after: "none" }`(`tools/amadeus-intent-autonomy.ts:218-227`)であり、既存の認可述語も同じ判別子を使う(`:512` verbatim `    if (!internalGate || projection.modeProvenance.kind !== "human-command") {`)。`modeProvenance.kind === "system-default"` なら**未宣言**として初回宣言を受理し、`"human-command"` なら **同値 no-op / 異値 loud** で停止して `amadeus-bolt set-autonomy` を案内する。起動フラグが人間が決めた既存 mode を無言で書き換えない。
  - **受け入れ基準**: (0) **birth 直後**(`modeProvenance.kind === "system-default"`、state は既定値 `none`)の Intent へ `--autonomy semi` → 0 exit で mode=semi を設定(loud にしない)。この1件が欠けると主用途が塞がる。(1) mode=semi(human-command 由来)の Intent へ `--autonomy semi` → 監査イベントを増やさず正常継続。(2) 同 Intent へ `--autonomy full` → 非 0 exit + `set-autonomy` の案内文。(3) (2) の状態で既存 grant が revoke されていないこと(`prepareNonFullCommand:386-390` の `revoke-full` 経路が起動フラグから到達不能であること)。
