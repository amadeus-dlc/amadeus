...
requirements.md 未解決事項の4件(FR-1/FR-2/FR-3/FR-5 の方式)を4問で扱う(FR-3 は1問内の2面 — 選挙 definition 上は q3/q4 の2 question として裁定)。component 境界・通信様式・データ所有の変更は無い(既存コンポーネントの欠陥修正 — components.md)ため、ステージ標準の境界質問は「該当なし」とし、方式裁定のみを問うた。
...
## Q3: #3149 の正の所在(クラスA)と回復経路(クラスB)

A面(クラスA): A. CLI を正としセンサーの束縛を直す / B. センサーを正とし CLI に converged→landed を追加。 B面(クラスB): A. 機械的同値証明のみ / B. human-presence 付き override のみ / C. 二段構え。 / X. Other

[Answer]: A面 = A、B面 = B — 選挙 E-260817-PBB3-FIX-METHODS の q3-3149-authority / q4-3149-orphan-recovery はいずれも tie hold(A面: s1=B GoA2 / s2=A GoA3。B面: s1=C GoA2 / s2=B GoA3)、ユーザー裁定で A / B 確定(2026-08-17)。converged は final のまま in-place finalisation + attestation ベース束縛(ADR-3)。回復経路は s2 の実測(実在3件すべて tree/patch-id 不一致 — 機械証明は観測レンジ外)により human-presence 付き override のみ(ADR-4)。
