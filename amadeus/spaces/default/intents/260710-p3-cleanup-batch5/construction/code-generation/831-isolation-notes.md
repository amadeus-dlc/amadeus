...
## E-B5e 裁定と実装(2026-07-11、b5-u6-831)

E-B5e 開票: Q1=(A) `AMADEUS_LOCK_BASE_DIR` override 単独採用(4/6、(A) 部分は6票一致。(B) ハッシュ幅拡大は併用不採用)、Q2=stamp guard は本 PR 同乗(5/6)、Q3=fixture 随伴是正の追認(6/6)。

### 実装
1. `auditLockDir` の `tmpdir()` を `lockBaseDir()` 経由にし、`AMADEUS_LOCK_BASE_DIR` で上書き可能にした(`lockStaleMs()`/`AMADEUS_LOCK_STALE_MS` と同型の env 注入。テスト専用分岐ではない)。
2. lock 系テスト(t76・t161)を module load 時に `mkdtempSync` した run 毎ユニーク base dir へ隔離。afterAll で撤去。
   - t76: `state()` の `spawnSync` と test13 の `spawn` に `env: process.env` を明示追加。**bun の spawnSync は既定 child env に runtime の process.env 変更を畳み込まない**ため(scratch 実測: no-env-opt で子は `<unset>`)、明示継承が必須。これがないと subprocess が override を継承せず、植えた lock と別 bucket をロックして merge が即成功(高速 fail)する。
   - t161: `cleanLocks` の sweep 対象を `tmpdir()` から run の base dir へ変更。per-run 私有 dir を掃くため、無差別 sweep が他 run の lock を消す干渉(#859 の timeout 方向 flake の一因)も解消。
3. 新規 `lockBaseDir` の両分岐(override 有/無)を t161 に in-process seam テストで被覆。

### 実証(裁定の完了条件)
- (i) 落ちる実証: seam scratch で、隔離無効(共有 tmpdir)時は run B の cleanup rmSync が run A の植えた lock を削除(survived=false=機序再現)、隔離有効(per-run base dir)時は disjoint パスで保存(survived=true)。exit=0。
- (ii) 隔離有効で t76+t161 を 3並列×各10回=30並行実行 → フレーク0。他 lock テスト(t115/t145/t163/t164/t162/t33/t66/t49/e2e t07)全 pass で退行なし。

### stamp guard(Q2 同乗、E-B5b 由来)との区別
stamp guard は releaseAuditLock/onExit の release 非対称 defect(他者 owner の lock を無検証削除)の是正であり、in-process の cross-identity release-collision に対する正しさ改善。#831 フレークの閉包は (A) の base-dir 隔離が担う(削除者は別プロセスのため in-process guard では閉じない)。両者は別便益として同一 PR に同乗。
