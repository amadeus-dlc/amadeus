...
`git diff --stat cac41363a HEAD -- tests/ .github/` の出力からの転記:

| ファイル | 変更 |
|---|---|
| `tests/lib/control-byte.ts` | 新規 63 行 |
| `tests/control-byte-gate.ts` | 新規 199 行 |
| `tests/unit/t-control-byte-predicate.test.ts` | 新規 66 行 |
| `tests/integration/t-control-byte-gate.integration.test.ts` | 新規 319 行 |
| `.github/workflows/ci.yml` | +22 行(独立ジョブ `control-byte-gate`) |
| `tests/fixtures/formal-verif-ci-baseline.sha256` | 1 行更新(ci.yml のピン同期) |
| `tests/integration/t-formal-verif-ci-workflow.integration.test.ts` | +4 行(同上) |

合計 7 files changed, 674 insertions(+), 1 deletion(-)。

コミット: `e7341b254`(実装本体)、`b5e514dd8`(allowlist assert・symlink 実測記録・CLI テスト拡充)。
...
| `bun test`(unit 述語 + integration ゲート + formal-verif ci workflow) | 0 | 33 pass / 0 fail / 125 expect / Ran 33 tests across **3 files**(宣言した path 集合の実在を事前確認し、runner 報告のファイル数と照合) |
...
| FR-CBG-7 CI blocking 配線 | 独立ジョブ `control-byte-gate`。落ちる実証で赤の実在を確認 |
