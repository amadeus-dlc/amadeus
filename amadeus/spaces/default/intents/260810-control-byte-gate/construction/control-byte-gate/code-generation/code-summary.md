...
上表は**出荷断面 `cc775f87b` で閉じている**。その後 §FR-CBG-7 の是正(PR #2880)で加えた差分は次のとおり。測定 ref は **PR #2880 の squash コミット `10fd33610`**、述語は `git diff --numstat 10fd33610~1 10fd33610 -- tests/ .github/`(`--stat` の表示幅ではなく numstat の insertions/deletions):
...
| `tests/integration/t222-ci-snapshot-branch.integration.test.ts` | 19 | 1 | needs 集合 pin 2箇所の伝播 + `require_result` の位置 pin(CodeRabbit 指摘への対応) |
...
合計 5 files changed, 81 insertions(+), 4 deletions(-)(8+46+19+7+1 = 81 / 0+1+1+1+1 = 4 の機械再計算と一致)。

> **本表自身の訂正記録**: 当初この表は測定 ref を `cc775f87b..HEAD` と書き、t222 を +15 −1、合計を 77/4 としていた。conductor の閉包検証で2つの陳腐化が判明した — (a) `HEAD` は着地とともに動くため、PR 着地後にその述語を再実行すると main の並行前進(#2879 / #2868 / metrics)まで含む 184 ファイルの差分になり再現不能 (b) 表を書いた後に CodeRabbit 指摘への対応(`require_result` の位置 pin)が t222 へ +4 行入った。**BLOCKER 2 とまったく同じ「測定後に積まれたコミットを反映していない」クラスの欠陥が、その是正の中で再発していた**。測定 ref を動かない squash コミットへ固定し、数値を再測定して訂正した。
