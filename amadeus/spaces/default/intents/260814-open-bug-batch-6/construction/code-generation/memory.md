...
- 2026-08-15T (Open question): build-and-test 到達不能(#3099)。engine の per-unit run-stage 経路で完走した Construction は、consume fanout が swarm pool 専用の outcome 台帳しか読まないため producer-outcome-pending で恒久停止する。梯子裁定 auto-decision-88de9a6253e8f085da280f43d76f9323 = A(park → #3099 を別 intent で修正 → resume)。pool イベントの後付け生成(選択肢 C)は検証劇場クラスとして却下。
