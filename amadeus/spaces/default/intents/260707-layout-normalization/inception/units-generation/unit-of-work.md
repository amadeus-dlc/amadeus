...
| --- | --- | --- | --- | --- |
| U1 Layout Decision Record | Issue #610 に対応する ADR または同等 design record を作成し、実装後の layout contract を記録する | design/docs artifact + migration record | documentation + implementation | M |
| U2 Contributor Documentation Update | README/docs/contributing guide に `packages/framework` source zone と root `scripts`/`dist` contract の違いを明文化する | public/contributor docs | documentation + path update | M |
...
- directory move は `packages/framework/core` と `packages/framework/harness` に限定する。`scripts/` と `dist/` は root に維持する。
