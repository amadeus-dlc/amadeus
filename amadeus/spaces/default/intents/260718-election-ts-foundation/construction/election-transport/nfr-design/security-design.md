...
- `ShortNotification` 型は electionId+配布ビューパス参照のみのフィールド構成(business-logic-model.md の型定義。BR-T1 のキー全数 assert の正本は **business-rules.md:9** — reviewer 是正: BR ラベルの帰属を訂正)。質問文・選択肢・推奨・先行票のフィールドを型宣言に持たない(parse-don't-validate の対偶 — 持てない型で送る)
