...
- GoA 行スキーマの拡張(複節コード対応等)— requirements.md NFR-4 が明示禁止(変更が必要になったら実装前停止→裁定)。既存 corpus のハイフン複節コード非可視問題は **Issue #1226** へ分離済み(leader 起票確認済み 2026-07-19 — corpus 9種中8種拒否の実測付き)
