...
| 型 | 形 | 由来 |
|---|---|---|
| `StoreError` | `"exists" \| "duplicate" \| "not-found" \| "io-error" \| "corrupt"` | FR-1a/3b、C2 表。**corrupt = 既存ファイルの parse 失敗(fail-closed load)— nfr-requirements レビュー Minor の申告付き追補**(io-error = fs 操作失敗と区別) |
...
| `ElectionState` | `"draft" \| "open" \| "collecting" \| "tallied" \| "rendered" \| "recorded" \| "hold"`(**7状態** — E-ETF-FD2 Q2=A 裁定。遷移の所有は U5、**election.json への保存は U2**) | ADR-3 追補 |
