...
## 状態機械(ADR-3 — 6状態の指令ループ、FR-0)

```
draft → open → collecting → tallied → recorded(終端)
任意状態 → hold(reason: HoldReason — tie/block/quorum-short/discussion-needed)→ 人間裁定後に report で復帰
```
...
| tallied | `render` | render verb 呼出形 | render 実行 → report |
| recorded 前段 | `verify` | verify verb 呼出形(self-check) | verify 実行 → report |
| 全完了 | `done` | 記録パス | 終了 |
...
```
report --election <id> --result <distributed|ballots-collected|tallied|rendered|verified>
...
Bolt 1 の U5 核 = 最小状態機械(draft→open→collecting→tallied→recorded の直進+zero-confirm パス)+next/report+open/vote/tally の最小 verb。hold 分岐・全 verb 配線・機械実行器 e2e の完全化は Bolt 4(cli-complete)。
