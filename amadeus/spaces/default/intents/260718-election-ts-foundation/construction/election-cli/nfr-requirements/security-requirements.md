...
- vote verb の票入力は U1 `Ballot.parse` の fail-closed 5クラス検証を通してのみ受理(requirements.md FR-3b — U5 は検証をバイパスする経路を持たない)。verb 引数の不正(未知 verb・引数不足)は loud エラー+exit 非0(business-logic-model.md の exit code 契約)
