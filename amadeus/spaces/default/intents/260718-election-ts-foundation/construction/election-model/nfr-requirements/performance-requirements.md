...
U1 は純関数層(business-logic-model.md エラー処理節: shuffleView/tally/canEarlyTally/classifyLate は全域関数)であり、入力規模はチーム選挙の実測レンジ(投票者 4〜14 名 — team.sh 登録実測、選択肢 ≤10)に閉じる。この規模で専用の性能 SLO を新設しない(observability-setup:c3 — 実在しない service SLO を作らない)。
...
- Bun 直接実行(technology-stack.md の Bun/TS スタック実測)で起動 ~20ms — CLI 応答性は runtime 特性で足り、追加最適化を要求しない
