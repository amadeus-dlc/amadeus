...
- FR-4b: 開票結果は「成立(採用/不採用)/保留(要人間: タイ・ブロック・定足数不足・**追加議論要 = GoA 5 が2票以上**)」の判別ユニオン。保留理由を型で運ぶ(C-01 — 人間裁定の保存)。定足数不足の定義 = **有効票(賛成側+反対側)が 0 件のときのみ**(新定数なし — E-ETF-FD ユーザー直接裁定 2026-07-19T03:00:35Z、Q1=A(norm (iii) の型写像補完・正準リスト(4)承認)/ Q2=A(最小定義))
