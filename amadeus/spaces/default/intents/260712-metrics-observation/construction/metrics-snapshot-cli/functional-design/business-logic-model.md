...
- test_pyramid: **classifyTestSize(tests/lib/test-size.ts:49)による静的走査**(テストファイルを glob して tier×size を自己完結で集計 — reviewer F5 の二択のうち静的側。tier(smoke/unit/integration/e2e)はファイルパスのディレクトリから導出(run-tests.ts の scopeOfFile :924 と同じ規約)。test-size-report.json(動的・部分実行で不完全)は読まない: 実行有無への依存と追加 fault 契約を作らないため)
