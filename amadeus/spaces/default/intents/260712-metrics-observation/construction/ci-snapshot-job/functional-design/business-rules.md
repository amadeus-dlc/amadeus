...
4. **配線検査テスト(P-2 の強制メカニズム、本ユニットの成果物)**: `tests/unit/t222-ci-snapshot-wiring.test.ts` を新設し、ci.yml のテキストに対して (a) `ci-success` の needs/require_result に `metrics-snapshot` が含まれない (b) `metrics-snapshot` job に `if: github.event_name == 'push' && github.ref == 'refs/heads/main'` ガードが存在する (c) job 単位 `permissions: contents: write` の存在、をアサートする。**方式は t152-windows-portability(:47-51)の readFileSync+toContain 文字列検査様式**(構造 YAML パースはしない — 新規依存を作らないため。nfr performance-requirements P-2 と対)。
