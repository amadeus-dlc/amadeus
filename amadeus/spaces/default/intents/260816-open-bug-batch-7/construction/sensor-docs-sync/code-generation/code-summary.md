...
branch `open-bug-batch-7/sensor-docs-sync`(head `a013a5bd2`、push 済み・conductor 統合済み)。方式 = D3 + クロスレビュー refinement(値照合述語、06 側陳腐値)。実測値は builder 報告からの転記。
...
- `docs/reference/07-sensor-system.md` / `.ja.md` — 表へ 4 行追加(nfr-budget / pr-convergence-report-format / question-budget / scope-sizing)+ 陳腐 2 行是正 + スキーマ例 2 行是正(追補、E-AD-528D74AF)
