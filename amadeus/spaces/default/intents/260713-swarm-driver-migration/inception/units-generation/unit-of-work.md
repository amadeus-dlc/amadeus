...
承認済みApplication Designの責務境界は維持する。C-01はdriver lifecycleの公開入口、C-02〜C-04は共通contractと決定的selection、C-05〜C-07はprovider adapter、C-08〜C-10はevidence・attempt・audit、C-11は既存referee、C-12はdistribution projectionである。ADR-001〜ADR-008の共通tool、batch coordinator、二段階evidence、atomic checkpoint、versioned event、native proof、legacy shim、platform境界も変更しない。
...
C-01の公開CLI lifecycleと、C-08 evidence verifier、C-09 attempt store、C-10 driver audit emitter、C-11 referee envelope連携を1つのstateful deep moduleへ閉じる。provider固有CLI command、provider raw event parser、harness tool invocationは所有しない。
...
- U-01が所有するversioned `DriverAdapter`、`DriverRegistration`、`ProbeResult`、`LaunchSpec`を消費し、`createCoordinator({ registry })`という内部factoryでlifecycleを組み立てる。U-02の単体testだけはfake registrationを注入できるが、provider Unitのintegration/live proofは必ず下記production registry assemblyを通す。
...
- Unit/integration test: probeがbatch/attemptごと1回、明示probe failureでworker/worktree 0件、post-dispatch fallback 0件、evidence全単射、audit redactionを検証する。
...
- 完了条件: native自己申告だけでは成功せず、native result、referee convergence、merge-backが揃うまでbatch成功eventを発行しない。再開後も成功二重発行と未完了Unit成功化が0件となる。
...
C-05の1つのClaude adapterで`claude-agent-teams`と`claude-ultracode`を別modeとして扱い、Claude Code harnessから共通driver lifecycleをend-to-endで利用可能にする。共通selector、attempt state、referee verdictは再実装しない。
...
- Agent Teamsはbatchごと1つの`claude -p` coordinator、experimental flag、in-process teammate、team config、共有task、Unit-child全単射を証明する。
- Ultra Codeはbatchごと1つのnon-interactive coordinator、Dynamic Workflow run/task/agent ID、Unit割当を証明し、xhighだけをnative successにしない。
...
- Deterministic integration: fake `claude` CLI、team/workflow state fixture、process/env/cwd/stdin、未知schema、child不足、mode marker欠落を検証する。
- macOS opt-in live proof: Claude conductorから公開C-01 CLIとproduction registry assemblyを通してAgent TeamsとUltra Codeをそれぞれ2 Unit以上で1回以上実行し、native証跡、各Unit成果、`check`、`finalize`を保存する。Codex/Kiro slotがfail-closed placeholderでもClaude harnessのselection対象外なのでClaude proofを妨げない。認証不足やsurface不明はpassにしない。
- 完了条件: Agent TeamsとUltra Codeの両方がClaude Task floorと機械的に区別され、明示modeを別方式へ置換しない。

### 制約

provider Unit内部で2 modeを別acceptance sliceにする。Unit内部をさらにAgent Team化しない。他teamをscanせず、execution由来のteam/runだけを読む。
...
| FR-11 | U-03 | Agent Teams fake integration + macOS 2 Unit live proof |
| FR-12 | U-03 | Ultra Code fake integration + macOS 2 Unit live proof |
...
1. **C-04 registry境界とprovider Unitのend-to-end完了条件が両立していない。** U-01はC-04の`DriverAdapter` registry contractを所有し、U-02は`DriverAdapter` interface versionを所有し、U-06はprovider実体の最終配線を所有するとされている。一方、U-03〜U-05はU-06より前に、対応harness conductorからC-01を通すend-to-end integrationとmacOS live proofを完了条件に持つ。C-01がprovider adapterを解決するproduction registryはU-06まで配線されないため、provider Unit単独でこの完了条件を満たす経路が定義されていない。test-only injectionを使うならC-04 contractの所有者、version、production registryとの差分、およびlive proofが何を通過すべきかをintegration contractへ明記する必要がある。そうでなければ、providerごとの静的registration descriptorを各Unitが所有してU-06は検証だけを行う、またはprovider Unitの受入範囲をadapter-levelへ限定して統合live proofをU-06へ移す必要がある。現在のままではU-03〜U-05の独立性とU-06の依存方向に隠れた循環がある。

### Validation results

- 6 Unitのboundary、responsibility、deployment model、relative complexity: 記載あり。
- FR-01〜FR-26: 26/26に主担当Unitと必須test/evidenceあり。
- USR-01〜USR-10、REL-01〜REL-02: 12/12にPrimary Unit、受入結果、test/evidenceあり。全Unitにscenarioあり。
- YAML DAG: 6 Unitを各1回宣言し、self-loop、未宣言参照、cycleなし。provider 3 Unit間にedgeなし。
- Integration points、shared resources、provider file ownership: 記載あり。ただし上記registry integration seamは未解消。
- Story map: cross-cutting concernとUnit内slice順を分離して記載あり。
- Units Generationは実装順・価値順・critical pathを選択しておらず、Unit内slice順だけを定義している。
- `amadeus-sensor-required-sections.ts`: 3成果物すべてPASS。`unit-of-work-dependency.md`は`edge_block: ok`。
- `amadeus-sensor-upstream-coverage.ts --consumes components,component-methods,services,component-dependency,decisions,requirements,stories`: 3成果物すべてPASS、未参照0件。
...
- Blocking findingなし。Iteration 1で指摘したC-04 registry境界の隠れた循環は解消された。U-01がversioned `DriverRegistration` contract、U-02が3つの静的fail-closed slotを備えたproduction registry assemblyとexhaustive mapping、U-03〜U-05が対応する1 slotだけ、U-06がplaceholder 0件・mapping網羅性・余剰descriptor 0件の検証だけを所有しており、contract、composition root、provider実装、完了検証の所有権は一意である。

### Validation results

- U-03〜U-05は、未実装の他provider slotが型付きplaceholderとしてcompile可能かつfail-closedであるため、各Unitを単独で完了できる。各providerのfake/live proofは、対応harness conductor → public C-01 command → production registry → native coordinator → evidence/refereeを通過し、test-only injectionに依存しない。
- YAML DAGは`U-01 → U-02 → {U-03, U-04, U-05} → U-06`の6 Unitで、self-loop、未宣言参照、cycle、provider間edgeなし。production registry assemblyはU-02で完了し、U-06は検証だけを行うため、暗黙の逆依存もない。
- 6 Unitのboundary、responsibility、deployment model、relative complexity: 記載あり。shared resourceとprovider fileの所有権も一意。
- FR-01〜FR-26: 26/26に主担当Unitと必須test/evidenceあり。
- USR-01〜USR-10、REL-01〜REL-02: 12/12にPrimary Unit、受入結果、test/evidenceあり。全Unitにscenarioあり。
- Story map: cross-cutting concernとUnit内slice順を分離して記載あり。
- Units Generationは実装順・価値順・critical pathを選択しておらず、Unit内slice順だけを定義している。
