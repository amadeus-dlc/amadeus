...
本DAGは`components`のC-01〜C-12、`component-methods`のversioned interface、`services`のbatch coordinator lifecycle、`component-dependency`の一方向依存、`decisions`のADR-001〜ADR-009、`requirements`のFR-01〜FR-26をUnit topologyへ変換する。`stories`はSKIP済みのため、USR-01〜USR-10とFR-24〜FR-26由来のREL-01〜REL-02をdependency妥当性の確認に使う。
...
| `claude-native-driver` | `swarm-execution-lifecycle` | `DriverAdapter`、`ProbeResult`、`AdapterExecutionPlan`、`CoordinatorTransport`、`EvidenceCapturePlan`、`CaptureCheckpoint`、binding/live-control、normalized evidence/failure contract | 完成済みU-02 contractのPTY/headless variantへClaude raw surfaceを正規化して渡し、common runtime、checkpoint、evidence verdict、refereeを編集しない |
...
| `swarm-execution-lifecycle` | provider 3 Unit | production registry assembly、3つの型付きfail-closed slot、attempt nonce、Unit manifest、expected wave、closed transport/capture plan、binding checkpoint、live control | U-02が静的mappingとprovider-neutral lifecycleを所有し、各providerは自分のslotとadapter projectionだけを実装する |
| provider 3 Unit | `swarm-execution-lifecycle` | normalized provider events、redacted probe result、capture plan/binding、live control signal、process terminal | U-02がcapture順序、evidence verdict、terminal stateを決め、provider自己申告やcontrol signalだけでsuccessにしない |
...
U-03、U-04、U-05は自分のmoduleの`registration_state`を実descriptorへ置換し、production composition root、共通contract/runtime、他provider module、driver mappingを編集しない。U-02は各provider Unitがreadyになる前にclosed `stdio-json | pty-interactive`、3種のcapture plan、binding、live control、capture lifecycleを完了条件として満たす。各Unitのfake integrationとmacOS live proofは、対応harness conductorから公開`amadeus-swarm-driver.ts`、production registry、C-01 lifecycle、native coordinator、evidence verifier、refereeまでを通す。`createCoordinator({ registry: fake })`による注入はU-02自身のlifecycle unit testだけに限定し、FR-11〜FR-14/FR-23のprovider proofには使用しない。
...
| coordinator/runtime/referee envelope + production registry assembly | U-02 | provider Unitは固定slotとadapter projectionだけを実装し、共通seamを編集しない | U-02が静的mapping、fail-closed placeholder、closed transport/capture、provider-neutral lifecycleを完成させる。provider固有分岐をcommon runtimeへ入れない |
| `amadeus-swarm-driver-adapters/claude.ts` | U-03 | U-02のplaceholderを実registrationへ置換し、Agent Teams PTYとUltra headlessのmode差を閉じる。U-06が検査 | provider別1 file ownership |
...
`claude-native-driver`、`codex-native-driver`、`kiro-native-driver`の間にはdirect/transitive edgeがなく、互いのprovider source file、harness source、provider fixtureを所有しない。closed transport/capture contractは3 Unitがreadyになる前にU-02の完了条件として成立し、provider Unitは共通seamを編集しない。この3 Unitは同じready setになり得て、3!通りの有効なtopological orderingと並行実行が存在する。
...
3. U-03〜U-05は完成済みU-02の公開adapter/transport/capture contractだけへ依存し、共通seamを編集せず、互いをimportしない。
