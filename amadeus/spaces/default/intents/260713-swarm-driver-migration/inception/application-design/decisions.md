...
本ADR集は`requirements`、brownfieldの`architecture`・`component-inventory`、`team-practices`、およびApplication DesignのQ1〜Q8を根拠とする。`stories`は実行計画でSKIPされている。各ADRはQ&Aで推奨案が選択済みであり、Application Design gateでまとめて確定する。
...
ClaudeのPTY/team state/hookまたはstream-json/workflow state、Codex JSONL/hooks、Kiro session metadataは形式が異なり、message本文やprovider応答を含み得る。共通toolが全raw schemaを解析するとprovider差分と機密性が中央moduleへ漏れる。adapterごとの独自auditはevent taxonomyを分裂させる。
...
|---|---|---|---|
| Claude Agent Teams | attempt専用session由来のexact team config `members` + shared task list | TaskCreated / TaskCompleted / TeammateIdle hook | members≥2、Unit task全件、全childのstart/completed stop、全owner idle、同一team相関 |
| Claude Ultra Code | `runId`から導出するDynamic Workflow completed snapshot + `transcriptDir/journal.jsonl` | Claude stream-json + Subagent hook | workflow run ID、native task/agent≥2、Unit全件、全childのstart/result/completed stop、ID全件一致 |
...
Requirements OQ-01の「headless Agent Teams」という調査仮説は実機で反証された。FR-11自体はnon-interactive processを要求していないため、OQ-01はinteractive PTY上のnative Teamをprovider state + hookで証明することで解消する。
...
## ADR-009 — Claude coordinator transportをmode別の閉じたvariantにする

**Status:** Proposed（Q8でOption A選択、stage gate待ち）
**Decision scope:** FR-11、FR-12、FR-19〜FR-23、NFR-02〜NFR-06

### Context

Code Generation入口のmacOS実機調査で、Claude Code 2.1.205はAgent Teams flagが有効でも非対話`claude -p`ではsession team初期化をskipし、通常のasync Agentだけを起動することが判明した。Task/Subagent eventは出るがteam/task stateとTeammateIdleがなく、Agent Teamsとして受理するとfloorとの区別を失う。一方、Ultra Codeはheadless `claude -p`でnative Workflow、completed snapshot、journal、hookを独立に相関できた。全Claude modeを同じstdio transportへ固定した設計前提が誤りである。
...
- Positive: Agent TeamsとUltra Codeを、それぞれ実在するnative surfaceで証明できる。
- Positive: provider固有のinteractive/headless差を閉じたtransport seamに封じ、Codex/Kiroと共通lifecycleを維持できる。
...
Option Bは実機でteam初期化されずFR-11を満たせないため却下した。Option Cはteam stateとTeammateIdleを欠く通常async Agentをnative Teamへ読み替え、FR-11の証跡条件とADR-003/ADR-006のfail-closed原則に反するため却下した。
...
中程度。Claudeが将来headless Agent Teamsを公式提供した場合、`claude-agent-teams`のtransport variantだけを`stdio-json`へ変更できる。通常async AgentをTeamとして受理する変更やtransport unionを開放plugin化する変更はnative保証と公開境界を変えるため再承認が必要。
...
| Claude Agent Teamsはinteractive PTY、Ultra Codeはheadless stdio JSON | ADR-006、ADR-009 |
...
| raw provider outputとcredentialを永続化しない | ADR-004〜ADR-006、ADR-009 |
...
|---|---|---|---|---|
| 1 | Critical | `component-methods.md` の `EvidenceCapturePlan` / `CaptureBinding` / `CaptureCheckpoint` | 前回Finding #2のevent-bound経路は`resolveCaptureBinding`、`capture-bound` self-edge、checkpoint接続で解消した。一方、fixed-pathはprovider arm前の`prepared-dispatched`でbindingを同時保存すると規定されるのに、`fixed-provider-path`は`exactPaths`しか持たず、必須の`nativeRunId`と`sourceEventDigest`を供給できない。resolverも`RawNativeEvent`を必須入力とするためarm前には呼べず、`CaptureCheckpoint`にはcapture kindがないためstoreは「event-boundだけself-edge可、hook-onlyはbinding不要」を型から判定できない。Agent Teamsのcapture-before-armを実装するには、argv解析・sentinel event・外部stateなど未記載の推測が必要になる。 | capture plan/checkpoint/bindingをkindで閉じたdiscriminated unionにする。fixed variantはadapterがarm前に`nativeRunId`とplan由来binding sourceを含むinitial bindingを返し、event-boundだけがevent digest付きresolver/self-edgeを使い、hook-onlyはbindingを持たない形にする。`CaptureCheckpoint`へkindを保存し、transition validatorがvariantごとの許可edgeを単独で検証できるようにする。 |
| 2 | Minor | `services.md` のAttempt lifecycle state diagram | `prepared --> dispatched`のラベルが`coordinator started`のままだが、`component-methods.md`とADR-009ではcapture observer開始後・provider arm前の`prepared-dispatched`保存を意味する。実装契約は後者で明確だが、図だけがspawn済みと読める。 | state diagramのラベルを`capture checkpointed before arm`等へ合わせ、spawnとcapture-boundの順序を図にも反映する。 |

### Validation Tool Results

| Tool | Result | Interpretation |
|---|---|---|
| `required-sections` | PASS（5/5成果物） | `components.md`、`component-methods.md`、`services.md`、`component-dependency.md`、`decisions.md`はいずれも必須のH2構造を満たす。 |
| `upstream-coverage` | PASS（5/5成果物） | 5成果物はいずれもruntime graphで解決された上流成果物を参照している。形式的traceabilityは満たすが、Finding #1のvariant構築可能性はこのsensorの検査範囲外である。 |

### Summary

前回Finding #1のPTY循環待ちは`observeControl`、provider-neutral signal、exit input、terminal後のretained evidence検証で解消し、signalもsuccess evidenceから分離されている。mode分離、event-bound binding、独立source AND、lease/fencing、referee境界、macOS/Linux方針は整合するが、fixed/event-bound/hook-onlyのcapture variantがまだ型として閉じておらず、Agent Teamsのcapture-before-armを推測なしで実装できないため、最終IterationをNOT-READYとする。

### Iteration 2後の解消（独立再判定なし）

- fixed-pathへarm前の`initialBinding`、native run ID、exact path digest、source plan digestを必須化した。
- event-boundだけがresolverと`capture-bound` self-edgeを使用し、hook-onlyはbindingを持たないdiscriminated unionへ`EvidenceCapturePlan`と`CaptureCheckpoint`を揃えた。
- state diagramの`prepared → dispatched`をarm前capture checkpointへ修正し、provider armとevent-bound bindingの順序を追記した。

reviewer最大2 iteration到達後の設計者修正であり、上記2件に対する独立READY判定は取得していない。最終sensor、TypeScript code block、Mermaid構造、whitespace検査を再実行し、結果と未取得判定をApplication Design gateへ明示する。
