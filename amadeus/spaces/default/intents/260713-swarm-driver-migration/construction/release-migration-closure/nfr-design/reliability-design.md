...
本設計は`performance-requirements.md`、`security-requirements.md`、`scalability-requirements.md`、`reliability-requirements.md`、`tech-stack-decisions.md`、`business-logic-model.md`を消費する。信頼性はall-match closure、same-tree provenance、immutable report、完全検索付きsingle Issue publisher、driver別transport/capture/resource receiptとprocess terminal後のredacted native indexによりfalse release closureを0件にすることで定義する。availability SLA、backup、multi-regionはlocal/CI checkerに非適用である。
...
| sealed native variant index | live validator | 4 driver不足、variant不一致、ready signal単独のfake補完0 |
| complete-search single-writer Issue ensure | issue state machine |未取得pageを含むduplicate/誤reference 0 |
...
macOS/Linux deterministic、macOS 4-driver credentialed live、4 harness dist、Claude/Codex self-install、production registry 4 driverのいずれもmissing/red/stale/extra/unknownを許さない。Agent Teamsのprocess terminal/terminal retained evidence、Ultraのexact `claude -p --verbose --effort ultracode --output-format stream-json --include-hook-events`を示すversioned invocation contract/canonical argv digestとheadless stdio、Codexのhook-only、Kiroのevent-bound captureを相互代替しない。Windows成功は要求も表明もしない。
...
| transport/capture/resource不一致、ready signal単独、process未terminal | blocked | terminal後のsealed replacement summary |
| Ultra invocation contract欠落、canonical argv digest不一致 | blocked | exact headless invocationのsealed replacement summary |
...
| Issue page/limit打切り、件数不一致、schema不明 | blocked、mutation 0 |完全検索可能な状態で再実行 |
...
finding/reportはcandidate、repository/tree/contract、domain/code/subject、expected/observed digest、platform/run SHA、driver/harness/profile、transport/capture/resource、必要なbinding/control/process terminal/terminal-retained-evidence digest、closed verdict/countを相関する。provider payload、prompt、credential、raw PTY/JSONL/CI log、absolute home/worktreeを含めない。
...
- Issue duplicate/closed/conflict時の追加mutation、検索page打切り／件数不一致／未知schemaのgreen化、0.2.0削除のscope内実装、raw/secret保存、fake/skip/floor/legacy live昇格、ready signal単独成功、Windows成功表明をmerge blockerにする。
