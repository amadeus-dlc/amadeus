...
同一invocationのrecoveryでは初回targetを`targetBeforeCommit`へexact matchし、その後は完了済みUnitの`CodeMergeOutcome`が作るcanonical prefixへexact matchする。`aidlc-merged`かつcode未完了ならcode stepから、`code-merged`かつcleanup pendingならcleanupから、両方完了かつaudit未完了ならUnit auditだけを再開する。prefix外commit、operation marker/digest不一致、旧operation group未停止、fencing不一致はfail-closedである。
...
| Referee resumable | check/process/finalize/claim/aidlc-merge/code-merge/audit failure | resumable |
