...
U-01が定義しU-02がassemblyするgeneric registration/capture contractをそのまま使う。
...
CodexUltraAdapterView.prepareResources(input)
  -> AdapterResourcePreparation(AuxiliaryResourcePlan[])
CodexUltraAdapterView.buildExecution(input, MaterializedAuxiliaryResourceSet)
...
sequenceDiagram
  ...
  alt capability or sentinel unavailable
    ...
    C->>A: prepareResources
    C->>S: materialize auxiliary resources
    S-->>C: MaterializedAuxiliaryResourceSet
    C->>A: buildExecution(materialized resources)
...
Codexはprovider-state directoryを読まない。U-01-owned/U-02-consumed closed unionでは`hook-only`を使い、bindingや`capture-bound` self-edgeを持たない。collaboration itemはprocess JSONL内の必須projectionであり、第三のI/O channelではない。pure `prepareResources`がevidence root、owner marker、scratch HOME等を`AuxiliaryResourcePlan[]`として宣言し、U-02のmaterialized setを受けたpure `buildExecution`が具体的env/capture planを返す。adapter自身はI/Oを行わない。
...
`AdapterResourcePreparation.resources`はattempt-owned directory 0700、owner marker 0600、scratch HOME/config fileを宣言する。U-02はresourceをexclusive materializeし、adapterは同じresource preparation digestを持つ`AdapterExecutionPlan`へ5 correlation key付き`LaunchSpec.env`とhook-only capture identityを純粋構築する。U-02はresource/tool-env/sandbox policy digestをcheckpointへ保存してからproviderをarmする。provider group terminal後にhook child processが残っていないことを待ち、directoryをsealしてからnormalizerへ渡し、owned resourceだけをcleanupする。bindingを作る、capture未開始、join timeout、owner mismatch、symlink、unknown file、toolからのroot write可能性はsuccessを禁止する。
...
- 旧capture variantの誤りは解消された。CodexはU-02の`hook-only`を使い、provider-state bindingと`capture-bound`を持たない。
- 旧auxiliary-resource findingは解消された。evidence root、owner marker、scratch HOME/configは`AdapterExecutionPlan.resources`で宣言し、U-02がmaterialize、checkpoint、owned cleanupを行う。
- model解決、hook trust、5 correlation key、model-tool env隔離、collaboration terminal itemとのAND相関は現行成果物間で一貫している。
