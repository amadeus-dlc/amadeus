...
E-HCRRA1=A(追加のみなら非該当)だったが、**E-HCRRA3=B(tie 語彙の置換 = 受理域縮小)採用により e1 留保の反転条件が発動し「該当」へ反転 → 正準リスト(4)のユーザーエスカレーションを実施、B で承認済み(03:47Z 台)**。以後の要件はこの承認済み契約変更を前提とする。再判定条件(e3/e4 の Q1 留保): 本要件の範囲(tie 限定の置換)を超える受理値・出力・既存テストピンの変更が生じたら都度エスカレーションへ。
...
typecheck / lint / --ci green+deslop+lcov(配線行個別確認)。t238/t241 非接触(W-1 — e4 バッチ面)、e4 並行合意の相互通知を維持。
...
- NFR-3: 既存 store 実データ(旧スキーマ 62本 — RE 実測)の load/verify を壊さない(resolutions 空配列・established のみの現況に対し無影響であることの sweep 確認)。
