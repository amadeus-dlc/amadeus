...
- NFR-1: 新受理形の検証は既存 fail-closed 様式(invalid-transition + valid ヒント)に整合。既存ピン(t236:310 系)・既存受理値・既存出力の不変は design/実装で**機械確認**する(e3 Q1 留保の verbatim 完全形 — CI green = t236 系ピンの機械的継続確認+FR-1 の語彙不変テストが担う)。
