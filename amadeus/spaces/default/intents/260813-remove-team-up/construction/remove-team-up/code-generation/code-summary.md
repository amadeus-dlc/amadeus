...
- 削除: `packages/framework/core/tools/team-up.sh`、`team-up-codex-safety-wait.ts`
- 削除: 名前付き `tests/**/*team-up*`、`t266-team-launcher-prerequisites`、`t267-clean-env-team-mode`、safety-wait fixture
- 変更: `amadeus-utility.ts` doctor 修復、`t226`、`team-msg.sh` コメント、Team Mode / messaging / Codex / glossary 対訳、`AGENTS.md` タイムアウト注記、test-time-factor allowlist
...
- FR-3 は縮退ではなく削除。
- FR-7: `team-msg.sh` は残置。role マップコメントからランチャ名を外した。
