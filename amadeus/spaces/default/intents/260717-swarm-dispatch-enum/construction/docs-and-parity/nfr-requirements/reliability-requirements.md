...
- RNR-D2(NFR-2 / C-15): effort 証拠限界の開示を docs に明記 — 「実測済み」と読める表現の禁止(BR-D3。禁止フレーズ語彙は U2 NR(RNR-W2)の候補「実適用を確認」「必ず ultra で実行」「保証」と同一集合を nfr-design で確定。受け入れ = 開示併記の grep 実在+禁止フレーズ 0)
