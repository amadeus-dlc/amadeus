...
- SNR-1(NFR-5 / C-24 承継): resolve の出力・エラーに env の他変数・token・credential を含めない。出力は `AMADEUS_USE_SWARM` の raw 値と許可値列挙のみ(`business-rules.md` BR-7 の様式。受け入れ = 出力形の fixture 検査)
