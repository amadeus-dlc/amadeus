...
- PD-W1(PNR-W1): wave 分割は SKILL 手順の条件分岐(「slot 不足なら残 unit を次 wave へ」)として記述 — 実装機構を持たない prose 契約。正しさの並行度非依存は referee check のステートレス per-unit 性(U1 RD 系で実測確認済み)に帰着
