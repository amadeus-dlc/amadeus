...
- RNR-W2(NFR-2 / C-15): `codex-ultra` の ultra 指定は「API 受理+child 完了」を証拠限界として SKILL に併記 — 実適用を確約する文言を書かない(受け入れ = 該当併記の grep 実在+禁止フレーズ 0 — 候補語彙: 「実適用を確認」「必ず ultra で実行」「保証」。確定語彙集合は nfr-design で固定(canonical = U1 driver-contract-core ND の RD-4。参照経路 = unit-of-work-dependency.md の Cross-unit 決定欄 CU-1))
