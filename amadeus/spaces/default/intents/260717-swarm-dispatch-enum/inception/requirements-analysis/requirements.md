...
### FR-5: Codex 通常経路の native 化(C-07)— hard stop 解除済み(C-13/C-14 成立)

Codex の通常 floor は同一セッション内の native subagent 並列 fan-out とし、headless `codex exec` per-unit worker 経路を撤去する(fallback としても残さない — scope Won't)。spawn・結果回収・失敗 Unit の retry を含む。C-13(prepared unit worktree への隔離書き込み)/ C-14(writable-root 境界)は live probe で成立(evidence = 本ディレクトリ `c13-probe-evidence.md`、child commit 4bf5487c2、隔離比較 5 項目すべて前後一致)。受け入れ = live spawn/collection/worktree evidence(取得済み)+source/dist scan で `codex exec` floor 記述ゼロ。
...
### FR-9: dispatch 指示を持たないハーネス(opencode / cursor)の扱い(Q1 裁定 = C、E-SDE-RA 23:27:58Z)

opencode / cursor は invoke-swarm dispatch 指示(SKILL.md)を持たない(RE 実測)ため decision table の consumer に含めない。S-09 の既存生成経路でツールコピー・audit 語彙のみ同期する。加えて docs へ「opencode / cursor は dispatch 指示を持たないため `AMADEUS_USE_SWARM` の影響を受けない」旨の言及を1行追加する — 留保転記(e4 GoA 2): この言及は enum 変更で更新される既存 `docs/harness-engineering/08-construction-and-swarm.md` 節への1行追記に限定し、新規ページ・新規節は作らない(無音 no-op ギャップの封鎖が目的 — e3 根拠)。受け入れ = 当該 docs 行の実在+opencode / cursor への dispatch 指示ファイル追加ゼロ。
...
- feasibility 実測済み: Codex native subagent の並列 spawn・結果回収・`ultra` 指定の API 受理(cid:feasibility:c1-2)。C-13/C-14 は live probe で成立(2026-07-18、`c13-probe-evidence.md`)
- **開示(C-14 条件)**: probe 実測時の sandbox は permission profile `disabled` / filesystem `unrestricted` — 制限プロファイル下の挙動は未実測。本要件は現行のユーザー実行構成での成立を前提とし、restricted profile での成立を主張しない
- **開示(C-15)**: `reasoning_effort=ultra` は受理+child 完了までが証拠限界。実適用 telemetry は probe でも未観測

## Out of scope

`scope-document.md` の Won't Have を全承継: effort telemetry 新設 / 汎用 subagent observability / driver registry・adapter hierarchy / 旧値 `1` 互換シム / headless `codex exec` fallback 残置 / Herdr 統合 / Kiro 系新 driver / referee 意味論変更 / upstream v2.3.0 referee 設計変更 / AWS 変更 / 固定 LOC・費用・納期 budget。opencode / cursor の consumer 同期(dispatch 指示ファイルの新設)も対象外(Q1=C 裁定)。ただし将来 opencode / cursor に invoke-swarm dispatch 指示(SKILL)が追加される場合は三値契約へ追随する — 留保転記(e1 後着票 GoA 2、E-SDE-RA)

## Open questions

なし(Q1 は E-SDE-RA 裁定 C で確定、C-13/C-14 は probe 成立で閉包。residual: sandbox restricted profile 下の挙動は未実測 — 前提の開示条項として保持し、design 以降で追加検証が必要になった場合は選挙へ)
