...
- 対象: C1(DriverName 三値置換・`resolveDriver` 純関数・`resolve` サブコマンド)+ C2(emitSwarmDegraded 追随、Fallback :291 維持)+ C7 のうち契約テスト(matrix 全セル in-process・FR-2 negative 副作用ゼロ・t28 enum 同期・t134/t135 語彙追随)
- 充足 FR: FR-1, FR-2, FR-4, FR-6, FR-7(回帰維持), FR-3 の audit 面
- 受け入れ: decision table 全セル green / rejected 経路の副作用ゼロ実測 / repo 全域 grep で `ultracode` 残存 0 / t134・t135・t28 green / local lcov で diff 追加行未カバー 0
- 概算行数レンジ: **215-405**(C1 60-110 + C2 5-15 + 契約テスト 150-280)
...
- 概算行数レンジ: **90-190**(SKILL/onboarding prose 40-90 + parity・journey テスト追随 50-100)
...
手書き実装+テスト+文書の合算レンジ: **345-675**(U1 215-405 + U2 90-190 + U3 40-80。生成物 dist/self-install を含まない)。固定 LOC 上限は合否基準にしない(C-10)— 凝集性と伝播面は AD の変更セット C1〜C8 に閉じる。
