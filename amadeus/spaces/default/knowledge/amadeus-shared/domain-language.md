...
- **正本は1箇所**: フレームワーク用語の定義正本は `docs/guide/glossary.md`(英)/ `docs/guide/glossary.ja.md`(日)。日本語文書の正表記・許容表記・避ける表記も `glossary.ja.md` の「表記規則」に従う。本ファイルはそれらを再定義せず、チーム固有の用語と表記の決定だけを持つ。既存正本にある用語はポインタで参照する。
