...
## 鮮度追補(2026-07-15T22:38:11Z、スキャン後の main 前進対応)

スキャン完了後に origin/main が 2 コミット前進(`6495e03a1..d6b489772`): `a7375b24c`(#1019 record-sync — `amadeus/` 配下のみ)+ `d6b489772`(#1028 metrics snapshot — `metrics/*.json` のみ)。実測:
- `git diff --name-only 6495e03a1 origin/main -- ':!amadeus/'` = `metrics/2026-07-15T22-21-36-829Z-6495e03a12d9.json` の1件のみ → **フォーカス面(tools/tests/stages)の diff 0件、observed `6495e03a1` の全結論は landed main `d6b489772` でも有効**。
- base 再評価(rescan-base-ancestry): #1019 で着地した `re-scans/260715-opencode-cursor-harness.md` の observed `6a23b0ec` は `git merge-base --is-ancestor 6a23b0ec origin/main` = **exit 1(非祖先、squash 別 SHA)** につき候補除外 — base `cf3dc88` の距離最小選定は landed main 基準でも不変。
- codekb 整合: #1019 が code-structure.md 冒頭へ「harness port 開放性の観測面(最新)」を追加していたため、merge union で本 intent 節を最新のまま先頭に維持し、harness-port 節を履歴へ降格(c3-relabel、shared-ledger-insert-collision の後着地側手順)。timestamp も同処理。
