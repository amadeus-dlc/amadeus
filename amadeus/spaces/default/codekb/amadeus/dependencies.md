...
`git diff 9458bbda8 b938898f3 -- package.json` の差分は **pi ブロックの追加のみ**であり、`dependencies` / `devDependencies` に変更はない。ビルドランタイムは bun のまま不変。
...
これは外部パッケージ依存ではなく、pi ハーネスへの成果物宣言（extension / skill の配置先）である。

### 内部エッジの追加

| from | to | 根拠 |
| --- | --- | --- |
| `amadeus-orchestrate.ts` | `amadeus-intent-autonomy-production.ts` | `:2183` `productionStageAutonomy({…})` の呼出 |
| `amadeus-orchestrate.ts` | `amadeus-approval-authorization.ts` | `:4445` `handleAuthorizedApprovalReport(pd, slug, authority: CarrierApprovalAuthority)`、dispatch `:4728` |
| `amadeus-orchestrate.ts` | `amadeus-directive.ts` | `:2160-2166` / `:2181-2196` が `phase_boundary` / autonomy フィールドを設定（宣言は `amadeus-directive.ts:97-100`, `:143`, `:144-149`） |
| `amadeus-state.ts` | `amadeus-goal-reconciliation.ts` | workflow completion が goal receipt を前提とする（#2171。`t-phase-check-gate-seam.test.ts` の `complete-workflow` が `seedGoalReceiptForFinalStage` へ再シード） |
| `amadeus-jump.ts` | `amadeus-state.ts` | `:8` で `verifyPhaseCheckArtifact` を import、`:545` で呼出（既存エッジだが本 intent の患部） |
| `harness/pi/**` | `core/tools/**` | pi の driver / extension が core CLI を呼ぶ（他ハーネスと同型） |
| `amadeus-intent-autonomy-runtime.ts` | `amadeus-intent-autonomy.ts` | 三つ組の層関係（`loop-monitor` / `quality-repair` も同型） |
| `amadeus-intent-autonomy-replay.ts` | `amadeus-intent-autonomy.ts` | 同上 |

### 内部エッジの削除

**`amadeus-grant-authorization.ts` は区間内で削除された。** 同ファイルを指す本文書中の全エッジは observed では存在しない。後継 `amadeus-approval-authorization.ts` への付け替えが必要である。

### 依存構造上の所見

- 三つ組（`X.ts` / `X-runtime.ts` / `X-replay.ts`）が `intent-autonomy` / `loop-monitor` / `quality-repair` の3系統に並存し、いずれも「domain ← runtime」「domain ← replay」の同型エッジを持つが、**3系統をまたぐ共有抽象への依存はない**。合計約7500行（`goal` 系を含めると4系統）。同型の層構造が抽象なしに複製されている状態であり、変更時は3〜4箇所を同時に追う必要がある。
- `amadeus-orchestrate.ts` は `phase_boundary` の算出（`:2160-2166`）と autonomy の射影（`:2181-2196`）の両方を持つが、**両者の相互作用を検証する依存先を持たない**。ガード `amadeus-state.ts:3472` は autonomy を参照せず、autonomy 側も phase-check artifact の存在を参照しない。この2経路は同一 directive を共有しながら互いに独立しており、`full` × phase boundary で初めて交差する。
