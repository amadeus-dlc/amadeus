...
- **区間の構造変化（`47574fbab` → `63e69d922`）**: 配布境界の患部 9 パスのうち区間内にコミットがあるのは `scripts/package.ts` の 1 件のみ（`#2031` `8448fdc6e`）で、内容は `writeHarnessData` の `harness.json` スキーマへの `name` フィールド追加（`scripts/package.ts:210-215`、`name: m.name` が新規行）と、それに伴う全 7 面 dist の再生成。**配布契約そのものは不変**。`scripts/promote-self.ts` / `.github/workflows/release.yml` / `.github/workflows/ci.yml` / `scripts/detect-ci-changes.sh` / `.gitignore` / `AGENTS.md` / `packages/setup` / `.claude/settings.json` はいずれも区間内 0 コミット（`git log --oneline 47574fbab..63e69d922 -- <path>` 実測）。区間の大半は `#2031`（213 ファイル）/ `#2049`（228 ファイル）の dist 再生成と metrics スナップショット群で、非 dot 面の変化は `.coderabbit.yaml` と `.gitattributes`（`#2057`: `.agents/**` を linguist-generated 化）に留まる。よって Issue #2043 が記述した配布構造は区間を通じて残存しており、乖離は区間内の新規導入ではない。

- **配布の三層構造**: 現行の配布は「正本 → dist → 消費面」の三層で追跡される。第 1 層が `packages/framework/core/` と `packages/framework/harness/<name>/`（`manifest.ts` を持つ 7 面 = claude / codex / cursor / kimi / kiro / kiro-ide / opencode、`discoverHarnessNames`（`scripts/package.ts:92-97`）が `manifest.ts` の実在で機械発見する — ハーネス名のハードコード列は存在しない）。第 2 層が `dist/<harness>/`（`dist/` 直下は 8 ディレクトリ = 7 ハーネス + `dist/plugins/`）で、**Git にコミットされる生成物**。第 3 層が消費面で、(a) リポジトリ自身の dogfood 面（`.claude` / `.codex` / `.agents` / `.cursor` / `.opencode` / `.kimi-code`）と (b) 利用者環境（installer が展開する `dist/<harness>/` サブツリー）に分かれる。source-only 化はこの第 2 層をリポジトリから外し、Release Asset として第 3 層へ供給し直す変更であり、三層のうち中間層の**所在**だけを動かして層構造自体は保つか、層を 2 つに畳むかが設計上の分岐点になる。

- **第 2 層 → dogfood 面の写像（promote-self）**: `scripts/promote-self.ts:53-60` の `managedDirs` が 6 エントリの写像を宣言する（`dist/claude/.claude` → `.claude`、`dist/codex/.codex` → `.codex`、`dist/codex/.agents` → `.agents`、`dist/cursor/.cursor` → `.cursor`、`dist/opencode/.opencode` → `.opencode`、`dist/kimi/.kimi-code` → `.kimi-code`）。**src 側がすべて `dist/` である**ことが構造上の要点で、dist が消えれば写像の入力が消える。ただし `:355-359` の再帰呼び出しが `scripts/package.ts` を `apply` / `--check` モードで起動しており、promote は build を内包している — dist をディスク上に持たなくても「その場でビルドして promote する」経路への移行余地がここにある（緩和材料）。dist は 8 ディレクトリあるが `managedDirs` は 6 面のみで、`kiro` / `kiro-ide` は promote 対象外（dogfood 面を持たない）。写像の外側では `PROJECT_INSTRUCTIONS`（`:65-74`）/ `CODEX_AGENTS_MARKER`（`:76`）/ `AMADEUS_IMPORT`（`:77` = `@.agents/rules/amadeus.md`）が `composeRootAgents`（`:83-99`）でルート `AGENTS.md` / `CLAUDE.md` を合成し、`preserved`（`:101-114`、10 エントリ）が手編集正本を上書きから守る。

- **利用者環境への配布経路（installer）**: `packages/setup` は現在 **codeload からのソース tarball 取得 → 展開 → その中の `dist/` を読む**という経路を取る。`packages/setup/src/internal/resolved-version-factory.ts:4-5` が ADR-003 として `CODELOAD_BASE = "https://codeload.github.com/amadeus-dlc/amadeus/tar.gz/refs/tags"` を固定し、`:14` で `${CODELOAD_BASE}/${tag}` の URL を組む。展開後は `packages/setup/src/internal/payload-factory.ts:12` の `resolveWrapperDir` が位置基準でラッパーディレクトリを解決し（tarball の先頭ディレクトリ名がタグと一致しないため、名前ではなく位置で解決する BR-F10）、`:38` で `const distDir = join(wrapper.value, "dist")`、`:42` の `readdirSync(distDir)` が失敗すると `:44` で `missing dist/ directory in extracted archive` を返す。ハーネス選択は `:53-58`（不在なら `harness "<name>" is not present in this distribution`）。ネットワーク境界は `packages/setup/src/ports/http.ts:5` の `ALLOWED_HOSTS = new Set(["api.github.com", "codeload.github.com"])` に閉じており（SEC-F02）、`:52` の初回ホスト検査と `:79` の redirect 先ホスト検査（`MAX_REDIRECTS = 5`、`:7`）が二重にゲートする。**Release Asset 配布へ移す場合、ダウンロード先ホストが `objects.githubusercontent.com` 等へ変わるため、この 3 ファイルは同時変更が必須**（アーカイブ URL の生成・展開後のレイアウト前提・許可ホスト集合が互いに独立に固定されている）。

- **Release Asset の不在（release.yml）**: `.github/workflows/release.yml`（206 行）の `github-release` ジョブ（`:133-158`）は **checkout / bun セットアップ / build ステップを一切持たない** 3 ステップ構成で、`softprops/action-gh-release@…v2`（`:152-154`）へ渡す入力は `tag_name`（`:156`）/ `generate_release_notes`（`:157`）/ `token`（`:158`）の 3 つのみ。**`files:` 入力が無い = Release Asset がゼロ**。対照として同ファイルの `publish` ジョブ（`:164-`）は `Checkout released commit`（`:169-172`）+ bun 1.3.13（`:174-177`）+ `bun run build`（`:188-190`、`packages/setup` の `dist/cli.js` 生成）を持つ。すなわちリリースワークフロー内にビルド能力を持つジョブは既に存在するが、それは npm publish 用であって配布アーカイブ生成用ではない。source-only 化は `github-release` ジョブへ「checkout + bun + dist ビルド + `files:` でのアップロード」を新設する構造変更を要求し、`prepare` → `github-release` → `publish` のジョブ依存も再考対象になる。

- **drift guard 3 種と source-only 化での意味変化**: `.github/workflows/ci.yml` の `drift-check` ジョブ（`:225-255`、発火条件 `:228` = `full == 'true' || drift == 'true'`）が 3 種のガードを直列に走らせる — (1) `bun run dist:check`（`:243-244`、= `scripts/package.ts --check`）、(2) `bun run promote:self:check`（`:246-247`、= `scripts/promote-self.ts --check`）、(3) `bun .claude/tools/amadeus-graph.ts compile --check`（`:254-255`）。ジョブは 6 ステップのみで **build ステップを持たない**（checkout + bun + install + 上記 3 つ）。(1) の意味論は `scripts/package.ts:31-34` のコメントが明示する — 「各ツリーを一時ディレクトリへビルドし、コミット済み `dist/` とバイト単位で差分を取り、ドリフトがあれば offending path を出して exit 1。dist はコミットされたまま。このガードは誰かが dist を手編集したか再生成を忘れたときに CI を落とす」。実装は `checkHarness`（`scripts/package.ts:698-712`、committed 参照 `:700` / temp `:701` / `buildTree` `:705` / MISSING・DIFFERS 判定 `:707-712`、ハーネス外バイト差分 `:728-742`）。**dist が非コミットになれば (1) の比較対象（committed 側）が消え、ガードは「生成物が正本と一致するか」ではなく「生成が成功するか」へ意味が変わる**。(2) も同型で、`check()`（`scripts/promote-self.ts:471-494`）は `MISSING`（`:477`）/ `DIFFERS`（`:481-485`、`SCOPE_GRID_RE` 分岐 `:481-482` と `STAGE_GRAPH_RE` 分岐 `:483-484`、既定のバイト比較 `:485`）/ `ORPHAN`（`:487`）を返すが、期待値マップ `expected` は dist ではなくビルド結果から得られるため、dist 非コミット化の影響は (1) より小さい。(3) は dist に依存しない。

- **drift 発火条件のパス依存（detect-ci-changes.sh）**: `scripts/detect-ci-changes.sh`（37 行）が変更パスから `full` / `drift` の 2 フラグを立てる。`drift` フィルタ（`:18-24`）は `:20` に `packages/framework/*|dist/*|.agents/*|.claude/*|.codex/*|.kiro/*|.cursor/*|.opencode/*|.kimi-code/*`、`:21` に `AGENTS.md|CLAUDE.md` を列挙する。**`dist/*` がフィルタの一員である**ため、dist を非コミット化すると drift 発火の主要トリガの 1 つが構造的に沈黙する（`packages/framework/*` が残るので完全な沈黙ではないが、消費面の手編集を dist 経由で捉える経路が消える）。あわせて既存の不整合が現存する — `.kiro-ide` パターンが無く、`.kiro/*` はルート面に実体が無い（dogfood 面は 6 面で kiro 系を含まない）。`full` フィルタ（`:9-16`）は `:13` に `scripts/*|tests/*|packages/framework/*|packages/setup/*|book-pack/*` を持ち、配布系スクリプトの変更は必ず full を立てる。

- **`.gitignore` の COMMITTED 契約（反転の起点）**: `.gitignore:16-19` が `# dist/ is generated, COMMITTED, and drift-guarded (see CONTRIBUTING.md).` に続けて `!/dist/` の否定パターンを置き、「ユーザーのグローバル `dist/` ignore が配布ツリーをコミットから無言で落とすこと」を防いでいる。source-only 化はこの否定を除去して `dist/` を通常の無視対象へ反転させる変更であり、**配布構造全体の起点**にあたる。周辺には既に例外群がある — `:22-24`（`dist/{claude,kiro,codex}/amadeus-docs/`）、`:27`（`dist/claude/todo-app/`）、`:29-32`（`packages/setup/dist/` は「フレームワークの `/dist/` とは別物、drift-guard 対象外、`tests/lib/setup-lazy-build.ts` が遅延ビルド」という明示注記付き）。この最後の注記は source-only 化後も**同名で意味の異なる 2 つの `dist/` が残る**ことを意味し、反転時に注記の再整理が要る。
