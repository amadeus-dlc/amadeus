...
## ambient error sink の修正を縛る既存契約と実測ラチェット（260814-ambient-error-sink、現在、observed `6e94189de`）

対象: [Issue #3004](https://github.com/amadeus-dlc/amadeus/issues/3004)。測定 ref = observed `6e94189dec9e8e2bd0aaeb53bcff7cf9cba27440`。本節は**修正方式を選ぶ前に満たさねばならない既存契約の正確な形**を記録する（方式の選定自体は所掌外）。

### Q-1. `t214` が肯定的に固定している契約（`tests/unit/t214-engine-error-logged-seam.test.ts`）

**インポート元が `dist`**: `:33 } from "../../dist/claude/.claude/tools/amadeus-orchestrate.ts";`。冒頭 `:17-19` が「stale dist reds here rather than passing against un-regenerated bytes」と理由を宣言している。**core だけを直して t214 を回すと旧バイトに対して緑になる** — `bun run build` が検証手順の一部である。

ヘッダが宣言する契約（`:21-24`、逐語、Architect 実読で一致確認）:

> `recordEngineError contract: best-effort ONE ERROR_LOGGED row when workflow state exists; no-op (no throw, no write) when it does not. Project-dir resolution mirrors main(): --project-dir from process.argv, else CLAUDE_PROJECT_DIR.`

| test | 行 | 駆動 | 拘束する段 |
|---|---|---|---|
| T1 | `:126-137` | `CLAUDE_PROJECT_DIR = proj`、`:131 recordEngineError("seam: something went wrong");`（**projectDir 省略**） | **env 段（ラダー rung 2）が生きること** |
| T2 | `:139-151` | `delete process.env.CLAUDE_PROJECT_DIR;` + `:145 process.argv = [..., "--project-dir", proj];`、`:147 recordEngineError("seam: argv project-dir");` | **argv 段が生きること** |
| T3 | `:153-160` | `removeWorkspaceRecord(proj)`、`:158 expect(() => recordEngineError("seam: no state")).not.toThrow();` | **state 不在時の no-op**（`:958` の early return）、throw しないこと |
| T4 | `:164-178` | `:171 handleNext(["--scope","zzznotascope"], proj)` | `emit` の error 分岐（explicit 形）、`message` が `"Unknown scope"` を含むこと |
| T5 | `:182-197` | argv に `--project-dir proj` を張って `runEngineMain()` | **`main` 経由 + top-level catch の argv 段** |

**品質上の含意**: T1 / T2 は「projectDir 省略形の ambient 解決」を**バグではなく契約として肯定的に固定している**。これは本 Issue が閉じようとしている挙動と正面から重なる。`recordEngineError` 自体を fail-closed 化する案は T1 / T2 を直接赤にするため、契約変更 = 仕様変更としてユーザー裁定が要る。

### Q-2. `t258` が固定している契約（`tests/integration/t258-engine-error-ambient-shard-pollution.test.ts`）

インポート元は **core**（`:36`）。beforeEach で `resetOtelPerProject()`（`:50`）、**argv 中和**（`:56 process.argv = ["bun", "amadeus-orchestrate.ts", "report"];`、理由コメント `:53-55`）、`_resetCloneIdForTests()`。

- T1（`:98-110`）: ambient / target の 2 fixture、`CLAUDE_PROJECT_DIR = ambient`、`:91 handleReport(["--result","__not_a_verdict__"], target)` → `auditShardsOf(target)` 1 件 **かつ** `auditShardsOf(ambient)` **空**
- T2（`:112-122`）: `target === ambient` の単一プロジェクト路の非退行
- T3 / T4（`:126-163`）: `auditShardName` の projectDir キー化（本件と直交）

**ギャップ**: t258 は **explicit 形のみ**を固定し、`undefined` 形は 0 件。#1389 の回帰網は「渡された projectDir が守られること」は保証するが、「渡さなかった場合に何が起きるか」は無防備のまま残されている。

### Q-3. 修正が t214 / t258 を緑のまま満たすための制約 7 条

1. `recordEngineError(message)`（projectDir 省略）は **throw せず env 段・argv 段で解決を続ける**こと（t214 T1 / T2、および `runEngineMain:6851` の本番 catch が依存）
2. **state 不在時は no-op（throw なし）**（t214 T3）。新設ガードを例外送出の形にしない
3. **`main` 経由（CLI）の ambient 解決は現状維持**（t214 T5）。したがって **`main` と in-process を区別するシームが必須**
4. **explicit 形の挙動はバイト単位で不変**（t258 T1 / T2、t214 T4）。着地先・行数・shard 名を変えない
5. **error directive の kind と message を変えない**（t258 T1 は shard 数、t214 T4 は `"Unknown scope"` を要求）。新設の拒否 directive は既存 message を上書きしない位置に置く
6. **`handleNext` の循環的複雑度を増やせない（ハード制約）** — 下記 Q-4
7. **`bun run build` で dist を再生成してから t214 を回す**（Q-1）

### Q-4. 複雑度ラチェットの実測（`handleNext` は上限ちょうど）

`tests/.complexity-baseline.json` は `threshold: 15`、entry は `{"path":"packages/framework/core/tools/amadeus-orchestrate.ts","name":"handleNext","ordinal":0,"ccn":22}`（Architect が JSON を直読して一致確認）。`evaluateComplexity`（`tests/complexity-gate.ts:245-270`）は `rec.ccn > recorded` を regression = fail とする **shrink-only ratchet** であるから、**`handleNext` の本体に `if` を 1 つ足すと確実に赤**になる。

実測 CCN（`python3 -m lizard packages/framework/core/tools/amadeus-orchestrate.ts -l typescript --csv`）: `handleNext=22` / `main=13` / `emit=10` / `recordEngineError=8` / `applyPendingAdvisoryGuard=7`。

既存コードはこの制約を明示的に扱っている — `:3022-3026` の逐語コメント「Composed here rather than as two statements in handleNext so the handler's decision count — already at the complexity ratchet's recorded ceiling — is unchanged」。**新ガードを `refuseBlockedNextEnvironment` の中に畳み込むのが既存様式**である。

補足（実測）: `handleReport` / `handleFailureRuling` / `handlePark` は lizard の TypeScript パーサが関数として検出しておらず（csv に該当行なし）、複雑度ゲートの measured 集合にも baseline にも存在しない。これらへの `if` 追加はゲート上は無制約だが、新規ヘルパは CCN ≤ 15 を守ること。

### Q-5. 既に実 record へ着地している行（述語併記、Architect 独立再実測）

**述語（再実行可能）**: 対象集合 `git ls-files 'amadeus/spaces/default/intents/*/audit/*.jsonl'`（**250 ファイル**）。各行を JSON parse し、`attributes.Event === "ERROR_LOGGED"` **かつ** `attributes.Tool === "amadeus-orchestrate"` **かつ** `attributes.Command === ""` を満たす行を数える。

| 量 | 実測値（observed 断面） |
|---|---|
| 対象ファイル数 | 250 |
| 全行数 | 148,805 |
| `attributes.Event === "ERROR_LOGGED"` の行数 | **1,148** |
| 上記 3 条件を満たす行数 | **45** |

`attributes.Error` 別内訳（45 の全数）: `Unknown --result "failed". report commits forward tr…` **32** / `report --result failed requires --failure <detail> —…` **9** / `Unknown subcommand: (none). Valid: next, report, par…` **2** / `seam: something went wrong` **1** / `seam: no state` **1**。

**帰属が一意に定まる 2 行**: `seam: something went wrong` と `seam: no state` は `tests/unit/t214-engine-error-logged-seam.test.ts:131` と `:158` にのみ存在する文字列リテラルであり、**どちらも projectDir を省略した `recordEngineError` 呼出**である。両行は実 record（`amadeus/spaces/default/intents/260807-projectdir-worktree-fix/audit/j5ik2o-mac-studio-lan-d13e4f0ca2c0.jsonl`、いずれも `2026-08-07T11:20:09Z`）に着地している。他の 43 行は projectDir provenance を持たないため個々の undefined 形の証明にはならないが、この 2 行は**メッセージ文字列が呼出行を一意に指すため undefined 形の直接証拠**になる。

**数え方の注意**: Developer scan は「既着地 43 行（41 + seam 2）」と記す。この 43 は上記述語の 45 から `Unknown subcommand: (none)` の 2 行を除いた部分集合であり、除外根拠は scan 本文に明示されていない。**受け入れ基準に使う場合は述語ヒット数 45 か、除外条件を明示した部分集合かを選び、選んだ側の述語をそのまま併記すること。** また Developer scan の集計欄「`ERROR_LOGGED` 2,242」は Architect の再実測（JSON parse 基準）では **1,148**、部分文字列一致（`grep -o "ERROR_LOGGED" | wc -l`）では **2,278** であり、いずれとも一致しない。**述語ヒット 45 と内訳は完全一致しているため本 Issue の主張自体は無傷**だが、集計欄の総数は方法依存であり転記に使わないこと。

### Q-6. 未実証の仮説（設計裁定への入力、**本スキャンでは実証していない**）

t214-seam は `:129` / `:156` で `process.env.CLAUDE_PROJECT_DIR = proj;` を明示的に張っており、rung 2 が rung 3 より上位である以上、`resolveProjectDir` 単体では実 record に着地しないはずである。さらに `seam: no state` のケースは `removeWorkspaceRecord(proj)` 済みで `:958` の early return が効くはずであり、**fixture 側の解決結果だけでは Q-5 の 2 行を説明できない**。

仮説: 監査行の宛先が `resolveProjectDir` ではなく **OTel の per-process ワークスペースピン**で決まっている可能性。`emitErrorAuditRow`（`amadeus-lib.ts:8059-8069`）は `otel/audit-emit.ts` の `emitAuditEvent` へ委譲し、そこは `ensureOtelBootstrap(projectDir)` の後 `appendAuditEntryViaEvents(...)` / `emitEvent(...)` を通る。`otel/bootstrap.ts:88-90` は登録済みプロバイダの workspace と要求 workspace の不一致を `assertSameProject`（`:45-51`）で throw するが、`recordEngineError` は `catch {}` で全例外を握り潰す（`:964-966`、逐語 `// Swallowed by contract — recording failure must not mask the original error.`）ため、当時のバイトによっては別 workspace へ着地しえた可能性がある。

**品質上の含意**: 修正の受け入れ検証を「fixture の shard 件数」だけで採ると、この経路が残っていても緑になりうる。**回帰テストは ambient 側 fixture の shard が空であること**（t258 T1 の形）を必ず含めること。この機序自体は本 Issue の完了条件の外であり、別途起票の候補である。
