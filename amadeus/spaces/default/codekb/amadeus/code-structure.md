...
## swarm/autonomy 患部の配置と区間のツール面デルタ（現在: 260728-gated-swarm-serializatio、observed `ec6f16ad8`）

260728-gated-swarm-serializatio 差分リフレッシュ（2026-07-28、observed `ec6f16ad81074f7ca4f252afa0d5d91ecbd48538`、base `0c4709102`（祖先 exit 0。`afb93a825` は非祖先 exit 1 で棄却）、距離 **36**、scope `amadeus-bugfix`、[Issue #1612](https://github.com/amadeus-dlc/amadeus/issues/1612)）。上流入力: Developer スキャン結果。数値はすべてコマンド出力からの転記（測定 ref: observed `ec6f16ad8`）。

**患部の配置（新規モジュールなし）**: 欠陥は既存 3 ファイル内に閉じる — `packages/framework/core/tools/amadeus-orchestrate.ts`（`readAutonomyMode:1164-1168` / `tryEmitSwarm:2511-2528` の `:2526` / フォールバック `orderedUnits:2636-2640`・`nextUncoveredUnit:2683-2704`・`emitPerUnitRunStage:2712-2789` / approve 側 `:3824-3826`）、`packages/framework/core/tools/amadeus-directive.ts`（`InvokeSwarmDirective:173-185`）、`packages/framework/core/amadeus-common/protocols/stage-protocol.md`（`:123-125`）。**区間 36 コミットはこの 3 面のいずれにも触れていない**（`git log 0c4709102..HEAD -- …/amadeus-orchestrate.ts` 空）。再生成面は `grep -rl tryEmitSwarm`（record/tests 除外）= **17 ファイル**（正本 1 + self-install 5 + dist 7 ハーネス）。詳細は `architecture.md` の同 intent 節。

**区間のツール面デルタ（`packages/framework` 31 ファイル）**: mirror 系ツールが `packages/framework/core/tools/amadeus-mirror-*.ts` = **15 ファイル**（`ls … | wc -l` 実測）へ、election 系が `amadeus-election.ts` / `-model.ts` / `-record.ts` / `-store.ts` / `-transport.ts` = **5 ファイル**、plugin 系が `amadeus-plugin.ts` / `-activation.ts` / `-compose.ts` = **3 ファイル**。ほかに `amadeus-graph.ts` / `amadeus-runner-gen.ts` / `amadeus-utility.ts` / `amadeus-version.ts` とハーネス manifest 7 面が区間で変更されている。区間の面別上位は `tests/integration` **35** / `packages/framework` **31** / dist 各ハーネス **23-25** / `docs/reference` **24** / `tests/unit` **23** / `docs/guide` **22** / self-install `tools` 各 **20-21**（record 除外の総変更ファイル数 **488**）。

**scope 定義面の訂正（本節が上書きする既存記述）**: 本ファイル下部（`## …` 内の scope 節、`tools/data/scope-grid.json` 行の直後）は composed scope の実在ファイルとして `amadeus-amadeus.md` を挙げているが、これは区間の `9f3d4583a`（#1629 legacy `amadeus` スコープ廃止）で**削除済み**であり observed `ec6f16ad8` には存在しない。現時点の実測は `ls .claude/scopes/*.md | wc -l` = **15**（`amadeus-amadeus-bugfix.md` / `amadeus-amadeus-document.md` / `amadeus-amadeus-feature.md` / `amadeus-amadeus-refactor.md` / `amadeus-bugfix.md` / `amadeus-chore.md` / `amadeus-enterprise.md` / `amadeus-feature.md` / `amadeus-infra.md` / `amadeus-installer-distribution.md` / `amadeus-mvp.md` / `amadeus-poc.md` / `amadeus-refactor.md` / `amadeus-security-patch.md` / `amadeus-workshop.md`）、`scope-grid.json` のキーも **15**（`amadeus-bugfix` / `amadeus-document` / `amadeus-feature` / `amadeus-refactor` / `bugfix` / `chore` / `enterprise` / `feature` / `infra` / `installer-distribution` / `mvp` / `poc` / `refactor` / `security-patch` / `workshop`）。キー総数 15 は偶然一致しているが**構成が変わっている**（`amadeus` が抜け `amadeus-document` が入った）点に注意する。あわせて `ec6f16ad8`（#1640）が `amadeus-feature` の EXECUTE 集合を軽量化している。

## plugin CLI・runner-gen・スキル正本のコード配置（260727-plugin-verb-skills、履歴 2026-07-28、差分リフレッシュ、observed `afb93a825`、別 worktree `plugin-dev` で並行進行）
