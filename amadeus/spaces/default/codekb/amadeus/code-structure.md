...
## 監査リーダー面の二分構造とテスト層配置（260807-intent-2328-tests-e2e-au、履歴、2026-08-07、observed `a5621236c`）
...
| 層 | パス | 位置 |
| --- | --- | --- |
| 規約 | `packages/framework/core/amadeus-common/protocols/stage-protocol-governance.md` | `:14-18`（§13 「When to verify」） |
| ガード | `packages/framework/core/tools/amadeus-state.ts` | `:379-396` 定義、`:390-394` 拒否文言、呼出 `:2263` / `:2413` / `:2539` / `:3472` |
| ガード（再利用） | `packages/framework/core/tools/amadeus-jump.ts` | `:8` import、`:545` 呼出 |
| directive 算出 | `packages/framework/core/tools/amadeus-orchestrate.ts` | `:2160-2166`（`phase_boundary`）、`:2181-2196`（autonomy 射影） |
| directive スキーマ | `packages/framework/core/tools/amadeus-directive.ts` | `:143`, `:144-149`, `:97-100`, `:403-411`, `:606-609`, `:633-637` |
| annex（8ハーネス） | `packages/framework/harness/<name>/skills/amadeus/SKILL.md` ほか | 下表 |
| seam テスト | `tests/unit/t-phase-check-gate-seam.test.ts` | 16 pass / 0 fail / 36 expect / 214ms（実測） |

### annex の配置と phase-check 記述の有無

| ハーネス | ファイル | 行数 | approval 条項 | phase-check 記述 |
| --- | --- | --- | --- | --- |
| `pi` | `harness/pi/skills/amadeus/SKILL.md` | 200 | `:98-103` | **あり（唯一）** |
| `claude` | `harness/claude/skills/amadeus/SKILL.md` | 251 | `:98-99` | なし（`:119` にポインタのみ） |
| `codex` | `harness/codex/skills/amadeus/SKILL.md` | 248 | `:96-97` | なし（`:117` にポインタのみ） |
| `kimi` | `harness/kimi/skills/amadeus/SKILL.md` | 260 | `:96-97` | なし（`:117` にポインタのみ） |
| `kiro` | `harness/kiro/skills/amadeus/SKILL.md` | 249 | `:92-93` | なし（`:119` にポインタのみ） |
| `kiro-ide` | `harness/kiro-ide/skills/amadeus/SKILL.md` | 249 | `:92-93` | なし（`:119` にポインタのみ） |
| `cursor` | `harness/cursor/commands/amadeus.md` | 82 | 実質なし | なし |
| `opencode` | `harness/opencode/commands/amadeus.md` | 81 | 実質なし | なし |

是正対象は上表の網掛けでない5行（claude / codex / kimi / kiro / kiro-ide）である。`cursor` / `opencode` は `commands/amadeus.md` が薄く、そもそも承認儀式を持たない。

### core tools の配置デルタ — 103 → 116

`packages/framework/core/tools/` に14ファイル追加、1ファイル削除。

**追加（行数は observed 実測）**

| ファイル | 行 |
| --- | --- |
| `amadeus-approval-authorization.ts` | 80 |
| `amadeus-goal.ts` | 582 |
| `amadeus-goal-reconciliation.ts` | 883 |
| `amadeus-intent-autonomy.ts` | 961 |
| `amadeus-intent-autonomy-production.ts` | 900 |
| `amadeus-intent-autonomy-runtime.ts` | 800 |
| `amadeus-intent-autonomy-replay.ts` | 175 |
| `amadeus-loop-monitor.ts` | 795 |
| `amadeus-loop-monitor-runtime.ts` | 816 |
| `amadeus-loop-monitor-replay.ts` | 553 |
| `amadeus-quality-repair.ts` | 838 |
| `amadeus-quality-repair-runtime.ts` | 951 |
| `amadeus-quality-repair-replay.ts` | 190 |
| `amadeus-pi-doctor.ts` | 392 |

**削除**: `amadeus-grant-authorization.ts`（後継は `amadeus-approval-authorization.ts`）。

### 三つ組の命名規約

区間内で追加された autonomy 系4系統は、いずれも同一の3層命名規約をとる。

```
amadeus-<domain>.ts          ドメインロジック
amadeus-<domain>-runtime.ts  ランタイム結線
amadeus-<domain>-replay.ts   replay
```

`intent-autonomy` はさらに `-production.ts`（本番結線）を持ち、`goal` は `-reconciliation.ts` を持つ。4系統合計は約7500行で、層をまたぐ共有抽象は存在しない。

### 新サブツリー — `packages/framework/harness/pi/`

```
packages/framework/harness/pi/
├── dot-gitignore
├── manifest.ts                          (97)
├── onboarding.fills.ts                  (31)
├── drivers/
│   ├── amadeus-pi-driver.ts             (659)
│   ├── amadeus-pi-driver-contract.ts    (231)
│   ├── amadeus-pi-guardian.ts           (377)
│   └── amadeus-pi-replay-store.ts       (336)
├── extensions/
│   └── amadeus-pi-extension.ts          (1313)
└── skills/amadeus/
    ├── SKILL.md                         (200)
    └── question-rendering.md
```

既存7ハーネスが `hooks/` または `commands/` を持つのに対し、`pi` は `drivers/` + `extensions/` 構成である。`package.json` には対応する pi ブロック（`extensions` / `skills` 宣言）が追加された。

### `scripts/` の追加 — 6本

`scripts/harness-manifest.ts` / `no-silent-drop-evidence.ts` / `no-silent-drop-evidence-adapter.ts` / `pi-conformance-evidence.ts` / `pi-live-rpc.ts` / `pi-package.ts`。

**`scripts/manifest-types.ts` は base 時点で既に存在する**（Developer scan の「追加7本」を実読で訂正 — 実際は6本）。

### `tests/` の配置デルタ

`.test.ts` は **883 → 927**、`.ts` 全体は **991 → 1066**。新設サブツリー `tests/harness/live-e2e/`（`adapter.ts` / `contract.ts` / `journey.ts` / `ledger.ts` / `lifecycle.ts` / `policy.ts` / `projector.ts` / `registry.ts` / `stream.ts` ほか、`testing/` 配下7ファイルを含む）と `tests/conformance/pi-*`、`tests/fixtures/pi-driver/` が加わった。

削除: `t-solo-standing-grant-domain`（unit / integration 両方）、`t-solo-standing-grant-harness`、`t-solo-standing-grant-opencode-mint`、`t257-amadeus-config`、`t343-amadeus-mirror-project-config`。
...
`amadeus-utility.ts` は全体5,205行で、`handleDoctor` はおよそ830–2200行の約1,371行を占める。`handleDoctor` 自体は export 済みで、既存の in-process テストは `process.exit`、stdout、env を monkeypatch して直接実行する。この方式で6ファイル104ケースが成功し、LCOV 437/771行 hit を得ている。

構造上の未解消点は、ハンドラが正式な戻り値を持たず、テストごとに副作用捕捉が重複すること、`worktreeBaseDir → resolveMainCheckout` が session cwd に依存すること、stage graph/harness の検査が env と cache を直接参照することにある。
