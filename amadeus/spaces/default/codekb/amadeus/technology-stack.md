...
`package.json` の `dependencies` / `devDependencies` に区間内の変更はない（`git diff` 実測）。ビルド・実行ランタイムは bun のまま、言語は TypeScript のまま不変。core tools はファイル数のみ **103 → 116** に増えた。

### 唯一の技術断面の変化 — pi ハーネスの宣言形式

`package.json` に pi 専用ブロックが追加された。これは外部依存ではなく、pi ランタイムに対する成果物の配置宣言である。
...
既存7ハーネスは hook スクリプト（`hooks/*.ts`、`*.kiro.hook`、`amadeus-hooks.snippet.toml`）または slash command（`commands/amadeus.md`）で結線されるのに対し、**pi は extension + skill をパッケージメタデータで宣言する形式**をとる。これはハーネス統合方式としては本リポジトリ初であり、`scripts/pi-package.ts` がパッケージングを、`scripts/harness-manifest.ts` が manifest 整合を担う。

pi サブツリーは他ハーネス同様 `dot-gitignore` を同梱する（`packages/framework/harness/pi/dot-gitignore`）。

### 検証系の技術追加

- **PBT（property-based testing）**: `.github/workflows/pbt.yml` が新設された。
- **live E2E**: `tests/harness/live-e2e/` サブツリーが新設され、実ハーネス（claude print / claude SDK / claude TUI / codex）に対する契約テストを持つ。`scripts/pi-live-rpc.ts` が pi の live RPC を担う。
- **適合性証跡**: `tests/conformance/pi-formal-evidence.schema.json` と `scripts/pi-conformance-evidence.ts` により、pi の適合性が JSON schema で固定される。

いずれも新しい外部ライブラリを導入せず、bun の標準テストランナと既存 CI 基盤の上に構築されている。
...
## CG 計画整合ガードの技術断面（260801-cg-plan-guard、履歴、observed `cb809c4de`）

- 判断: 技術スタック変化なし（TypeScript/Bun、既存 engine 内のガード追加のみ）。詳細は `re-scans/260801-cg-plan-guard.md` に委ねる。
