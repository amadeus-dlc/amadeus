...
| installer | `scripts/amadeus-install.ts`（`npm run amadeus:install` / `bun run scripts/amadeus-install.ts --target <workspace>`） （smoke pass 時に固定 marker で shell 未 seed の info 行を表示 = #573）| エンジン（`.agents/amadeus/` 7 dir）、`amadeus*` skill、変換済み `AMADEUS.md`、`.claude/*` symlink、hooks wiring を対象 workspace へ配布する唯一の配布手段。冪等（#451） |
