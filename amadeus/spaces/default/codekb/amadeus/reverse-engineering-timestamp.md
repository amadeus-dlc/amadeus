...
## 実行メタデータ（現在: 260810-numeric-provenance-guard）

- Date: `2026-08-10`
- Base commit: `df1c874cfb397fafe877a72f00a82664a59689ae`（`re-scans/` 中で最新の observed = 直前 intent `260810-plugin-harness-dir-token` の observed。HEAD の祖先であることを実測確認。区間 `git rev-list --count df1c874cfb..HEAD` = **11 commits**（`40056d0ec` まで）+ record 2 commits。`cid:reverse-engineering:rescan-base-ancestry`）
- Observed commit: `40056d0ec`（`origin/main`。コード面は本 worktree HEAD `8402e5c5eceec24232e4d8de4f5adec0f5341b09` と **byte 同一** — `git diff --name-only 40056d0ec HEAD` = 14 ファイルで全て本 intent の record 配下。`cid:reverse-engineering:c2-observed-mainline-commit`。短縮 SHA の手動展開はしない — `cid:requirements-analysis:sha-no-manual-expansion`）
...
- 行番号引用の currency（**実測の記録であり免除の主張ではない**）: `git diff --name-only c909b6130..40056d0ec`（測定区間は `review..observed` に固定、`..HEAD` ではない — `cid:reverse-engineering:E-XBB-RE-S13-c2`）は **sensor 正本に非交差** — `packages/framework/core/sensors/**` および `packages/framework/core/tools/amadeus-sensor*.ts` に変更なし。交差はテスト面（`t514` 等）のみで、そちらは scan が現行断面で再解決済み
- Verification: git 状態変更・GitHub 書込・engine 操作（`amadeus-orchestrate.ts` / `amadeus-state.ts` / `amadeus-log.ts` / `amadeus-bolt.ts`）は**すべてゼロ**。coverage 実行もゼロ（`cid:code-generation:c1-coverage-single-owner`）。scan は read-only、書き込みは codekb 配下のみ
- **⭐ 主要所見 — 新センサー追加の配線点は 2 つだけで、registry も switch も存在しない**: dispatcher は manifest 駆動（`amadeus-sensor.ts:213-229` の `resolveScriptPath` が `command:` から `.ts` basename を抜き `__FILE_DIR` の隣で解決）、ハーネス投影も 8/8 manifest が `{ src: "sensors", dst: "sensors" }` をディレクトリ単位で持つため自動。よって「manifest 1 件 + `amadeus-sensor-<id>.ts` 1 件 + 対象 stage の `sensors:` 1 行」で完結する。**例外は per-sensor 引数アーム**（`:455-500`）だが、追加入力を要求しない設計なら `amadeus-sensor.ts` は無改変
- **advisory はグラフに書かれない**: `amadeus-graph.ts:809-811` が `default_severity !== "advisory"` のときだけ `entry.severity` を書く（`:134-139` 逐語 `ABSENT for the framework default "advisory"`）。**第1段を advisory で出せばグラフ golden は byte 不変**
- **`nfr-budget` が完全な鋳型**: `amadeus-sensor-nfr-budget.ts`（1,185 行）に数値主張検査の既存実装が揃う — 数値 regex の vacuity guard（単位トークン必須で id 数字・見出し番号・日付の誤検出を封じる、`:242-270`）、`idBlocks` 型の近傍窓（`:278-297`）、`verdict()` の pass 機械導出（`:930-935`）、観測レンジ内閾値の根拠コメント様式（`:818` / `:829` が `cid:code-generation:c1-threshold-inside-observed-range` を明示引用）。⚠ ただし**母集団の向きが逆**（あちらは「しきい値の不在」検出）で、単位必須をそのまま持ち込むと単位なし件数語が母集団から落ちる — 母集団定義は下流の設計裁定
- **cutoff は `answer-evidence` の写像**: `intentDateFromPath`（`amadeus-sensor-answer-evidence.ts:62-69`）が outputPath の `intents/<dir>/` から `dir.slice(0,6)` を parse し、`date < CUTOFF` または undatable を `skipped("pre-cutoff")` で fail-open（`:83-85`）。**8,503 件の既存コーパスへ遡及適用しないための必須機構**
- **閾値を置かない選択肢にも先例がある**: `scope-sizing` は閾値を持たず `measureCapabilities`（`:146-159`）で数えて `source` を併記するのみ。manifest description 逐語 `so the depth-versus-size band can be set once the distribution exists`。corpus 分布が未知の段階では「測定のみ・閾値なし」が正当な第1段の形
- **引用実在チェッカーは不在（反証確認済み）**: 述語 4 本（`file:line` / `verifyCitation` 系の grep、sensors・tools の命名 grep、`provenance` 全域 grep）でいずれも該当ゼロ（`cid:requirements-analysis:absence-claim-grep-verify`）。`amadeus-mirror-provenance.ts` は GitHub mirror の所有権、`amadeus-advisory-choice.ts` の provenance は TLA モデル digest 検証で無関係。唯一の近縁 `amadeus-norm-metrics.ts:10-24` は memory 層の cid **引用回数の集計**であって実在検証ではない
- **供給側は既に一貫様式を持つ**: `stage-stats`（`amadeus-stage-stats.ts:964` / `:938-956` / 恒等式行 `:970-971`）と `subagent-stats`（`amadeus-subagent-stats.ts:193-198`）が `# <tool> — measurement ref` ヘッダ + `key: value` 行を出力。第1段の受理語彙はこの様式を対象にできるが、**コーパス内の出現率は UNMEASURED**
- tNNN 予約: 連番系列の使用済み最大 **`t531`**（`t2790-plugin-staging-seed-harness-dir.integration.test.ts` は Issue 番号由来の系列外）、本 intent は **`t532`** 以降を予約。層は integration（実 FS を触るため — `cid:code-generation:fs-tests-integration-first`）、in-process seam は named export の直 import（`cid:requirements-analysis:bun-coverage-spawn-blindspot`）。PR 発行前・マージ直前に固定 base SHA の `tests/` で再確認すること（`cid:code-generation:c1-tnnn-collision-on-regrounding`）
- corpus 規模（実測、測定 ref = HEAD `8402e5c5`）: intent record 内 `*.md` **8,503** / codekb `*.md` **135** / intent ディレクトリ **154** / `code-summary.md` **366** / `requirements.md` **139** / `build-test-results.md` **129** / `verification/*.md` **313**
- Updated artifacts: `component-inventory.md` に「センサー機構の構成要素棚卸し」節を新設（配線 2 点・advisory 契約・再利用 seam 8 件・供給側様式・不在の反証確認）。同節は下段の履歴節「センサー実行面」（`260805-pr-convergence-plugin`、observed `8409c2039`）が記す「出荷センサーは **8 件**」を **13 件へ更新**する — 履歴節は当時の断面として本文を保持（`cid:reverse-engineering:c3-relabel` / `cid:requirements-analysis:historical-section-cite-check-at-observed`）。直前の現在節（`260810-plugin-harness-dir-token`）は本文保持のまま履歴へ降格
- Reviewed-and-unchanged artifacts（**沈黙のスキップではなく、レビュー済みで無変更**）: `architecture.md` / `code-structure.md` / `code-quality-assessment.md` / `technology-stack.md` / `dependencies.md` / `api-documentation.md` / `business-overview.md` の 7 面。理由 — (1) `architecture.md` は既に `### 機構 C — センサー発火の対象決定が \`matches\` glob のみ（#1742）` と `### 5. センサーリスク` でセンサー機構の構造面を扱っており、本 scan が確定したのは**構造の変更ではなく既存構造の棚卸し**（配線点・seam・供給様式）であるため、コンポーネント面である `component-inventory.md` への集約が正しい所在。(2) `code-quality-assessment.md` のセンサー関連節（#680 / #847 / #757）はいずれも個別欠陥の記録で、本 scan は新たな欠陥を発見していない（発見したのは先例と再利用可能面）。(3) 残る 5 面は本 focus が**技術スタック・外部依存・公開 API 契約・ディレクトリ配置・業務価値のいずれも変更していない**ため、既存記述を無効化する事実が区間 `df1c874cfb..40056d0ec` に存在しない
- Per-intent record: `re-scans/260810-numeric-provenance-guard.md`（述語 P0〜P11、§1〜§9 の全実測、第1段への含意 5 点、UNMEASURED 3 件の正本）
- UNMEASURED（3 件、下流で確定すべき事項）: (U-1) `.claude/sensors/` と core manifest の**内容差**（サイズ差 8 バイトの原因） / (U-2) 成果物コーパス内で **measurement ref 様式が実際に使われている出現率** / (U-3) **数値主張そのものの corpus 内出現分布**（第1段の閾値導出に必須 — `cid:code-generation:c1-threshold-inside-observed-range` の両側契約を満たすには別途スイープが要る）
- 適用範囲外（明示）: 第1段の母集団定義（どの数値を「主張」と見なすか）、受理語彙の確定、閾値を置くか測定のみに留めるか、`idBlocks` の export 化可否 — いずれも requirements-analysis / application-design の所掌。本 RE は裁定を証拠から下せる状態にすることのみを行った
