...
## 実行メタデータ（現在: 260814-ambient-error-sink）

- Date: `2026-08-14`
- Base commit: `5f6b5bf97068f59dee53dcd4a2f6564967c3d164`（`reverse-engineering-timestamp.md` + `re-scans/*.md` の全 observed のうち **HEAD の祖先で距離最小**。`git merge-base --is-ancestor 5f6b5bf97 6e94189de` = **rc=0**、`git rev-list --count 5f6b5bf97..6e94189de` = **3**。`cid:reverse-engineering:rescan-base-ancestry`）
- Observed commit: `6e94189dec9e8e2bd0aaeb53bcff7cf9cba27440`（= 本 worktree HEAD = `origin/main`。`cid:reverse-engineering:c2-observed-mainline-commit`）
- Scope: `self-fix`、Brownfield、単一 repo `amadeus`、depth `Minimal`、build `bun`
- Focus: [Issue #3004](https://github.com/amadeus-dlc/amadeus/issues/3004) — in-process 呼出で `projectDir` を省略すると engine のエラー記録・state 書込が開発者の実 workspace へ着地する。本 RE の主題は**逸出面の全数棚卸しと、修正を縛る既存契約の確定**であり、患部重点の差分リフレッシュとして実施した
- Scan mode: **xrev differential scan**（`cid:reverse-engineering:c1-xrev-scan-mode` / `c1-xrev-single-issue`）— run `xrev-260814-3004`。Issue #3004 のクロスレビューコメント 2 件を Developer scan の一次入力とし、Architect が observed 断面の verbatim 実読で抜き打ち二重化した
- 行番号引用の currency（**実測の記録であり免除の主張ではない**）: `git diff --name-only 5f6b5bf97 6e94189de -- packages/ tests/ .claude/` → **`tests/integration/t528-report-ack-kind.integration.test.ts` の 1 ファイルのみ / rc=0**。`amadeus-orchestrate.ts` と `amadeus-lib.ts` は base→observed で**無変更**であるから、xrev verdict と前 intent 節の行番号引用は observed でも行番号込みで有効。**表現形式の移行検査**（`cid:reverse-engineering:c5-xrev-currency-schema-migration`）: 区間 3 commits に患部のスキーマ・セレクタ形式を変える移行はなく、c5 の構造的不成立条件には該当しない
- 中核知見: **(1) in-process 射程は 4 入口ではなく 3 入口** — `handlePark` は core `amadeus-orchestrate.ts:6407` にも `dist/claude/.claude/tools/amadeus-orchestrate.ts` にも export がなく（`grep -c "export function handlePark" dist/…` = **0**）、`undefined` が入る経路は `main:6817` の CLI だけ（Issue 本文への訂正）。**(2) 修正は `_handlerProjectDir` の型付けでは閉じない** — `amadeus-orchestrate.ts` の `resolveProjectDir` 26 箇所（`grep -c` 実測）のうち `_handlerProjectDir` を読むのは 4 箇所のみで、残る 22 箇所はハンドラのパラメータを直接解決するため、Issue の被害 (iii)（`spawnState` による ambient state 変更）は**入口で `undefined` を止めない限り残る**。**(3) 拒否は ambient を汚さない形が既存にある** — `emit()`（`:775`）の `applyPendingAdvisoryGuard` は `pending` 空判定（`:828`）と kind 判定（`:829`）の後に `resolveProjectDir` を呼ぶため、error directive はこの面で ambient に触れない。`emitStateNeutralError`（`:903-905`、`recordError=false`）がその形。**(4) 修正を縛る既存契約 7 条** — t214 T1/T2 が `recordEngineError` の projectDir 省略形の env 段・argv 段を**肯定的に固定**、T3 が state 不在時 no-op、T5 が `main` 経由の ambient を固定、t258 が explicit 形をバイト固定。加えて `handleNext` は複雑度ラチェットの記録上限ちょうど（`tests/.complexity-baseline.json` `threshold:15` / `handleNext ccn:22`、shrink-only）で **`if` を 1 つ足すと確実に赤**。**(5) 既に実 record へ着地した行 45**（述語: 追跡 `audit/*.jsonl` 250 ファイル / 全 148,805 行に対し `Event=ERROR_LOGGED` かつ `Tool=amadeus-orchestrate` かつ `Command=""`）。うち `seam: something went wrong` / `seam: no state` の 2 行は `t214-seam:131` / `:158` の projectDir 省略呼出にのみ存在する文字列で、**undefined 形の直接証拠**。**(6) 未実証仮説**: この 2 行は fixture 側の解決結果では説明できず、OTel の per-process ワークスペースピンが宛先を決めている可能性がある（**本 RE では実証していない**、別途起票の候補）
- Verification: git 状態変更・GitHub 書込・`bun run build`・engine/state 操作は**すべてゼロ**。書き込みは `codekb/amadeus/` 配下のみ（既存 4 面 + 新規 re-scan 1 件）
- Updated artifacts: `architecture.md`（新現在節「in-process 入口と emit 集約点における ambient 逸出面の全数」— A 入口 5 箇所と export 実態、B emit 集約点 3 面 + advisory latch、C CLI と in-process を区別する型・分岐の不在という設計欠落、D `_handlerProjectDir` 外の 22 面、E 拒否ガードの precedent と文言の不在。**前 intent 節の E1/E2 を部分集合として包含する上位互換の棚卸しであることを本文冒頭で明記**）/ `code-quality-assessment.md`（新現在節「ambient error sink の修正を縛る既存契約と実測ラチェット」— Q-1 t214 の 5 test、Q-2 t258 のギャップ、Q-3 制約 7 条、Q-4 複雑度ラチェット、Q-5 着地行 45 と述語、Q-6 未実証仮説）/ `code-structure.md`（新現在節「undefined 形の回帰テストが要求するシーム」— 層の制約、系統 A env 段 / B marker 段 / C テスト自身の安全性）。直前の現在節 3 面は本文保持のまま履歴へ降格（`cid:reverse-engineering:c3-relabel`）
- Reviewed-and-unchanged artifacts（**沈黙のスキップではなく、レビュー済みで無変更**）: `api-documentation.md`（公開シグネチャに変化なし。本 intent は既存シグネチャの**呼出契約の運用**を扱う）/ `component-inventory.md`（component の新設・廃止・責務移動ゼロ）/ `technology-stack.md`（`base..observed` で bun / TypeScript / Biome / fast-check 不変）/ `dependencies.md`（依存エッジの追加削除なし）/ `business-overview.md`（業務価値面の記述に変化なし）。**この 5 面は本 intent の節を持たないため、後続ステージがここから本 intent の事実を引いてはならない**（`cid:requirements-analysis:c4-consume-header-is-not-citable-content`）
- Per-intent record: `re-scans/260814-ambient-error-sink.md`（base/observed 選定根拠、scan mode と currency、Architect の抜き打ち照合 6 件の結果、更新面・無変更面の一覧、後続が本 intent の主張をどの面から引けるかの対応表、Developer scan への訂正 2 件の正本）
- 適用範囲外（明示）: 修正方式の選定（入口で拒否 / 型で表現 / `main` 側で事前解決）、拒否メッセージの文言、`handlePark` を対象に含めるか、Q-6 の仮説を本 Issue に含めるかは **requirements-analysis / application-design の所掌**

## 実行メタデータ（履歴: 260814-t528-ambient-isolation）

- Date: `2026-08-14`
- Base commit: `89532174c30ef9cc7ff29496cd6916586fdda00a`（`reverse-engineering-timestamp.md` + `re-scans/*.md` の全 observed のうち **HEAD の祖先で距離最小**、`git rev-list --count 89532174c..HEAD` = **9**。本 intent に prior record は無い。`cid:reverse-engineering:rescan-base-ancestry`）
...
- Scope: `self-fix`、Brownfield、単一 repo `amadeus`、depth `Minimal`、build `bun`
- Focus: [Issue #2981](https://github.com/amadeus-dlc/amadeus/issues/2981) — `tests/integration/t528-report-ack-kind.integration.test.ts` の失敗集合が実行文脈で入れ替わる。本 RE の主題は**その入れ替わりの機序の特定**であり、患部重点の差分リフレッシュとして実施した
- Scan mode: **xrev differential scan**（`cid:reverse-engineering:c1-xrev-scan-mode` / `c1-xrev-single-issue`）— run `xrev-260814-2981`、target-sha `52f1f1b2575ea35bd23b761697b2d17a5e9a7ac3`、currency 成立。クロスレビュー verdict を Developer scan の一次入力とし、Architect が observed 断面の verbatim 実読で二重化
- 行番号引用の currency（**実測の記録であり免除の主張ではない**）: `git log --oneline 52f1f1b25..HEAD` の実測は **1 commit**（`5f6b5bf97 docs(norms): E-260813-RECORD-BUNDLING-NORM を選挙ストア索引へ登録する`）。`git diff --name-only 52f1f1b25..HEAD -- <患部 5 パス>`（`t528`、`amadeus-lib.ts`、`amadeus-orchestrate.ts`、`fixtures.ts`、`amadeus-intent-autonomy-production.ts`）は**空出力 / rc=0** で交差なし → 行番号再解決は構造的 no-op（`cid:reverse-engineering:E-XBB-RE-S13-c2`）。**表現形式の移行検査**（`cid:reverse-engineering:c5-xrev-currency-schema-migration`）: 患部のスキーマ・セレクタ形式を変える移行は区間内に無く、c5 の構造的不成立条件には該当しない
- 中核知見: **#2981 の「入れ替わり」は単一欠陥ではなく独立2機序の重ね合わせである。機序 A**（xrev 確定、本 RE で再現）= `t528:124` の `handleReport(…, undefined)` → `resolveProjectDir(undefined)`（`amadeus-lib.ts:232-269`）が実 workspace へ ambient 解決 → `runsQualityRepair`（`amadeus-orchestrate.ts:5780-5783`）が実 record の autonomy projection を読み、`semi`/`full` なら failed-result 分岐（`:6020-6023`）が `FORWARD_RESULTS` 検査（`:6039-6045`）より上で先取りする。本 worktree の読取専用プローブ実測では `mode = "full"` / `runsQualityRepair = true` で **test #3 は必ず落ちる**。**機序 B**（Developer scan で新規特定）= `STOCK_GRAPH`（`t528:46-54`）が gitignore 対象の `dist/claude/.claude/tools/data/stage-graph.json` を指し、不在時に `loadStageGraph()`（`amadeus-lib.ts:6954-6967`）が throw → `nodeForSlug`（`:2846-2848`）へ到達する **#4/#5 のちょうど2件**が落ちる（#6 は `handleNext` Branch 0 の read-only latch `:3118-3120` がグラフ手前で返るため緑）。repo 外 scratch プローブの落ちる実証で報告失敗集合と完全一致した。**未記載側面**: 機序 A の error directive は `emit()` 集約点（`:802-804`）→ `recordEngineError`（`:941-968`）の ambient 段を通り、`:958` の state 実在ガードを実 record が満たすため `emitErrorAuditRow`（`:962`）が**実 intent record の監査シャードへ `ERROR_LOGGED` 1 行を書く**（state 本体は `--failure` ガードで未到達）。既存回帰 `t258`（#1389）は explicit target 形（`:91`）しか固定せず `undefined` 形は未被覆。**`dist/` 依存は t528 固有ではなく横断クラス**で、`AMADEUS_SRC`（`fixtures.ts:59`）/ `AMADEUS_MEMORY_SRC`（`:93`）/ `STOCK_GRAPH` を含む述語和集合は **278 / 1102 テストファイル（約 25%）**。**患部行は base 以降無変更**（`git diff 89532174c..HEAD -- amadeus-orchestrate.ts | grep -c "^[+-].*\(resolveProjectDir\|runsQualityRepair\|failureAdmission\|handleStageFailureReport\)"` = 0）
- Verification: git 状態変更・GitHub 書込・`bun run build`・engine/state 操作は**すべてゼロ**。書き込みは `codekb/amadeus/` 配下のみ。プローブはすべて repo 外 scratch で実行し、書込を伴うものは `CLAUDE_PROJECT_DIR` を scratch へ固定した
- Updated artifacts: `code-quality-assessment.md`（新現在節「テスト実行文脈への依存 — 失敗集合が入れ替わる二重機序」— 機序 A/B の対比表、Q-1 ambient 解決の露出、Q-2 監査シャード汚染と t258 のギャップ、Q-3 `dist/` 依存クラス 278/1102、#6 が緑である構造的説明、落ちる実証、未検証 H1/H2）/ `architecture.md`（新現在節「projectDir 解決の段構造と in-process 呼出における ambient 逸出」— 6 段ラダー表、逸出点 E1/E2 と閉包点の相違、explicit 経路上に残る ambient 依存 5 面と `pluginHostRoot()` の未閉シーム）/ `code-structure.md`（新現在節「テスト基盤の `dist/` 依存と env 伝播」— 患部ファイル表、外部前提の 3 層図とテキストフォールバック、クラス規模の実測述語、fixture ライフサイクル）。直前の現在節は本文保持のまま履歴へ降格（`cid:reverse-engineering:c3-relabel`）
- Reviewed-and-unchanged artifacts（**沈黙のスキップではなく、レビュー済みで無変更**）: `api-documentation.md`（本 intent は既存関数の**呼出契約の運用**を扱い、公開シグネチャ・入力ドメインの新規事実を持たない。`resolveProjectDir` / `handleReport` の契約自体は base から不変）/ `component-inventory.md`（component の新設・廃止・責務移動はゼロ。患部は既存 component 間の引数受け渡し）/ `technology-stack.md`（`base..observed` で bun / TypeScript / Biome / fast-check 不変、新規 runtime dependency なし）/ `dependencies.md`（依存エッジの追加削除なし）/ `business-overview.md`（テスト実行の再現性は開発者体験の問題で、業務価値面の記述に変化なし）。**この 5 面は本 intent の節を持たないため、後続ステージがここから本 intent の事実を引いてはならない**（`cid:requirements-analysis:c4-consume-header-is-not-citable-content`）
- Per-intent record: `re-scans/260814-t528-ambient-isolation.md`（base/observed 選定根拠、述語 P0〜P9、t528 の 6 テスト × projectDir 表、機序 A/B の全経路、Developer scan への行番号訂正 3 件、UNMEASURED-1〜4 の正本）
- 適用範囲外（明示）: 修正方式の選定 — テスト側を explicit projectDir へ直すか production 側（`_handlerProjectDir` / `recordEngineError` の ambient 段）を fail-closed にするか、`dist/` 前提を loud fail 化するか、`pluginHostRoot()` に projectDir を通すか — はすべて requirements-analysis / application-design の所掌
