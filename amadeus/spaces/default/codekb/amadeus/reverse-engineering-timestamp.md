...
## 実行メタデータ（現在: 260728-gated-swarm-serializatio）

- Date: `2026-07-28`
- Base commit: `0c4709102cfa1d13e5aca6b49c65f31a903d72f2`（`git merge-base --is-ancestor 0c4709102 HEAD` **exit 0 = 祖先**、`git rev-list --count 0c4709102..HEAD` = **36**。cid:reverse-engineering:rescan-base-ancestry）。**`afb93a825`（直前「現在」節 = intent `260727-plugin-verb-skills` の observed）は base 候補から棄却**した — `git merge-base --is-ancestor afb93a825 HEAD` **exit 1 = 非祖先**（別 worktree `plugin-dev` / ブランチ `worktree-plugin-dev` 上で並行進行中の intent であり、本 HEAD の履歴に載っていない）。祖先性を満たす最新の observed が `0c4709102` である。
- Observed commit: `ec6f16ad81074f7ca4f252afa0d5d91ecbd48538`（= 現 HEAD、`git rev-parse HEAD` 実測。worktree `unit-flow`、ブランチ `intent/1612-gated-serialization`）
- 区間規模: 36 コミット（先頭 `ec6f16ad8`、末尾 `68f2d6699 chore(release): v0.1.6`）。変更ファイル数は record（`amadeus/`）除外で **488**。面別上位は `tests/integration` **35** / `packages/framework` **31** / `dist` 各ハーネス **23-25** / `docs/reference` **24** / `tests/unit` **23** / `docs/guide` **22** / self-install `tools` 各 **20-21** / `dist/plugins` **6** / `tests/smoke` **3** / `scripts/formal-verif` **3**。`packages/framework` 31 の内訳は mirror 系ツール **11**（`amadeus-mirror-*.ts`）/ plugin 系 **3** / election 系 **2** / `amadeus-graph.ts`・`amadeus-runner-gen.ts`・`amadeus-utility.ts`・`amadeus-version.ts` / ハーネス manifest **7** ほか。docs **48** ファイル。テストは新規 A=**25**（e2e 1 / integration 15 / unit 9）、既存 M=**41**（ratchet・registry・allowlist・complexity-baseline の 4 台帳を含む）。（測定 ref: observed `ec6f16ad8`）
- 区間の内訳（主要系統 4 本）: (1) **mirror-project-status**（U1〜U5）= `4f8607f05`(#1593) / `1e7c2602b`(#1603) / `7d713c2fb`(#1604) / `36d59f061`(#1605) / `d9457b723`(#1606) / `56abb3fa3`(#1617) — GitHub Project Status 同期 (2) **plugin 系** = `f1d561904`(#1596) / `b32be0ec2`(#1611) / `85e66a9ed`(#1616) / `2000f29c8`(#1618) / `53bdfcd7c`(#1624) / `242e4175a`(#1634) / `7e26c8033`(#1632) / `89bddbb11`(#1626) / `faceb738b`(#1627) — `/amadeus plugin <verb>`、install verb、plugin stage runner-gen、`amadeus-plugin` スキル、scope-grid clobber 修正、composed-plugin carve-out (3) **election solo-mode** = `d2d20131e`(#1609) / `ccc30d06d`(#1639) (4) **scope/docs/metrics** = `ec6f16ad8`(#1640 `amadeus-feature` 軽量化) / `9f3d4583a`(#1629 legacy `amadeus` スコープ廃止) / `76599e366`(#1635) / `3eba39a90`(#1584) / `d5e8912f0`(#1587) / `daa18009e`(#1600) / metrics スナップショット系。リリース `68f2d6699` = **v0.1.6**。
- **区間と欠陥領域の交差ゼロ（本 scan の最重要の否定的事実）**: `git log --oneline 0c4709102..HEAD -- packages/framework/core/tools/amadeus-orchestrate.ts` が **空**、同ファイルの `git diff --stat` も **空**。`stage-protocol.md` も区間の変更一覧に不在。すなわち [Issue #1612](https://github.com/amadeus-dlc/amadeus/issues/1612) の欠陥は本区間で導入されたものではなく、**#841 のバッチ進行実装以来この形で内在**している。差分リフレッシュは欠陥領域の新規リスクを持ち込まない一方、**修正の起点断面は既存実装そのもの**である。
- Scope: `amadeus-bugfix`（intent `260728-gated-swarm-serializatio`）、Brownfield、単一 repo `amadeus`
- Focus: **swarm dispatch と Construction autonomy のゲーティング**。走査対象は (a) `tryEmitSwarm` のガード列と `:2526` の autonomy 拒否 (b) `readAutonomyMode` の 3 値→2 値潰し (c) フォールバック直列化経路（`orderedUnits` / `nextUncoveredUnit` / `emitPerUnitRunStage`）(d) approve 側 `isAutonomousSwarm` 除外条件との連動（デッドロック警告）(e) `invoke-swarm` directive 形と「バッチ末尾ゲート」機構の不在 (f) 規範 `stage-protocol.md:123-125` (g) 欠陥挙動を契約化している既存テスト。詳細は `architecture.md` の同名節。
- 差分リフレッシュ（cid:reverse-engineering:c1）: フルスキャン不実施。差分区間 + 本 intent の欠陥領域に限定して走査した。上流入力は Developer スキャン結果（実測済みスキャンノート、全文読了）。
- Architect 段の独立再検証（cid:reverse-engineering:cite-shift-vs-nonshift-separation）: 上流スキャンノートの荷重引用を observed `ec6f16ad8` に対して直読 spot-check し **訂正 0 件** — `amadeus-orchestrate.ts:2511-2528`（`:2526` `if (readAutonomyMode(stateContent) !== "autonomous") return false;`）、同 `:1164-1168`（`readAutonomyMode` 本体と `:1161-1163` の設計コメント）、同 `:3824-3826`（`isAutonomousSwarm` と除外条件）および `:3810-3821` のデッドロック警告コメント、同 `:2636-2640`（`return batches.flat();`）/ `:2683-2704`（`return { unit: uncovered[0], uncovered };`）、`stage-protocol.md:123-125`（Subsequent Bolt gate の規範文）、`amadeus-directive.ts:167-185`（`InvokeSwarmDirective` の 3 フィールド、gate フィールド不在）。ancestry の 2 判定（exit 0 / exit 1）と区間コミット数 36 も自ら再実行して一致確認した。
- 測定 ref: 本節および本 scan で更新した全成果物の数値・SHA・file:line は observed `ec6f16ad8` での `git rev-parse` / `git merge-base --is-ancestor` / `git rev-list --count` / `git log --oneline` / `git diff --name-only`・`--stat` / `grep -n` / `sed -n` / `grep -rl` 出力からの転記（cid:requirements-analysis:numbers-from-command-output-only、cid:reverse-engineering:measurement-ref-in-artifacts）。
- 更新した成果物: `reverse-engineering-timestamp.md`（本ファイル）/ `architecture.md`（swarm dispatch と autonomy ゲーティングの新節、旧「現在」節は履歴降格）/ `code-structure.md`（区間の面別内訳と新規ツール面の追記）/ `technology-stack.md`・`dependencies.md`（v0.1.6 とスコープ集合変更の追記）。`business-overview.md` / `api-documentation.md` / `component-inventory.md` / `code-quality-assessment.md` は区間が既存記述を実質的に陳腐化させないため**無更新**（最小タッチ方針）。旧「現在」マーカー（`260727-plugin-verb-skills`）は本ファイルの H2 見出しで履歴ラベルへ降格した（cid:reverse-engineering:c3-relabel）。**履歴節の当時記述は削除せず全文保存**する。
- Sensors: RE ステージが宣言する 3 センサー（required-sections / upstream-coverage / answer-evidence）は、codekb 出力パス `amadeus/spaces/default/codekb/amadeus/**` が各 manifest の filter に構造的に不適合で発火不能（cid:reverse-engineering:re-sensors-codekb-filter-mismatch、cid:reverse-engineering:c3-codekb-sensor）。**センサー成功として扱わず**、代替として更新成果物へ `grep -c '^## '`（H2 ≥ 2）と正準 3 語彙の conflict マーカー検査（ヒット 0、cid:code-generation:conflict-marker-grep-before-commit）を機械実行した。
- Delivery boundary: 本 scan は codekb 成果物の差分更新のみを成果物とし、患部コード（`amadeus-orchestrate.ts` / `stage-protocol.md` / `amadeus-directive.ts`）・テスト・CI 設定・生成配布物・intent record / state / audit・GitHub Issue への書込は一切行わない。修正方式（`readAutonomyMode` を 3 値へ戻すか / `invoke-swarm` に gate フィールドを足すか vs 次 `next` 再入でゲート付き directive を返すか / `:2526` と `:3825` を同時に変える形 / 欠陥を契約化した既存テストの改訂範囲）は後続の requirements-analysis 以降で裁定する。
