...
「並列にしたくない」は実行方針「直列限定」で表現し、「並列を止めながら進める」は gated(`none` — 実装ドリフト下では `semi` も同扱い)の batch 承認で表現する。機構に固有名はなく、実行形態は「直列実行/並列実行」とだけ呼ぶ。
...
> 本部は現行契約の記述のみを置き、問題の指摘を含めない(問題提起は Part 2 の §7)。契約の正本は承認済み RFC(RFC 0001 等)であり、実装が正本と乖離している箇所は正本契約を記した上で乖離を「実装ドリフト」として注記する。同一事実が複数箇所に現れる場合(§1 の対比表・§4 の図は要約投影)、規範の正は §2・§3・§5・§6 の表と条文である。
...
前3列を通過すれば **swarm は常に発動へ向かい**、モードが決めるのは「batch ごとに人間が承認するか(`none` — 実装ドリフト下では `semi` も同扱い)、連続で走るか(`semi`・`full` の正本契約)」だけである。swarm 自体を避ける手段はモードの側にはなく、並列 batch を計画しない(delivery plan 側の判断)ことだけがそれにあたる。
