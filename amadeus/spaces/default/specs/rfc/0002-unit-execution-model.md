...
ドライバ設定を読む面、決定表を記す文書、用語集、当該テスト群、全ハーネス配布と自己インストール面に加え、§8.4 の機械名貫通の対象(検証ツール・監査イベント発行部・イベントレジストリ・階層設定キー・旧語彙を読む集計/リプレイ系)。全数列挙と改名の段階順序(一括か段階か)は実装 intent の調査段で確定する(Unresolved 参照)。
