...
completed self-feature intents): `feasibility` (9/10 trivial GO),
...
`self-fix` for a bounded correction to an existing contract and
`self-refactor` for a behavior-preserving structural change.

Not inferable by keyword; select it explicitly with `--scope self-feature`.
