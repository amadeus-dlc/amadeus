...
// (amadeus-runtime-compile.ts) on every transition-class audit emit. Pure
...
// --- Path helpers ---
//
// The amadeus-docs data-path helpers (runtimeGraphPath, memoryFilePath,
// unitDependencyPath, relativeMemoryPath, worktreeRuntimeGraphPath) are imported
// from amadeus-lib.ts — the single chokepoint for the amadeus-docs tree. memoryPath
...
// amadeus-orchestrate.ts handleSingleReport). Those rows belong to no main
...
export function compile(opts: CompileOptions): { skipped?: string; written?: string } {
  ...
  // (DEFAULT_TIMEOUT_SECONDS, a non-exported const at amadeus-sensor.ts:44) in an
  ...
  const ORPHAN_CUTOFF_SECONDS = 60; // = DEFAULT_TIMEOUT_SECONDS (amadeus-sensor.ts:44, not exported)
...
// into a Bolt's worktree at <wt>/amadeus-docs/runtime-graph.json on Bolt
...
// the transition regex per amadeus-runtime-compile.ts:87). Content-merge
...
Subcommands:
  ...
  fragment-fork --slug <slug>       Byte-copy main runtime-graph.json into a Bolt's
                                    worktree at <worktree>/amadeus-docs/runtime-graph.json.
...
hook (amadeus-runtime-compile.ts) on every transition-class audit emit.
Manual invocation is a debug surface.

fragment-fork / fragment-merge are invoked by amadeus-bolt.ts during the
