...
//
// (1) STAGE-RUNNERS: one thin `skills/amadeus-<stage>/SKILL.md`
...
//     typeable `/amadeus-<stage>` command. The authoritative authoring path is
...
//     ONE `/amadeus-init` runner over `/amadeus --init` instead.
//
// (2) SCOPE-RUNNERS: one thin `skills/amadeus-<scope>/SKILL.md` per
...
import { errorMessage, harnessDir } from "./amadeus-lib.ts";
import { type GraphStage, loadGraph } from "./amadeus-graph.ts";
...
// how the engine resolves amadeus-common/ and the stage files.
...
// generation and ship ONE `/amadeus-init` runner that wraps `/amadeus --init`
...
// The dir name for the init-phase runner: `/amadeus-init`. It wraps the whole
...
1. Ask the engine for the single-stage directive:
   ...
   \`\`\`bash
   bun ${harnessDir()}/tools/amadeus-orchestrate.ts next --stage ${node.slug} --single
   \`\`\`
...
2. When the stage's work is done, commit the single-stage record:
   ...
   \`\`\`bash
   bun ${harnessDir()}/tools/amadeus-orchestrate.ts report --single --stage ${node.slug} --result completed
   \`\`\`
...
// Render the `/amadeus-init` runner: a thin wrapper over the deterministic
...
1. Birth the intent (run the initialization phase). Parse the user's
   ...
   **\`--label\`**: a 2-3 word kebab-case essence of what's being built
   ...
   \`\`\`bash
   bun ${harnessDir()}/tools/amadeus-utility.ts intent-birth --scope <name> --arguments "<description>" --label "<2-3 word essence>"
   \`\`\`
...
// The dir name for the composer shortcut runner: `/amadeus-compose`. A thin
// typeable wrapper over `/amadeus compose ...` - the same one-door path, never a
// divergent flow.
const COMPOSE_RUNNER_DIR = "amadeus-compose";

// Render the `/amadeus-compose` runner: a thin wrapper over the engine's compose
...
// `/amadeus-init`, its parity is held by the packager's dist-level `--check`
...
// The on-disk stage-runner set: every `skills/amadeus-<slug>/` dir whose SKILL.md
...
function handleCheck(): void {
  ...
  if (missing.length > 0) {
    console.log(`MISSING runners (stage in graph, no skills/amadeus-<slug>/): ${missing.join(", ")}`);
  }
  if (orphans.length > 0) {
    console.log(`ORPHAN runners (skills/amadeus-<slug>/ with no matching stage): ${orphans.join(", ")}`);
  }
  console.log(`Run \`bun ${harnessDir()}/tools/amadeus-runner-gen.ts write\` to regenerate.`);
  process.exit(1);
}
...
// quoted). Mirrors amadeus-lib.ts scalarField for the fields the generator reads.
...
// file is `.claude/scopes/amadeus-<name>.md`; the canonical name is its `name`
// frontmatter field (not the filename, which carries the amadeus- prefix).
...
1. \`directive = bun ${harnessDir()}/tools/amadeus-orchestrate.ts next --scope ${scope} $ARGUMENTS\`
...
3. \`bun ${harnessDir()}/tools/amadeus-orchestrate.ts report --stage <directive.stage> --result <outcome> [--user-input "<text>"]\` when the directive names a stage; omit \`--stage\` only for non-stage report round-trips.
4. Treat the directive returned by the report as the next loop step. Continue immediately for \`run-stage\`, \`invoke-swarm\`, and \`print\`; stop for \`ask\`, \`select-intent\`, \`error\`, \`parked\`, or \`done\`.
...
function handleScopes(rest: string[]): void {
  ...
  if (check) {
    ...
    if (drift.length > 0) {
      ...
      console.error("Re-run `bun amadeus-runner-gen.ts scopes` to regenerate.");
      process.exit(1);
    }
