...
const EXTRACT_RELATIVE_DIR = "extracted";
...
export function createFetcher(http: Http, tmpWrite: TmpWrite): Fetcher {
  ...
  return Object.freeze({
    async fetchArchive(version: ResolvedVersion): Promise<Result<ExtractedPayload, FetchError>> {
      ...
      const extraction = await extractArchive(archivePath, extractDir, tmpWrite);
...
async function extractArchive(archivePath: string, extractDir: string, tmpWrite: TmpWrite): Promise<Result<void, FetchError>> {
