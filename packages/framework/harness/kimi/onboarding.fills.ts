...
- Kimi's \`SessionStart\` atomically establishes a main-session baseline. The \`Stop\` hook forwards only when its non-empty \`session_id\` matches both that baseline and \`.current-session\`, no delegated \`agent_name\` is present, and no subagent role is active. Missing, malformed, updating, ended-session, unknown, and delegated states fail closed before state or audit writes. The lifecycle carrier records ambient subagent presence rather than authenticating an arbitrary process caller, so the conductor must not mutate while a subagent is active; reviewer tool allowlists (\`Read\`, \`Grep\`, \`Glob\`) remain defense in depth.
