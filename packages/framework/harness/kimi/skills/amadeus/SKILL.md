...
description: >
  ...
  `.kimi-code/scopes/`; run
  `bun .kimi-code/tools/amadeus-utility.ts help` for the authoritative list
...
| `parked` | The workflow was parked at a clean inter-stage boundary (`directive.stage`) for a later session. Tell the user it is parked and how to resume (`/skill:amadeus --resume`), then STOP the loop. No stage was advanced and nothing was marked complete. |
...
| `ask` | Render `directive.question` via `AskUserQuestion` (this harness's binding for the protocol's structured questions — see `question-rendering.md` beside this file; when unavailable, render numbered prose), then feed the resolved option label or free text back on the next `report` via `--user-input`. The engine never asks the user itself — it defers the human turn to you. |
...
The orchestration engine emits eight kinds today: `run-stage`, `invoke-swarm`, `ask`, `select-intent`, `print`, `error`, `done`, `parked` (`invoke-swarm` is emitted for an eligible Construction batch under an `autonomous` OR `gated` grant — under `gated` the engine emits an `ask` batch-end gate between batches). The `dispatch-subagent` and `present-gate` arms remain documented placeholders so the loop is complete-shaped; until the engine emits those two, you will only ever act on the eight. Do not implement those two placeholder behaviours speculatively.
...
1. Pass the directive width to preparation: `bun .kimi-code/tools/amadeus-swarm.ts prepare --batch <n> --units <all> --concurrency <directive.cap> [--base <branch>] [--repo <name>]`. `max-parallel-units` resolves project → space → intent, defaults to the hard cap 4, and an invocation may only narrow it.
2. Call `bun .kimi-code/tools/amadeus-swarm.ts acquire --batch <n> --idempotency-key <stable-delivery-id>` until it returns `capacity-exhausted` or `no-ready-unit`. Dispatch only the returned unconfirmed active attempt permits. After native acceptance, immediately call `bun .kimi-code/tools/amadeus-swarm.ts confirm-dispatch --batch <n> --attempt <attempt-id> --native-handle <handle> --idempotency-key <stable-delivery-id>`; a claim alone is not a start fact.
3. After `bun .kimi-code/tools/amadeus-swarm.ts check <unit> --check-cmd "<command>"`, call `bun .kimi-code/tools/amadeus-swarm.ts settle-release --batch <n> --attempt <attempt-id> --outcome <succeeded|failed> --idempotency-key <stable-delivery-id>`. A non-success outcome automatically cancels transitive dependents; `settle-release-cancel-dependents` is the explicit equivalent for a local terminal failure. The same event set releases the slot and promotes dependency-ready FIFO work.
...
5. Call `bun .kimi-code/tools/amadeus-swarm.ts finalize --batch <n> --units <all> --claimed <converged> --check-cmd "<command>"` only after the pool exists and is terminal. An absent/uninitialized, open, draining, queued, or active pool is rejected.
...
This equivalence governs how `#nnn` and Issue URLs are read as Intent input (e.g. during Birth or a continuation); it does not change engine routing or `next`/`report` argument parsing.

### Composing a workflow plan (the adaptive composer)

The engine can name a COMPOSER DISPATCH instead of a scope confirm: on `/skill:amadeus compose "<task>"`, `--new-scope`, `--report <path>`, or when the human answers a cold-start compose offer with "compose", `next` emits a `print` whose message names the composer agent. Act on it like any dispatch: run the composer via the Agent tool (`subagent_type`: `amadeus-composer-agent`) with the message's instructions as the prompt (the agent loads its own persona). The composer runs the read-only `detect` scan, reads the stock scopes, and returns a structured proposal: `{ mode: matched|custom, scopeName, grid, rationale[] }` with a reason for every SKIP.

Render that proposal to the human and present an approve/edit/reject gate via `AskUserQuestion` (Approve / Edit the grid / Reject; numbered prose as the fallback). This gate is a hard turn-stop, like a stage gate: never treat silence as approval, and never write scope data or birth a workflow before an explicit approve. On edit, fold the human's changes into the grid and re-present. On reject, stop; the human can name a scope directly instead. (A one-shot non-interactive `kimi -p` invocation cannot answer the gate mid-run, so a compose that needs approval only completes in an interactive session.)

**On approve (front/report), the write and the birth run in the SAME turn - no second `/skill:amadeus` invocation:**
...
3. Continue into the normal birth: run `bun .kimi-code/tools/amadeus-orchestrate.ts next --scope <name>` and act on its birth print exactly as "Acting on a directive" describes (the `--label` essence and the duplicate-intent guard ride the same path as any birth).
...
The orchestration engine resolves scope-level stage routing internally (it reads the compiled scope grid the table below summarises). The summary table is kept here as human-readable data — not dispatch logic — and is regenerated, never hand-edited. (One carve-out: the dispatched composer agent APPENDS approved composed scopes to the runtime scope registry (`.kimi-code/scopes/amadeus-<name>.md` + a `scope-grid.json` entry) - that is the sanctioned write path for composed scopes, not a hand-edit; this summary table itself stays generated.)

Source of truth: one file per scope under `.kimi-code/scopes/amadeus-<name>.md` (identity + keywords + description) plus each stage's `scopes:` frontmatter (membership), transposed into the compiled grid at `bun .kimi-code/tools/amadeus-graph.ts compile`. Adding a scope is the same muscle memory as authoring a sensor or agent — drop `.kimi-code/scopes/amadeus-<name>.md`, tag the member stages' `scopes:` lists, recompile, then `bun .kimi-code/tools/amadeus-utility.ts scope-table` to regenerate the table below + commit. CI runs `scope-table --check` to prevent drift.
...
## Harness notes (Kimi Code)

- **Gates**: `AskUserQuestion` is the primary binding for structured questions; numbered prose is the sanctioned fallback when it is unavailable (auto permission mode, headless `kimi -p`). BOTH paths route through the same `mint` target. For a stage approval, call `gate-reserve` first, include its Presence Reservation Id in the approval question payload, and require the numbered-prose reply to echo it. Only that same-session, same-intent, same-stage route can mint the reservation; ordinary and §13 turns remain unreserved and cannot approve. Gate responses go directly to carrier-bearing `report` without an intervening `QUESTION_ANSWERED` event.
- **Stop and mutation authorization use a fail-closed session baseline plus ambient subagent presence**: `SessionStart` atomically establishes the host-stamped main-session baseline. Kimi `Stop` forwards to the stateful core hook only when its non-empty `session_id` matches both that baseline and the current-session carrier, no delegated `agent_name` is present, and the active-role set is empty. The live Kimi 0.28.1 Stop capture has no `agent_name`; the installed Kimi 0.29.0 bundle constructs Stop with `stopHookActive` and reports delegated lifecycle separately through `SubagentStart`/`SubagentStop` `agentName`. Missing, unreadable, malformed, updating, ended-session, reviewer, support, explore, and other ambiguous states are silent Stop no-ops and reject `next`, `report`, `park`, `gate-reserve`, `gate-reject`, and direct state mutations before state or audit writes. The role carrier proves ambient presence, not the process identity of an arbitrary tool caller, so the conductor must never mutate while a subagent is active; this is an execution contract, with the reviewer tool allowlists as defense in depth. Product and architecture reviewer profiles expose only `Read`, `Grep`, and `Glob`; stage `support_agents` remain inline and Kimi's built-in `explore` profile remains read-only.
- **Stage visibility rides the TodoList tool.** There is no custom statusline; keep the todo list current instead — one `in_progress` item per running stage, the active item's text ending with the stage's `[slug]` suffix (e.g. `Running Intent Capture [intent-capture]`). The `PostToolUse(TodoList)` hook reads that suffix to sync `amadeus-state.md`. Mark the prior item completed when a stage finishes. `/skill:amadeus --status` is always available on demand.
- **Hooks live in the user-level config**: Kimi Code has no project-level config file, so the `[[hooks]]` + `[[permission.rules]]` wiring ships as `.kimi-code/hooks/amadeus-hooks.snippet.toml` and is merged into `~/.kimi-code/config.toml` as a marker-fenced managed block (`# >>> amadeus-kimi-hooks >>>`) by the setup CLI (backup + confirm + atomic write). The block first denies agent-side Bash composition matching `Bash(*.kimi-code/hooks/*)`, then pre-allows `Bash(bun .kimi-code/tools/*)` and the required git prefixes. That deny is defense-in-depth, not a caller-authentication boundary. Host-managed hook invocations still route through `bun .kimi-code/hooks/amadeus-kimi-adapter.ts <target>`, resolved from the session's project dir and fail-open where the project is not installed.
- **Construction swarm = native subagent fan-out**: one Agent child per unit in this session, each confined to its worktree (worktree-relative paths only, no git operation outside the assigned tree). Resolve the driver via `amadeus-swarm.ts resolve --harness kimi`: `subagent` dispatches here; `claude-ultra`/`codex-ultra` loud-degrade to the `subagent` floor (`--degraded-from <requested>` → `SWARM_DEGRADED`); an unknown value is rejected (exit 1) and stops the batch fail-closed. There is no kimi-ultra.
- **Session lifecycle**: `SessionEnd` exists (matcher `exit`) and emits `SESSION_ENDED` directly — no next-start reconcile hack is needed (contrast Codex, which infers an unclosed prior session at the next start). `PreCompact` (`manual|auto`) drives state validation; PostCompact re-injection is not wired on this harness.
...
| environment-provisioning | 4.2 | Environment Provisioning | Operation | CONDITIONAL | amadeus-aws-platform-agent | amadeus-devsecops-agent | inline |
...
| feedback-optimization | 4.7 | Feedback & Optimization | Operation | CONDITIONAL | amadeus-operations-agent | amadeus-compliance-agent | inline |
