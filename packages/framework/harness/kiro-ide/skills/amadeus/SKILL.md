...
| `present-gate` | A solo standing grant expired or was revoked before commit. Present the normal approval gate and report `approved` after a human response. |

The orchestration engine emits eight kinds today: `run-stage`, `invoke-swarm`, `present-gate`, `ask`, `print`, `error`, `done`, `parked`. `present-gate` is the fail-closed fallback for a solo standing-grant race; `dispatch-subagent` remains a placeholder.
...
`run-stage` folds the approval decision into its `gate` field. Bootstrap stages use `false`, an ordinary uncovered gate uses `true`, and a solo standing grant covering an ordinary gate uses `"granted"`. One case is unresolved:
...
- **`gate: "granted"`** — run the same body, reviewer, verification, and §13 learnings ritual as `gate: true`, but do not stop for a human. Call `report --stage "<directive.stage>" --result grant-approved`. Commit-time revalidation records the `Grant Id`; expiry or revocation emits `present-gate` and falls back to normal human approval. Team mode retains its leader/delegation flow.
...
**Per-unit iteration (`directive.unit`).** When `directive.unit` is present, this `run-stage` is ONE iteration of a per-unit Construction stage. A `false` gate means an uncovered unit: write its artifacts and re-run `next` without reporting. Once every unit is built, the engine emits `true` or, for a covered solo grant, `"granted"`; follow the corresponding branch above. The reviewer fires once per unit.
