#!/usr/bin/env bun
// amadeus-codex-adapter.ts — the Codex CLI hook shim (AUTHORED shell file; the
// amadeus-*.ts hook bodies beside it are PACKAGED core, byte-shared with the
// Claude Code harness). Modeled on kiro's amadeus-kiro-adapter.ts: ONE shim
...
//   bun .codex/hooks/amadeus-codex-adapter.ts <target>
...
const DEDUPE_ROOT = join(
  tmpdir(),
  `amadeus-codex-dedupe-${createHash("sha256").update(projectDir).digest("hex").slice(0, 16)}`,
);
  ...
  try {
    if (existsSync(heartbeatFile)) {
      ...
      if (prior.session_id && prior.session_id !== codex.session_id) {
        ...
        runCore("amadeus-session-end.ts", JSON.stringify({ reason }));
      }
    }
...
switch (target) {
  ...
  case "audit-and-sensors": {
    ...
    if ((codex.tool_name ?? "") === "apply_patch") {
      ...
      for (const f of patchedFiles(command)) {
        ...
        runCore("amadeus-audit-logger.ts", fwd);
        runCore("amadeus-sensor-fire.ts", fwd);
      }
    }
    persistResponse("", 0);
    process.exit(0);
  ...
  case "state-sync": {
    ...
    if ((codex.tool_name ?? "") === "update_plan") {
      ...
      if (active?.step) {
        ...
        runCore("amadeus-sync-statusline.ts", fwd);
      }
    }
    persistResponse("", 0);
    process.exit(0);
  ...
  case "runtime-compile": {
    ...
    runCore("amadeus-runtime-compile.ts", rawInput);
    persistResponse("", 0);
    process.exit(0);
  }
  ...
  case "validate-state": {
    ...
    runCore("amadeus-validate-state.ts", rawInput);
    persistResponse("", 0);
    process.exit(0);
  }
  ...
  case "log-subagent": {
    ...
    runCore("amadeus-log-subagent.ts", rawInput);
    persistResponse("", 0);
    process.exit(0);
  }
  ...
  case "stop": {
    ...
    const r = runCore("amadeus-stop.ts", rawInput);
