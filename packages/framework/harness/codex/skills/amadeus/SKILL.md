...
| `ask` | Render `directive.question` as numbered prose per `question-rendering.md`, then stop for the human answer. If `directive.response_action.kind` is `select-intent`, render exactly its `options`; on reply, resolve the chosen number (including full-width digits) or text to the option slug, run `bun .codex/tools/amadeus-utility.ts intent "<slug>"`, then re-run `next`. Never pass the raw ordinal to a tool and never call `report` for this action. Otherwise run exactly `bun .codex/tools/amadeus-orchestrate.ts report --user-input "<resolved option label or free text>"`: an ordinary `ask` answer is not a stage verdict, so do **not** pass `--result` or `--stage`. The engine never asks the user itself — it defers the human turn to you. |
