...
5. Call `bun .cursor/tools/amadeus-swarm.ts finalize --batch <directive.batch> --units <all> --claimed <converged> --check-cmd "<command>" [--repo <name>] [--target <branch>] [--strategy <squash|merge|rebase>]` only after the pool exists and is terminal; absent, open, draining, queued, or active pools are rejected.
