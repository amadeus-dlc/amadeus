...
5. Call `bun .cursor/tools/amadeus-swarm.ts finalize --batch <directive.batch> --units <all> --claimed <converged> --check-cmd "<command>"` only after the pool exists and is terminal; absent, open, draining, queued, or active pools are rejected. finalize itself merges AIDLC data then Git source via `amadeus-worktree merge` (`--target` defaults to the current branch, `--strategy` defaults to squash). Pass `--repo` when prepare did. A unit is not converged unless both land.
