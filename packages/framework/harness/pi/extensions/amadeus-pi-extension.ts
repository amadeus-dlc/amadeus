...
async function loadCoreRuntime(): Promise<CoreRuntime> {
  coreRuntimePromise ??= Promise.all([
    ...
    import(coreModuleUrl("tools/amadeus-plugin.ts")),
    import(coreModuleUrl("otel/event-registry.ts")),
  ]).then(([audit, lib, journal, plugin, registry]) => {
    if (
      ...
      || typeof plugin.handlePluginCli !== "function"
...
async function autoComposePlugins(projectDir: string, context: PiExtensionContext): Promise<void> {
  try {
    ...
    const code = core.handlePluginCli([
      "compose",
      "--if-stale",
      "--project-root",
      join(projectDir, ".pi"),
