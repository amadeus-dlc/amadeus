 ...
 *
 * Vendored from @earendil-works/pi-coding-agent 0.83.0
 * (examples/extensions/subagent, Apache-2.0), extended with a session-wide
 * concurrency valve: every subagent process launch funnels through one
 * module-scope semaphore, so a single Pi session never runs more than
 * PI_SUBAGENT_MAX_CONCURRENCY (default 4) subagent processes at once.
 * Excess launches wait in a FIFO queue, like a fixed-size thread pool.
 *
 * This file is intentionally self-contained (the harness manifest ships one
 * file per native extension resource) and intentionally free of repo imports:
 * it is loaded by Pi's own extension runtime, never by the repo's tests or
 * typecheck. See packages/framework/harness/pi/manifest.ts.
 */

// ---------------------------------------------------------------------------
// Session-wide concurrency valve (Amadeus addition; not in the upstream example)
// ---------------------------------------------------------------------------

const DEFAULT_MAX_CONCURRENCY = 4;

function readSessionMaxConcurrency(env: Record<string, string | undefined> = process.env): number {
	const raw = env.PI_SUBAGENT_MAX_CONCURRENCY;
...
const SESSION_MAX_CONCURRENCY = readSessionMaxConcurrency();
const subagentSemaphore = new Semaphore(SESSION_MAX_CONCURRENCY);

// ---------------------------------------------------------------------------
// Agent discovery and configuration (upstream agents.ts, inlined)
// ---------------------------------------------------------------------------
...
async function runSingleAgent(
	...
	const releaseSlot = await subagentSemaphore.acquire();
	try {
		...
		releaseSlot();
		// Best-effort cleanup of our own mkdtemp dir: rmSync with force does not
		// throw on a missing path, so no fall-through catch block is needed.
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: [
			...
			`Session-wide limit: at most ${SESSION_MAX_CONCURRENCY} subagent process(es) run at once across all calls; further launches wait in a queue (configure with PI_SUBAGENT_MAX_CONCURRENCY).`,
