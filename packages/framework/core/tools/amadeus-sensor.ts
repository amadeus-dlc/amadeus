// amadeus-sensor.ts — sensor dispatcher CLI for the AIDLC framework.
//
// Subcommands:
...
// vocabulary's 1:1 producer rule), mirroring amadeus-state.ts's
// producesDirsForStage / amadeus-orchestrate.ts's resolveArtifactPath seams:
...
function exitForMarkerExemption(sensorId: string, outputPath: string): void {
	const outputStem = basename(outputPath).replace(/\.md$/, "");
	const markerSensor = sensorId === "required-sections" || sensorId === "upstream-coverage";
...
export async function handleFire(args: string[], projectDirArg?: string): Promise<void> {
	...
	// amadeus-docs/.amadeus-sensors/<stage-slug>/<sensor-id>-<fire-id>.md
...
function emitTerminal(
	...
	if (outcome.kind === "failed") {
		...
		// path under amadeus-docs/.amadeus-sensors/.
		const fields: Record<string, string> = {
			...baseFields,
