...
Read the state template from `{{HARNESS_DIR}}/knowledge/amadeus-shared/state-template.md`.
...
- Total Stages: count EXECUTE stages only (not SKIP). Authoritative counts come from the compiled scope grid (`{{HARNESS_DIR}}/tools/data/scope-grid.json`, transposed from each stage's `scopes:` frontmatter; run `bun {{HARNESS_DIR}}/tools/amadeus-utility.ts scope-table` for the live table). Today's values:
  | Scope | EXECUTE / Total |
  |-------|-----------------|
