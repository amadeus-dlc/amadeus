...
Load amadeus-delivery-agent persona from `agents/amadeus-delivery-agent.md` and knowledge from `{{HARNESS_DIR}}/knowledge/amadeus-delivery-agent/`.
...
**If practices-discovery executed**, read `{{HARNESS_DIR}}/rules/amadeus-team.md` via `extractMarkdownSection` for three sections that influence Bolt planning:
