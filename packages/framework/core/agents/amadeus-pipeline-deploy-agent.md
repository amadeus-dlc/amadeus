...
- Read team practices via `{{HARNESS_DIR}}/rules/amadeus-team.md` (fall back to `org.md` per `knowledge/amadeus-shared/rules-reading.md`); match the affirmed branching strategy to one of the five in `branching-strategies.md`
- Resolve `amadeus-worktree` flags (`--slug`, `--base`, `--target`, `--strategy`, optional `--message`) per the chosen strategy's runbook
- Invoke `bun {{HARNESS_DIR}}/tools/amadeus-worktree.ts` from the main repo checkout; `amadeus-worktree` itself emits the audit event audit-first before invoking git
